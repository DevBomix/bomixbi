...
begin
    ...
    begin
       Application.MessageBox('Necessário definir um Tipo de Observação!' + #13 + '','Atenção',mb_iconerror + mb_ok);
       BTN_BuscarClick(nil);
       Exit;
    end;

    if TXT_Observacao.Text = '' then
    begin
       Application.MessageBox('Descreva a Observação!' + #13 + '','Atenção',mb_iconerror + mb_ok);
       TXT_Observacao.SetFocus;
       Exit;
    end;


    VLC_Insert := VLC_Insert + ' Insert Into BomixBI.[dbo].[Icq_TB_InspecaoObservacao] (DataTurno, DataCadastro, Turno, Setor, Area, Produto_ID, Lote, Observacao_FK, Observacao, Inspetor_FK) Values (' + #13;
...
procedure TProcessoInspecao_Observacao.BTN_BuscarClick(
  Sender: TObject);
begin
