...
begin
    ...
    begin
       Application.MessageBox('Defina a Hora da Inspeção!' + #13 + '','Atenção',mb_iconerror + mb_ok);
       Exit;
    end;

    if (TXT_Inspector_Area.Text = 'INJECAO') AND (TXT_Dosagem.Text = ' .   ') then
    begin
       Application.MessageBox('Defina a Dosagem!' + #13 + '','Atenção',mb_iconerror + mb_ok);
       Exit;
    end;

    If (TXT_Inspector_Area.Text = 'INJECAO') AND (PNL_AnalisePeso_Cavidade1_Status.Caption = '') then
    begin
       Application.MessageBox('Necessária a Analise de Cavidade!' + #13 + '','Atenção',mb_iconerror + mb_ok);
       Exit;
    end;
    ...
    // Modifico o código porque ao fechar esta tela é deletado todas as informações caso o usuário desista
...
begin
    ...
    // Este if é porque sempre gravará o primeiro defeito, porém os proximo apenas se forem adicionados como NC
    // A logica é que se o primeiro sendo CONFORME não existe outros defeitos
...
begin
  ...
  try
      ...
      ON EConvertError do
      begin
          Application.MessageBox('Hora inválida!','Error', mb_Ok + mb_iconError);
          TXT_Hora.Text := '';
          TXT_Hora.SetFocus;
      end;
  end;
