...
begin
    ...
    begin
       Application.MessageBox('Você não pode excluir observação feita por outro colaborador!' + #13 + '','Atenção',mb_iconerror + mb_ok);
       Exit;
    end;
...
begin
    ...
    begin
       Application.MessageBox('Exclua a ultima inspeção feita para este produto antes desta!' + #13 + '','Atenção',mb_iconerror + mb_ok);
       Exit;
    end;

    if DS_Inspecao.DataSet.FieldByName('Matricula_FK').AsString <> Sys_FuncaoSistema.PNL_Matricula.Caption then
    begin
       Application.MessageBox('Você não pode excluir Inspeção feita por outro colaborador!' + #13 + '','Atenção',mb_iconerror + mb_ok);
       Exit;
    end;
