...
type
  TBI_FuncionarioEntregaMaterias = class(TPadraoBI)
    ...
    procedure FichadeEntregadeMaterias1Click(Sender: TObject);
    procedure BTN_ImprimirClick(Sender: TObject);
    procedure FuncionrioxMateriais1Click(Sender: TObject);
    procedure MateriasAgrupadoPorMs1Click(Sender: TObject);
  private
...
uses BI_FuncionarioEntregaMaterias_InserirMaterial_Unit, conexaodados_unit,
  ...
  BI_FuncionarioEntregaMaterias_FuncXMat_Unit,
  BI_FuncionarioEntregaMaterias_MateriasAgrupadoPorMes_Unit;
...
function TBI_FuncionarioEntregaMaterias.Get_ConsultaBomix: String;
Var
...
begin
   ...
   VCL_Select := VCL_Select + ' 		Select ' + #13;
    VCL_Select := VCL_Select + ' 			Recno, Funcionario.IDTotvs, Funcionario.Empresa, Matricula, Funcionario.Chapa, NomeCompleto as Funcionario, Funcao, TipoFuncao, Departamento, Admissao, Demissao, ' + #13;
    ...
    // Filtro de Datas ************************************************************************************************************************************************
    if CBX_TipoData.Text = 'ADMISSÃO' then
    begin
    ...
    if CBX_TipoData.Text = 'DEMISSÃO' then
    begin
...
procedure TBI_FuncionarioEntregaMaterias.FichadeEntregadeMaterias1Click(
  Sender: TObject);
Var
  VLC_Select : String;
begin
  ...
  begin
       Application.MessageBox('Acesso negado para imprimir este relatório.' + #13 + 'Consulte o Administrador do sistema','Atenção',mb_iconwarning + mb_ok);
   end
   else
   begin
         VLC_Select :=  VLC_Select + ' Select ' + #13;
         VLC_Select :=  VLC_Select + ' 			Ordem, Chapa, Recno, Funcionario.Empresa, Funcionario.IDTotvs, Matricula, NomeCompleto as Funcionario, ' + #13;
...
procedure TBI_FuncionarioEntregaMaterias.FuncionrioxMateriais1Click(
  Sender: TObject);
Var
   VLC_Select : String;
begin

  VLC_Select := Get_ConsultaPrincipal + ' Order by Empresa, NomeCompleto, Acessorio_FK ';

  Application.CreateForm(TBI_FuncionarioEntregaMaterias_FuncXMat, BI_FuncionarioEntregaMaterias_FuncXMat);

  BI_FuncionarioEntregaMaterias_FuncXMat.Query_EntregaMaterial.Close;
  BI_FuncionarioEntregaMaterias_FuncXMat.CDS_EntregaMaterial.Close;
  BI_FuncionarioEntregaMaterias_FuncXMat.Query_EntregaMaterial.SQL.Clear;
  BI_FuncionarioEntregaMaterias_FuncXMat.Query_EntregaMaterial.SQL.Text := VLC_Select;
  BI_FuncionarioEntregaMaterias_FuncXMat.CDS_EntregaMaterial.Open;

  Memo1.Lines.Text :=  VLC_Select;

  BI_FuncionarioEntregaMaterias_FuncXMat.ppEntregaMaterial.Print;

end;

procedure TBI_FuncionarioEntregaMaterias.MateriasAgrupadoPorMs1Click(
  Sender: TObject);
Var
   VLC_Select : String;
begin

  Application.CreateForm(TBI_FuncionarioEntregaMaterias_MateriasAgrupadoPorMes, BI_FuncionarioEntregaMaterias_MateriasAgrupadoPorMes);

  VLC_Select :=  VLC_Select + ' Select ' + #13;
  VLC_Select :=  VLC_Select + ' 	Year(DataEntrega) as Ano, BomixBi.dbo.GetMesPorExtenso(DataEntrega) as Mes, Acessorio_FK, Acessorio, ' +#13;
  VLC_Select :=  VLC_Select + ' 	Sum(Quantidade) as Quantidade' + #13;
  VLC_Select :=  VLC_Select + ' from (  ' + #13;
  VLC_Select :=  VLC_Select + Get_ConsultaPrincipal + #13;
  VLC_Select :=  VLC_Select + ' ) TB ' + #13;
  VLC_Select :=  VLC_Select + ' Group by Acessorio_FK, Acessorio, Year(DataEntrega), BomixBi.dbo.GetMesPorExtenso(DataEntrega) ' + #13;
  VLC_Select :=  VLC_Select + ' Order by Year(DataEntrega) desc, BomixBi.dbo.GetMesPorExtenso(DataEntrega) desc, Acessorio '  + #13;

  Memo1.Lines.Text :=  VLC_Select;

  BI_FuncionarioEntregaMaterias_MateriasAgrupadoPorMes.CDS.Close;
  BI_FuncionarioEntregaMaterias_MateriasAgrupadoPorMes.Query.Close;
  BI_FuncionarioEntregaMaterias_MateriasAgrupadoPorMes.Query.SQL.Clear;
  BI_FuncionarioEntregaMaterias_MateriasAgrupadoPorMes.Query.SQL.Text := VLC_Select;
  BI_FuncionarioEntregaMaterias_MateriasAgrupadoPorMes.CDS.Open;

  BI_FuncionarioEntregaMaterias_MateriasAgrupadoPorMes.ppMateriasAgrupadpPorMes.Print;
