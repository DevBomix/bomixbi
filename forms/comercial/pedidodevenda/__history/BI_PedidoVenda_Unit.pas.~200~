...
begin
    ...
    begin
      ...
      begin
        Application.MessageBox('Não permito criar Ordem de Separação para Orçamentos','Atenção',mb_iconwarning + mb_ok);
        Exit;
      end;
...
begin
  ...
  begin
       Application.MessageBox('Acesso negado para Ordem de Carregamento.' + #13 + 'Consulte o Administrador do sistema','Atenção',mb_iconwarning + mb_ok);
   end
   else
   begin

        if VerificarSeTemLiberacaoCredito(CDSPedidoVenda_ID.AsString) = 'Não' then
        begin
               Application.MessageBox('Bloqueado por Liberação de Crédito. Verificar com o Setor Comercial','Atenção',mb_iconwarning + mb_ok);
               Exit;
        end;
...
begin
  inherited;
    ...
    if SHP_OpcaoPorPedidodeVenda.Brush.Color = $00FF8000 then
    begin
         LBL_PeriodoPedidoPCP.Caption :=  ' Data de Saída Entre ' + ' ' + DateToStr(DTP_DataInicial.Date) + ' a ' + DateToStr(DTP_DataFinal.Date);
         ppLBL_DataPedidoPCP.Caption :=  'Data Impressão: ' + DateTimeToStr(Date());
    ...
    begin
         ppLabel36.Caption :=  ' Data de Saída Entre ' + ' ' + DateToStr(DTP_DataInicial.Date) + ' a ' + DateToStr(DTP_DataFinal.Date);
         ppLabel48.Caption :=  'Data Impressão: ' + DateTimeToStr(Date());
...
begin
            ...
            if CBX_TipoData.Text = 'EMISSÃO' then
            begin
            ...
            begin
                VLC_Select := VLC_Select + '  AND  Item.ZerarRotulo = ' + '''' + 'Não' + '''' + #13;
            end;
           ...
           // Verificar se Gerar OP
            ...
            begin
                VLC_Select := VLC_Select + '  AND  Item.GerarOP = ' + '''' + 'Não' + '''' + #13;
            end;
            ...
            // Verificar se está selecionado para a Ordem de Carregamento
...
begin
  ...
  begin
      ...
      CategoryPanel2.Caption := 'Gerência (Modificado)';
      Exit;
  end;
  ...
  begin
      ...
      CategoryPanel2.Caption := 'Gerência';
  end;
  ...
  begin
   ...
   begin
        ...
        begin
            ...
            begin
                Application.MessageBox('Nenhum Pedido relacionado para Carregamento','Atenção',mb_iconwarning + mb_ok);
                Exit;
            end;
...
begin
   ...
   // Visualização Por Pedidos de Venda
   if SHP_OpcaoPorPedidodeVenda.Brush.Color = $00FF8000 then
  ...
  // Atualizar Também na base BomixBI
  ...
  With Sys_ModuloConexaoDados.ADOCommand1 do begin  CommandText := VLC_Update; Execute;  end;
   ...
   begin
       ...
       begin
          ...
          if LBL_DataPCPAnterior.Caption = '' then
          begin
              VLC_Update := VLC_Update + '''' + 'Inclusão' + '''' + ',';
              VLC_Update := VLC_Update + '''' + DateToStr(DT_DataPCP.Date) + '''' + ',';
          end
          else
          begin
              VLC_Update := VLC_Update + '''' + 'Edição' + '''' + ',';
...
begin
  ...
  begin
       Application.MessageBox('Acesso negado para definir a Data do PCP.' + #13 + 'Consulte o Administrador do sistema','Atenção',mb_iconwarning + mb_ok);
   end
   else
   begin
...
begin
   ...
   if SHP_OpcaoPorPedidodeVenda.Brush.Color = $00FF8000 then
    ...
    VLC_Update := VLC_Update + '''' + VLC_Pedido + '''' + ',';
    VLC_Update := VLC_Update + '''' + 'Data PCP' + '''' + ',';
    VLC_Update := VLC_Update + '''' + 'Exclusão' + '''' + ',';
    VLC_Update := VLC_Update + '''' + Sys_FuncaoSistema.PNL_LoginAtivo.Caption + '''' + ',';
    VLC_Update := VLC_Update + '''' + Sys_FuncaoSistema.TransformarDataFormatoSQLServer(DateTimeToStr(Date()),1) + '''' + ',';
    VLC_Update := VLC_Update + '''' + DateToStr(DT_DataPCP.Date) + '''' + ',';
    VLC_Update := VLC_Update + '''' + Copy(Sys_FuncaoSistema.PNL_EmpresaTotvs.Caption,1,4) + '''' + ')';
...
begin
    ...
    VLC_Select := VLC_Select + ' Case when Rtrim(Arte.StatusArte) = ' + '''' + 'ARTE NOVA' + '''' + ' then ' + '''' + 'Inclusão' + '''' + ' else ' + '''' + 'Edição' + '''' + ' end as Status, ' + #13;
...
begin
  ...
  begin
      ...
      begin
          ...
          begin
               Application.MessageBox('Não permitido definir carga para Pedido sem Data do PCP','Atenção',mb_iconwarning + mb_ok);
               Exit;
          end;

          if VerificarSeTemLiberacaoCreditoParaDataPCP(CDSPedidoVenda_ID.AsString) = 'Não' then
          begin
             Application.MessageBox('Bloqueado por Liberação de Crédito. Verificar com o Setor Comercial','Atenção',mb_iconwarning + mb_ok);
             Exit;
          end;
...
begin
  ...
  begin
      ...
      begin
          ...
          If CDSTipoRegistro.AsString = 'O' then
          begin
              Application.MessageBox('Não permitido definir carga para Orçamento','Atenção',mb_iconwarning + mb_ok);
              Exit;
          end;
