...
begin
    ...
    begin
          ...
          VLC_Select := VLC_Select + ' 		TipoCarga, Carga, RazaoSocial as Cliente, Case when Carga.SetorMercado = ' + '''' + 'ALIMENTOS' + '''' + ' then ' + '''' + 'ALIMENTOS' + '''' + ' else ' + '''' + '' + '''' + ' end as SetorMercado, ' + #13;
          ...
          VLC_Select := VLC_Select + ' 		Saldo, Saldo * PesoItem  as Peso, Pallete, Carga.Observacao, Carga.DataPCP' + #13;
          VLC_Select := VLC_Select + ' 	from BomixBI.dbo.Fat_TB_PedidoVenda_OrdemCarregamento Carga (Nolock) ' + #13;
          VLC_Select := VLC_Select + ' 	Inner Join BomixBI.dbo.Fat_TB_PedidoVenda Pedido ON Recno_Pedido = Pedido.Recno ' + #13;
          ...
          VLC_Select := VLC_Select + ' 	AND Carga.Empresa = ' + '''' + Copy(Sys_FuncaoSistema.PNL_EmpresaTotvs.Caption,1,4) + '''' + #13;
    ...
    begin
          ...
          VLC_Select := VLC_Select + ' 		TipoCarga, Carga, RazaoSocial as Cliente, Case when Carga.SetorMercado = ' + '''' + 'ALIMENTOS' + '''' + ' then ' + '''' + 'ALIMENTOS' + '''' + ' else ' + '''' + '' + '''' + ' end as SetorMercado, ' + #13;
          VLC_Select := VLC_Select + ' 		Municipio as Cidade, UnidadeFederativa as UF, PedidoVenda_FK, Sequencia, Produto_ID, Produto, Quantidade, ' +#13;
          VLC_Select := VLC_Select + ' 		Saldo, Saldo * PesoItem  as Peso, Pallete, Carga.Observacao, Historico.DataPCP ' + #13;
          VLC_Select := VLC_Select + ' 	from BomixBI.dbo.Fat_TB_PedidoVenda_OrdemCarregamento_Historico CargaCarga (Nolock) ' + #13;
          VLC_Select := VLC_Select + ' 	Inner Join BomixBI.dbo.Fat_TB_PedidoVenda Pedido ON Recno_Pedido = Pedido.Recno ' + #13;
          VLC_Select := VLC_Select + ' 	Where Carga <> ' + '''' + '' + '''' + #13;
          VLC_Select := VLC_Select + '  AND DataCarregamento = CONVERT(Datetime,Convert(Varchar(10),' + '''' + DateTimeToStr(DT_DataCarragamento.Date) + '''' +  ',103),103)' + #13;
          VLC_Select := VLC_Select + '  AND TipoCarga = ' +  '''' + VLC_TipoCarga + '''' + #13;
    ...
    ppLBL_Data.Caption :=  'Data Impressão: ' + DateTimeToStr(Date()) + ' // Hora: ' + TimeToStr(Time());
