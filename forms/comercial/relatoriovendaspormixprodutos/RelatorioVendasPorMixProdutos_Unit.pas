...
begin
  ...
  VLC_Query := VLC_Query + '								AND Item.Faturamento <> ' + '''' +  'Não Comercial' + '''' + #13;
  ...
  VLC_Query := VLC_Query + ' CONVERT(Datetime,Convert(Varchar(10),' + '''' + DateTimeToStr(DT_DataEmissao1.Date) + '''' +  ',103),103)';
  VLC_Query := VLC_Query + '								AND CONVERT(Datetime,Convert(Varchar(10),' + '''' + DateTimeToStr(DT_DataEmissao2.Date) + '''' +  ',103),103)' + #13;
  VLC_Query := VLC_Query + '								AND ItemDevolucao.Faturamento <> ' + '''' +  'Não Comercial' + '''' + #13;
  VLC_Query := VLC_Query + '								AND F2_Empresa = ' + '''' + Copy(Sys_FuncaoSistema.PNL_EmpresaTotvs.Caption,1,4) + '''' + #13;
...
begin
    ...
    VLC_Select := VLC_Select + '       	   AND RetirarMix = '+ '''' + 'Não' + '''' + #13;
...
begin
 ...
 VLC_Select := VLC_Select + '                from BomixBI.dbo.Fat_TB_NotaFiscalVenda_CombinacaoMixAgrupado (nolock)' + #13;
  ...
  begin
      VLC_Select := VLC_Select + '               AND ProdutoMixado =  ' + '''' + 'Não' + '''';
  end;
...
begin
  ...
  begin
     ...
     ppLabel34.Caption :=  ' Data de Saída Entre ' + ' ' + DateToStr(DT_DataEmissao1.Date) + ' a ' + DateToStr(DT_DataEmissao2.Date);
...
begin
  ...
  begin
     // ppMemoFiltro.Lines.Text := 'Filtros: ' + GetFiltros;
     ppLabel2.Caption :=  ' Data de Saída Entre ' + ' ' + DateToStr(DT_DataEmissao1.Date) + ' a ' + DateToStr(DT_DataEmissao2.Date);
     ppLabel16.Caption :=  'Data Impressão: ' + DateTimeToStr(Date());
...
begin
  ...
  begin
     // ppMemoFiltro.Lines.Text := 'Filtros: ' + GetFiltros;
     LBL_PeriodoListaPorGerencia.Caption :=  ' Data de Saída Entre ' + ' ' + DateToStr(DT_DataEmissao1.Date) + ' a ' + DateToStr(DT_DataEmissao2.Date);
     ppLBL_DataListaPorGerencia.Caption :=  'Data Impressão: ' + DateTimeToStr(Date());
...
begin
  ...
  begin
     // ppMemoFiltro.Lines.Text := 'Filtros: ' + GetFiltros;
     ppLabel72.Caption :=  ' Data de Saída Entre ' + ' ' + DateToStr(DT_DataEmissao1.Date) + ' a ' + DateToStr(DT_DataEmissao2.Date);
     ppLabel74.Caption :=  'Impresso Por: ' + Sys_FuncaoSistema.PNL_UsuarioAtivo.Caption + '  ' + 'Data Impressão: ' + DateTimeToStr(Date());
...
begin
  ...
  begin
     // ppMemoFiltro.Lines.Text := 'Filtros: ' + GetFiltros;
     ppLabel30.Caption :=  ' Data de Saída Entre ' + ' ' + DateToStr(DT_DataEmissao1.Date) + ' a ' + DateToStr(DT_DataEmissao2.Date);
     ppLabel32.Caption :=  'Data Impressão: ' + DateTimeToStr(Date());
...
begin
  ...
  begin
     ...
     ppLabel28.Caption :=  'Data Impressão: ' + DateTimeToStr(Date());
...
begin
  ...
  if CBX_Tipo.Text = 'MIX EM LISTA - AGRUPADO POR GERÊNCIA' then
  begin
  ...
  if CBX_Tipo.Text = 'MIX EM LISTA - AGRUPADO POR GRUPO DE PRODUTO (SINTÉTICO)' then
  begin
      Imprimir_MixEmListaGrupoProdutoSintetico;
  end;

  if CBX_Tipo.Text = 'MIX EM LISTA - PREÇO X PRAZO DO CLIENTE' then
  begin
      Imprimir_MixEmListaUltimasVendasCliente;
  end;

  if CBX_Tipo.Text = 'MIX EM GRADE - ANÁLISE PESO' then
  begin
      Imprimir_MixEmGradePorPeso;
  end;

  if CBX_Tipo.Text = 'MIX EM GRADE - ANÁLISE PREÇO' then
  begin
...
begin
  inherited;
   ...
   if aText = 'Média MediaRS' then
      aText := 'Média R$';

   if aText = 'Somar de Peso' then
      aText := 'Peso';

   if aText = 'Média MediaKG' then
      aText := 'Média KG';
