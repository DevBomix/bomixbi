unit RelatorioParametrizadoVendas_VendedorXVendedor_Unit;
...
type
  TRelatorioParametrizadoVendas_VendedorXVendedor = class(TForm)
    ppDB: TppDBPipeline;
    Query: TADOQuery;
    DS: TDataSource;
    CDS: TClientDataSet;
...
var
  RelatorioParametrizadoVendas_VendedorXVendedor: TRelatorioParametrizadoVendas_VendedorXVendedor;
...
function TRelatorioParametrizadoVendas_VendedorXVendedor.GetSelect: String;
Var
  VLC_Select : String;
begin
   ...
   begin
         ...
         VLC_Select := VLC_Select + '   ' + '''' + 'Soma / Subtrai' + '''' + ' as ComissaoVendedor, Vendedor, SUM(Quantidade) as Quantidade, Sum(ValorUnitario) as ValorUnitario,' + #13;
         ...
         VLC_Select := VLC_Select + '    SELECT' + #13;
         VLC_Select := VLC_Select + '' + #13;
         VLC_Select := VLC_Select + '		ComissaoVendedor, Vendedor,' + #13;
         ...
         VLC_Select := VLC_Select + '    Group By ComissaoVendedor, Vendedor' + #13;
         VLC_Select := VLC_Select + '' + #13;
         VLC_Select := VLC_Select + ') TB Group by Vendedor' + #13;
   ...
   begin
         ...
         VLC_Select := VLC_Select + 'SELECT ' + #13;
         VLC_Select := VLC_Select + ' ' + #13;
         VLC_Select := VLC_Select + '     ComissaoVendedor, Vendedor, SUM(Quantidade) as Quantidade, Sum(ValorUnitario) as ValorUnitario,' + #13;
         VLC_Select := VLC_Select + '     Case When SUM(Peso) > 0 Then Round((Sum(ValorPCC)/ SUM(Peso)),2) else 0 end as MediaKG,' + #13;
         VLC_Select := VLC_Select + '     Case When SUM(Peso) > 0 Then Round((Sum(ValorPCCMP)/ SUM(Peso)),2) else 0 end as MediaKGMP,' + #13;
         VLC_Select := VLC_Select + '     SUM(Peso) as Peso, Sum(ValorLiquido) as ValorLiquido, Sum(ValorLiquidoMP) as ValorLiquidoMP,' + #13;
         VLC_Select := VLC_Select + '     Sum(ValorPCC) as ValorPCC, Sum(ValorPCCMP) as ValorPCCMP,' + #13;
         ...
         VLC_Select := VLC_Select + ') TB ' + #13;
         VLC_Select := VLC_Select + 'Group by ComissaoVendedor, Vendedor  ' + #13;
         VLC_Select := VLC_Select + 'Order By ComissaoVendedor, Vendedor  ' + #13;
...
function TRelatorioParametrizadoVendas_VendedorXVendedor.ImprimirRelatorio: Boolean;
Var
   VLC_Select : String;
begin
...
procedure TRelatorioParametrizadoVendas_VendedorXVendedor.ppRelatorioStartPage(
  Sender: TObject);
begin
...
procedure TRelatorioParametrizadoVendas_VendedorXVendedor.TXT_MediaComissaoMPPrint(
  Sender: TObject);
begin
...
procedure TRelatorioParametrizadoVendas_VendedorXVendedor.TXT_MediaComissaoPrint(
  Sender: TObject);
begin
  if TXT_PesoTotalComissao.Value > 0 then
