...
begin
  ...
  ppLBL_DataGerenciaXGerencia.Caption := 'Data Impressão: ' + DateTimeToStr(Date());
  ppLBL_NumeroPaginaGerenciaXGerencia.Caption := ('Página ' + IntToStr(ppGerenciaXGerencia.Page) + ' de ' + IntToStr(ppGerenciaXGerencia.PageCount));
  ...
  if RelatorioParametrizadoVendas.CBX_TipoPedido.Text = 'PARAMETRIZADO DE PEDIDOS DE VENDA' then
       ppLabel278.Caption := 'Relatório Parametrizado de Pedidos (Gerência x Gerência)'
  else
       ppLabel278.Caption := 'Relatório Parametrizado de Notas (Gerência x Gerência)';
