...
begin
  ...
  VLC_Select := VLC_Select + '    		 CASE' + #13;
  VLC_Select := VLC_Select + ' 			    	WHEN (PosVenda.ConfirmacaoChegada is NULL) AND (CIT.DataPrevista < GETDATE())  THEN ' + '''' + 'NÃO REALIZADA' + '''' + #13;
...
begin
  ...
  begin
       Application.MessageBox('Acesso negado para Edição do Pós Venda.' + #13 + 'Consulte o Administrador do sistema','Atenção',mb_iconwarning + mb_ok);
   end
   else
   begin
...
begin
    ...
    // Somente se não estiver em modo de edição de dados
...
begin
  PageControl1.ActivePage := TabSheet2;
  ppLBL_Periodo.Text :=  ' Emissão Entre ' + ' ' + DateToStr(DTP_DataInicial.Date) + ' a ' + DateToStr(DTP_DataFinal.Date);
...
begin
  ...
  begin
       Application.MessageBox('Acesso negado para Relatório de Pós Venda.' + #13 + 'Consulte o Administrador do sistema','Atenção',mb_iconwarning + mb_ok);
   end
   else
   begin
        inherited;
