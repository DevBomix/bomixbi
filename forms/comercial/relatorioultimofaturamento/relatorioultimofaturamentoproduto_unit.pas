...
begin
  ...
  begin
      VLC_Select := VLC_Select +  ' AND Nota.Cliente = ' + '''' + CBX_Igual_Cliente.Text + '''' + #13;
  end;

  if CBX_Igual_UF.Text <> '' then
  begin
      VLC_Select := VLC_Select +  ' AND Nota.UF = ' + '''' + CBX_Igual_UF.Text + '''' + #13;
  end;

  if CBX_Igual_Vendedor.Text <> 'TODAS' then
  begin
      VLC_Select := VLC_Select +  ' AND Nota.Vendedor = ' + '''' + CBX_Igual_Gerencia.Text + '''' + #13;
  end;
...
procedure TRelatorioUltimoFaturamentoProduto.ListarVendedor;
var
  VLC_Select : String;
  Query : TADOQuery;
  VLN_Contador : Integer;
...
procedure TRelatorioUltimoFaturamentoProduto.BNT_CancelarClick(Sender: TObject);
begin
  inherited;
  Close;
end;
