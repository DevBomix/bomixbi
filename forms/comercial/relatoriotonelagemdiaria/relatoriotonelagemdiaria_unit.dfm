inherited Relatorio_TonelagemDiaria: TRelatorio_TonelagemDiaria
  Caption = 'Relat'#243'rio de Tonelagem Di'#225'ria'
  ClientHeight = 256
  ClientWidth = 688
  OnShow = FormShow
  ExplicitWidth = 694
  ExplicitHeight = 285
  PixelsPerInch = 96
  TextHeight = 13
  inherited Panel_Titulo: TPanel
    inherited LBL_Titulo: TLabel
      Width = 162
      Caption = 'Tonelagem Di'#225'ria'
      ExplicitWidth = 162
    end
  end
  inherited Panel_Padrao: TPanel
    Top = 142
    ExplicitTop = 142
  end
  inherited MemoSQL: TMemo
    Width = 362
    ExplicitWidth = 362
  end
  inherited Panel_Botoes: TPanel
    Top = 217
    Width = 688
    ExplicitTop = 217
    ExplicitWidth = 688
    inherited Panel_Navegador: TPanel
      Left = 443
      ExplicitLeft = 443
      inherited BNT_Confirmar: TSpeedButton
        Left = 129
        ExplicitLeft = 129
      end
    end
  end
  object Panel4: TPanel [4]
    Left = 7
    Top = 55
    Width = 672
    Height = 53
    Color = 14869218
    ParentBackground = False
    TabOrder = 4
    object Shape1: TShape
      Left = 4
      Top = 5
      Width = 650
      Height = 43
      Pen.Width = 2
    end
    object Edit25: TEdit
      Left = 12
      Top = 14
      Width = 170
      Height = 24
      TabStop = False
      Color = 10907648
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
      ReadOnly = True
      TabOrder = 0
      Text = ' Tipo Relat'#243'rio'
    end
    object CBX_TipoRelatorio: TComboBox
      Left = 196
      Top = 14
      Width = 449
      Height = 24
      CharCase = ecUpperCase
      Color = 3618815
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 1
      Text = 'TONELAGEM DI'#193'RIA - GERAL'
      Items.Strings = (
        'TONELAGEM DI'#193'RIA - GER'#202'NCIA'
        'TONELAGEM DI'#193'RIA - VENDEDOR'
        'TONELAGEM DI'#193'RIA - GERAL')
    end
  end
  object Panel7: TPanel [5]
    Left = 7
    Top = 110
    Width = 672
    Height = 32
    BevelOuter = bvNone
    Color = 16053492
    ParentBackground = False
    TabOrder = 5
    object CBX_OperadorDataEmissao: TComboBox
      Left = 197
      Top = 4
      Width = 134
      Height = 23
      Color = clWhite
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
      TabStop = False
      Text = 'Entre'
      Items.Strings = (
        'Iniciado por'
        'Igual'
        'Terminado por'
        'Cont'#233'm'
        'Est'#225' contido'
        'Maior'
        'Menor'
        'Maior ou Igual'
        'Menor ou Igual'
        'Entre')
    end
    object CBX_Igual_DataEmissao: TComboBox
      Left = 660
      Top = 4
      Width = 322
      Height = 23
      CharCase = ecUpperCase
      Color = clWhite
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 1
      TabStop = False
      Visible = False
    end
    object TXT_Valor1_DataEmissao: TMaskEdit
      Left = 334
      Top = 4
      Width = 159
      Height = 23
      CharCase = ecUpperCase
      Color = clWhite
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 2
      Text = ''
    end
    object TXT_Valor2_DataEmissao: TMaskEdit
      Left = 495
      Top = 4
      Width = 160
      Height = 23
      CharCase = ecUpperCase
      Color = clWhite
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 3
      Text = ''
    end
    object DT_Valor1_Periodo: TDateTimePicker
      Left = 335
      Top = 4
      Width = 159
      Height = 24
      Date = 41335.000000000000000000
      Time = 0.424179409732460000
      Font.Charset = DEFAULT_CHARSET
      Font.Color = 16744448
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 4
    end
    object DT_Valor2_Periodo: TDateTimePicker
      Left = 496
      Top = 4
      Width = 160
      Height = 24
      Date = 41335.000000000000000000
      Time = 0.424179409732460000
      Font.Charset = DEFAULT_CHARSET
      Font.Color = 16744448
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 5
    end
    object PNL_Mensal: TPanel
      Left = 197
      Top = 2
      Width = 460
      Height = 29
      Color = 9803157
      TabOrder = 6
      object Bevel2: TBevel
        Left = 178
        Top = 2
        Width = 4
        Height = 27
      end
      object CBX_Periodo: TComboBox
        Left = 4
        Top = 3
        Width = 169
        Height = 23
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 0
        Text = 'MENSAL'
        OnChange = CBX_PeriodoChange
        Items.Strings = (
          'MENSAL'
          'BIMESTRAL'
          'TRIMESTRAL'
          'SEMESTRAL'
          'ANUAL')
      end
      object CBX_Ano: TComboBox
        Left = 352
        Top = 3
        Width = 93
        Height = 23
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 1
        Text = '2013'
        OnChange = CBX_AnoChange
        Items.Strings = (
          '2013'
          '2014'
          '2015'
          '2016'
          '2017'
          '2018'
          '2019'
          '2020')
      end
      object CBX_Mes: TComboBox
        Left = 188
        Top = 3
        Width = 158
        Height = 23
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 2
        Text = 'MES ATUAL'
        OnChange = CBX_MesChange
        Items.Strings = (
          '01. JANEIRO'#9
          '02. FEVEREIRO'#9
          '03. MAR'#199'O'#9
          '04. ABRIL'#9
          '05. MAIO'#9
          '06. JUNHO'#9
          '07. JULHO'#9
          '08. AGOSTO'#9
          '09. SETEMBRO'#9
          '10. OUTUBRO'#9
          '11. NOVEMBRO'#9
          '12. DEZEMBRO'#9
          '')
      end
    end
    object CBX_TipoData: TComboBox
      Left = 12
      Top = 4
      Width = 183
      Height = 24
      Color = 10907648
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 7
      Text = 'Emiss'#227'o'
      Items.Strings = (
        'Emiss'#227'o'
        'Data PCP')
    end
  end
  object Panel16: TPanel [6]
    Left = 7
    Top = 176
    Width = 672
    Height = 32
    BevelOuter = bvNone
    Color = 16053492
    ParentBackground = False
    TabOrder = 6
    object TXT_Valor2_Gerencia: TMaskEdit
      Left = 495
      Top = 4
      Width = 160
      Height = 23
      CharCase = ecUpperCase
      Color = clWhite
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 2
      Text = ''
    end
    object CBX_OperadorGerencia: TComboBox
      Left = 197
      Top = 4
      Width = 134
      Height = 23
      Color = clWhite
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
      TabStop = False
      Text = 'Igual'
      Items.Strings = (
        'Iniciado por'
        'Igual'
        'Terminado por'
        'Cont'#233'm'
        'Est'#225' contido'
        'Maior'
        'Menor'
        'Maior ou Igual'
        'Menor ou Igual'
        'Entre')
    end
    object TXT_Valor1_Gerencia: TMaskEdit
      Left = 334
      Top = 4
      Width = 159
      Height = 23
      CharCase = ecUpperCase
      Color = clWhite
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 1
      Text = ''
    end
    object Panel17: TPanel
      Left = 7
      Top = 4
      Width = 188
      Height = 23
      Alignment = taLeftJustify
      Caption = '  Ger'#234'ncia'
      Color = 10907648
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentBackground = False
      ParentFont = False
      TabOrder = 3
      object PNL_SinalGerencia: TPanel
        Left = 174
        Top = 1
        Width = 13
        Height = 21
        Cursor = crHandPoint
        Align = alRight
        BevelOuter = bvNone
        Color = clAqua
        ParentBackground = False
        TabOrder = 0
        OnClick = PNL_SinalGerenciaClick
      end
    end
    object CBX_Igual_Gerencia: TComboBox
      Left = 334
      Top = 4
      Width = 322
      Height = 23
      CharCase = ecUpperCase
      Color = clWhite
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 4
      TabStop = False
      Text = 'TODAS'
    end
  end
  object Panel1: TPanel [7]
    Left = 8
    Top = 144
    Width = 671
    Height = 32
    BevelOuter = bvNone
    Color = 16053492
    ParentBackground = False
    TabOrder = 7
    object TXT_Valor2_Vendedor: TMaskEdit
      Left = 495
      Top = 4
      Width = 160
      Height = 23
      CharCase = ecUpperCase
      Color = clWhite
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 2
      Text = ''
    end
    object CBX_OperadorVendedor: TComboBox
      Left = 197
      Top = 4
      Width = 134
      Height = 23
      Color = clWhite
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
      TabStop = False
      Text = 'Igual'
      Items.Strings = (
        'Iniciado por'
        'Igual'
        'Terminado por'
        'Cont'#233'm'
        'Est'#225' contido'
        'Maior'
        'Menor'
        'Maior ou Igual'
        'Menor ou Igual'
        'Entre')
    end
    object TXT_Valor1_Vendedor: TMaskEdit
      Left = 334
      Top = 4
      Width = 159
      Height = 23
      CharCase = ecUpperCase
      Color = clWhite
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 1
      Text = ''
    end
    object Panel2: TPanel
      Left = 7
      Top = 4
      Width = 188
      Height = 23
      Alignment = taLeftJustify
      Caption = ' Vendedor'
      Color = 10907648
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentBackground = False
      ParentFont = False
      TabOrder = 3
      object PNL_SinalVendedor: TPanel
        Left = 174
        Top = 1
        Width = 13
        Height = 21
        Cursor = crHandPoint
        Align = alRight
        BevelOuter = bvNone
        Color = clAqua
        ParentBackground = False
        TabOrder = 0
        OnClick = PNL_SinalVendedorClick
      end
    end
    object CBX_Igual_Vendedor: TComboBox
      Left = 334
      Top = 4
      Width = 322
      Height = 23
      CharCase = ecUpperCase
      Color = clWhite
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 4
      TabStop = False
      Text = 'TODOS'
      Items.Strings = (
        '')
    end
  end
  inherited ACL_Atalhos: TActionList
    Left = 601
    Top = 207
  end
  object CDS: TClientDataSet
    Aggregates = <>
    AggregatesActive = True
    AutoCalcFields = False
    Params = <>
    ProviderName = 'DSP'
    Left = 845
    Top = 84
    object CDSData: TDateTimeField
      FieldName = 'Data'
    end
    object CDSDia: TStringField
      FieldName = 'Dia'
      Size = 3
    end
    object CDSDiaDaSemana: TStringField
      FieldName = 'DiaDaSemana'
    end
    object CDSAnoBomix: TStringField
      FieldName = 'AnoBomix'
      Size = 4
    end
    object CDSMesNomeBomix: TStringField
      FieldName = 'MesNomeBomix'
      Size = 14
    end
    object CDSGerente: TStringField
      FieldName = 'Gerente'
      Size = 100
    end
    object CDSVendedor_FK: TStringField
      FieldName = 'Vendedor_FK'
      Size = 6
    end
    object CDSVendedor: TStringField
      FieldName = 'Vendedor'
      Size = 100
    end
    object CDSPesoAFaturar: TFloatField
      FieldName = 'PesoAFaturar'
      ReadOnly = True
    end
    object CDSPesoFaturado: TFloatField
      FieldName = 'PesoFaturado'
      ReadOnly = True
    end
    object CDSPesoTotalDia: TFloatField
      FieldName = 'PesoTotalDia'
      ReadOnly = True
    end
    object CDSPesoOrcamento: TFloatField
      FieldName = 'PesoOrcamento'
      ReadOnly = True
    end
    object CDSPesoNaoDatado: TFloatField
      FieldName = 'PesoNaoDatado'
      ReadOnly = True
    end
    object CDSPesoArteBloqueada: TFloatField
      FieldName = 'PesoArteBloqueada'
      ReadOnly = True
    end
    object CDSEmpresa: TStringField
      FieldName = 'Empresa'
      Size = 6
    end
  end
  object Query: TADOQuery
    Connection = Sys_ModuloConexaoDados.ADOConnection1
    CursorType = ctStatic
    DataSource = DS
    Parameters = <>
    SQL.Strings = (
      '   Select '
      
        '    Empresa, Data, Dia, DiaDaSemana, AnoBomix, MesNomeBomix, Ger' +
        'ente, Vendedor_FK, Vendedor,'
      
        '    Sum(PesoOrcamento) as PesoOrcamento, Sum(PesoArteBloqueada) ' +
        'as PesoArteBloqueada, Sum(PesoNaoDatado) as PesoNaoDatado, '
      
        '    SUM(PesoAFaturar) as PesoAFaturar, SUM(PesoFaturado) as Peso' +
        'Faturado, '
      '    SUM(PesoTotalDia) as PesoTotalDia '
      
        '  from BomixBI.[dbo].[Fat_TB_TonelagemDiaria] (nolock) Tonelagem' +
        '  '
      
        #9'Where Data Between '#39'2021-01-01 00:00:00'#39' AND '#39'2021-01-31 23:59:' +
        '00'#39
      '  AND Empresa = '#39'010101'#39
      
        '  Group by Empresa, Data, Dia, DiaDaSemana, AnoBomix, MesNomeBom' +
        'ix, Gerente, Vendedor_FK, Vendedor'
      '  Order by Gerente, Vendedor, MesNomeBomix, Data desc')
    Left = 875
    Top = 84
    object QueryData: TDateTimeField
      FieldName = 'Data'
    end
    object QueryDia: TStringField
      FieldName = 'Dia'
      Size = 3
    end
    object QueryDiaDaSemana: TStringField
      FieldName = 'DiaDaSemana'
    end
    object QueryAnoBomix: TStringField
      FieldName = 'AnoBomix'
      Size = 4
    end
    object QueryMesNomeBomix: TStringField
      FieldName = 'MesNomeBomix'
      Size = 14
    end
    object QueryGerente: TStringField
      FieldName = 'Gerente'
      Size = 100
    end
    object QueryVendedor_FK: TStringField
      FieldName = 'Vendedor_FK'
      Size = 6
    end
    object QueryVendedor: TStringField
      FieldName = 'Vendedor'
      Size = 100
    end
    object QueryPesoAFaturar: TFloatField
      FieldName = 'PesoAFaturar'
      ReadOnly = True
    end
    object QueryPesoFaturado: TFloatField
      FieldName = 'PesoFaturado'
      ReadOnly = True
    end
    object QueryPesoTotalDia: TFloatField
      FieldName = 'PesoTotalDia'
      ReadOnly = True
    end
    object QueryPesoOrcamento: TFloatField
      FieldName = 'PesoOrcamento'
      ReadOnly = True
    end
    object QueryPesoNaoDatado: TFloatField
      FieldName = 'PesoNaoDatado'
      ReadOnly = True
    end
    object QueryPesoArteBloqueada: TFloatField
      FieldName = 'PesoArteBloqueada'
      ReadOnly = True
    end
    object QueryEmpresa: TStringField
      FieldName = 'Empresa'
      Size = 6
    end
  end
  object DS: TDataSource
    DataSet = CDS
    Left = 905
    Top = 84
  end
  object DSP: TDataSetProvider
    DataSet = Query
    Left = 936
    Top = 84
  end
  object ppReportDiario: TppReport
    AutoStop = False
    ColumnPositions.Strings = (
      '6350'
      '105001')
    DataPipeline = ppDB
    PassSetting = psTwoPass
    PrinterSetup.BinName = 'Default'
    PrinterSetup.DocumentName = 'Report'
    PrinterSetup.Duplex = dpNone
    PrinterSetup.PaperName = 'A4'
    PrinterSetup.PrinterName = 'Default'
    PrinterSetup.SaveDeviceSettings = False
    PrinterSetup.mmMarginBottom = 6350
    PrinterSetup.mmMarginLeft = 6350
    PrinterSetup.mmMarginRight = 6350
    PrinterSetup.mmMarginTop = 6350
    PrinterSetup.mmPaperHeight = 297000
    PrinterSetup.mmPaperWidth = 210000
    PrinterSetup.PaperSize = 9
    AllowPrintToArchive = True
    AllowPrintToFile = True
    ArchiveFileName = '($MyDocuments)\ReportArchive.raf'
    DeviceType = 'Screen'
    DefaultFileDeviceType = 'XLSData'
    EmailSettings.ReportFormat = 'PDF'
    LanguageID = 'Default'
    OpenFile = True
    OutlineSettings.CreateNode = True
    OutlineSettings.CreatePageNodes = True
    OutlineSettings.Enabled = True
    OutlineSettings.Visible = True
    ThumbnailSettings.Enabled = True
    ThumbnailSettings.Visible = True
    ThumbnailSettings.DeadSpace = 30
    ThumbnailSettings.PageHighlight.Width = 3
    PDFSettings.EmbedFontOptions = [efUseSubset]
    PDFSettings.EncryptSettings.AllowCopy = True
    PDFSettings.EncryptSettings.AllowInteract = True
    PDFSettings.EncryptSettings.AllowModify = True
    PDFSettings.EncryptSettings.AllowPrint = True
    PDFSettings.EncryptSettings.AllowExtract = True
    PDFSettings.EncryptSettings.AllowAssemble = True
    PDFSettings.EncryptSettings.AllowQualityPrint = True
    PDFSettings.EncryptSettings.Enabled = False
    PDFSettings.EncryptSettings.KeyLength = kl40Bit
    PDFSettings.EncryptSettings.EncryptionType = etRC4
    PDFSettings.FontEncoding = feAnsi
    PDFSettings.ImageCompressionLevel = 25
    PDFSettings.PDFAFormat = pafNone
    PreviewFormSettings.PageBorder.mmPadding = 0
    PreviewFormSettings.WindowState = wsMaximized
    PreviewFormSettings.ZoomSetting = zs100Percent
    RTFSettings.DefaultFont.Charset = DEFAULT_CHARSET
    RTFSettings.DefaultFont.Color = clWindowText
    RTFSettings.DefaultFont.Height = -13
    RTFSettings.DefaultFont.Name = 'Arial'
    RTFSettings.DefaultFont.Style = []
    TextFileName = '($MyDocuments)\Report.xls'
    TextSearchSettings.DefaultString = '<Texto a localizar>'
    TextSearchSettings.Enabled = True
    XLSSettings.AppName = 'ReportBuilder'
    XLSSettings.Author = 'ReportBuilder'
    XLSSettings.Subject = 'Report'
    XLSSettings.Title = 'Report'
    XLSSettings.WorksheetName = 'Report'
    Left = 770
    Top = 84
    Version = '19.03'
    mmColumnWidth = 98651
    DataPipelineName = 'ppDB'
    object ppHeaderBand16: TppHeaderBand
      Background.Brush.Style = bsClear
      Border.mmPadding = 0
      PrintHeight = phDynamic
      mmBottomOffset = 0
      mmHeight = 13758
      mmPrintPosition = 0
      object ppLine142: TppLine
        DesignLayer = ppDesignLayer1
        UserName = 'Line1'
        Border.mmPadding = 0
        Pen.Width = 3
        Weight = 2.250000000000000000
        mmHeight = 2646
        mmLeft = 1324
        mmTop = 10850
        mmWidth = 193411
        BandType = 0
        LayerName = Foreground
      end
      object ppImage15: TppImage
        DesignLayer = ppDesignLayer1
        UserName = 'ppIMG_Logo'
        AlignHorizontal = ahCenter
        AlignVertical = avCenter
        MaintainAspectRatio = False
        Stretch = True
        Border.mmPadding = 0
        Picture.Data = {
          
          FFFF}
        mmHeight = 6879
        mmLeft = 1853
        mmTop = 3442
        mmWidth = 23813
        BandType = 0
        LayerName = Foreground
      end
      object ppLabel278: TppLabel
        DesignLayer = ppDesignLayer1
        UserName = 'ppLBL_TituloRelatorio'
        HyperlinkEnabled = False
        Border.mmPadding = 0
        Caption = 'Relat'#243'rio de Tonelagem Di'#225'ria'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 18
        Font.Style = [fsBold]
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        Transparent = True
        mmHeight = 7673
        mmLeft = 28312
        mmTop = 3442
        mmWidth = 91546
        BandType = 0
        LayerName = Foreground
      end
    end
    object ppDetailBand16: TppDetailBand
      Background1.Brush.Style = bsClear
      Background2.Brush.Style = bsClear
      Border.mmPadding = 0
      mmBottomOffset = 0
      mmHeight = 5027
      mmPrintPosition = 0
      object TXT_GerenteGerenciaXGerencia: TppDBText
        DesignLayer = ppDesignLayer1
        UserName = 'DBText138'
        HyperlinkEnabled = False
        Border.mmPadding = 0
        DataField = 'Dia'
        DataPipeline = ppDB
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = [fsBold]
        TextAlignment = taCentered
        Transparent = True
        DataPipelineName = 'ppDB'
        mmHeight = 3440
        mmLeft = 21167
        mmTop = 794
        mmWidth = 17198
        BandType = 4
        LayerName = Foreground
      end
      object ppDBCalc7: TppDBCalc
        DesignLayer = ppDesignLayer1
        UserName = 'DBCalc7'
        Border.mmPadding = 0
        DataField = 'Dia'
        DataPipeline = ppDB
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        ResetGroup = ppGroup3
        TextAlignment = taRightJustified
        Transparent = True
        DBCalcType = dcCount
        DataPipelineName = 'ppDB'
        mmHeight = 3175
        mmLeft = 15610
        mmTop = 794
        mmWidth = 5821
        BandType = 4
        LayerName = Foreground
      end
      object ppDBText2: TppDBText
        DesignLayer = ppDesignLayer1
        UserName = 'DBText2'
        HyperlinkEnabled = False
        Border.mmPadding = 0
        DataField = 'DiaDaSemana'
        DataPipeline = ppDB
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        Transparent = True
        DataPipelineName = 'ppDB'
        mmHeight = 3175
        mmLeft = 39688
        mmTop = 794
        mmWidth = 33867
        BandType = 4
        LayerName = Foreground
      end
      object ppDBText10: TppDBText
        DesignLayer = ppDesignLayer1
        UserName = 'DBText12'
        HyperlinkEnabled = False
        Border.mmPadding = 0
        DataField = 'PesoAFaturar'
        DataPipeline = ppDB
        DisplayFormat = '#,0;-#,0'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDB'
        mmHeight = 3175
        mmLeft = 136058
        mmTop = 794
        mmWidth = 18252
        BandType = 4
        LayerName = Foreground
      end
      object ppDBText3: TppDBText
        DesignLayer = ppDesignLayer1
        UserName = 'DBText3'
        HyperlinkEnabled = False
        Border.mmPadding = 0
        DataField = 'PesoFaturado'
        DataPipeline = ppDB
        DisplayFormat = '#,0;-#,0'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDB'
        mmHeight = 3175
        mmLeft = 155898
        mmTop = 794
        mmWidth = 18252
        BandType = 4
        LayerName = Foreground
      end
      object ppDBText4: TppDBText
        DesignLayer = ppDesignLayer1
        UserName = 'DBText4'
        HyperlinkEnabled = False
        Border.mmPadding = 0
        DataField = 'PesoTotalDia'
        DataPipeline = ppDB
        DisplayFormat = '#,0;-#,0'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDB'
        mmHeight = 3175
        mmLeft = 175473
        mmTop = 794
        mmWidth = 18252
        BandType = 4
        LayerName = Foreground
      end
      object ppShape2: TppShape
        DesignLayer = ppDesignLayer1
        UserName = 'Shape2'
        Brush.Color = 14145495
        Pen.Style = psClear
        mmHeight = 5553
        mmLeft = 1854
        mmTop = 0
        mmWidth = 12437
        BandType = 4
        LayerName = Foreground
      end
      object ppDBText33: TppDBText
        DesignLayer = ppDesignLayer1
        UserName = 'DBText33'
        HyperlinkEnabled = False
        Border.mmPadding = 0
        DataField = 'PesoOrcamento'
        DataPipeline = ppDB
        DisplayFormat = '#,0;-#,0'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDB'
        mmHeight = 3175
        mmLeft = 75398
        mmTop = 794
        mmWidth = 18252
        BandType = 4
        LayerName = Foreground
      end
      object ppDBText40: TppDBText
        DesignLayer = ppDesignLayer1
        UserName = 'DBText40'
        HyperlinkEnabled = False
        Border.mmPadding = 0
        DataField = 'PesoNaoDatado'
        DataPipeline = ppDB
        DisplayFormat = '#,0;-#,0'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDB'
        mmHeight = 3175
        mmLeft = 115908
        mmTop = 794
        mmWidth = 18252
        BandType = 4
        LayerName = Foreground
      end
      object ppDBText45: TppDBText
        DesignLayer = ppDesignLayer1
        UserName = 'DBText45'
        HyperlinkEnabled = False
        Border.mmPadding = 0
        DataField = 'PesoArteBloqueada'
        DataPipeline = ppDB
        DisplayFormat = '#,0;-#,0'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDB'
        mmHeight = 3175
        mmLeft = 96046
        mmTop = 794
        mmWidth = 18256
        BandType = 4
        LayerName = Foreground
      end
    end
    object ppFooterBand16: TppFooterBand
      Background.Brush.Style = bsClear
      Border.mmPadding = 0
      mmBottomOffset = 0
      mmHeight = 6350
      mmPrintPosition = 0
      object ppLine144: TppLine
        DesignLayer = ppDesignLayer1
        UserName = 'Line2'
        Border.mmPadding = 0
        Pen.Width = 3
        Weight = 2.250000000000000000
        mmHeight = 2646
        mmLeft = 529
        mmTop = 0
        mmWidth = 195527
        BandType = 8
        LayerName = Foreground
      end
      object LBL_Status: TppLabel
        DesignLayer = ppDesignLayer1
        UserName = 'LBL_Status'
        HyperlinkEnabled = False
        Border.mmPadding = 0
        Caption = 'LBL_Status'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = [fsBold]
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        TextAlignment = taRightJustified
        Transparent = True
        mmHeight = 4191
        mmLeft = 176498
        mmTop = 1323
        mmWidth = 19558
        BandType = 8
        LayerName = Foreground
      end
    end
    object ppSummaryBand16: TppSummaryBand
      Background.Brush.Style = bsClear
      Border.mmPadding = 0
      mmBottomOffset = 0
      mmHeight = 0
      mmPrintPosition = 0
    end
    object ppGroup1: TppGroup
      BreakName = 'Gerente'
      DataPipeline = ppDB
      GroupFileSettings.NewFile = False
      GroupFileSettings.EmailFile = False
      KeepTogether = True
      OutlineSettings.CreateNode = True
      StartOnOddPage = False
      UserName = 'Group1'
      mmNewColumnThreshold = 0
      mmNewPageThreshold = 0
      DataPipelineName = 'ppDB'
      NewFile = False
      object ppGroupHeaderBand1: TppGroupHeaderBand
        Background.Brush.Style = bsClear
        Border.mmPadding = 0
        mmBottomOffset = 0
        mmHeight = 0
        mmPrintPosition = 0
      end
      object ppGroupFooterBand1: TppGroupFooterBand
        Background.Brush.Style = bsClear
        Border.mmPadding = 0
        HideWhenOneDetail = False
        mmBottomOffset = 0
        mmHeight = 0
        mmPrintPosition = 0
      end
    end
    object ppGroup4: TppGroup
      BreakName = 'Vendedor'
      DataPipeline = ppDB
      GroupFileSettings.NewFile = False
      GroupFileSettings.EmailFile = False
      KeepTogether = True
      OutlineSettings.CreateNode = True
      StartOnOddPage = False
      UserName = 'Group4'
      mmNewColumnThreshold = 0
      mmNewPageThreshold = 0
      DataPipelineName = 'ppDB'
      NewFile = False
      object ppGroupHeaderBand4: TppGroupHeaderBand
        Background.Brush.Style = bsClear
        Border.mmPadding = 0
        mmBottomOffset = 0
        mmHeight = 6879
        mmPrintPosition = 0
        object Linha_GerenciaXGerencia00: TppShape
          DesignLayer = ppDesignLayer1
          UserName = 'Shape36'
          mmHeight = 6344
          mmLeft = 1589
          mmTop = 0
          mmWidth = 193146
          BandType = 3
          GroupNo = 1
          LayerName = Foreground
        end
        object ppDBText5: TppDBText
          DesignLayer = ppDesignLayer1
          UserName = 'DBText5'
          HyperlinkEnabled = False
          Border.mmPadding = 0
          DataField = 'Vendedor_FK'
          DataPipeline = ppDB
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 10
          Font.Style = [fsBold]
          Transparent = True
          DataPipelineName = 'ppDB'
          mmHeight = 4233
          mmLeft = 4235
          mmTop = 794
          mmWidth = 12700
          BandType = 3
          GroupNo = 1
          LayerName = Foreground
        end
        object ppDBText6: TppDBText
          DesignLayer = ppDesignLayer1
          UserName = 'DBText6'
          HyperlinkEnabled = False
          Border.mmPadding = 0
          DataField = 'Vendedor'
          DataPipeline = ppDB
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 10
          Font.Style = [fsBold]
          Transparent = True
          DataPipelineName = 'ppDB'
          mmHeight = 4233
          mmLeft = 19051
          mmTop = 794
          mmWidth = 79111
          BandType = 3
          GroupNo = 1
          LayerName = Foreground
        end
        object ppDBText7: TppDBText
          DesignLayer = ppDesignLayer1
          UserName = 'DBText7'
          HyperlinkEnabled = False
          Border.mmPadding = 0
          DataField = 'Gerente'
          DataPipeline = ppDB
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 10
          Font.Style = [fsBold]
          Transparent = True
          DataPipelineName = 'ppDB'
          mmHeight = 4498
          mmLeft = 100808
          mmTop = 794
          mmWidth = 90223
          BandType = 3
          GroupNo = 1
          LayerName = Foreground
        end
      end
      object ppGroupFooterBand4: TppGroupFooterBand
        Background.Brush.Style = bsClear
        Border.mmPadding = 0
        HideWhenOneDetail = False
        mmBottomOffset = 0
        mmHeight = 0
        mmPrintPosition = 0
      end
    end
    object ppGroup2: TppGroup
      BreakName = 'AnoBomix'
      DataPipeline = ppDB
      GroupFileSettings.NewFile = False
      GroupFileSettings.EmailFile = False
      KeepTogether = True
      OutlineSettings.CreateNode = True
      StartOnOddPage = False
      UserName = 'Group2'
      mmNewColumnThreshold = 0
      mmNewPageThreshold = 0
      DataPipelineName = 'ppDB'
      NewFile = False
      object ppGroupHeaderBand2: TppGroupHeaderBand
        Background.Brush.Style = bsClear
        Border.mmPadding = 0
        mmBottomOffset = 0
        mmHeight = 0
        mmPrintPosition = 0
        object ppDBText9: TppDBText
          DesignLayer = ppDesignLayer1
          UserName = 'DBText9'
          HyperlinkEnabled = False
          Border.mmPadding = 0
          DataField = 'AnoBomix'
          DataPipeline = ppDB
          Font.Charset = ANSI_CHARSET
          Font.Color = clWhite
          Font.Name = 'Arial'
          Font.Size = 10
          Font.Style = [fsBold]
          TextAlignment = taCentered
          Transparent = True
          DataPipelineName = 'ppDB'
          mmHeight = 4763
          mmLeft = 4233
          mmTop = 0
          mmWidth = 14288
          BandType = 3
          GroupNo = 2
          LayerName = Foreground
        end
        object ppDBText11: TppDBText
          DesignLayer = ppDesignLayer1
          UserName = 'DBText11'
          HyperlinkEnabled = False
          Border.mmPadding = 0
          DataField = 'MesNomeBomix'
          DataPipeline = ppDB
          Font.Charset = ANSI_CHARSET
          Font.Color = clWhite
          Font.Name = 'Arial'
          Font.Size = 10
          Font.Style = [fsBold]
          Transparent = True
          DataPipelineName = 'ppDB'
          mmHeight = 4763
          mmLeft = 23283
          mmTop = 0
          mmWidth = 40746
          BandType = 3
          GroupNo = 2
          LayerName = Foreground
        end
      end
      object ppGroupFooterBand2: TppGroupFooterBand
        Background.Brush.Style = bsClear
        Border.mmPadding = 0
        HideWhenOneDetail = False
        mmBottomOffset = 0
        mmHeight = 0
        mmPrintPosition = 0
      end
    end
    object ppGroup3: TppGroup
      BreakName = 'MesNomeBomix'
      DataPipeline = ppDB
      GroupFileSettings.NewFile = False
      GroupFileSettings.EmailFile = False
      KeepTogether = True
      OutlineSettings.CreateNode = True
      StartOnOddPage = False
      UserName = 'Group3'
      mmNewColumnThreshold = 0
      mmNewPageThreshold = 0
      DataPipelineName = 'ppDB'
      NewFile = False
      object ppGroupHeaderBand3: TppGroupHeaderBand
        Background.Brush.Style = bsClear
        Border.mmPadding = 0
        mmBottomOffset = 0
        mmHeight = 6085
        mmPrintPosition = 0
        object ppShape1: TppShape
          DesignLayer = ppDesignLayer1
          UserName = 'Shape1'
          Brush.Color = clGray
          mmHeight = 6085
          mmLeft = 1853
          mmTop = 0
          mmWidth = 192882
          BandType = 3
          GroupNo = 3
          LayerName = Foreground
        end
        object ppDBText8: TppDBText
          DesignLayer = ppDesignLayer1
          UserName = 'DBText8'
          HyperlinkEnabled = False
          Border.mmPadding = 0
          DataField = 'AnoBomix'
          DataPipeline = ppDB
          Font.Charset = ANSI_CHARSET
          Font.Color = clWhite
          Font.Name = 'Arial'
          Font.Size = 10
          Font.Style = [fsBold]
          TextAlignment = taCentered
          Transparent = True
          DataPipelineName = 'ppDB'
          mmHeight = 4763
          mmLeft = 4235
          mmTop = 792
          mmWidth = 14288
          BandType = 3
          GroupNo = 2
          LayerName = Foreground
        end
        object ppLabel3: TppLabel
          DesignLayer = ppDesignLayer1
          UserName = 'Label3'
          HyperlinkEnabled = False
          AutoSize = False
          Border.mmPadding = 0
          Caption = 'A Faturar'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWhite
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
          FormFieldSettings.FormFieldType = fftNone
          TextAlignment = taRightJustified
          Transparent = True
          mmHeight = 4233
          mmLeft = 132834
          mmTop = 792
          mmWidth = 20902
          BandType = 3
          GroupNo = 3
          LayerName = Foreground
        end
        object ppLabel1: TppLabel
          DesignLayer = ppDesignLayer1
          UserName = 'LBL_QTDGerenciaXGerencia1'
          HyperlinkEnabled = False
          AutoSize = False
          Border.mmPadding = 0
          Caption = 'Faturado'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWhite
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
          FormFieldSettings.FormFieldType = fftNone
          TextAlignment = taRightJustified
          Transparent = True
          mmHeight = 4233
          mmLeft = 154264
          mmTop = 792
          mmWidth = 20889
          BandType = 3
          GroupNo = 3
          LayerName = Foreground
        end
        object ppLabel2: TppLabel
          DesignLayer = ppDesignLayer1
          UserName = 'Label2'
          HyperlinkEnabled = False
          AutoSize = False
          Border.mmPadding = 0
          Caption = 'Total'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWhite
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
          FormFieldSettings.FormFieldType = fftNone
          TextAlignment = taRightJustified
          Transparent = True
          mmHeight = 4233
          mmLeft = 171719
          mmTop = 792
          mmWidth = 20897
          BandType = 3
          GroupNo = 3
          LayerName = Foreground
        end
        object ppDBText1: TppDBText
          DesignLayer = ppDesignLayer1
          UserName = 'DBText1'
          HyperlinkEnabled = False
          Border.mmPadding = 0
          DataField = 'MesNomeBomix'
          DataPipeline = ppDB
          Font.Charset = ANSI_CHARSET
          Font.Color = clWhite
          Font.Name = 'Arial'
          Font.Size = 10
          Font.Style = [fsBold]
          Transparent = True
          DataPipelineName = 'ppDB'
          mmHeight = 4763
          mmLeft = 23285
          mmTop = 792
          mmWidth = 40746
          BandType = 3
          GroupNo = 3
          LayerName = Foreground
        end
        object ppLabel13: TppLabel
          DesignLayer = ppDesignLayer1
          UserName = 'Label13'
          HyperlinkEnabled = False
          AutoSize = False
          Border.mmPadding = 0
          Caption = 'Or'#231'amento'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWhite
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
          FormFieldSettings.FormFieldType = fftNone
          TextAlignment = taRightJustified
          Transparent = True
          mmHeight = 4233
          mmLeft = 73543
          mmTop = 794
          mmWidth = 20902
          BandType = 3
          GroupNo = 3
          LayerName = Foreground
        end
        object ppLabel21: TppLabel
          DesignLayer = ppDesignLayer1
          UserName = 'Label21'
          HyperlinkEnabled = False
          AutoSize = False
          Border.mmPadding = 0
          Caption = 'N'#227'o Datado'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWhite
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
          FormFieldSettings.FormFieldType = fftNone
          TextAlignment = taRightJustified
          Transparent = True
          mmHeight = 4233
          mmLeft = 113258
          mmTop = 794
          mmWidth = 20902
          BandType = 3
          GroupNo = 3
          LayerName = Foreground
        end
        object ppLabel25: TppLabel
          DesignLayer = ppDesignLayer1
          UserName = 'Label25'
          HyperlinkEnabled = False
          AutoSize = False
          Border.mmPadding = 0
          Caption = 'Arte Bloq.'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWhite
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
          FormFieldSettings.FormFieldType = fftNone
          TextAlignment = taRightJustified
          Transparent = True
          mmHeight = 4233
          mmLeft = 92334
          mmTop = 794
          mmWidth = 20902
          BandType = 3
          GroupNo = 3
          LayerName = Foreground
        end
      end
      object ppGroupFooterBand3: TppGroupFooterBand
        Background.Brush.Style = bsClear
        Border.mmPadding = 0
        HideWhenOneDetail = False
        mmBottomOffset = 0
        mmHeight = 6879
        mmPrintPosition = 0
        object ppShape3: TppShape
          DesignLayer = ppDesignLayer1
          UserName = 'Shape3'
          mmHeight = 5556
          mmLeft = 11911
          mmTop = 531
          mmWidth = 182833
          BandType = 5
          GroupNo = 3
          LayerName = Foreground
        end
        object TXT_PesoTotalComissao1: TppDBCalc
          DesignLayer = ppDesignLayer1
          UserName = 'TXT_PesoTotalComissao1'
          HyperlinkEnabled = False
          Border.mmPadding = 0
          DataField = 'PesoAFaturar'
          DataPipeline = ppDB
          DisplayFormat = '#,0;-#,0'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = []
          ParentDataPipeline = False
          ResetGroup = ppGroup3
          TextAlignment = taRightJustified
          Transparent = True
          DataPipelineName = 'ppDB'
          mmHeight = 3440
          mmLeft = 136015
          mmTop = 1588
          mmWidth = 18252
          BandType = 5
          GroupNo = 3
          LayerName = Foreground
        end
        object ppLabel15: TppLabel
          DesignLayer = ppDesignLayer1
          UserName = 'Label15'
          HyperlinkEnabled = False
          AutoSize = False
          Border.mmPadding = 0
          Caption = 'Total'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = [fsBold]
          FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
          FormFieldSettings.FormFieldType = fftNone
          TextAlignment = taRightJustified
          Transparent = True
          mmHeight = 4233
          mmLeft = 13229
          mmTop = 1058
          mmWidth = 13229
          BandType = 5
          GroupNo = 3
          LayerName = Foreground
        end
        object ppDBCalc1: TppDBCalc
          DesignLayer = ppDesignLayer1
          UserName = 'DBCalc1'
          HyperlinkEnabled = False
          Border.mmPadding = 0
          DataField = 'PesoFaturado'
          DataPipeline = ppDB
          DisplayFormat = '#,0;-#,0'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = []
          ParentDataPipeline = False
          ResetGroup = ppGroup3
          TextAlignment = taRightJustified
          Transparent = True
          DataPipelineName = 'ppDB'
          mmHeight = 3440
          mmLeft = 155854
          mmTop = 1588
          mmWidth = 18252
          BandType = 5
          GroupNo = 3
          LayerName = Foreground
        end
        object ppDBCalc2: TppDBCalc
          DesignLayer = ppDesignLayer1
          UserName = 'DBCalc2'
          HyperlinkEnabled = False
          Border.mmPadding = 0
          DataField = 'PesoTotalDia'
          DataPipeline = ppDB
          DisplayFormat = '#,0;-#,0'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = []
          ParentDataPipeline = False
          ResetGroup = ppGroup3
          TextAlignment = taRightJustified
          Transparent = True
          DataPipelineName = 'ppDB'
          mmHeight = 3440
          mmLeft = 175429
          mmTop = 1588
          mmWidth = 18252
          BandType = 5
          GroupNo = 3
          LayerName = Foreground
        end
        object ppDBText12: TppDBText
          DesignLayer = ppDesignLayer1
          UserName = 'DBText10'
          HyperlinkEnabled = False
          Border.mmPadding = 0
          DataField = 'AnoBomix'
          DataPipeline = ppDB
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = [fsBold]
          TextAlignment = taCentered
          Transparent = True
          DataPipelineName = 'ppDB'
          mmHeight = 3968
          mmLeft = 27684
          mmTop = 1058
          mmWidth = 10048
          BandType = 5
          GroupNo = 3
          LayerName = Foreground
        end
        object ppDBText13: TppDBText
          DesignLayer = ppDesignLayer1
          UserName = 'DBText101'
          HyperlinkEnabled = False
          Border.mmPadding = 0
          DataField = 'MesNomeBomix'
          DataPipeline = ppDB
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = [fsBold]
          Transparent = True
          DataPipelineName = 'ppDB'
          mmHeight = 3969
          mmLeft = 39344
          mmTop = 1059
          mmWidth = 32808
          BandType = 5
          GroupNo = 3
          LayerName = Foreground
        end
        object ppDBCalc14: TppDBCalc
          DesignLayer = ppDesignLayer1
          UserName = 'DBCalc14'
          HyperlinkEnabled = False
          Border.mmPadding = 0
          DataField = 'PesoNaoDatado'
          DataPipeline = ppDB
          DisplayFormat = '#,0;-#,0'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = []
          ParentDataPipeline = False
          ResetGroup = ppGroup3
          TextAlignment = taRightJustified
          Transparent = True
          DataPipelineName = 'ppDB'
          mmHeight = 3440
          mmLeft = 115908
          mmTop = 1586
          mmWidth = 18252
          BandType = 5
          GroupNo = 3
          LayerName = Foreground
        end
        object ppDBCalc15: TppDBCalc
          DesignLayer = ppDesignLayer1
          UserName = 'DBCalc15'
          HyperlinkEnabled = False
          Border.mmPadding = 0
          DataField = 'PesoOrcamento'
          DataPipeline = ppDB
          DisplayFormat = '#,0;-#,0'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = []
          ParentDataPipeline = False
          ResetGroup = ppGroup3
          TextAlignment = taRightJustified
          Transparent = True
          DataPipelineName = 'ppDB'
          mmHeight = 3440
          mmLeft = 75399
          mmTop = 1588
          mmWidth = 18252
          BandType = 5
          GroupNo = 3
          LayerName = Foreground
        end
        object ppDBCalc20: TppDBCalc
          DesignLayer = ppDesignLayer1
          UserName = 'DBCalc20'
          HyperlinkEnabled = False
          Border.mmPadding = 0
          DataField = 'PesoArteBloqueada'
          DataPipeline = ppDB
          DisplayFormat = '#,0;-#,0'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = []
          ParentDataPipeline = False
          ResetGroup = ppGroup3
          TextAlignment = taRightJustified
          Transparent = True
          DataPipelineName = 'ppDB'
          mmHeight = 3440
          mmLeft = 96041
          mmTop = 1588
          mmWidth = 18256
          BandType = 5
          GroupNo = 3
          LayerName = Foreground
        end
      end
    end
    object raCodeModule1: TraCodeModule
    end
    object ppDesignLayers1: TppDesignLayers
      object ppDesignLayer1: TppDesignLayer
        UserName = 'Foreground'
        LayerType = ltBanded
        Index = 0
      end
    end
    object ppParameterList16: TppParameterList
    end
  end
  object ppDB: TppDBPipeline
    DataSource = DS
    CloseDataSource = True
    UserName = 'DB'
    Left = 740
    Top = 84
  end
  object ppDBPipeline1: TppDBPipeline
    DataSource = DS_DiarioGerencia
    CloseDataSource = True
    UserName = 'DB1'
    Left = 748
    Top = 140
  end
  object ppReport1: TppReport
    AutoStop = False
    ColumnPositions.Strings = (
      '6350'
      '105001')
    DataPipeline = ppDBPipeline1
    PassSetting = psTwoPass
    PrinterSetup.BinName = 'Default'
    PrinterSetup.DocumentName = 'Report'
    PrinterSetup.Duplex = dpNone
    PrinterSetup.PaperName = 'A4'
    PrinterSetup.PrinterName = 'Default'
    PrinterSetup.SaveDeviceSettings = False
    PrinterSetup.mmMarginBottom = 6350
    PrinterSetup.mmMarginLeft = 6350
    PrinterSetup.mmMarginRight = 6350
    PrinterSetup.mmMarginTop = 6350
    PrinterSetup.mmPaperHeight = 297000
    PrinterSetup.mmPaperWidth = 210000
    PrinterSetup.PaperSize = 9
    AllowPrintToArchive = True
    AllowPrintToFile = True
    ArchiveFileName = '($MyDocuments)\ReportArchive.raf'
    DeviceType = 'Screen'
    DefaultFileDeviceType = 'XLSData'
    EmailSettings.ReportFormat = 'PDF'
    LanguageID = 'Default'
    OpenFile = True
    OutlineSettings.CreateNode = True
    OutlineSettings.CreatePageNodes = True
    OutlineSettings.Enabled = True
    OutlineSettings.Visible = True
    ThumbnailSettings.Enabled = True
    ThumbnailSettings.Visible = True
    ThumbnailSettings.DeadSpace = 30
    ThumbnailSettings.PageHighlight.Width = 3
    PDFSettings.EmbedFontOptions = [efUseSubset]
    PDFSettings.EncryptSettings.AllowCopy = True
    PDFSettings.EncryptSettings.AllowInteract = True
    PDFSettings.EncryptSettings.AllowModify = True
    PDFSettings.EncryptSettings.AllowPrint = True
    PDFSettings.EncryptSettings.AllowExtract = True
    PDFSettings.EncryptSettings.AllowAssemble = True
    PDFSettings.EncryptSettings.AllowQualityPrint = True
    PDFSettings.EncryptSettings.Enabled = False
    PDFSettings.EncryptSettings.KeyLength = kl40Bit
    PDFSettings.EncryptSettings.EncryptionType = etRC4
    PDFSettings.FontEncoding = feAnsi
    PDFSettings.ImageCompressionLevel = 25
    PDFSettings.PDFAFormat = pafNone
    PreviewFormSettings.PageBorder.mmPadding = 0
    PreviewFormSettings.WindowState = wsMaximized
    PreviewFormSettings.ZoomSetting = zs100Percent
    RTFSettings.DefaultFont.Charset = DEFAULT_CHARSET
    RTFSettings.DefaultFont.Color = clWindowText
    RTFSettings.DefaultFont.Height = -13
    RTFSettings.DefaultFont.Name = 'Arial'
    RTFSettings.DefaultFont.Style = []
    TextFileName = '($MyDocuments)\Report.xls'
    TextSearchSettings.DefaultString = '<Texto a localizar>'
    TextSearchSettings.Enabled = True
    XLSSettings.AppName = 'ReportBuilder'
    XLSSettings.Author = 'ReportBuilder'
    XLSSettings.Subject = 'Report'
    XLSSettings.Title = 'Report'
    XLSSettings.WorksheetName = 'Report'
    Left = 778
    Top = 140
    Version = '19.03'
    mmColumnWidth = 98651
    DataPipelineName = 'ppDBPipeline1'
    object ppHeaderBand1: TppHeaderBand
      Background.Brush.Style = bsClear
      Border.mmPadding = 0
      PrintHeight = phDynamic
      mmBottomOffset = 0
      mmHeight = 13758
      mmPrintPosition = 0
      object ppLine1: TppLine
        DesignLayer = ppDesignLayer2
        UserName = 'Line1'
        Border.mmPadding = 0
        Pen.Width = 3
        Weight = 2.250000000000000000
        mmHeight = 2646
        mmLeft = 1324
        mmTop = 10850
        mmWidth = 193411
        BandType = 0
        LayerName = Foreground1
      end
      object ppImage1: TppImage
        DesignLayer = ppDesignLayer2
        UserName = 'ppIMG_Logo'
        AlignHorizontal = ahCenter
        AlignVertical = avCenter
        MaintainAspectRatio = False
        Stretch = True
        Border.mmPadding = 0
        Picture.Data = {
          
          FFFF}
        mmHeight = 6879
        mmLeft = 1853
        mmTop = 3442
        mmWidth = 23813
        BandType = 0
        LayerName = Foreground1
      end
      object ppLabel4: TppLabel
        DesignLayer = ppDesignLayer2
        UserName = 'ppLBL_TituloRelatorio'
        HyperlinkEnabled = False
        Border.mmPadding = 0
        Caption = 'Relat'#243'rio de Tonelagem Ger'#234'ncia'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 18
        Font.Style = [fsBold]
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        Transparent = True
        mmHeight = 7673
        mmLeft = 28310
        mmTop = 3440
        mmWidth = 101072
        BandType = 0
        LayerName = Foreground1
      end
    end
    object ppDetailBand1: TppDetailBand
      Background1.Brush.Style = bsClear
      Background2.Brush.Style = bsClear
      Border.mmPadding = 0
      mmBottomOffset = 0
      mmHeight = 5027
      mmPrintPosition = 0
      object ppDBText14: TppDBText
        DesignLayer = ppDesignLayer2
        UserName = 'DBText138'
        HyperlinkEnabled = False
        Border.mmPadding = 0
        DataField = 'Dia'
        DataPipeline = ppDBPipeline1
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 9
        Font.Style = [fsBold]
        TextAlignment = taCentered
        Transparent = True
        DataPipelineName = 'ppDBPipeline1'
        mmHeight = 3440
        mmLeft = 18785
        mmTop = 794
        mmWidth = 17198
        BandType = 4
        LayerName = Foreground1
      end
      object ppDBCalc3: TppDBCalc
        DesignLayer = ppDesignLayer2
        UserName = 'DBCalc7'
        Border.mmPadding = 0
        DataField = 'Dia'
        DataPipeline = ppDBPipeline1
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 9
        Font.Style = []
        ResetGroup = ppGroup8
        TextAlignment = taRightJustified
        Transparent = True
        DBCalcType = dcCount
        DataPipelineName = 'ppDBPipeline1'
        mmHeight = 3175
        mmLeft = 13229
        mmTop = 794
        mmWidth = 5821
        BandType = 4
        LayerName = Foreground1
      end
      object ppDBText15: TppDBText
        DesignLayer = ppDesignLayer2
        UserName = 'DBText2'
        HyperlinkEnabled = False
        Border.mmPadding = 0
        DataField = 'DiaDaSemana'
        DataPipeline = ppDBPipeline1
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 9
        Font.Style = []
        Transparent = True
        DataPipelineName = 'ppDBPipeline1'
        mmHeight = 3175
        mmLeft = 37306
        mmTop = 794
        mmWidth = 33867
        BandType = 4
        LayerName = Foreground1
      end
      object ppDBText16: TppDBText
        DesignLayer = ppDesignLayer2
        UserName = 'DBText12'
        HyperlinkEnabled = False
        Border.mmPadding = 0
        DataField = 'PesoAFaturar'
        DataPipeline = ppDBPipeline1
        DisplayFormat = '#,0;-#,0'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 9
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDBPipeline1'
        mmHeight = 3175
        mmLeft = 132348
        mmTop = 794
        mmWidth = 19047
        BandType = 4
        LayerName = Foreground1
      end
      object ppDBText17: TppDBText
        DesignLayer = ppDesignLayer2
        UserName = 'DBText3'
        HyperlinkEnabled = False
        Border.mmPadding = 0
        DataField = 'PesoFaturado'
        DataPipeline = ppDBPipeline1
        DisplayFormat = '#,0;-#,0'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 9
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDBPipeline1'
        mmHeight = 3175
        mmLeft = 152453
        mmTop = 794
        mmWidth = 19047
        BandType = 4
        LayerName = Foreground1
      end
      object ppDBText18: TppDBText
        DesignLayer = ppDesignLayer2
        UserName = 'DBText4'
        HyperlinkEnabled = False
        Border.mmPadding = 0
        DataField = 'PesoTotalDia'
        DataPipeline = ppDBPipeline1
        DisplayFormat = '#,0;-#,0'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 9
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDBPipeline1'
        mmHeight = 3175
        mmLeft = 172558
        mmTop = 794
        mmWidth = 19047
        BandType = 4
        LayerName = Foreground1
      end
      object ppShape4: TppShape
        DesignLayer = ppDesignLayer2
        UserName = 'Shape2'
        Brush.Color = 14145495
        Pen.Style = psClear
        mmHeight = 5553
        mmLeft = 1854
        mmTop = 0
        mmWidth = 8996
        BandType = 4
        LayerName = Foreground1
      end
      object ppDBText41: TppDBText
        DesignLayer = ppDesignLayer2
        UserName = 'DBText41'
        HyperlinkEnabled = False
        Border.mmPadding = 0
        DataField = 'PesoArteBloqueada'
        DataPipeline = ppDBPipeline1
        DisplayFormat = '#,0;-#,0'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 9
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDBPipeline1'
        mmHeight = 3175
        mmLeft = 91565
        mmTop = 794
        mmWidth = 19047
        BandType = 4
        LayerName = Foreground1
      end
      object ppDBText42: TppDBText
        DesignLayer = ppDesignLayer2
        UserName = 'DBText42'
        HyperlinkEnabled = False
        Border.mmPadding = 0
        DataField = 'PesoNaoDatado'
        DataPipeline = ppDBPipeline1
        DisplayFormat = '#,0;-#,0'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 9
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDBPipeline1'
        mmHeight = 3175
        mmLeft = 112199
        mmTop = 794
        mmWidth = 19047
        BandType = 4
        LayerName = Foreground1
      end
      object ppDBText43: TppDBText
        DesignLayer = ppDesignLayer2
        UserName = 'DBText43'
        HyperlinkEnabled = False
        Border.mmPadding = 0
        DataField = 'PesoOrcamento'
        DataPipeline = ppDBPipeline1
        DisplayFormat = '#,0;-#,0'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 9
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDBPipeline1'
        mmHeight = 3175
        mmLeft = 71703
        mmTop = 794
        mmWidth = 19050
        BandType = 4
        LayerName = Foreground1
      end
    end
    object ppFooterBand1: TppFooterBand
      Background.Brush.Style = bsClear
      Border.mmPadding = 0
      mmBottomOffset = 0
      mmHeight = 6350
      mmPrintPosition = 0
      object ppLine2: TppLine
        DesignLayer = ppDesignLayer2
        UserName = 'Line2'
        Border.mmPadding = 0
        Pen.Width = 3
        Weight = 2.250000000000000000
        mmHeight = 2646
        mmLeft = 529
        mmTop = 0
        mmWidth = 195527
        BandType = 8
        LayerName = Foreground1
      end
      object ppLabel5: TppLabel
        DesignLayer = ppDesignLayer2
        UserName = 'LBL_Status'
        HyperlinkEnabled = False
        Border.mmPadding = 0
        Caption = 'LBL_Status'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = [fsBold]
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        TextAlignment = taRightJustified
        Transparent = True
        mmHeight = 4191
        mmLeft = 176498
        mmTop = 1323
        mmWidth = 19558
        BandType = 8
        LayerName = Foreground1
      end
    end
    object ppSummaryBand1: TppSummaryBand
      Background.Brush.Style = bsClear
      Border.mmPadding = 0
      mmBottomOffset = 0
      mmHeight = 0
      mmPrintPosition = 0
    end
    object ppGroup5: TppGroup
      BreakName = 'Gerente'
      DataPipeline = ppDBPipeline1
      GroupFileSettings.NewFile = False
      GroupFileSettings.EmailFile = False
      KeepTogether = True
      OutlineSettings.CreateNode = True
      StartOnOddPage = False
      UserName = 'Group1'
      mmNewColumnThreshold = 0
      mmNewPageThreshold = 0
      DataPipelineName = 'ppDBPipeline1'
      NewFile = False
      object ppGroupHeaderBand5: TppGroupHeaderBand
        Background.Brush.Style = bsClear
        Border.mmPadding = 0
        mmBottomOffset = 0
        mmHeight = 0
        mmPrintPosition = 0
      end
      object ppGroupFooterBand5: TppGroupFooterBand
        Background.Brush.Style = bsClear
        Border.mmPadding = 0
        HideWhenOneDetail = False
        mmBottomOffset = 0
        mmHeight = 6350
        mmPrintPosition = 0
      end
    end
    object ppGroup6: TppGroup
      BreakName = 'Gerente'
      DataPipeline = ppDBPipeline1
      GroupFileSettings.NewFile = False
      GroupFileSettings.EmailFile = False
      KeepTogether = True
      OutlineSettings.CreateNode = True
      StartOnOddPage = False
      UserName = 'Group4'
      mmNewColumnThreshold = 0
      mmNewPageThreshold = 0
      DataPipelineName = 'ppDBPipeline1'
      NewFile = False
      object ppGroupHeaderBand6: TppGroupHeaderBand
        Background.Brush.Style = bsClear
        Border.mmPadding = 0
        mmBottomOffset = 0
        mmHeight = 6879
        mmPrintPosition = 0
        object ppShape5: TppShape
          DesignLayer = ppDesignLayer2
          UserName = 'Shape36'
          mmHeight = 6344
          mmLeft = 1589
          mmTop = 0
          mmWidth = 193146
          BandType = 3
          GroupNo = 1
          LayerName = Foreground1
        end
        object ppDBText21: TppDBText
          DesignLayer = ppDesignLayer2
          UserName = 'DBText7'
          HyperlinkEnabled = False
          Border.mmPadding = 0
          DataField = 'Gerente'
          DataPipeline = ppDBPipeline1
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 10
          Font.Style = [fsBold]
          Transparent = True
          DataPipelineName = 'ppDBPipeline1'
          mmHeight = 4498
          mmLeft = 3703
          mmTop = 530
          mmWidth = 90223
          BandType = 3
          GroupNo = 1
          LayerName = Foreground1
        end
      end
      object ppGroupFooterBand6: TppGroupFooterBand
        Background.Brush.Style = bsClear
        Border.mmPadding = 0
        HideWhenOneDetail = False
        mmBottomOffset = 0
        mmHeight = 6879
        mmPrintPosition = 0
      end
    end
    object ppGroup7: TppGroup
      BreakName = 'AnoBomix'
      DataPipeline = ppDBPipeline1
      GroupFileSettings.NewFile = False
      GroupFileSettings.EmailFile = False
      KeepTogether = True
      OutlineSettings.CreateNode = True
      StartOnOddPage = False
      UserName = 'Group2'
      mmNewColumnThreshold = 0
      mmNewPageThreshold = 0
      DataPipelineName = 'ppDBPipeline1'
      NewFile = False
      object ppGroupHeaderBand7: TppGroupHeaderBand
        Background.Brush.Style = bsClear
        Border.mmPadding = 0
        mmBottomOffset = 0
        mmHeight = 0
        mmPrintPosition = 0
        object ppDBText22: TppDBText
          DesignLayer = ppDesignLayer2
          UserName = 'DBText9'
          HyperlinkEnabled = False
          Border.mmPadding = 0
          DataField = 'AnoBomix'
          DataPipeline = ppDBPipeline1
          Font.Charset = ANSI_CHARSET
          Font.Color = clWhite
          Font.Name = 'Arial'
          Font.Size = 10
          Font.Style = [fsBold]
          TextAlignment = taCentered
          Transparent = True
          DataPipelineName = 'ppDBPipeline1'
          mmHeight = 4763
          mmLeft = 4233
          mmTop = 0
          mmWidth = 14288
          BandType = 3
          GroupNo = 2
          LayerName = Foreground1
        end
        object ppDBText23: TppDBText
          DesignLayer = ppDesignLayer2
          UserName = 'DBText11'
          HyperlinkEnabled = False
          Border.mmPadding = 0
          DataField = 'MesNomeBomix'
          DataPipeline = ppDBPipeline1
          Font.Charset = ANSI_CHARSET
          Font.Color = clWhite
          Font.Name = 'Arial'
          Font.Size = 10
          Font.Style = [fsBold]
          Transparent = True
          DataPipelineName = 'ppDBPipeline1'
          mmHeight = 4763
          mmLeft = 23283
          mmTop = 0
          mmWidth = 40746
          BandType = 3
          GroupNo = 2
          LayerName = Foreground1
        end
      end
      object ppGroupFooterBand7: TppGroupFooterBand
        Background.Brush.Style = bsClear
        Border.mmPadding = 0
        HideWhenOneDetail = False
        mmBottomOffset = 0
        mmHeight = 0
        mmPrintPosition = 0
      end
    end
    object ppGroup8: TppGroup
      BreakName = 'MesNomeBomix'
      DataPipeline = ppDBPipeline1
      GroupFileSettings.NewFile = False
      GroupFileSettings.EmailFile = False
      KeepTogether = True
      OutlineSettings.CreateNode = True
      StartOnOddPage = False
      UserName = 'Group3'
      mmNewColumnThreshold = 0
      mmNewPageThreshold = 0
      DataPipelineName = 'ppDBPipeline1'
      NewFile = False
      object ppGroupHeaderBand8: TppGroupHeaderBand
        Background.Brush.Style = bsClear
        Border.mmPadding = 0
        mmBottomOffset = 0
        mmHeight = 6085
        mmPrintPosition = 0
        object ppShape6: TppShape
          DesignLayer = ppDesignLayer2
          UserName = 'Shape1'
          Brush.Color = clGray
          mmHeight = 6085
          mmLeft = 1853
          mmTop = 0
          mmWidth = 192882
          BandType = 3
          GroupNo = 3
          LayerName = Foreground1
        end
        object ppDBText24: TppDBText
          DesignLayer = ppDesignLayer2
          UserName = 'DBText8'
          HyperlinkEnabled = False
          Border.mmPadding = 0
          DataField = 'AnoBomix'
          DataPipeline = ppDBPipeline1
          Font.Charset = ANSI_CHARSET
          Font.Color = clWhite
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          TextAlignment = taCentered
          Transparent = True
          DataPipelineName = 'ppDBPipeline1'
          mmHeight = 4763
          mmLeft = 4235
          mmTop = 792
          mmWidth = 14288
          BandType = 3
          GroupNo = 2
          LayerName = Foreground1
        end
        object ppLabel7: TppLabel
          DesignLayer = ppDesignLayer2
          UserName = 'Label3'
          HyperlinkEnabled = False
          AutoSize = False
          Border.mmPadding = 0
          Caption = 'A Faturar'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWhite
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
          FormFieldSettings.FormFieldType = fftNone
          TextAlignment = taRightJustified
          Transparent = True
          mmHeight = 4233
          mmLeft = 131244
          mmTop = 792
          mmWidth = 20902
          BandType = 3
          GroupNo = 3
          LayerName = Foreground1
        end
        object ppLabel8: TppLabel
          DesignLayer = ppDesignLayer2
          UserName = 'LBL_QTDGerenciaXGerencia1'
          HyperlinkEnabled = False
          AutoSize = False
          Border.mmPadding = 0
          Caption = 'Faturado'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWhite
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
          FormFieldSettings.FormFieldType = fftNone
          TextAlignment = taRightJustified
          Transparent = True
          mmHeight = 4233
          mmLeft = 150819
          mmTop = 1057
          mmWidth = 20889
          BandType = 3
          GroupNo = 3
          LayerName = Foreground1
        end
        object ppLabel9: TppLabel
          DesignLayer = ppDesignLayer2
          UserName = 'Label2'
          HyperlinkEnabled = False
          AutoSize = False
          Border.mmPadding = 0
          Caption = 'Total'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWhite
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
          FormFieldSettings.FormFieldType = fftNone
          TextAlignment = taRightJustified
          Transparent = True
          mmHeight = 4233
          mmLeft = 169864
          mmTop = 1057
          mmWidth = 20897
          BandType = 3
          GroupNo = 3
          LayerName = Foreground1
        end
        object ppDBText25: TppDBText
          DesignLayer = ppDesignLayer2
          UserName = 'DBText1'
          HyperlinkEnabled = False
          Border.mmPadding = 0
          DataField = 'MesNomeBomix'
          DataPipeline = ppDBPipeline1
          Font.Charset = ANSI_CHARSET
          Font.Color = clWhite
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          Transparent = True
          DataPipelineName = 'ppDBPipeline1'
          mmHeight = 4763
          mmLeft = 23285
          mmTop = 792
          mmWidth = 40746
          BandType = 3
          GroupNo = 3
          LayerName = Foreground1
        end
        object ppLabel6: TppLabel
          DesignLayer = ppDesignLayer2
          UserName = 'Label6'
          HyperlinkEnabled = False
          AutoSize = False
          Border.mmPadding = 0
          Caption = 'Or'#231'amento'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWhite
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
          FormFieldSettings.FormFieldType = fftNone
          TextAlignment = taRightJustified
          Transparent = True
          mmHeight = 4233
          mmLeft = 70377
          mmTop = 1057
          mmWidth = 20902
          BandType = 3
          GroupNo = 3
          LayerName = Foreground1
        end
        object ppLabel22: TppLabel
          DesignLayer = ppDesignLayer2
          UserName = 'Label22'
          HyperlinkEnabled = False
          AutoSize = False
          Border.mmPadding = 0
          Caption = 'N'#227'o Datado'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWhite
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
          FormFieldSettings.FormFieldType = fftNone
          TextAlignment = taRightJustified
          Transparent = True
          mmHeight = 4233
          mmLeft = 111404
          mmTop = 1059
          mmWidth = 20902
          BandType = 3
          GroupNo = 3
          LayerName = Foreground1
        end
        object ppLabel23: TppLabel
          DesignLayer = ppDesignLayer2
          UserName = 'Label23'
          HyperlinkEnabled = False
          AutoSize = False
          Border.mmPadding = 0
          Caption = 'Arte. Bloq.'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWhite
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
          FormFieldSettings.FormFieldType = fftNone
          TextAlignment = taRightJustified
          Transparent = True
          mmHeight = 4233
          mmLeft = 90785
          mmTop = 1057
          mmWidth = 20902
          BandType = 3
          GroupNo = 3
          LayerName = Foreground1
        end
      end
      object ppGroupFooterBand8: TppGroupFooterBand
        Background.Brush.Style = bsClear
        Border.mmPadding = 0
        HideWhenOneDetail = False
        mmBottomOffset = 0
        mmHeight = 6879
        mmPrintPosition = 0
        object ppShape7: TppShape
          DesignLayer = ppDesignLayer2
          UserName = 'Shape3'
          mmHeight = 5556
          mmLeft = 11642
          mmTop = 529
          mmWidth = 183092
          BandType = 5
          GroupNo = 3
          LayerName = Foreground1
        end
        object ppDBCalc4: TppDBCalc
          DesignLayer = ppDesignLayer2
          UserName = 'TXT_PesoTotalComissao1'
          HyperlinkEnabled = False
          Border.mmPadding = 0
          DataField = 'PesoAFaturar'
          DataPipeline = ppDBPipeline1
          DisplayFormat = '#,0;-#,0'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = []
          ParentDataPipeline = False
          ResetGroup = ppGroup8
          TextAlignment = taRightJustified
          Transparent = True
          DataPipelineName = 'ppDBPipeline1'
          mmHeight = 3440
          mmLeft = 132305
          mmTop = 1588
          mmWidth = 19047
          BandType = 5
          GroupNo = 3
          LayerName = Foreground1
        end
        object ppLabel10: TppLabel
          DesignLayer = ppDesignLayer2
          UserName = 'Label15'
          HyperlinkEnabled = False
          AutoSize = False
          Border.mmPadding = 0
          Caption = 'Total'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
          FormFieldSettings.FormFieldType = fftNone
          TextAlignment = taRightJustified
          Transparent = True
          mmHeight = 4233
          mmLeft = 12978
          mmTop = 1058
          mmWidth = 12435
          BandType = 5
          GroupNo = 3
          LayerName = Foreground1
        end
        object ppDBCalc5: TppDBCalc
          DesignLayer = ppDesignLayer2
          UserName = 'DBCalc1'
          HyperlinkEnabled = False
          Border.mmPadding = 0
          DataField = 'PesoFaturado'
          DataPipeline = ppDBPipeline1
          DisplayFormat = '#,0;-#,0'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = []
          ParentDataPipeline = False
          ResetGroup = ppGroup8
          TextAlignment = taRightJustified
          Transparent = True
          DataPipelineName = 'ppDBPipeline1'
          mmHeight = 3440
          mmLeft = 152409
          mmTop = 1588
          mmWidth = 19047
          BandType = 5
          GroupNo = 3
          LayerName = Foreground1
        end
        object ppDBCalc6: TppDBCalc
          DesignLayer = ppDesignLayer2
          UserName = 'DBCalc2'
          HyperlinkEnabled = False
          Border.mmPadding = 0
          DataField = 'PesoTotalDia'
          DataPipeline = ppDBPipeline1
          DisplayFormat = '#,0;-#,0'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = []
          ParentDataPipeline = False
          ResetGroup = ppGroup8
          TextAlignment = taRightJustified
          Transparent = True
          DataPipelineName = 'ppDBPipeline1'
          mmHeight = 3440
          mmLeft = 172514
          mmTop = 1588
          mmWidth = 19047
          BandType = 5
          GroupNo = 3
          LayerName = Foreground1
        end
        object ppDBText26: TppDBText
          DesignLayer = ppDesignLayer2
          UserName = 'DBText10'
          HyperlinkEnabled = False
          Border.mmPadding = 0
          DataField = 'AnoBomix'
          DataPipeline = ppDBPipeline1
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          TextAlignment = taCentered
          Transparent = True
          DataPipelineName = 'ppDBPipeline1'
          mmHeight = 3969
          mmLeft = 26737
          mmTop = 1058
          mmWidth = 10054
          BandType = 5
          GroupNo = 3
          LayerName = Foreground1
        end
        object ppDBText27: TppDBText
          DesignLayer = ppDesignLayer2
          UserName = 'DBText101'
          HyperlinkEnabled = False
          Border.mmPadding = 0
          DataField = 'MesNomeBomix'
          DataPipeline = ppDBPipeline1
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          Transparent = True
          DataPipelineName = 'ppDBPipeline1'
          mmHeight = 3969
          mmLeft = 38378
          mmTop = 1058
          mmWidth = 32808
          BandType = 5
          GroupNo = 3
          LayerName = Foreground1
        end
        object ppDBCalc16: TppDBCalc
          DesignLayer = ppDesignLayer2
          UserName = 'DBCalc16'
          HyperlinkEnabled = False
          Border.mmPadding = 0
          DataField = 'PesoOrcamento'
          DataPipeline = ppDBPipeline1
          DisplayFormat = '#,0;-#,0'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = []
          ParentDataPipeline = False
          ResetGroup = ppGroup8
          TextAlignment = taRightJustified
          Transparent = True
          DataPipelineName = 'ppDBPipeline1'
          mmHeight = 3440
          mmLeft = 71690
          mmTop = 1588
          mmWidth = 19047
          BandType = 5
          GroupNo = 3
          LayerName = Foreground1
        end
        object ppDBCalc17: TppDBCalc
          DesignLayer = ppDesignLayer2
          UserName = 'DBCalc17'
          HyperlinkEnabled = False
          Border.mmPadding = 0
          DataField = 'PesoNaoDatado'
          DataPipeline = ppDBPipeline1
          DisplayFormat = '#,0;-#,0'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = []
          ParentDataPipeline = False
          ResetGroup = ppGroup8
          TextAlignment = taRightJustified
          Transparent = True
          DataPipelineName = 'ppDBPipeline1'
          mmHeight = 3440
          mmLeft = 112199
          mmTop = 1588
          mmWidth = 19047
          BandType = 5
          GroupNo = 3
          LayerName = Foreground1
        end
        object ppDBCalc18: TppDBCalc
          DesignLayer = ppDesignLayer2
          UserName = 'DBCalc18'
          HyperlinkEnabled = False
          Border.mmPadding = 0
          DataField = 'PesoArteBloqueada'
          DataPipeline = ppDBPipeline1
          DisplayFormat = '#,0;-#,0'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = []
          ParentDataPipeline = False
          ResetGroup = ppGroup8
          TextAlignment = taRightJustified
          Transparent = True
          DataPipelineName = 'ppDBPipeline1'
          mmHeight = 3440
          mmLeft = 91546
          mmTop = 1588
          mmWidth = 19050
          BandType = 5
          GroupNo = 3
          LayerName = Foreground1
        end
      end
    end
    object raCodeModule2: TraCodeModule
    end
    object ppDesignLayers2: TppDesignLayers
      object ppDesignLayer2: TppDesignLayer
        UserName = 'Foreground1'
        LayerType = ltBanded
        Index = 0
      end
    end
    object ppParameterList1: TppParameterList
    end
  end
  object CDS_DiarioGerencia: TClientDataSet
    Aggregates = <>
    AggregatesActive = True
    AutoCalcFields = False
    Params = <>
    ProviderName = 'DSP_DiarioGerencia'
    Left = 853
    Top = 140
    object CDS_DiarioGerenciaEmpresa: TStringField
      FieldName = 'Empresa'
      Size = 6
    end
    object CDS_DiarioGerenciaData: TDateTimeField
      FieldName = 'Data'
    end
    object CDS_DiarioGerenciaDia: TStringField
      FieldName = 'Dia'
      Size = 3
    end
    object CDS_DiarioGerenciaDiaDaSemana: TStringField
      FieldName = 'DiaDaSemana'
    end
    object CDS_DiarioGerenciaAnoBomix: TStringField
      FieldName = 'AnoBomix'
      Size = 4
    end
    object CDS_DiarioGerenciaMesNomeBomix: TStringField
      FieldName = 'MesNomeBomix'
      Size = 14
    end
    object CDS_DiarioGerenciaPesoOrcamento: TFloatField
      FieldName = 'PesoOrcamento'
      ReadOnly = True
    end
    object CDS_DiarioGerenciaPesoArteBloqueada: TFloatField
      FieldName = 'PesoArteBloqueada'
      ReadOnly = True
    end
    object CDS_DiarioGerenciaPesoNaoDatado: TFloatField
      FieldName = 'PesoNaoDatado'
      ReadOnly = True
    end
    object CDS_DiarioGerenciaPesoAFaturar: TFloatField
      FieldName = 'PesoAFaturar'
      ReadOnly = True
    end
    object CDS_DiarioGerenciaPesoFaturado: TFloatField
      FieldName = 'PesoFaturado'
      ReadOnly = True
    end
    object CDS_DiarioGerenciaPesoTotalDia: TFloatField
      FieldName = 'PesoTotalDia'
      ReadOnly = True
    end
    object CDS_DiarioGerenciaGerente: TStringField
      FieldName = 'Gerente'
      Size = 100
    end
  end
  object Query_DiarioGerencia: TADOQuery
    Connection = Sys_ModuloConexaoDados.ADOConnection1
    CursorType = ctStatic
    DataSource = DS_DiarioGerencia
    Parameters = <>
    SQL.Strings = (
      '   Select  '
      
        '       Empresa, Data, Dia, DiaDaSemana, AnoBomix, MesNomeBomix, ' +
        'Gerente, '
      
        '       Sum(PesoOrcamento) as PesoOrcamento, Sum(PesoArteBloquead' +
        'a) as PesoArteBloqueada, Sum(PesoNaoDatado) as PesoNaoDatado, '
      
        '       SUM(PesoAFaturar) as PesoAFaturar, SUM(PesoFaturado) as P' +
        'esoFaturado, '
      '       SUM(PesoTotalDia) as PesoTotalDia '
      '   from ( '
      '               Select '
      
        '                    Empresa, Data, Dia, DiaDaSemana, AnoBomix, M' +
        'esNomeBomix, Gerente, Vendedor_FK, Vendedor,'
      
        '                    Sum(PesoOrcamento) as PesoOrcamento, Sum(Pes' +
        'oArteBloqueada) as PesoArteBloqueada, Sum(PesoNaoDatado) as Peso' +
        'NaoDatado, '
      
        '                    SUM(PesoAFaturar) as PesoAFaturar, SUM(PesoF' +
        'aturado) as PesoFaturado, '
      '                    SUM(PesoTotalDia) as PesoTotalDia '
      
        '                from BomixBI.[dbo].[Fat_TB_TonelagemDiaria] (nol' +
        'ock) Tonelagem  '
      
        '                Where Data Between '#39'2020-11-26 00:00:00'#39' AND '#39'20' +
        '20-12-31 23:59:00'#39
      '                AND Empresa = '#39'010101'#39
      
        '                Group by Empresa, Data, Dia, DiaDaSemana, AnoBom' +
        'ix, MesNomeBomix, Gerente, Vendedor_FK, Vendedor'
      '   ) TB'
      
        '   Group by Empresa, Data, Dia, DiaDaSemana, AnoBomix, MesNomeBo' +
        'mix, Gerente'
      '   Order by Empresa, Gerente, MesNomeBomix, Data desc')
    Left = 883
    Top = 140
    object Query_DiarioGerenciaEmpresa: TStringField
      FieldName = 'Empresa'
      Size = 6
    end
    object Query_DiarioGerenciaData: TDateTimeField
      FieldName = 'Data'
    end
    object Query_DiarioGerenciaDia: TStringField
      FieldName = 'Dia'
      Size = 3
    end
    object Query_DiarioGerenciaDiaDaSemana: TStringField
      FieldName = 'DiaDaSemana'
    end
    object Query_DiarioGerenciaAnoBomix: TStringField
      FieldName = 'AnoBomix'
      Size = 4
    end
    object Query_DiarioGerenciaMesNomeBomix: TStringField
      FieldName = 'MesNomeBomix'
      Size = 14
    end
    object Query_DiarioGerenciaPesoOrcamento: TFloatField
      FieldName = 'PesoOrcamento'
      ReadOnly = True
    end
    object Query_DiarioGerenciaPesoArteBloqueada: TFloatField
      FieldName = 'PesoArteBloqueada'
      ReadOnly = True
    end
    object Query_DiarioGerenciaPesoNaoDatado: TFloatField
      FieldName = 'PesoNaoDatado'
      ReadOnly = True
    end
    object Query_DiarioGerenciaPesoAFaturar: TFloatField
      FieldName = 'PesoAFaturar'
      ReadOnly = True
    end
    object Query_DiarioGerenciaPesoFaturado: TFloatField
      FieldName = 'PesoFaturado'
      ReadOnly = True
    end
    object Query_DiarioGerenciaPesoTotalDia: TFloatField
      FieldName = 'PesoTotalDia'
      ReadOnly = True
    end
    object Query_DiarioGerenciaGerente: TStringField
      FieldName = 'Gerente'
      Size = 100
    end
  end
  object DS_DiarioGerencia: TDataSource
    DataSet = CDS_DiarioGerencia
    Left = 913
    Top = 140
  end
  object DSP_DiarioGerencia: TDataSetProvider
    DataSet = Query_DiarioGerencia
    Left = 944
    Top = 140
  end
  object DSP_DiarioGeral: TDataSetProvider
    DataSet = Query_DiarioGeral
    Left = 936
    Top = 12
  end
  object DS_DiarioGeral: TDataSource
    DataSet = CDS_DiarioGeral
    Left = 905
    Top = 12
  end
  object Query_DiarioGeral: TADOQuery
    Connection = Sys_ModuloConexaoDados.ADOConnection1
    CursorType = ctStatic
    DataSource = DS_DiarioGeral
    Parameters = <>
    SQL.Strings = (
      '   Select  '
      '       Empresa, Data, Dia, DiaDaSemana, AnoBomix, MesNomeBomix, '
      
        '       Sum(PesoOrcamento) as PesoOrcamento, Sum(PesoArteBloquead' +
        'a) as PesoArteBloqueada, Sum(PesoNaoDatado) as PesoNaoDatado, '
      
        '       SUM(PesoAFaturar) as PesoAFaturar, SUM(PesoFaturado) as P' +
        'esoFaturado, '
      '       SUM(PesoTotalDia) as PesoTotalDia '
      '   from ( '
      '               Select '
      
        '                    Empresa, Data, Dia, DiaDaSemana, AnoBomix, M' +
        'esNomeBomix, Gerente, Vendedor_FK, Vendedor,'
      
        '                    Sum(PesoOrcamento) as PesoOrcamento, Sum(Pes' +
        'oArteBloqueada) as PesoArteBloqueada, Sum(PesoNaoDatado) as Peso' +
        'NaoDatado,'
      
        '                    SUM(PesoAFaturar) as PesoAFaturar, SUM(PesoF' +
        'aturado) as PesoFaturado, '
      '                    SUM(PesoTotalDia) as PesoTotalDia '
      
        '                from BomixBI.[dbo].[Fat_TB_TonelagemDiaria] (nol' +
        'ock) Tonelagem  '
      
        '                Where Data Between '#39'2021-01-01 00:00:00'#39' AND '#39'20' +
        '21-01-31 23:59:00'#39
      '                AND Empresa = '#39'010101'#39
      
        '                Group by Empresa, Data, Dia, DiaDaSemana, AnoBom' +
        'ix, MesNomeBomix, Gerente, Vendedor_FK, Vendedor'
      '   ) TB'
      
        '   Group by Empresa, Data, Dia, DiaDaSemana, AnoBomix, MesNomeBo' +
        'mix '
      '   Order by MesNomeBomix, Data desc')
    Left = 875
    Top = 12
    object Query_DiarioGeralEmpresa: TStringField
      FieldName = 'Empresa'
      Size = 6
    end
    object Query_DiarioGeralData: TDateTimeField
      FieldName = 'Data'
    end
    object Query_DiarioGeralDia: TStringField
      FieldName = 'Dia'
      Size = 3
    end
    object Query_DiarioGeralDiaDaSemana: TStringField
      FieldName = 'DiaDaSemana'
    end
    object Query_DiarioGeralAnoBomix: TStringField
      FieldName = 'AnoBomix'
      Size = 4
    end
    object Query_DiarioGeralMesNomeBomix: TStringField
      FieldName = 'MesNomeBomix'
      Size = 14
    end
    object Query_DiarioGeralPesoOrcamento: TFloatField
      FieldName = 'PesoOrcamento'
      ReadOnly = True
    end
    object Query_DiarioGeralPesoArteBloqueada: TFloatField
      FieldName = 'PesoArteBloqueada'
      ReadOnly = True
    end
    object Query_DiarioGeralPesoNaoDatado: TFloatField
      FieldName = 'PesoNaoDatado'
      ReadOnly = True
    end
    object Query_DiarioGeralPesoAFaturar: TFloatField
      FieldName = 'PesoAFaturar'
      ReadOnly = True
    end
    object Query_DiarioGeralPesoFaturado: TFloatField
      FieldName = 'PesoFaturado'
      ReadOnly = True
    end
    object Query_DiarioGeralPesoTotalDia: TFloatField
      FieldName = 'PesoTotalDia'
      ReadOnly = True
    end
  end
  object CDS_DiarioGeral: TClientDataSet
    Aggregates = <>
    AggregatesActive = True
    AutoCalcFields = False
    Params = <>
    ProviderName = 'DSP_DiarioGeral'
    Left = 845
    Top = 12
    object CDS_DiarioGeralEmpresa: TStringField
      FieldName = 'Empresa'
      Size = 6
    end
    object CDS_DiarioGeralData: TDateTimeField
      FieldName = 'Data'
    end
    object CDS_DiarioGeralDia: TStringField
      FieldName = 'Dia'
      Size = 3
    end
    object CDS_DiarioGeralDiaDaSemana: TStringField
      FieldName = 'DiaDaSemana'
    end
    object CDS_DiarioGeralAnoBomix: TStringField
      FieldName = 'AnoBomix'
      Size = 4
    end
    object CDS_DiarioGeralMesNomeBomix: TStringField
      FieldName = 'MesNomeBomix'
      Size = 14
    end
    object CDS_DiarioGeralPesoOrcamento: TFloatField
      FieldName = 'PesoOrcamento'
      ReadOnly = True
    end
    object CDS_DiarioGeralPesoArteBloqueada: TFloatField
      FieldName = 'PesoArteBloqueada'
      ReadOnly = True
    end
    object CDS_DiarioGeralPesoNaoDatado: TFloatField
      FieldName = 'PesoNaoDatado'
      ReadOnly = True
    end
    object CDS_DiarioGeralPesoAFaturar: TFloatField
      FieldName = 'PesoAFaturar'
      ReadOnly = True
    end
    object CDS_DiarioGeralPesoFaturado: TFloatField
      FieldName = 'PesoFaturado'
      ReadOnly = True
    end
    object CDS_DiarioGeralPesoTotalDia: TFloatField
      FieldName = 'PesoTotalDia'
      ReadOnly = True
    end
  end
  object ppReportDiarioGeral: TppReport
    AutoStop = False
    ColumnPositions.Strings = (
      '6350'
      '105001')
    DataPipeline = ppDBPipeline2
    PassSetting = psTwoPass
    PrinterSetup.BinName = 'Default'
    PrinterSetup.DocumentName = 'Report'
    PrinterSetup.Duplex = dpNone
    PrinterSetup.PaperName = 'A4'
    PrinterSetup.PrinterName = 'Default'
    PrinterSetup.SaveDeviceSettings = False
    PrinterSetup.mmMarginBottom = 6350
    PrinterSetup.mmMarginLeft = 6350
    PrinterSetup.mmMarginRight = 6350
    PrinterSetup.mmMarginTop = 6350
    PrinterSetup.mmPaperHeight = 297000
    PrinterSetup.mmPaperWidth = 210000
    PrinterSetup.PaperSize = 9
    AllowPrintToArchive = True
    AllowPrintToFile = True
    ArchiveFileName = '($MyDocuments)\ReportArchive.raf'
    DeviceType = 'Screen'
    DefaultFileDeviceType = 'XLSData'
    EmailSettings.ReportFormat = 'PDF'
    LanguageID = 'Default'
    OpenFile = True
    OutlineSettings.CreateNode = True
    OutlineSettings.CreatePageNodes = True
    OutlineSettings.Enabled = True
    OutlineSettings.Visible = True
    ThumbnailSettings.Enabled = True
    ThumbnailSettings.Visible = True
    ThumbnailSettings.DeadSpace = 30
    ThumbnailSettings.PageHighlight.Width = 3
    PDFSettings.EmbedFontOptions = [efUseSubset]
    PDFSettings.EncryptSettings.AllowCopy = True
    PDFSettings.EncryptSettings.AllowInteract = True
    PDFSettings.EncryptSettings.AllowModify = True
    PDFSettings.EncryptSettings.AllowPrint = True
    PDFSettings.EncryptSettings.AllowExtract = True
    PDFSettings.EncryptSettings.AllowAssemble = True
    PDFSettings.EncryptSettings.AllowQualityPrint = True
    PDFSettings.EncryptSettings.Enabled = False
    PDFSettings.EncryptSettings.KeyLength = kl40Bit
    PDFSettings.EncryptSettings.EncryptionType = etRC4
    PDFSettings.FontEncoding = feAnsi
    PDFSettings.ImageCompressionLevel = 25
    PDFSettings.PDFAFormat = pafNone
    PreviewFormSettings.PageBorder.mmPadding = 0
    PreviewFormSettings.WindowState = wsMaximized
    PreviewFormSettings.ZoomSetting = zs100Percent
    RTFSettings.DefaultFont.Charset = DEFAULT_CHARSET
    RTFSettings.DefaultFont.Color = clWindowText
    RTFSettings.DefaultFont.Height = -13
    RTFSettings.DefaultFont.Name = 'Arial'
    RTFSettings.DefaultFont.Style = []
    TextFileName = '($MyDocuments)\Report.xls'
    TextSearchSettings.DefaultString = '<Texto a localizar>'
    TextSearchSettings.Enabled = True
    XLSSettings.AppName = 'ReportBuilder'
    XLSSettings.Author = 'ReportBuilder'
    XLSSettings.Subject = 'Report'
    XLSSettings.Title = 'Report'
    XLSSettings.WorksheetName = 'Report'
    Left = 770
    Top = 12
    Version = '19.03'
    mmColumnWidth = 98651
    DataPipelineName = 'ppDBPipeline2'
    object ppHeaderBand2: TppHeaderBand
      Background.Brush.Style = bsClear
      Border.mmPadding = 0
      PrintHeight = phDynamic
      mmBottomOffset = 0
      mmHeight = 14817
      mmPrintPosition = 0
      object ppLine3: TppLine
        DesignLayer = ppDesignLayer3
        UserName = 'Line1'
        Border.mmPadding = 0
        Pen.Width = 3
        Weight = 2.250000000000000000
        mmHeight = 2646
        mmLeft = 1324
        mmTop = 10850
        mmWidth = 193411
        BandType = 0
        LayerName = Foreground2
      end
      object ppImage2: TppImage
        DesignLayer = ppDesignLayer3
        UserName = 'ppIMG_Logo'
        AlignHorizontal = ahCenter
        AlignVertical = avCenter
        MaintainAspectRatio = False
        Stretch = True
        Border.mmPadding = 0
        Picture.Data = {
          
          FFFF}
        mmHeight = 6879
        mmLeft = 1853
        mmTop = 3442
        mmWidth = 23813
        BandType = 0
        LayerName = Foreground2
      end
      object ppLabel11: TppLabel
        DesignLayer = ppDesignLayer3
        UserName = 'ppLBL_TituloRelatorio'
        HyperlinkEnabled = False
        Border.mmPadding = 0
        Caption = 'Relat'#243'rio de Tonelagem Di'#225'ria'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 18
        Font.Style = [fsBold]
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        Transparent = True
        mmHeight = 7673
        mmLeft = 28312
        mmTop = 3442
        mmWidth = 91546
        BandType = 0
        LayerName = Foreground2
      end
    end
    object ppDetailBand2: TppDetailBand
      Background1.Brush.Style = bsClear
      Background2.Brush.Style = bsClear
      Border.mmPadding = 0
      mmBottomOffset = 0
      mmHeight = 5027
      mmPrintPosition = 0
      object ppDBText19: TppDBText
        DesignLayer = ppDesignLayer3
        UserName = 'DBText138'
        HyperlinkEnabled = False
        Border.mmPadding = 0
        DataField = 'Dia'
        DataPipeline = ppDBPipeline2
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = [fsBold]
        TextAlignment = taCentered
        Transparent = True
        DataPipelineName = 'ppDBPipeline2'
        mmHeight = 3440
        mmLeft = 21928
        mmTop = 794
        mmWidth = 17198
        BandType = 4
        LayerName = Foreground2
      end
      object ppDBCalc8: TppDBCalc
        DesignLayer = ppDesignLayer3
        UserName = 'DBCalc7'
        Border.mmPadding = 0
        DataField = 'Dia'
        DataPipeline = ppDBPipeline2
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        ResetGroup = ppGroup12
        TextAlignment = taRightJustified
        Transparent = True
        DBCalcType = dcCount
        DataPipelineName = 'ppDBPipeline2'
        mmHeight = 3175
        mmLeft = 16371
        mmTop = 794
        mmWidth = 5821
        BandType = 4
        LayerName = Foreground2
      end
      object ppDBText20: TppDBText
        DesignLayer = ppDesignLayer3
        UserName = 'DBText2'
        HyperlinkEnabled = False
        Border.mmPadding = 0
        DataField = 'DiaDaSemana'
        DataPipeline = ppDBPipeline2
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        Transparent = True
        DataPipelineName = 'ppDBPipeline2'
        mmHeight = 3175
        mmLeft = 40448
        mmTop = 794
        mmWidth = 33867
        BandType = 4
        LayerName = Foreground2
      end
      object ppDBText28: TppDBText
        DesignLayer = ppDesignLayer3
        UserName = 'DBText12'
        HyperlinkEnabled = False
        Border.mmPadding = 0
        DataField = 'PesoAFaturar'
        DataPipeline = ppDBPipeline2
        DisplayFormat = '#,0;-#,0'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDBPipeline2'
        mmHeight = 3175
        mmLeft = 138708
        mmTop = 794
        mmWidth = 17457
        BandType = 4
        LayerName = Foreground2
      end
      object ppDBText29: TppDBText
        DesignLayer = ppDesignLayer3
        UserName = 'DBText3'
        HyperlinkEnabled = False
        Border.mmPadding = 0
        DataField = 'PesoFaturado'
        DataPipeline = ppDBPipeline2
        DisplayFormat = '#,0;-#,0'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDBPipeline2'
        mmHeight = 3175
        mmLeft = 157753
        mmTop = 794
        mmWidth = 17457
        BandType = 4
        LayerName = Foreground2
      end
      object ppDBText30: TppDBText
        DesignLayer = ppDesignLayer3
        UserName = 'DBText4'
        HyperlinkEnabled = False
        Border.mmPadding = 0
        DataField = 'PesoTotalDia'
        DataPipeline = ppDBPipeline2
        DisplayFormat = '#,0;-#,0'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDBPipeline2'
        mmHeight = 3175
        mmLeft = 176003
        mmTop = 794
        mmWidth = 17457
        BandType = 4
        LayerName = Foreground2
      end
      object ppShape8: TppShape
        DesignLayer = ppDesignLayer3
        UserName = 'Shape2'
        Brush.Color = 14145495
        Pen.Style = psClear
        mmHeight = 5553
        mmLeft = 1854
        mmTop = 0
        mmWidth = 13758
        BandType = 4
        LayerName = Foreground2
      end
      object ppDBText31: TppDBText
        DesignLayer = ppDesignLayer3
        UserName = 'DBText31'
        HyperlinkEnabled = False
        Border.mmPadding = 0
        DataField = 'PesoNaoDatado'
        DataPipeline = ppDBPipeline2
        DisplayFormat = '#,0;-#,0'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDBPipeline2'
        mmHeight = 3175
        mmLeft = 119885
        mmTop = 795
        mmWidth = 17457
        BandType = 4
        LayerName = Foreground2
      end
      object ppDBText32: TppDBText
        DesignLayer = ppDesignLayer3
        UserName = 'DBText32'
        HyperlinkEnabled = False
        Border.mmPadding = 0
        DataField = 'PesoArteBloqueada'
        DataPipeline = ppDBPipeline2
        DisplayFormat = '#,0;-#,0'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDBPipeline2'
        mmHeight = 3175
        mmLeft = 100576
        mmTop = 794
        mmWidth = 17457
        BandType = 4
        LayerName = Foreground2
      end
      object ppDBText44: TppDBText
        DesignLayer = ppDesignLayer3
        UserName = 'DBText44'
        HyperlinkEnabled = False
        Border.mmPadding = 0
        DataField = 'PesoOrcamento'
        DataPipeline = ppDBPipeline2
        DisplayFormat = '#,0;-#,0'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDBPipeline2'
        mmHeight = 3175
        mmLeft = 80168
        mmTop = 794
        mmWidth = 17463
        BandType = 4
        LayerName = Foreground2
      end
    end
    object ppFooterBand2: TppFooterBand
      Background.Brush.Style = bsClear
      Border.mmPadding = 0
      mmBottomOffset = 0
      mmHeight = 6350
      mmPrintPosition = 0
      object ppLine4: TppLine
        DesignLayer = ppDesignLayer3
        UserName = 'Line2'
        Border.mmPadding = 0
        Pen.Width = 3
        Weight = 2.250000000000000000
        mmHeight = 2646
        mmLeft = 529
        mmTop = 0
        mmWidth = 195527
        BandType = 8
        LayerName = Foreground2
      end
      object ppLabel12: TppLabel
        DesignLayer = ppDesignLayer3
        UserName = 'LBL_Status'
        HyperlinkEnabled = False
        Border.mmPadding = 0
        Caption = 'LBL_Status'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = [fsBold]
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        TextAlignment = taRightJustified
        Transparent = True
        mmHeight = 4191
        mmLeft = 176498
        mmTop = 1323
        mmWidth = 19558
        BandType = 8
        LayerName = Foreground2
      end
    end
    object ppSummaryBand2: TppSummaryBand
      Background.Brush.Style = bsClear
      Border.mmPadding = 0
      mmBottomOffset = 0
      mmHeight = 0
      mmPrintPosition = 0
    end
    object ppGroup11: TppGroup
      BreakName = 'AnoBomix'
      DataPipeline = ppDBPipeline2
      GroupFileSettings.NewFile = False
      GroupFileSettings.EmailFile = False
      OutlineSettings.CreateNode = True
      StartOnOddPage = False
      UserName = 'Group2'
      mmNewColumnThreshold = 0
      mmNewPageThreshold = 0
      DataPipelineName = 'ppDBPipeline2'
      NewFile = False
      object ppGroupHeaderBand11: TppGroupHeaderBand
        Background.Brush.Style = bsClear
        Border.mmPadding = 0
        mmBottomOffset = 0
        mmHeight = 0
        mmPrintPosition = 0
        object ppDBText34: TppDBText
          DesignLayer = ppDesignLayer3
          UserName = 'DBText9'
          HyperlinkEnabled = False
          Border.mmPadding = 0
          DataField = 'AnoBomix'
          DataPipeline = ppDBPipeline2
          Font.Charset = ANSI_CHARSET
          Font.Color = clWhite
          Font.Name = 'Arial'
          Font.Size = 10
          Font.Style = [fsBold]
          TextAlignment = taCentered
          Transparent = True
          DataPipelineName = 'ppDBPipeline2'
          mmHeight = 4763
          mmLeft = 4233
          mmTop = 0
          mmWidth = 14288
          BandType = 3
          GroupNo = 2
          LayerName = Foreground2
        end
        object ppDBText35: TppDBText
          DesignLayer = ppDesignLayer3
          UserName = 'DBText11'
          HyperlinkEnabled = False
          Border.mmPadding = 0
          DataField = 'MesNomeBomix'
          DataPipeline = ppDBPipeline2
          Font.Charset = ANSI_CHARSET
          Font.Color = clWhite
          Font.Name = 'Arial'
          Font.Size = 10
          Font.Style = [fsBold]
          Transparent = True
          DataPipelineName = 'ppDBPipeline2'
          mmHeight = 4763
          mmLeft = 23283
          mmTop = 0
          mmWidth = 40746
          BandType = 3
          GroupNo = 2
          LayerName = Foreground2
        end
      end
      object ppGroupFooterBand11: TppGroupFooterBand
        Background.Brush.Style = bsClear
        Border.mmPadding = 0
        HideWhenOneDetail = False
        mmBottomOffset = 0
        mmHeight = 0
        mmPrintPosition = 0
      end
    end
    object ppGroup12: TppGroup
      BreakName = 'MesNomeBomix'
      DataPipeline = ppDBPipeline2
      GroupFileSettings.NewFile = False
      GroupFileSettings.EmailFile = False
      OutlineSettings.CreateNode = True
      StartOnOddPage = False
      UserName = 'Group3'
      mmNewColumnThreshold = 0
      mmNewPageThreshold = 0
      DataPipelineName = 'ppDBPipeline2'
      NewFile = False
      object ppGroupHeaderBand12: TppGroupHeaderBand
        Background.Brush.Style = bsClear
        Border.mmPadding = 0
        mmBottomOffset = 0
        mmHeight = 6085
        mmPrintPosition = 0
        object ppShape10: TppShape
          DesignLayer = ppDesignLayer3
          UserName = 'Shape1'
          Brush.Color = clGray
          mmHeight = 6085
          mmLeft = 1853
          mmTop = 0
          mmWidth = 192882
          BandType = 3
          GroupNo = 3
          LayerName = Foreground2
        end
        object ppDBText36: TppDBText
          DesignLayer = ppDesignLayer3
          UserName = 'DBText8'
          HyperlinkEnabled = False
          Border.mmPadding = 0
          DataField = 'AnoBomix'
          DataPipeline = ppDBPipeline2
          Font.Charset = ANSI_CHARSET
          Font.Color = clWhite
          Font.Name = 'Arial'
          Font.Size = 10
          Font.Style = [fsBold]
          TextAlignment = taCentered
          Transparent = True
          DataPipelineName = 'ppDBPipeline2'
          mmHeight = 4763
          mmLeft = 4235
          mmTop = 792
          mmWidth = 14288
          BandType = 3
          GroupNo = 2
          LayerName = Foreground2
        end
        object ppLabel14: TppLabel
          DesignLayer = ppDesignLayer3
          UserName = 'Label3'
          HyperlinkEnabled = False
          AutoSize = False
          Border.mmPadding = 0
          Caption = 'A Faturar'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWhite
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
          FormFieldSettings.FormFieldType = fftNone
          TextAlignment = taRightJustified
          Transparent = True
          mmHeight = 4233
          mmLeft = 135749
          mmTop = 792
          mmWidth = 20902
          BandType = 3
          GroupNo = 3
          LayerName = Foreground2
        end
        object ppLabel16: TppLabel
          DesignLayer = ppDesignLayer3
          UserName = 'LBL_QTDGerenciaXGerencia1'
          HyperlinkEnabled = False
          AutoSize = False
          Border.mmPadding = 0
          Caption = 'Faturado'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWhite
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
          FormFieldSettings.FormFieldType = fftNone
          TextAlignment = taRightJustified
          Transparent = True
          mmHeight = 4233
          mmLeft = 155324
          mmTop = 792
          mmWidth = 20889
          BandType = 3
          GroupNo = 3
          LayerName = Foreground2
        end
        object ppLabel17: TppLabel
          DesignLayer = ppDesignLayer3
          UserName = 'Label2'
          HyperlinkEnabled = False
          AutoSize = False
          Border.mmPadding = 0
          Caption = 'Total'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWhite
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
          FormFieldSettings.FormFieldType = fftNone
          TextAlignment = taRightJustified
          Transparent = True
          mmHeight = 4233
          mmLeft = 171719
          mmTop = 792
          mmWidth = 20897
          BandType = 3
          GroupNo = 3
          LayerName = Foreground2
        end
        object ppDBText37: TppDBText
          DesignLayer = ppDesignLayer3
          UserName = 'DBText1'
          HyperlinkEnabled = False
          Border.mmPadding = 0
          DataField = 'MesNomeBomix'
          DataPipeline = ppDBPipeline2
          Font.Charset = ANSI_CHARSET
          Font.Color = clWhite
          Font.Name = 'Arial'
          Font.Size = 10
          Font.Style = [fsBold]
          Transparent = True
          DataPipelineName = 'ppDBPipeline2'
          mmHeight = 4763
          mmLeft = 23285
          mmTop = 792
          mmWidth = 40746
          BandType = 3
          GroupNo = 3
          LayerName = Foreground2
        end
        object ppLabel19: TppLabel
          DesignLayer = ppDesignLayer3
          UserName = 'Label19'
          HyperlinkEnabled = False
          AutoSize = False
          Border.mmPadding = 0
          Caption = 'Or'#231'amento'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWhite
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
          FormFieldSettings.FormFieldType = fftNone
          TextAlignment = taRightJustified
          Transparent = True
          mmHeight = 4233
          mmLeft = 76460
          mmTop = 794
          mmWidth = 20902
          BandType = 3
          GroupNo = 1
          LayerName = Foreground2
        end
        object ppLabel20: TppLabel
          DesignLayer = ppDesignLayer3
          UserName = 'Label20'
          HyperlinkEnabled = False
          AutoSize = False
          Border.mmPadding = 0
          Caption = 'N'#227'o Datado'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWhite
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
          FormFieldSettings.FormFieldType = fftNone
          TextAlignment = taRightJustified
          Transparent = True
          mmHeight = 4233
          mmLeft = 118028
          mmTop = 1059
          mmWidth = 20902
          BandType = 3
          GroupNo = 1
          LayerName = Foreground2
        end
        object ppLabel24: TppLabel
          DesignLayer = ppDesignLayer3
          UserName = 'Label201'
          HyperlinkEnabled = False
          AutoSize = False
          Border.mmPadding = 0
          Caption = 'Arte Bloq.'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWhite
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
          FormFieldSettings.FormFieldType = fftNone
          TextAlignment = taRightJustified
          Transparent = True
          mmHeight = 4233
          mmLeft = 97897
          mmTop = 1059
          mmWidth = 20902
          BandType = 3
          GroupNo = 1
          LayerName = Foreground2
        end
      end
      object ppGroupFooterBand12: TppGroupFooterBand
        Background.Brush.Style = bsClear
        Border.mmPadding = 0
        HideWhenOneDetail = False
        mmBottomOffset = 0
        mmHeight = 6879
        mmPrintPosition = 0
        object ppShape11: TppShape
          DesignLayer = ppDesignLayer3
          UserName = 'Shape3'
          mmHeight = 5556
          mmLeft = 16931
          mmTop = 528
          mmWidth = 177800
          BandType = 5
          GroupNo = 1
          LayerName = Foreground2
        end
        object ppDBCalc9: TppDBCalc
          DesignLayer = ppDesignLayer3
          UserName = 'TXT_PesoTotalComissao1'
          HyperlinkEnabled = False
          Border.mmPadding = 0
          DataField = 'PesoAFaturar'
          DataPipeline = ppDBPipeline2
          DisplayFormat = '#,0;-#,0'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = []
          ParentDataPipeline = False
          ResetGroup = ppGroup12
          TextAlignment = taRightJustified
          Transparent = True
          DataPipelineName = 'ppDBPipeline2'
          mmHeight = 3440
          mmLeft = 138661
          mmTop = 1588
          mmWidth = 17457
          BandType = 5
          GroupNo = 1
          LayerName = Foreground2
        end
        object ppLabel18: TppLabel
          DesignLayer = ppDesignLayer3
          UserName = 'Label15'
          HyperlinkEnabled = False
          AutoSize = False
          Border.mmPadding = 0
          Caption = 'Total'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 10
          Font.Style = [fsBold]
          FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
          FormFieldSettings.FormFieldType = fftNone
          TextAlignment = taRightJustified
          Transparent = True
          mmHeight = 4233
          mmLeft = 15533
          mmTop = 1058
          mmWidth = 10845
          BandType = 5
          GroupNo = 1
          LayerName = Foreground2
        end
        object ppDBCalc10: TppDBCalc
          DesignLayer = ppDesignLayer3
          UserName = 'DBCalc1'
          HyperlinkEnabled = False
          Border.mmPadding = 0
          DataField = 'PesoFaturado'
          DataPipeline = ppDBPipeline2
          DisplayFormat = '#,0;-#,0'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = []
          ParentDataPipeline = False
          ResetGroup = ppGroup12
          TextAlignment = taRightJustified
          Transparent = True
          DataPipelineName = 'ppDBPipeline2'
          mmHeight = 3440
          mmLeft = 157705
          mmTop = 1588
          mmWidth = 17457
          BandType = 5
          GroupNo = 1
          LayerName = Foreground2
        end
        object ppDBCalc11: TppDBCalc
          DesignLayer = ppDesignLayer3
          UserName = 'DBCalc2'
          HyperlinkEnabled = False
          Border.mmPadding = 0
          DataField = 'PesoTotalDia'
          DataPipeline = ppDBPipeline2
          DisplayFormat = '#,0;-#,0'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = []
          ParentDataPipeline = False
          ResetGroup = ppGroup12
          TextAlignment = taRightJustified
          Transparent = True
          DataPipelineName = 'ppDBPipeline2'
          mmHeight = 3440
          mmLeft = 175955
          mmTop = 1588
          mmWidth = 17457
          BandType = 5
          GroupNo = 1
          LayerName = Foreground2
        end
        object ppDBText38: TppDBText
          DesignLayer = ppDesignLayer3
          UserName = 'DBText10'
          HyperlinkEnabled = False
          Border.mmPadding = 0
          DataField = 'AnoBomix'
          DataPipeline = ppDBPipeline2
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          TextAlignment = taCentered
          Transparent = True
          DataPipelineName = 'ppDBPipeline2'
          mmHeight = 3969
          mmLeft = 28496
          mmTop = 1058
          mmWidth = 10054
          BandType = 5
          GroupNo = 1
          LayerName = Foreground2
        end
        object ppDBText39: TppDBText
          DesignLayer = ppDesignLayer3
          UserName = 'DBText101'
          HyperlinkEnabled = False
          Border.mmPadding = 0
          DataField = 'MesNomeBomix'
          DataPipeline = ppDBPipeline2
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          Transparent = True
          DataPipelineName = 'ppDBPipeline2'
          mmHeight = 3969
          mmLeft = 40933
          mmTop = 1058
          mmWidth = 32808
          BandType = 5
          GroupNo = 1
          LayerName = Foreground2
        end
        object ppDBCalc12: TppDBCalc
          DesignLayer = ppDesignLayer3
          UserName = 'DBCalc12'
          HyperlinkEnabled = False
          Border.mmPadding = 0
          DataField = 'PesoNaoDatado'
          DataPipeline = ppDBPipeline2
          DisplayFormat = '#,0;-#,0'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = []
          ParentDataPipeline = False
          ResetGroup = ppGroup12
          TextAlignment = taRightJustified
          Transparent = True
          DataPipelineName = 'ppDBPipeline2'
          mmHeight = 3440
          mmLeft = 119885
          mmTop = 1588
          mmWidth = 17457
          BandType = 5
          GroupNo = 1
          LayerName = Foreground2
        end
        object ppDBCalc13: TppDBCalc
          DesignLayer = ppDesignLayer3
          UserName = 'DBCalc13'
          HyperlinkEnabled = False
          Border.mmPadding = 0
          DataField = 'PesoArteBloqueada'
          DataPipeline = ppDBPipeline2
          DisplayFormat = '#,0;-#,0'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = []
          ParentDataPipeline = False
          ResetGroup = ppGroup12
          TextAlignment = taRightJustified
          Transparent = True
          DataPipelineName = 'ppDBPipeline2'
          mmHeight = 3440
          mmLeft = 100576
          mmTop = 1588
          mmWidth = 17457
          BandType = 5
          GroupNo = 1
          LayerName = Foreground2
        end
        object ppDBCalc19: TppDBCalc
          DesignLayer = ppDesignLayer3
          UserName = 'DBCalc19'
          HyperlinkEnabled = False
          Border.mmPadding = 0
          DataField = 'PesoOrcamento'
          DataPipeline = ppDBPipeline2
          DisplayFormat = '#,0;-#,0'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = []
          ParentDataPipeline = False
          ResetGroup = ppGroup12
          TextAlignment = taRightJustified
          Transparent = True
          DataPipelineName = 'ppDBPipeline2'
          mmHeight = 3440
          mmLeft = 80169
          mmTop = 1587
          mmWidth = 17463
          BandType = 5
          GroupNo = 1
          LayerName = Foreground2
        end
      end
    end
    object raCodeModule3: TraCodeModule
    end
    object ppDesignLayers3: TppDesignLayers
      object ppDesignLayer3: TppDesignLayer
        UserName = 'Foreground2'
        LayerType = ltBanded
        Index = 0
      end
    end
    object ppParameterList2: TppParameterList
    end
  end
  object ppDBPipeline2: TppDBPipeline
    DataSource = DS_DiarioGeral
    CloseDataSource = True
    UserName = 'DB2'
    Left = 740
    Top = 12
  end
end
