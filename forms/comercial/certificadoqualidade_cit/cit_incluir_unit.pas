...
begin
  ...
  begin
       Application.MessageBox('Nenhuma transportadora selecionada!','Atenção',mb_iconwarning + mb_ok);
  ...
  begin
       Application.MessageBox('Nenhum Motorista definido!','Atenção',mb_iconwarning + mb_ok);
  ...
  begin
       Application.MessageBox('Nenhuma CIT vinculada!','Atenção',mb_iconwarning + mb_ok);
       Exit;
  end;

  If TXT_Lacre01.Text = '' then
  begin
       Application.MessageBox('Lacre 01 não definido!','Atenção',mb_iconwarning + mb_ok);
...
begin
    ...
    if CDS_NotasCITNota.Value <> '' then
        Label10.Caption := 'Itens da CIT (' + 'Data Prevista: ' + DateToStr(CDS_NotasCITDataPrevista.Value) + ')  -  Posição de Entrega : 0' + IntToStr(CDS_NotasCITPosicao.Value)
    else
