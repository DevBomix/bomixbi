...
begin
  ...
  if Query.RecordCount > 0 then Result := 'Não' else Result := 'Sim';
...
begin

 {
  ...
  if GetVerificarSeImprimiuDanfe(CIT_Inserir.CDS_BuscarNotasNota.AsString) = 'Não' then
  begin
       Application.MessageBox('Por favor imprima antes a Danfe desta Nota para garantir que o XML sairá com a hora de entrega gravada!' + #13 + '','Atenção',mb_iconerror + mb_ok);
       Exit;
  end;

 }

  VLC_Insert := VLC_Insert + ' Insert Into BomixBI.dbo.[Fat_TB_NotaFiscalVendaCIT_Itens] (Empresa, Cit_FK, Nota_FK, Cliente_FK, Loja, Cliente, Cidade, UF, SetorMercado, Sequencia, DataPrevista) ' + #13;
  ...
  VLC_Insert := VLC_Insert + '''' + CIT_Inserir.CDS_BuscarNotasCliente_ID.AsString + '''' + ',';
  VLC_Insert := VLC_Insert + '''' + CIT_Inserir.CDS_BuscarNotasLoja.AsString + '''' + ',';
  VLC_Insert := VLC_Insert + '''' + CIT_Inserir.CDS_BuscarNotasCliente.AsString + '''' + ',';
  VLC_Insert := VLC_Insert + '''' + CIT_Inserir.CDS_BuscarNotasCidade.AsString + '''' + ',';
  VLC_Insert := VLC_Insert + '''' + CIT_Inserir.CDS_BuscarNotasUF.AsString + '''' + ',';
  VLC_Insert := VLC_Insert + '''' + CIT_Inserir.CDS_BuscarNotasSetorMercado.AsString + '''' + ',';
