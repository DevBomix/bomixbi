...
begin
  ...
  VLC_Select := VLC_Select + ' 			  AND Substring(F2_FILIAL,1,4) = ' + '''' + Copy(Sys_FuncaoSistema.PNL_EmpresaTotvs.Caption,1,4) + '''' + #13;
  ...
  VLC_Select := VLC_Select + ' 				AND Motorista.Empresa = ' + '''' + Copy(Sys_FuncaoSistema.PNL_EmpresaTotvs.Caption,1,4) + '''' + #13;
  ...
  VLC_Select := VLC_Select + '    AND Veiculo.Empresa = ' + '''' + Copy(Sys_FuncaoSistema.PNL_EmpresaTotvs.Caption,1,4) + '''' + #13;
...
begin
   ...
   begin
       Application.MessageBox('Nenhum Motorista selecionado!','Atenção',mb_iconwarning + mb_ok);
