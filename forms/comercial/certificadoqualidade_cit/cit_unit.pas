...
begin
  ...
  begin
       Application.MessageBox('Acesso negado para Aprovação de CIT' + #13 + 'Consulte o Administrador do sistema','Atenção',mb_iconwarning + mb_ok);
   end
   else
   begin
...
begin
  ...
  begin
       Application.MessageBox('Acesso negado para Exclusão da CIT.' + #13 + 'Consulte o Administrador do sistema','Atenção',mb_iconwarning + mb_ok);
   end
   else
   begin
        ...
        begin
            ...
            VLC_Mensagem := VLC_Mensagem + 'Segue a confirmação do CANCELAMENTO da CIT ' + CIT_Exclusao.CDSCIT.AsString + #13;
...
begin
  ...
  // Se a pessoa tem acesso a aprovar ela poderá visualizar o relatório
  ...
  begin
      ...
      begin
          Application.MessageBox('CIT sem aprovação para impressão','Atenção',mb_iconwarning + mb_ok);
          Exit;
      end;
  end;
...
begin
  ...
  If Sys_FuncaoSistema.GetVerificaSeExistePermissao(129,Sys_FuncaoSistema.PNL_LoginAtivo.Caption) = False then
  begin
       Application.MessageBox('Acesso negado para Inclusão da CIT.' + #13 + 'Consulte o Administrador do sistema','Atenção',mb_iconwarning + mb_ok);
   end
   else
   begin
  ...
  begin
   ...
   begin
          ...
          begin
                  if DS.DataSet.FieldByName('StatusAprovacao').AsInteger = 0 then
                  begin
                      Application.MessageBox('Certificado sem aprovação para impressão','Atenção',mb_iconwarning + mb_ok);
                      Exit;
                  end;
          end;
          ...
          begin
                Application.MessageBox('Necessário primeiro fazer a CIT antes de tirar o Certificado de Qualidade. Precisamos da informação dos Lacres','Atenção',mb_iconwarning + mb_ok);
                Exit;
          end;

          if CDSImprimirCIT.AsString = 'Não' then
          begin
                Application.MessageBox('Necessário primeiro fazer a CIT antes de tirar o Certificado de Qualidade. Precisamos da informação dos Lacres','Atenção',mb_iconwarning + mb_ok);
                Exit;
          end;
