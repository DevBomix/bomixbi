...
begin
  inherited;
  ppLabel22.Text := 'Pedidos datados até 24 hrs: ' + FormatFloat('#,##0.00',(((ppDBCalc_24Horas.Value)*100)/ppDBCalc_Total.Value)) + '%';
end;

procedure TRelatorioAnaliseDataPCP.ppLabel25Print(Sender: TObject);
begin
  inherited;
 ppLabel25.Text := 'Pedidos datados até 48 hrs: ' + FormatFloat('#,##0.00',(((ppDBCalc_48Horas.Value)*100)/ppDBCalc_Total.Value)) + '%';
end;
...
begin
  inherited;
  ppLabelFraseInicial.Text := 'É visto que ' + FormatFloat('#,##0.00',(((ppDBCalc_MesmoDia.Value + ppDBCalc_24Horas.Value)*100)/ppDBCalc_Total.Value)) + ' % dos pedidos estão sendo datados até 24 hrs.';
...
begin
  ...
  begin
     LBL_PeriodoPedidoPCP.Caption :=  ' Data de Saída Entre ' + ' ' + DateToStr(DT_DataEmissao1.Date) + ' a ' + DateToStr(DT_DataEmissao2.Date);
...
begin
     ...
     CDS_Diario.Open;
      ...
      begin
         ppLabel6.Caption :=  ' Data de Saída Entre ' + ' ' + DateToStr(DT_DataEmissao1.Date) + ' a ' + DateToStr(DT_DataEmissao2.Date);
...
begin
  if CBX_TipoPedido.Text = 'RELATÓRIO ANALITICO'  then
  begin
      RelatorioAnalitico;
  end;

  if CBX_TipoPedido.Text = 'RELATÓRIO DIÁRIO'  then
  begin
      RelatorioDiario;
  end;
