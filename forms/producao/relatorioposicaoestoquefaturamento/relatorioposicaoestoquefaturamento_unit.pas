...
begin
  ...
  begin
      ...
      begin
          VLC_Where := VLC_Where + ' AND Personalizado = ' + '''' + 'Não' + '''' + #13;
      end;
  end;
  ...
  if CBX_Ordernamento.Text = 'POR EMISSÃO - CRESCENTE' then
      VLC_Order := ' Order by Emissao';

  if CBX_Ordernamento.Text = 'POR EMISSÃO - DECRESCENTE' then
  ...
  begin
      ppLBL_Data.Caption := 'Impressão: ' + DateTimeToStr(Date()) + '  por ' + Sys_FuncaoSistema.PNL_UsuarioAtivo.Caption;
      ppReport.Print;
  end;
