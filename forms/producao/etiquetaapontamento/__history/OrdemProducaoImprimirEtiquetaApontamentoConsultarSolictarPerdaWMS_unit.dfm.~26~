inherited OrdemProducao_ImprimirEtiquetaApontamento_GerarConsultaWMS: TOrdemProducao_ImprimirEtiquetaApontamento_GerarConsultaWMS
  Caption = 'Solicitar Perda WMS'
  ClientHeight = 558
  ClientWidth = 885
  ExplicitWidth = 891
  ExplicitHeight = 587
  ...
  object Panel1: TPanel [3]
    ...
    object PNL_Prioridade: TPanel
      ...
      object Panel2: TPanel
        ...
        Height = 32
        Align = alTop
        Caption = 'Quantidade a Socilitar'
        TabOrder = 1
      end
      object TXT_Quantidade: TMaskEdit
        Left = 35
        Top = 65
        Width = 235
        Height = 42
  ...
  inherited CDS: TClientDataSet
    ...
    object CDSProduzida: TFloatField
      FieldName = 'Produzida'
    end
    object CDSSaldo: TFloatField
      FieldName = 'Saldo'
    end
    object CDSPerda_ID: TStringField
      FieldName = 'Perda_ID'
      Size = 10
    end
    object CDSPerda: TStringField
      FieldName = 'Perda'
      Size = 150
    end
  end
  inherited Query: TADOQuery
    SQL.Strings = (
      ' Select '
      ' '#9'OP.Recno, OP.Empresa, Setor, Area, OrdemProducao, Status, '
      ' '#9'OP.Cliente, '
      ' '#9'Emissao, Injetora as Recurso, '
      ' '#9'OP.Produto_ID, OP.Produto, '
      
        ' '#9'Quantidade, Produzida, Saldo, Produto.Liso_ID as Perda_ID, Lis' +
        'o.Produto as Perda '
      ' from BomixBI.dbo.Pcp_TB_OrdemProducao OP (nolock) '
      
        ' Inner Join BomixBI.dbo.Pcp_TB_Produto Produto (nolock) ON Produ' +
        'to.Empresa = '#39'0101'#39
      ' '#9#9#9#9#9#9#9#9#9#9#9#9#9'  AND Produto.Produto_ID = OP.Produto_ID '
      
        ' Inner Join BomixBI.dbo.Pcp_TB_Produto Liso (nolock) ON Liso.Emp' +
        'resa = '#39'0101'#39
      ' '#9#9#9#9#9#9#9#9#9#9#9'       AND Produto.Liso_ID = Liso.Produto_ID '
      ' Where Area = '#39'PERSONALIZACAO'#39
      ' AND OP.Empresa = '#39'0101'#39
      ' AND Prioridade < 500 '
      ' AND Status not in ('#39'FINALIZADA'#39','#39'SUSPENSA'#39')'
      ' AND Produzida > 0 '
      
        ' AND (OP.Produto_ID + OrdemProducao + OP.Produto + OP.Cliente + ' +
        'Produto.Liso_ID + Liso.Produto) like '#39'%P50827001001%'#39' '#11
      'Order by OP.Recno desc  ')
    Left = 380
    Top = 160
    ...
    object QueryProduzida: TFloatField
      FieldName = 'Produzida'
    end
    object QuerySaldo: TFloatField
      FieldName = 'Saldo'
    end
    object QueryPerda_ID: TStringField
      FieldName = 'Perda_ID'
      Size = 10
    end
    object QueryPerda: TStringField
      FieldName = 'Perda'
      Size = 150
    end
  end
