...
function TRelatorio_PosicaoEstoque_SaldoPerda.GetSelect: String;
Var
  VLC_Select : String;
begin
  inherited;
    ...
    VLC_Select := VLC_Select + '	Item_FK, ItemSemTipoAlca as Item, Produto_ID, Produto, Bloqueado,  ' + #13;
    VLC_Select := VLC_Select + '	B2_QATU as Saldo, Produto.UndMedida, B2_QATU * Produto.PesoSemAlca as PesoKG, Produto.PesoSemAlca as PesoItem ' + #13;
    VLC_Select := VLC_Select + 'from P12OFICIAL.dbo.SB2010 B2 (nolock) ' + #13;
    VLC_Select := VLC_Select + 'Inner Join (  ' + #13;
    VLC_Select := VLC_Select + '			Select  ' + #13;
    VLC_Select := VLC_Select + '				Distinct B1_FILIAL, B1_FSPRODC   ' + #13;
    VLC_Select := VLC_Select + '			from P12OFICIAL.dbo.SB1010 (nolock)  ' + #13;
    VLC_Select := VLC_Select + '			Where B1_FSPRODC <> ' + '''' + '' + '''' + #13;
    ...
    VLC_Select := VLC_Select + ') TB ON B2_FILIAL   = ' + '''' + Sys_FuncaoSistema.PNL_EmpresaTotvs.Caption + '''' + #13;
    VLC_Select := VLC_Select + '	AND TB.B1_FSPRODC = B2_COD  ' + #13;
...
function TRelatorio_PosicaoEstoque_SaldoPerda.ImprimirRelatorio: Boolean;
Var
   VLC_Select : String;
begin
