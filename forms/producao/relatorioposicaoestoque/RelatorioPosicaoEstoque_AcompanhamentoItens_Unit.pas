...
type
  TRelatorio_PosicaoEstoque_AcompanhamentoItens = class(TForm)
    ...
    QuerySaldoNegativo: TStringField;
    CDSSaldoNegativo: TStringField;
    ppLabel6: TppLabel;
    ppDBText7: TppDBText;
...
begin
  ...
  VLC_Select := VLC_Select + ' Select *, Cast(SaldoTotalB8 as decimal) - Cast(SaldoB2 as decimal) as Diferenca, Case when Cast(SaldoTotalB8 as decimal) - Cast(SaldoB2 as decimal) = 0 then ' + '''' + '' + '''' + ' else ' + '''' + 'DESBALANCEADO' + '''' + ' end as Desbalanceado  from (' + #13;
  ...
  VLC_Select := VLC_Select + '      		Produto.Classificacao + ' + '''' + ' - ' + '''' + ' + Produto.ClassificacaoDesc as Classificacao,   ' + #13;
  ...
  VLC_Select := VLC_Select + '				Where Total_B8.B8_FILIAL = ' + '''' + Sys_FuncaoSistema.PNL_EmpresaTotvs.Caption + '''' + #13;
  VLC_Select := VLC_Select + '				AND Total_B8.D_E_L_E_T_  <> ' + '''' + '*' + '''' + #13;
  VLC_Select := VLC_Select + '				AND Total_B8.B8_SALDO <> 0   ' + #13;
  ...
  VLC_Select := VLC_Select + '       						' + '''' + 'De ' + '''' + ' + Convert(Varchar(10),(Convert(Datetime,B8_DATA,112)),103) + ' + '''' + ' até ' + '''' + #13;
  ...
  VLC_Select := VLC_Select + '         ' + #13;
  VLC_Select := VLC_Select + '         ' + #13;
  VLC_Select := VLC_Select + ' 			Case when (  ' + #13;
  VLC_Select := VLC_Select + ' 			Select    ' + #13;
  VLC_Select := VLC_Select + ' 				Top 1 R_E_C_N_O_   ' + #13;
  ...
  VLC_Select := VLC_Select + ' 			AND B8_PRODUTO = B8.B8_PRODUTO  ' + #13;
  VLC_Select := VLC_Select + ' 			AND B8_LOCAL = B8.B8_LOCAL  ' + #13;
  VLC_Select := VLC_Select + ' 			AND B8_SALDO < 0) is NULL then ' + '''' + 'Não' + '''' + ' else ' + '''' + 'Sim' + '''' + ' end as SaldoNegativo ' + #13;
  VLC_Select := VLC_Select + '         ' + #13;
  VLC_Select := VLC_Select + '         ' + #13;
  ...
  if Relatorio_PosicaoEstoque.CBX_Igual_SaldoNegativo.Text <> 'AMBOS' then
  VLC_Select := VLC_Select + ' AND Upper(SaldoNegativo) = ' + '''' + Relatorio_PosicaoEstoque.CBX_Igual_SaldoNegativo.Text + '''' + #13;
  ...
  begin
        if Relatorio_PosicaoEstoque.CBX_Igual_Desbalanceado.Text = 'SIM' then
             VLC_Select := VLC_Select + ' AND Case when Cast(SaldoTotalB8 as decimal) - Cast(SaldoB2 as decimal) = 0 then ' + '''' + '' + '''' + ' else ' + '''' + 'DESBALANCEADO' + '''' + ' end = ' + '''' + 'DESBALANCEADO' + '''' + #13
        else
             VLC_Select := VLC_Select + ' AND Case when Cast(SaldoTotalB8 as decimal) - Cast(SaldoB2 as decimal) = 0 then ' + '''' + '' + '''' + ' else ' + '''' + 'DESBALANCEADO' + '''' + ' end = ' + '''' + '' + '''' + #13;
  end;
