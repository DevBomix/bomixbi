...
begin
    ...
    VLC_Select := VLC_Select + ' 				When Estoque.D3_TM = ' + '''' + '499' + '''' + ' then ' + '''' + 'DEVOLUCÃO TOTVS' + '''' + #13;
    ...
    VLC_Select := VLC_Select + '   ' + #13;
    VLC_Select := VLC_Select + ' 			Case   ' + #13;
    VLC_Select := VLC_Select + ' 				WHEN D3_CF = ' + '''' + 'DE0' + '''' + ' then ' + '''' + 'DEVOLUÇÃO MANUAL' + '''' + #13;
    VLC_Select := VLC_Select + ' 				WHEN D3_CF = ' + '''' + 'DE1' + '''' + ' then ' + '''' + 'DEVOLUÇÃO AUTOMÁTICA - ESTORNO DA PRODUÇÃO' + '''' + #13;
    VLC_Select := VLC_Select + ' 				WHEN D3_CF = ' + '''' + 'DE2' + '''' + ' then ' + '''' + 'DEVOLUÇÃO AUTOMÁTICA DE MATERIAL DE APROPRIAÇÃO INDIRETA - ESTORNO DA PRODUÇÃO' + '''' + #13;
    VLC_Select := VLC_Select + ' 				WHEN D3_CF = ' + '''' + 'DE3' + '''' + ' then ' + '''' + 'ESTORNO DE TRANSFERÊNCIA PARA LOCAL DE APROPRIAÇÃO INDIRETA' + '''' + #13;
    VLC_Select := VLC_Select + ' 				WHEN D3_CF = ' + '''' + 'DE4' + '''' + ' then ' + '''' + 'DEVOLUÇÃO DE TRANSFERÊNCIA ENTRE LOCAIS' + '''' + #13;
    VLC_Select := VLC_Select + ' 				WHEN D3_CF = ' + '''' + 'DE5' + '''' + ' then ' + '''' + 'DEVOLUÇÃO DE MATERIAL APROPRIADO EM OP - EXCLUSÃO DE NOTA FISCAL DE ENTRADA' + '''' + #13;
    VLC_Select := VLC_Select + ' 				WHEN D3_CF = ' + '''' + 'DE6' + '''' + ' then ' + '''' + 'DEVOLUÇÃO VALORIZADA' + '''' + #13;
    VLC_Select := VLC_Select + ' 				WHEN D3_CF = ' + '''' + 'DE7' + '''' + ' then ' + '''' + 'DEVOLUÇÃO DE TRANSFERÊNCIA DE UM PARA N' + '''' + #13;
    VLC_Select := VLC_Select + ' 				WHEN D3_CF = ' + '''' + 'RE0' + '''' + ' then ' + '''' + 'REQUISIÇÃO MANUAL' + '''' + #13;
    VLC_Select := VLC_Select + ' 				WHEN D3_CF = ' + '''' + 'RE1' + '''' + ' then ' + '''' + 'REQUISIÇÃO AUTOMÁTICA' + '''' + #13;
    VLC_Select := VLC_Select + ' 				WHEN D3_CF = ' + '''' + 'RE2' + '''' + ' then ' + '''' + 'REQUISIÇÃO AUTOMÁTICA DE MATERIAL DE APROPRIAÇÃO INDIRETA' + '''' + #13;
    VLC_Select := VLC_Select + ' 				WHEN D3_CF = ' + '''' + 'RE3' + '''' + ' then ' + '''' + 'TRANSFERÊNCIA PARA LOCAL DE APROPRIAÇÃO INDIRETA' + '''' + #13;
    VLC_Select := VLC_Select + ' 				WHEN D3_CF = ' + '''' + 'RE4' + '''' + ' then ' + '''' + 'REQUISIÇÃO POR TRANSFERÊNCIA' + '''' + #13;
    VLC_Select := VLC_Select + ' 				WHEN D3_CF = ' + '''' + 'RE5' + '''' + ' then ' + '''' + 'REQUISIÇÃO INFORMANDO OP NA NOTA FISCAL DE ENTRADA' + '''' + #13;
    VLC_Select := VLC_Select + ' 				WHEN D3_CF = ' + '''' + 'RE6' + '''' + ' then ' + '''' + 'REQUISIÇÃO  VALORIZADA' + '''' + #13;
    VLC_Select := VLC_Select + ' 				WHEN D3_CF = ' + '''' + 'RE7' + '''' + ' then ' + '''' + 'REQUISIÇÃO PARA TRANSFERÊNCIA DE UM PARA  N' + '''' + #13;
    VLC_Select := VLC_Select + ' 				WHEN D3_CF = ' + '''' + 'PR0' + '''' + ' then ' + '''' + 'PRODUÇÃO MANUAL' + '''' + #13;
    VLC_Select := VLC_Select + ' 			END AS Classificacao, ' + #13;
    VLC_Select := VLC_Select + '   ' + #13;
    VLC_Select := VLC_Select + ' 			RTRIM(Estoque.D3_DOC) AS Documento, Case when D3_OP = ' + '''' + '' + '''' + ' then D3_DOC else D3_OP end as OP, Estoque.D3_NUMSEQ AS Sequencia, ' + #13;
    VLC_Select := VLC_Select + ' 			RTRIM(Estoque.D3_COD) AS Produto_FK, Rtrim(Produto.B1_DESC) as Produto, Estoque.D3_TIPO AS ProdutoTipo, ISNULL(D5_QUANT,D3_QUANT) AS Quantidade, ' + #13;
    VLC_Select := VLC_Select + ' 			Rtrim(Estoque.D3_UM) AS Unidade, Isnull(Rtrim(D5_LOTECTL),D3_LOTECTL) AS Lote, ' + '''' + '' + ''''+ ' as LoteFornecedor, ' + #13;
    VLC_Select := VLC_Select + ' 			Isnull(Case When Year(D5_DTVALID) = 1900 then NULL else CONVERT(Datetime, D5_DTVALID, 112) end,CONVERT(Datetime, D3_DTVALID, 112)) AS DataValidade, ' + #13;
    VLC_Select := VLC_Select + ' ' + #13;
    ...
    VLC_Select := VLC_Select + ' 			Case when Estoque.D3_ESTORNO = ' + '''' + 'S' + '''' + ' then ' + '''' + 'SIM' + '''' + ' else ' + '''' + 'NÃO' + '''' + ' End as Estornado ' + #13;
    VLC_Select := VLC_Select + ' ' + #13;
    ...
    VLC_Select := VLC_Select + ' 		Left Join P12OFICIAL.dbo.SD5010 D5 (nolock) ON D5_FILIAL = ' + '''' + Sys_FuncaoSistema.PNL_EmpresaTotvs.Caption + '''' + #13;
    ...
    VLC_Select := VLC_Select + ' 														   AND D5_PRODUTO = Estoque.D3_COD ' + #13;
    ...
    // Lote
    If PadraoBuscadorFiltro.RetornarWhereFiltro(Relatorio_MovimentosInternos.CBX_OperadorLote,Relatorio_MovimentosInternos.PNL_SinalLote,Relatorio_MovimentosInternos.TXT_Valor1_Lote,Relatorio_MovimentosInternos.TXT_Valor2_Lote,'Rtrim(Rtrim(D5.D5_LOTECTL))','Caracter') <> '' then
         VLC_Select := VLC_Select + 'AND ' + PadraoBuscadorFiltro.RetornarWhereFiltro(Relatorio_MovimentosInternos.CBX_OperadorLote,Relatorio_MovimentosInternos.PNL_SinalLote,Relatorio_MovimentosInternos.TXT_Valor1_Lote,Relatorio_MovimentosInternos.TXT_Valor2_Lote,'Rtrim(Rtrim(D5.D5_LOTECTL))','Caracter');
    ...
    // Ordem de Produção
    If PadraoBuscadorFiltro.RetornarWhereFiltro(Relatorio_MovimentosInternos.CBX_OperadorOrdemProducao,Relatorio_MovimentosInternos.PNL_SinalOrdemProducao,Relatorio_MovimentosInternos.TXT_Valor1_OrdemProducao,Relatorio_MovimentosInternos.TXT_Valor2_OrdemProducao,'Rtrim(Estoque.D3_OP)','Caracter') <> '' then
         VLC_Select := VLC_Select + 'AND ' + PadraoBuscadorFiltro.RetornarWhereFiltro(Relatorio_MovimentosInternos.CBX_OperadorOrdemProducao,Relatorio_MovimentosInternos.PNL_SinalOrdemProducao,Relatorio_MovimentosInternos.TXT_Valor1_OrdemProducao,Relatorio_MovimentosInternos.TXT_Valor2_OrdemProducao,'Rtrim(Estoque.D3_OP)','Caracter');
...
begin
    ...
    VLC_Select := VLC_Select + '   ' + #13;
    VLC_Select := VLC_Select + ' 			RTRIM(Estoque.D1_DOC) AS Documento, RTRIM(Estoque.D1_DOC) AS OP, Estoque.D1_NUMSEQ AS Sequencia, ' + #13;
    ...
    VLC_Select := VLC_Select + ' 			Rtrim(Estoque.D1_UM) AS Unidade, Rtrim(Estoque.D1_LOTECTL) AS Lote, Rtrim(Estoque.D1_LOTEFOR) AS LoteFornecedor, Case When Year(Estoque.D1_DTVALID) = 1900 then NULL else CONVERT(Datetime, Estoque.D1_DTVALID, 112) end AS DataValidade, ' + #13;
    VLC_Select := VLC_Select + ' ' + #13;
    ...
    VLC_Select := VLC_Select + ' 		' + '''' + 'NÃO' + '''' + '  as Estornado ' + #13;
    VLC_Select := VLC_Select + ' ' + #13;
    ...
    // Documento
    If PadraoBuscadorFiltro.RetornarWhereFiltro(Relatorio_MovimentosInternos.CBX_OperadorDocumento,Relatorio_MovimentosInternos.PNL_SinalDocumento,Relatorio_MovimentosInternos.TXT_Valor1_Documento,Relatorio_MovimentosInternos.TXT_Valor2_Documento,'Rtrim(Estoque.D1_DOC)','Caracter') <> '' then
         VLC_Select := VLC_Select + 'AND ' + PadraoBuscadorFiltro.RetornarWhereFiltro(Relatorio_MovimentosInternos.CBX_OperadorDocumento,Relatorio_MovimentosInternos.PNL_SinalDocumento,Relatorio_MovimentosInternos.TXT_Valor1_Documento,Relatorio_MovimentosInternos.TXT_Valor2_Documento,'Rtrim(Estoque.D1_DOC)','Caracter');

    // Ordem de Produção
    If PadraoBuscadorFiltro.RetornarWhereFiltro(Relatorio_MovimentosInternos.CBX_OperadorOrdemProducao,Relatorio_MovimentosInternos.PNL_SinalOrdemProducao,Relatorio_MovimentosInternos.TXT_Valor1_OrdemProducao,Relatorio_MovimentosInternos.TXT_Valor2_OrdemProducao,'Rtrim(Estoque.D1_DOC)','Caracter') <> '' then
         VLC_Select := VLC_Select + 'AND ' + PadraoBuscadorFiltro.RetornarWhereFiltro(Relatorio_MovimentosInternos.CBX_OperadorOrdemProducao,Relatorio_MovimentosInternos.PNL_SinalOrdemProducao,Relatorio_MovimentosInternos.TXT_Valor1_OrdemProducao,Relatorio_MovimentosInternos.TXT_Valor2_OrdemProducao,'Rtrim(Estoque.D1_DOC)','Caracter');
...
begin
    ...
    VLC_Select := VLC_Select + '   ' + #13;
    VLC_Select := VLC_Select + ' 			RTRIM(Estoque.D2_DOC) AS Documento, RTRIM(Estoque.D2_DOC) AS OP, Estoque.D2_NUMSEQ AS Sequencia, ' + #13;
    ...
    VLC_Select := VLC_Select + ' 			Rtrim(Estoque.D2_UM) AS Unidade, Rtrim(Estoque.D2_LOTECTL) AS Lote, ' + '''' + '' + ''''+ ' as LoteFornecedor, Case When Year(Estoque.D2_DTVALID) = 1900 then NULL else CONVERT(Datetime, Estoque.D2_DTVALID, 112) end AS DataValidade, ' + #13;
    VLC_Select := VLC_Select + ' ' + #13;
    ...
    // Documento
    If PadraoBuscadorFiltro.RetornarWhereFiltro(Relatorio_MovimentosInternos.CBX_OperadorDocumento,Relatorio_MovimentosInternos.PNL_SinalDocumento,Relatorio_MovimentosInternos.TXT_Valor1_Documento,Relatorio_MovimentosInternos.TXT_Valor2_Documento,'Rtrim(Estoque.D2_DOC)','Caracter') <> '' then
         VLC_Select := VLC_Select + 'AND ' + PadraoBuscadorFiltro.RetornarWhereFiltro(Relatorio_MovimentosInternos.CBX_OperadorDocumento,Relatorio_MovimentosInternos.PNL_SinalDocumento,Relatorio_MovimentosInternos.TXT_Valor1_Documento,Relatorio_MovimentosInternos.TXT_Valor2_Documento,'Rtrim(Estoque.D2_DOC)','Caracter');

    // Ordem de Produção
    If PadraoBuscadorFiltro.RetornarWhereFiltro(Relatorio_MovimentosInternos.CBX_OperadorOrdemProducao,Relatorio_MovimentosInternos.PNL_SinalOrdemProducao,Relatorio_MovimentosInternos.TXT_Valor1_OrdemProducao,Relatorio_MovimentosInternos.TXT_Valor2_OrdemProducao,'Rtrim(Estoque.D2_DOC)','Caracter') <> '' then
         VLC_Select := VLC_Select + 'AND ' + PadraoBuscadorFiltro.RetornarWhereFiltro(Relatorio_MovimentosInternos.CBX_OperadorOrdemProducao,Relatorio_MovimentosInternos.PNL_SinalOrdemProducao,Relatorio_MovimentosInternos.TXT_Valor1_OrdemProducao,Relatorio_MovimentosInternos.TXT_Valor2_OrdemProducao,'Rtrim(Estoque.D2_DOC)','Caracter');
...
begin
    ...
    VLC_Select := VLC_Select + ' Order by Empresa, Armazem_ID, Emissao desc, Produto_FK, Tipo  ' + #13;
