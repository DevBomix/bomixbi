object Relatorio_ApontamentoProducao_MiguelAnalitico: TRelatorio_ApontamentoProducao_MiguelAnalitico
  Left = 0
  Top = 0
  Caption = 'Relatorio_ApontamentoProducao_MiguelAnalitico'
  ClientHeight = 221
  ClientWidth = 518
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  PixelsPerInch = 96
  TextHeight = 13
  object ppReport: TppReport
    AutoStop = False
    DataPipeline = Pipeline
    PrinterSetup.BinName = 'Default'
    PrinterSetup.DocumentName = 'Report'
    PrinterSetup.Orientation = poLandscape
    PrinterSetup.PaperName = 'A4'
    PrinterSetup.PrinterName = 'pdfFactory Pro'
    PrinterSetup.SaveDeviceSettings = False
    PrinterSetup.mmMarginBottom = 7620
    PrinterSetup.mmMarginLeft = 7620
    PrinterSetup.mmMarginRight = 7620
    PrinterSetup.mmMarginTop = 7620
    PrinterSetup.mmPaperHeight = 210000
    PrinterSetup.mmPaperWidth = 297000
    PrinterSetup.PaperSize = 9
    AllowPrintToArchive = True
    AllowPrintToFile = True
    ArchiveFileName = '($MyDocuments)\ReportArchive.raf'
    DeviceType = 'Screen'
    DefaultFileDeviceType = 'XLSData'
    EmailSettings.ReportFormat = 'PDF'
    LanguageID = 'Default'
    OpenFile = False
    OutlineSettings.CreateNode = True
    OutlineSettings.CreatePageNodes = True
    OutlineSettings.Enabled = True
    OutlineSettings.Visible = True
    ThumbnailSettings.Enabled = True
    ThumbnailSettings.Visible = True
    ThumbnailSettings.DeadSpace = 30
    PDFSettings.EmbedFontOptions = [efUseSubset]
    PDFSettings.EncryptSettings.AllowCopy = True
    PDFSettings.EncryptSettings.AllowInteract = True
    PDFSettings.EncryptSettings.AllowModify = True
    PDFSettings.EncryptSettings.AllowPrint = True
    PDFSettings.EncryptSettings.Enabled = False
    PDFSettings.EncryptSettings.KeyLength = kl40Bit
    PDFSettings.FontEncoding = feAnsi
    PDFSettings.ImageCompressionLevel = 25
    PreviewFormSettings.WindowState = wsMaximized
    PreviewFormSettings.ZoomSetting = zsPageWidth
    RTFSettings.DefaultFont.Charset = DEFAULT_CHARSET
    RTFSettings.DefaultFont.Color = clWindowText
    RTFSettings.DefaultFont.Height = -13
    RTFSettings.DefaultFont.Name = 'Arial'
    RTFSettings.DefaultFont.Style = []
    TextFileName = '($MyDocuments)\Report.xls'
    TextSearchSettings.DefaultString = '<Texto a localizar>'
    TextSearchSettings.Enabled = True
    XLSSettings.AppName = 'ReportBuilder'
    XLSSettings.Author = 'ReportBuilder'
    XLSSettings.Subject = 'Report'
    XLSSettings.Title = 'Report'
    Left = 199
    Top = 31
    Version = '15.04'
    mmColumnWidth = 284300
    DataPipelineName = 'Pipeline'
    object ppHeaderBand2: TppHeaderBand
      Background.Brush.Style = bsClear
      mmBottomOffset = 0
      mmHeight = 14288
      mmPrintPosition = 0
      object ppLine7: TppLine
        UserName = 'Line1'
        Pen.Width = 3
        Weight = 2.250000000000000000
        mmHeight = 2646
        mmLeft = 1588
        mmTop = 11642
        mmWidth = 278073
        BandType = 0
        LayerName = BandLayer6
      end
      object ppImage2: TppImage
        UserName = 'ppIMG_Logo'
        AlignHorizontal = ahCenter
        AlignVertical = avCenter
        MaintainAspectRatio = False
        Stretch = True
        Picture.Data = {
          
          FFFF}
        mmHeight = 8731
        mmLeft = 1588
        mmTop = 2118
        mmWidth = 27252
        BandType = 0
        LayerName = BandLayer6
      end
      object ppLabel30: TppLabel
        UserName = 'ppLBL_TituloRelatorio'
        HyperlinkEnabled = False
        Caption = 'An'#225'lise de Perdas, Efici'#234'ncia e Produtividade'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 20
        Font.Style = [fsBold]
        Transparent = True
        mmHeight = 8466
        mmLeft = 32014
        mmTop = 2117
        mmWidth = 157428
        BandType = 0
        LayerName = BandLayer6
      end
      object ppLabel29: TppLabel
        UserName = 'LBL_Periodo'
        HyperlinkEnabled = False
        Caption = 'LBL_Periodo'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = [fsBold]
        TextAlignment = taRightJustified
        Transparent = True
        mmHeight = 4233
        mmLeft = 257705
        mmTop = 2117
        mmWidth = 21960
        BandType = 0
        LayerName = BandLayer6
      end
    end
    object ppDetailBand4: TppDetailBand
      Background1.Brush.Style = bsClear
      Background2.Brush.Style = bsClear
      PrintHeight = phDynamic
      mmBottomOffset = 0
      mmHeight = 13758
      mmPrintPosition = 0
      object ppDBText43: TppDBText
        UserName = 'DBText36'
        HyperlinkEnabled = False
        DataField = 'Turno'
        DataPipeline = Pipeline
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = [fsBold]
        TextAlignment = taCentered
        Transparent = True
        VerticalAlignment = avCenter
        DataPipelineName = 'Pipeline'
        mmHeight = 13229
        mmLeft = 2381
        mmTop = 265
        mmWidth = 7408
        BandType = 4
        LayerName = BandLayer6
      end
      object ppDBText3: TppDBText
        UserName = 'DBText3'
        HyperlinkEnabled = False
        DataField = 'PerdaUN'
        DataPipeline = Pipeline
        DisplayFormat = '#,0;-#,0'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 6
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        VerticalAlignment = avCenter
        DataPipelineName = 'Pipeline'
        mmHeight = 13229
        mmLeft = 74160
        mmTop = 265
        mmWidth = 5292
        BandType = 4
        LayerName = BandLayer6
      end
      object TXT_Perda_PorTurno: TppDBText
        OnPrint = TXT_Perda_PorTurnoPrint
        UserName = 'TXT_Perda_PorTurno'
        HyperlinkEnabled = False
        DataField = 'PerdaKGPerc'
        DataPipeline = Pipeline
        DisplayFormat = '#,0.00;-#,0.00'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 6
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        VerticalAlignment = avCenter
        DataPipelineName = 'Pipeline'
        mmHeight = 13229
        mmLeft = 83386
        mmTop = 265
        mmWidth = 5821
        BandType = 4
        LayerName = BandLayer6
      end
      object ppDBMemo1: TppDBMemo
        UserName = 'DBMemo1'
        CharWrap = True
        DataField = 'Causa_Perda'
        DataPipeline = Pipeline
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 5
        Font.Style = []
        RemoveEmptyLines = False
        Transparent = True
        VerticalAlignment = avCenter
        DataPipelineName = 'Pipeline'
        mmHeight = 13229
        mmLeft = 92944
        mmTop = 265
        mmWidth = 50778
        BandType = 4
        LayerName = BandLayer6
        mmBottomOffset = 0
        mmOverFlowOffset = 0
        mmStopPosition = 0
        mmMinHeight = 0
        mmLeading = 0
      end
      object ppDBText8: TppDBText
        UserName = 'DBText8'
        HyperlinkEnabled = False
        DataField = 'CicloReal'
        DataPipeline = Pipeline
        DisplayFormat = '#,0.00;(#,0.00)'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 6
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        VerticalAlignment = avCenter
        DataPipelineName = 'Pipeline'
        mmHeight = 13229
        mmLeft = 154056
        mmTop = 264
        mmWidth = 5556
        BandType = 4
        LayerName = BandLayer6
      end
      object ppDBText9: TppDBText
        UserName = 'DBText9'
        HyperlinkEnabled = False
        DataField = 'HoraInicio'
        DataPipeline = Pipeline
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 6
        Font.Style = []
        TextAlignment = taCentered
        Transparent = True
        VerticalAlignment = avCenter
        DataPipelineName = 'Pipeline'
        mmHeight = 13229
        mmLeft = 161729
        mmTop = 264
        mmWidth = 8464
        BandType = 4
        LayerName = BandLayer6
      end
      object ppDBText10: TppDBText
        UserName = 'DBText10'
        HyperlinkEnabled = False
        DataField = 'HoraFim'
        DataPipeline = Pipeline
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 6
        Font.Style = []
        TextAlignment = taCentered
        Transparent = True
        VerticalAlignment = avCenter
        DataPipelineName = 'Pipeline'
        mmHeight = 13229
        mmLeft = 171254
        mmTop = 264
        mmWidth = 7667
        BandType = 4
        LayerName = BandLayer6
      end
      object ppDBMemo2: TppDBMemo
        UserName = 'DBMemo2'
        CharWrap = True
        DataField = 'Causa_Improdutividade'
        DataPipeline = Pipeline
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 5
        Font.Style = []
        RemoveEmptyLines = False
        SuppressRepeatedValues = True
        Transparent = True
        VerticalAlignment = avCenter
        DataPipelineName = 'Pipeline'
        mmHeight = 13229
        mmLeft = 235253
        mmTop = 265
        mmWidth = 43355
        BandType = 4
        LayerName = BandLayer6
        mmBottomOffset = 0
        mmOverFlowOffset = 0
        mmStopPosition = 0
        mmMinHeight = 0
        mmLeading = 0
      end
      object ppDBMemo3: TppDBMemo
        UserName = 'DBMemo3'
        CharWrap = True
        DataField = 'Item'
        DataPipeline = Pipeline
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 6
        Font.Style = []
        RemoveEmptyLines = False
        TextAlignment = taCentered
        Transparent = True
        VerticalAlignment = avCenter
        DataPipelineName = 'Pipeline'
        mmHeight = 13229
        mmLeft = 9789
        mmTop = 265
        mmWidth = 23775
        BandType = 4
        LayerName = BandLayer6
        mmBottomOffset = 0
        mmOverFlowOffset = 0
        mmStopPosition = 0
        mmMinHeight = 0
        mmLeading = 0
      end
      object ppDBText5: TppDBText
        UserName = 'DBText5'
        HyperlinkEnabled = False
        DataField = 'CicloPadrao'
        DataPipeline = Pipeline
        DisplayFormat = '#,0.00;(#,0.00)'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 6
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        VerticalAlignment = avCenter
        DataPipelineName = 'Pipeline'
        mmHeight = 13229
        mmLeft = 146074
        mmTop = 264
        mmWidth = 5556
        BandType = 4
        LayerName = BandLayer6
      end
      object ppDBText15: TppDBText
        UserName = 'DBText15'
        HyperlinkEnabled = False
        BlankWhenZero = True
        DataField = 'TempoParadaMin'
        DataPipeline = Pipeline
        DisplayFormat = '#,0;-#,0'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 6
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        VerticalAlignment = avCenter
        DataPipelineName = 'Pipeline'
        mmHeight = 13229
        mmLeft = 222551
        mmTop = 265
        mmWidth = 8467
        BandType = 4
        LayerName = BandLayer6
      end
      object SHP_Perda: TppShape
        UserName = 'SHP_Perda'
        Brush.Color = clGreen
        Shape = stCircle
        mmHeight = 2646
        mmLeft = 86294
        mmTop = 1588
        mmWidth = 2646
        BandType = 4
        LayerName = BandLayer6
      end
      object TXT_Eficiencia_PorTurno: TppDBText
        OnPrint = TXT_Eficiencia_PorTurnoPrint
        UserName = 'TXT_Eficiencia_PorTurno'
        HyperlinkEnabled = False
        DataField = 'EficienciaNominal'
        DataPipeline = Pipeline
        DisplayFormat = '#,0.00;-#,0.00'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 6
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        VerticalAlignment = avCenter
        DataPipelineName = 'Pipeline'
        mmHeight = 13229
        mmLeft = 191030
        mmTop = 529
        mmWidth = 11377
        BandType = 4
        LayerName = BandLayer6
      end
      object TXT_Produtividade_PorTurno: TppDBText
        OnPrint = TXT_Produtividade_PorTurnoPrint
        UserName = 'TXT_Produtividade_PorTurno'
        HyperlinkEnabled = False
        DataField = 'Produtividade'
        DataPipeline = Pipeline
        DisplayFormat = '#,0.00;-#,0.00'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 6
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        VerticalAlignment = avCenter
        DataPipelineName = 'Pipeline'
        mmHeight = 13229
        mmLeft = 206676
        mmTop = 265
        mmWidth = 11113
        BandType = 4
        LayerName = BandLayer6
      end
      object SHP_Produtividade: TppShape
        UserName = 'SHP_Produtividade'
        Brush.Color = clGreen
        Shape = stCircle
        mmHeight = 2646
        mmLeft = 214313
        mmTop = 1586
        mmWidth = 2646
        BandType = 4
        LayerName = BandLayer6
      end
      object SHP_Eficiencia: TppShape
        UserName = 'SHP_Produtividade1'
        Brush.Color = clGreen
        Shape = stCircle
        mmHeight = 2646
        mmLeft = 198968
        mmTop = 1588
        mmWidth = 2646
        BandType = 4
        LayerName = BandLayer6
      end
      object ppDBText11: TppDBText
        UserName = 'DBText11'
        HyperlinkEnabled = False
        DataField = 'NominalTurno'
        DataPipeline = Pipeline
        DisplayFormat = '#,0;-#,0'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 6
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        VerticalAlignment = avCenter
        DataPipelineName = 'Pipeline'
        mmHeight = 13229
        mmLeft = 35178
        mmTop = 0
        mmWidth = 10055
        BandType = 4
        LayerName = BandLayer6
      end
      object TXT_CapacidadeReal_PorTurno: TppDBText
        UserName = 'TXT_CapacidadeReal_PorTurno'
        HyperlinkEnabled = False
        DataField = 'CapacidadeDisponivel'
        DataPipeline = Pipeline
        DisplayFormat = '#,0;-#,0'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 6
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        VerticalAlignment = avCenter
        DataPipelineName = 'Pipeline'
        mmHeight = 13229
        mmLeft = 47351
        mmTop = 265
        mmWidth = 10050
        BandType = 4
        LayerName = BandLayer6
      end
      object ppDBText2: TppDBText
        UserName = 'DBText2'
        HyperlinkEnabled = False
        DataField = 'Producao'
        DataPipeline = Pipeline
        DisplayFormat = '#,0;-#,0'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 6
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        VerticalAlignment = avCenter
        DataPipelineName = 'Pipeline'
        mmHeight = 13229
        mmLeft = 60324
        mmTop = 265
        mmWidth = 10319
        BandType = 4
        LayerName = BandLayer6
      end
      object SHP_Producao: TppShape
        UserName = 'SHP_Perda1'
        Brush.Color = clGreen
        Shape = stCircle
        mmHeight = 2646
        mmLeft = 66674
        mmTop = 1588
        mmWidth = 2646
        BandType = 4
        LayerName = BandLayer6
      end
      object TXT_Semafaro: TppDBText
        OnPrint = TXT_SemafaroPrint
        UserName = 'TXT_Semafaro'
        HyperlinkEnabled = False
        DataField = 'Semafaro'
        DataPipeline = Pipeline
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 6
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        Visible = False
        VerticalAlignment = avCenter
        DataPipelineName = 'Pipeline'
        mmHeight = 4763
        mmLeft = 69320
        mmTop = 6615
        mmWidth = 4763
        BandType = 4
        LayerName = BandLayer6
      end
      object ppDBText1: TppDBText
        UserName = 'TXT_CapacidadeReal_PorTurno1'
        HyperlinkEnabled = False
        DataField = 'TempoProducao'
        DataPipeline = Pipeline
        DisplayFormat = '#,0;-#,0'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 6
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        VerticalAlignment = avCenter
        DataPipelineName = 'Pipeline'
        mmHeight = 13229
        mmLeft = 180978
        mmTop = 265
        mmWidth = 7930
        BandType = 4
        LayerName = BandLayer6
      end
      object SHP_Ciclo: TppShape
        OnPrint = SHP_CicloPrint
        UserName = 'SHP_Perda2'
        Brush.Color = clGreen
        Shape = stCircle
        mmHeight = 2646
        mmLeft = 156117
        mmTop = 1588
        mmWidth = 2646
        BandType = 4
        LayerName = BandLayer6
      end
    end
    object ppFooterBand2: TppFooterBand
      Background.Brush.Style = bsClear
      mmBottomOffset = 0
      mmHeight = 19844
      mmPrintPosition = 0
      object ppLabel40: TppLabel
        UserName = 'ppLBL_UsuarioListaPorGerencia'
        HyperlinkEnabled = False
        Caption = 'ppLBL_Usuario'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = [fsBold]
        TextAlignment = taRightJustified
        Transparent = True
        mmHeight = 4233
        mmLeft = 180442
        mmTop = 2381
        mmWidth = 99219
        BandType = 8
        LayerName = BandLayer6
      end
      object ppLine9: TppLine
        UserName = 'Line12'
        Pen.Width = 3
        Weight = 2.250000000000000000
        mmHeight = 2646
        mmLeft = 1590
        mmTop = 265
        mmWidth = 280721
        BandType = 8
        LayerName = BandLayer6
      end
      object ppShape6: TppShape
        UserName = 'Shape5'
        mmHeight = 16669
        mmLeft = 1588
        mmTop = 1588
        mmWidth = 165100
        BandType = 8
        LayerName = BandLayer6
      end
      object ppLabel10: TppLabel
        UserName = 'Label10'
        Caption = '<= 0.4          > 0.4'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 7
        Font.Style = []
        mmHeight = 3175
        mmLeft = 79640
        mmTop = 13758
        mmWidth = 20373
        BandType = 8
        LayerName = BandLayer6
      end
      object ppLabel9: TppLabel
        UserName = 'Label9'
        Caption = '>= 98.5        Entre 98 e 98.5        <=98'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 7
        Font.Style = []
        mmHeight = 3175
        mmLeft = 79640
        mmTop = 7938
        mmWidth = 43127
        BandType = 8
        LayerName = BandLayer6
      end
      object ppLabel6: TppLabel
        UserName = 'Label6'
        Caption = '>= 98        < 98'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 7
        Font.Style = []
        mmHeight = 3175
        mmLeft = 34660
        mmTop = 13229
        mmWidth = 17198
        BandType = 8
        LayerName = BandLayer6
      end
      object ppLabel2: TppLabel
        UserName = 'Label2'
        Caption = 'Indicadores'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = [fsBold]
        TextAlignment = taCentered
        mmHeight = 3704
        mmLeft = 3969
        mmTop = 2381
        mmWidth = 17198
        BandType = 8
        LayerName = BandLayer6
      end
      object ppLabel3: TppLabel
        UserName = 'Label3'
        Caption = '% Produtividade'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 7
        Font.Style = [fsBold]
        mmHeight = 2910
        mmLeft = 3968
        mmTop = 7938
        mmWidth = 20374
        BandType = 8
        LayerName = BandLayer6
      end
      object ppLabel4: TppLabel
        UserName = 'Label4'
        Caption = '>= 98        < 98'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 7
        Font.Style = []
        mmHeight = 3175
        mmLeft = 34660
        mmTop = 7938
        mmWidth = 17198
        BandType = 8
        LayerName = BandLayer6
      end
      object ppShape7: TppShape
        UserName = 'Shape7'
        Brush.Color = clGreen
        Shape = stCircle
        mmHeight = 2646
        mmLeft = 30956
        mmTop = 7938
        mmWidth = 2646
        BandType = 8
        LayerName = BandLayer6
      end
      object ppShape8: TppShape
        UserName = 'Shape8'
        Brush.Color = clRed
        Shape = stCircle
        mmHeight = 2646
        mmLeft = 43127
        mmTop = 7938
        mmWidth = 2646
        BandType = 8
        LayerName = BandLayer6
      end
      object ppShape9: TppShape
        UserName = 'Shape9'
        Brush.Color = clRed
        Shape = stCircle
        mmHeight = 2646
        mmLeft = 43127
        mmTop = 13229
        mmWidth = 2646
        BandType = 8
        LayerName = BandLayer6
      end
      object ppShape10: TppShape
        UserName = 'Shape10'
        Brush.Color = clGreen
        Shape = stCircle
        mmHeight = 2646
        mmLeft = 30956
        mmTop = 13229
        mmWidth = 2646
        BandType = 8
        LayerName = BandLayer6
      end
      object ppLabel5: TppLabel
        UserName = 'Label5'
        Caption = '% Efici'#234'ncia Nominal'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 7
        Font.Style = [fsBold]
        mmHeight = 2911
        mmLeft = 3968
        mmTop = 13229
        mmWidth = 25930
        BandType = 8
        LayerName = BandLayer6
      end
      object ppLabel7: TppLabel
        UserName = 'Label7'
        Caption = '% Produ'#231#227'o'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 7
        Font.Style = [fsBold]
        mmHeight = 2910
        mmLeft = 57679
        mmTop = 7938
        mmWidth = 16142
        BandType = 8
        LayerName = BandLayer6
      end
      object ppShape11: TppShape
        UserName = 'Shape11'
        Brush.Color = clGreen
        Shape = stCircle
        mmHeight = 2646
        mmLeft = 76200
        mmTop = 7938
        mmWidth = 2646
        BandType = 8
        LayerName = BandLayer6
      end
      object ppLabel8: TppLabel
        UserName = 'Label8'
        Caption = '% Perdas KG'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 7
        Font.Style = [fsBold]
        mmHeight = 2911
        mmLeft = 57679
        mmTop = 13229
        mmWidth = 17201
        BandType = 8
        LayerName = BandLayer6
      end
      object ppShape14: TppShape
        UserName = 'Shape14'
        Brush.Color = clRed
        Shape = stCircle
        mmHeight = 2646
        mmLeft = 89959
        mmTop = 13758
        mmWidth = 2646
        BandType = 8
        LayerName = BandLayer6
      end
      object ppShape15: TppShape
        UserName = 'Shape15'
        Brush.Color = clGreen
        Shape = stCircle
        mmHeight = 2646
        mmLeft = 76200
        mmTop = 13758
        mmWidth = 2646
        BandType = 8
        LayerName = BandLayer6
      end
      object ppShape12: TppShape
        UserName = 'Shape12'
        Brush.Color = clRed
        Shape = stCircle
        mmHeight = 2646
        mmLeft = 112713
        mmTop = 7938
        mmWidth = 2646
        BandType = 8
        LayerName = BandLayer6
      end
      object ppShape13: TppShape
        UserName = 'Shape13'
        Brush.Color = clYellow
        Shape = stCircle
        mmHeight = 2646
        mmLeft = 89959
        mmTop = 7938
        mmWidth = 2646
        BandType = 8
        LayerName = BandLayer6
      end
      object ppLine2: TppLine
        UserName = 'Line3'
        Position = lpLeft
        Weight = 0.750000000000000000
        mmHeight = 13495
        mmLeft = 126998
        mmTop = 3174
        mmWidth = 2646
        BandType = 8
        LayerName = BandLayer6
      end
      object ppLabel11: TppLabel
        UserName = 'Label11'
        Caption = 'Tempo dos Turnos'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = [fsBold]
        TextAlignment = taCentered
        mmHeight = 3704
        mmLeft = 130174
        mmTop = 2381
        mmWidth = 28046
        BandType = 8
        LayerName = BandLayer6
      end
      object ppLabel12: TppLabel
        UserName = 'Label12'
        Caption = ' - Turno 01 -> 500 minutos'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 7
        Font.Style = []
        mmHeight = 3175
        mmLeft = 133086
        mmTop = 6879
        mmWidth = 29104
        BandType = 8
        LayerName = BandLayer6
      end
      object ppLabel13: TppLabel
        UserName = 'Label13'
        Caption = ' - Turno 02 -> 495 minutos'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 7
        Font.Style = []
        mmHeight = 3175
        mmLeft = 133086
        mmTop = 10319
        mmWidth = 29104
        BandType = 8
        LayerName = BandLayer6
      end
      object ppLabel14: TppLabel
        UserName = 'Label14'
        Caption = ' - Turno 03 -> 445 minutos'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 7
        Font.Style = []
        mmHeight = 3175
        mmLeft = 133086
        mmTop = 13758
        mmWidth = 29104
        BandType = 8
        LayerName = BandLayer6
      end
    end
    object ppSummaryBand4: TppSummaryBand
      Background.Brush.Style = bsClear
      mmBottomOffset = 0
      mmHeight = 0
      mmPrintPosition = 0
    end
    object ppPageStyle2: TppPageStyle
      Background.Brush.Style = bsClear
      EndPage = 0
      SinglePage = 0
      StartPage = 0
      mmBottomOffset = 0
      mmHeight = 0
      mmPrintPosition = 0
    end
    object ppGroup1: TppGroup
      BreakName = 'Setor'
      DataPipeline = Pipeline
      GroupFileSettings.NewFile = False
      GroupFileSettings.EmailFile = False
      OutlineSettings.CreateNode = True
      ReprintOnSubsequentPage = False
      StartOnOddPage = False
      UserName = 'Group_Setor'
      mmNewColumnThreshold = 0
      mmNewPageThreshold = 0
      DataPipelineName = 'Pipeline'
      NewFile = False
      object ppGroupHeaderBand1: TppGroupHeaderBand
        Background.Brush.Style = bsClear
        mmBottomOffset = 0
        mmHeight = 0
        mmPrintPosition = 0
      end
      object ppGroupFooterBand1: TppGroupFooterBand
        Background.Brush.Style = bsClear
        HideWhenOneDetail = False
        mmBottomOffset = 0
        mmHeight = 0
        mmPrintPosition = 0
      end
    end
    object ppGroup2: TppGroup
      BreakName = 'Recurso_ID'
      DataPipeline = Pipeline
      GroupFileSettings.NewFile = False
      GroupFileSettings.EmailFile = False
      KeepTogether = True
      OutlineSettings.CreateNode = True
      StartOnOddPage = False
      UserName = 'Group_Recurso_ID'
      mmNewColumnThreshold = 0
      mmNewPageThreshold = 0
      DataPipelineName = 'Pipeline'
      NewFile = False
      object ppGroupHeaderBand2: TppGroupHeaderBand
        Background.Brush.Style = bsClear
        mmBottomOffset = 0
        mmHeight = 20902
        mmPrintPosition = 0
        object ppShape5: TppShape
          UserName = 'Shape36'
          mmHeight = 20108
          mmLeft = 1589
          mmTop = 795
          mmWidth = 278078
          BandType = 3
          GroupNo = 1
          LayerName = BandLayer6
        end
        object ppMemo1: TppMemo
          UserName = 'Memo1'
          Border.BorderPositions = [bpLeft, bpTop, bpRight, bpBottom]
          Border.Visible = True
          Caption = 'Memo1'
          CharWrap = False
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 6
          Font.Style = [fsBold]
          Lines.Strings = (
            'Perdas'
            '(UN)')
          RemoveEmptyLines = False
          TextAlignment = taCentered
          Transparent = True
          VerticalAlignment = avCenter
          mmHeight = 8996
          mmLeft = 72532
          mmTop = 11906
          mmWidth = 9790
          BandType = 3
          GroupNo = 1
          LayerName = BandLayer6
          mmBottomOffset = 0
          mmOverFlowOffset = 0
          mmStopPosition = 0
          mmMinHeight = 0
          mmLeading = 0
        end
        object ppMemo8: TppMemo
          UserName = 'Memo8'
          Border.BorderPositions = [bpLeft, bpTop, bpRight, bpBottom]
          Border.Visible = True
          Caption = 'Memo8'
          CharWrap = False
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 6
          Font.Style = [fsBold]
          Lines.Strings = (
            'Causas - Perdas')
          RemoveEmptyLines = False
          TextAlignment = taCentered
          Transparent = True
          VerticalAlignment = avCenter
          mmHeight = 8996
          mmLeft = 91318
          mmTop = 11906
          mmWidth = 53173
          BandType = 3
          GroupNo = 1
          LayerName = BandLayer6
          mmBottomOffset = 0
          mmOverFlowOffset = 0
          mmStopPosition = 0
          mmMinHeight = 0
          mmLeading = 0
        end
        object ppMemo10: TppMemo
          UserName = 'Memo10'
          Border.BorderPositions = [bpLeft, bpTop, bpRight, bpBottom]
          Border.Visible = True
          Caption = 'Memo10'
          CharWrap = False
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 6
          Font.Style = [fsBold]
          Lines.Strings = (
            #205'nicio')
          RemoveEmptyLines = False
          TextAlignment = taCentered
          Transparent = True
          VerticalAlignment = avCenter
          mmHeight = 8996
          mmLeft = 161159
          mmTop = 11906
          mmWidth = 9787
          BandType = 3
          GroupNo = 1
          LayerName = BandLayer6
          mmBottomOffset = 0
          mmOverFlowOffset = 0
          mmStopPosition = 0
          mmMinHeight = 0
          mmLeading = 0
        end
        object ppMemo4: TppMemo
          UserName = 'Memo4'
          Border.BorderPositions = [bpLeft, bpTop, bpRight, bpBottom]
          Border.Visible = True
          Caption = 'Memo4'
          CharWrap = False
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 6
          Font.Style = [fsBold]
          Lines.Strings = (
            'Perdas KG%')
          RemoveEmptyLines = False
          TextAlignment = taCentered
          Transparent = True
          VerticalAlignment = avCenter
          mmHeight = 8996
          mmLeft = 82057
          mmTop = 11906
          mmWidth = 9525
          BandType = 3
          GroupNo = 1
          LayerName = BandLayer6
          mmBottomOffset = 0
          mmOverFlowOffset = 0
          mmStopPosition = 0
          mmMinHeight = 0
          mmLeading = 0
        end
        object ppDBText46: TppDBText
          UserName = 'DBText46'
          HyperlinkEnabled = False
          DataField = 'Recurso'
          DataPipeline = Pipeline
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 10
          Font.Style = [fsBold]
          Transparent = True
          VerticalAlignment = avCenter
          DataPipelineName = 'Pipeline'
          mmHeight = 5027
          mmLeft = 2645
          mmTop = 6086
          mmWidth = 57152
          BandType = 3
          GroupNo = 1
          LayerName = BandLayer6
        end
        object ppMemo2: TppMemo
          UserName = 'Memo2'
          Border.BorderPositions = [bpLeft, bpTop, bpRight, bpBottom]
          Border.Visible = True
          Caption = 'Memo2'
          CharWrap = False
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 6
          Font.Style = [fsBold]
          Lines.Strings = (
            'Turno')
          RemoveEmptyLines = False
          TextAlignment = taCentered
          Transparent = True
          VerticalAlignment = avCenter
          mmHeight = 8996
          mmLeft = 1588
          mmTop = 11906
          mmWidth = 8200
          BandType = 3
          GroupNo = 1
          LayerName = BandLayer6
          mmBottomOffset = 0
          mmOverFlowOffset = 0
          mmStopPosition = 0
          mmMinHeight = 0
          mmLeading = 0
        end
        object ppMemo11: TppMemo
          UserName = 'Memo101'
          Border.BorderPositions = [bpLeft, bpTop, bpRight, bpBottom]
          Border.Visible = True
          Caption = 'Memo101'
          CharWrap = False
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 6
          Font.Style = [fsBold]
          Lines.Strings = (
            'T'#233'rmino')
          RemoveEmptyLines = False
          TextAlignment = taCentered
          Transparent = True
          VerticalAlignment = avCenter
          mmHeight = 8996
          mmLeft = 170684
          mmTop = 11906
          mmWidth = 9787
          BandType = 3
          GroupNo = 1
          LayerName = BandLayer6
          mmBottomOffset = 0
          mmOverFlowOffset = 0
          mmStopPosition = 0
          mmMinHeight = 0
          mmLeading = 0
        end
        object ppMemo9: TppMemo
          UserName = 'Memo9'
          Border.BorderPositions = [bpLeft, bpTop, bpRight, bpBottom]
          Border.Visible = True
          Caption = 'Memo9'
          CharWrap = False
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 6
          Font.Style = [fsBold]
          Lines.Strings = (
            'Ciclo'
            'Real')
          RemoveEmptyLines = False
          TextAlignment = taCentered
          Transparent = True
          VerticalAlignment = avCenter
          mmHeight = 8996
          mmLeft = 152693
          mmTop = 11906
          mmWidth = 8731
          BandType = 3
          GroupNo = 1
          LayerName = BandLayer6
          mmBottomOffset = 0
          mmOverFlowOffset = 0
          mmStopPosition = 0
          mmMinHeight = 0
          mmLeading = 0
        end
        object ppMemo15: TppMemo
          UserName = 'Memo15'
          Border.BorderPositions = [bpLeft, bpTop, bpRight, bpBottom]
          Border.Visible = True
          Caption = 'Memo15'
          CharWrap = False
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 6
          Font.Style = [fsBold]
          Lines.Strings = (
            '% '
            'Produtividade')
          RemoveEmptyLines = False
          TextAlignment = taCentered
          Transparent = True
          VerticalAlignment = avCenter
          mmHeight = 8996
          mmLeft = 207434
          mmTop = 11906
          mmWidth = 16140
          BandType = 3
          GroupNo = 1
          LayerName = BandLayer6
          mmBottomOffset = 0
          mmOverFlowOffset = 0
          mmStopPosition = 0
          mmMinHeight = 0
          mmLeading = 0
        end
        object ppMemo16: TppMemo
          UserName = 'Memo16'
          Border.BorderPositions = [bpLeft, bpTop, bpRight, bpBottom]
          Border.Visible = True
          Caption = 'Memo16'
          CharWrap = False
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 6
          Font.Style = [fsBold]
          Lines.Strings = (
            'Tempo Parada (MIN)')
          RemoveEmptyLines = False
          TextAlignment = taCentered
          Transparent = True
          VerticalAlignment = avCenter
          mmHeight = 8996
          mmLeft = 223309
          mmTop = 11906
          mmWidth = 11113
          BandType = 3
          GroupNo = 1
          LayerName = BandLayer6
          mmBottomOffset = 0
          mmOverFlowOffset = 0
          mmStopPosition = 0
          mmMinHeight = 0
          mmLeading = 0
        end
        object ppMemo17: TppMemo
          UserName = 'Memo17'
          Border.BorderPositions = [bpLeft, bpTop, bpRight, bpBottom]
          Border.Visible = True
          Caption = 'Memo17'
          CharWrap = False
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 6
          Font.Style = [fsBold]
          Lines.Strings = (
            'Causas - Improdutividade')
          RemoveEmptyLines = False
          TextAlignment = taCentered
          Transparent = True
          VerticalAlignment = avCenter
          mmHeight = 8996
          mmLeft = 234157
          mmTop = 11906
          mmWidth = 45508
          BandType = 3
          GroupNo = 1
          LayerName = BandLayer6
          mmBottomOffset = 0
          mmOverFlowOffset = 0
          mmStopPosition = 0
          mmMinHeight = 0
          mmLeading = 0
        end
        object ppLabel1: TppLabel
          UserName = 'Label1'
          HyperlinkEnabled = False
          AutoSize = False
          Caption = 'M'#225'quina'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 10
          Font.Style = [fsBold]
          Transparent = True
          mmHeight = 4233
          mmLeft = 2381
          mmTop = 1323
          mmWidth = 16141
          BandType = 3
          GroupNo = 1
          LayerName = BandLayer6
        end
        object ppDBText17: TppDBText
          UserName = 'DBText17'
          HyperlinkEnabled = False
          DataField = 'Cavidade'
          DataPipeline = Pipeline
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 10
          Font.Style = [fsBold]
          TextAlignment = taCentered
          Transparent = True
          VerticalAlignment = avCenter
          DataPipelineName = 'Pipeline'
          mmHeight = 9790
          mmLeft = 84138
          mmTop = 1318
          mmWidth = 8202
          BandType = 3
          GroupNo = 1
          LayerName = BandLayer6
        end
        object ppMemo5: TppMemo
          UserName = 'Memo5'
          Border.BorderPositions = [bpLeft, bpTop, bpRight, bpBottom]
          Caption = 'Memo5'
          CharWrap = False
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 10
          Font.Style = [fsBold]
          Lines.Strings = (
            'N'#186' de Cavidades')
          RemoveEmptyLines = False
          TextAlignment = taCentered
          Transparent = True
          VerticalAlignment = avCenter
          mmHeight = 9790
          mmLeft = 62442
          mmTop = 1318
          mmWidth = 20373
          BandType = 3
          GroupNo = 1
          LayerName = BandLayer6
          mmBottomOffset = 0
          mmOverFlowOffset = 0
          mmStopPosition = 0
          mmMinHeight = 0
          mmLeading = 0
        end
        object ppMemo18: TppMemo
          UserName = 'Memo18'
          Border.BorderPositions = [bpLeft, bpTop, bpRight, bpBottom]
          Border.Visible = True
          Caption = 'Memo18'
          CharWrap = False
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 6
          Font.Style = [fsBold]
          Lines.Strings = (
            'Item')
          RemoveEmptyLines = False
          TextAlignment = taCentered
          Transparent = True
          VerticalAlignment = avCenter
          mmHeight = 8996
          mmLeft = 9509
          mmTop = 11906
          mmWidth = 24617
          BandType = 3
          GroupNo = 1
          LayerName = BandLayer6
          mmBottomOffset = 0
          mmOverFlowOffset = 0
          mmStopPosition = 0
          mmMinHeight = 0
          mmLeading = 0
        end
        object ppMemo19: TppMemo
          UserName = 'Memo19'
          Border.BorderPositions = [bpLeft, bpTop, bpRight, bpBottom]
          Border.Visible = True
          Caption = 'Memo19'
          CharWrap = False
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 6
          Font.Style = [fsBold]
          Lines.Strings = (
            'Ciclo'
            'Padr'#227'o')
          RemoveEmptyLines = False
          TextAlignment = taCentered
          Transparent = True
          VerticalAlignment = avCenter
          mmHeight = 8996
          mmLeft = 144226
          mmTop = 11906
          mmWidth = 8731
          BandType = 3
          GroupNo = 1
          LayerName = BandLayer6
          mmBottomOffset = 0
          mmOverFlowOffset = 0
          mmStopPosition = 0
          mmMinHeight = 0
          mmLeading = 0
        end
        object ppMemo14: TppMemo
          UserName = 'Memo14'
          Border.BorderPositions = [bpLeft, bpTop, bpRight, bpBottom]
          Border.Visible = True
          Caption = 'Memo14'
          CharWrap = False
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 6
          Font.Style = [fsBold]
          Lines.Strings = (
            '% Efici'#234'ncia Nominal')
          RemoveEmptyLines = False
          TextAlignment = taCentered
          Transparent = True
          VerticalAlignment = avCenter
          mmHeight = 8996
          mmLeft = 191030
          mmTop = 11905
          mmWidth = 16669
          BandType = 3
          GroupNo = 1
          LayerName = BandLayer6
          mmBottomOffset = 0
          mmOverFlowOffset = 0
          mmStopPosition = 0
          mmMinHeight = 0
          mmLeading = 0
        end
        object ppMemo12: TppMemo
          UserName = 'Memo12'
          Border.BorderPositions = [bpLeft, bpTop, bpRight, bpBottom]
          Border.Visible = True
          Caption = 'Memo12'
          CharWrap = False
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 6
          Font.Style = [fsBold]
          Lines.Strings = (
            'Capacidade'
            'Instalada')
          RemoveEmptyLines = False
          TextAlignment = taCentered
          Transparent = True
          VerticalAlignment = avCenter
          mmHeight = 8996
          mmLeft = 33855
          mmTop = 11905
          mmWidth = 13232
          BandType = 3
          GroupNo = 1
          LayerName = BandLayer6
          mmBottomOffset = 0
          mmOverFlowOffset = 0
          mmStopPosition = 0
          mmMinHeight = 0
          mmLeading = 0
        end
        object ppMemo13: TppMemo
          UserName = 'Memo13'
          Border.BorderPositions = [bpLeft, bpTop, bpRight, bpBottom]
          Border.Visible = True
          Caption = 'Memo13'
          CharWrap = False
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 6
          Font.Style = [fsBold]
          Lines.Strings = (
            'Capacidade Real')
          RemoveEmptyLines = False
          TextAlignment = taCentered
          Transparent = True
          VerticalAlignment = avCenter
          mmHeight = 8996
          mmLeft = 46823
          mmTop = 11905
          mmWidth = 13755
          BandType = 3
          GroupNo = 1
          LayerName = BandLayer6
          mmBottomOffset = 0
          mmOverFlowOffset = 0
          mmStopPosition = 0
          mmMinHeight = 0
          mmLeading = 0
        end
        object ppMemo3: TppMemo
          UserName = 'Memo3'
          Border.BorderPositions = [bpLeft, bpTop, bpRight, bpBottom]
          Border.Visible = True
          Caption = 'Memo3'
          CharWrap = False
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 6
          Font.Style = [fsBold]
          Lines.Strings = (
            'Qtd Produzida')
          RemoveEmptyLines = False
          TextAlignment = taCentered
          Transparent = True
          VerticalAlignment = avCenter
          mmHeight = 8996
          mmLeft = 60324
          mmTop = 11906
          mmWidth = 12435
          BandType = 3
          GroupNo = 1
          LayerName = BandLayer6
          mmBottomOffset = 0
          mmOverFlowOffset = 0
          mmStopPosition = 0
          mmMinHeight = 0
          mmLeading = 0
        end
        object ppMemo6: TppMemo
          UserName = 'Memo6'
          Border.BorderPositions = [bpLeft, bpTop, bpRight, bpBottom]
          Border.Visible = True
          Caption = 'Memo6'
          CharWrap = False
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 6
          Font.Style = [fsBold]
          Lines.Strings = (
            'Tempo Produ'#231#227'o')
          RemoveEmptyLines = False
          TextAlignment = taCentered
          Transparent = True
          VerticalAlignment = avCenter
          mmHeight = 8996
          mmLeft = 180180
          mmTop = 11906
          mmWidth = 11115
          BandType = 3
          GroupNo = 1
          LayerName = BandLayer6
          mmBottomOffset = 0
          mmOverFlowOffset = 0
          mmStopPosition = 0
          mmMinHeight = 0
          mmLeading = 0
        end
      end
      object ppGroupFooterBand2: TppGroupFooterBand
        Background.Brush.Style = bsClear
        HideWhenOneDetail = False
        mmBottomOffset = 0
        mmHeight = 6085
        mmPrintPosition = 0
        object ppShape1: TppShape
          UserName = 'Shape1'
          mmHeight = 5556
          mmLeft = 1589
          mmTop = 528
          mmWidth = 278078
          BandType = 5
          GroupNo = 1
          LayerName = BandLayer6
        end
        object ppLabel41: TppLabel
          UserName = 'Label41'
          HyperlinkEnabled = False
          AutoSize = False
          Caption = 'Dia'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 7
          Font.Style = [fsBold]
          TextAlignment = taCentered
          Transparent = True
          VerticalAlignment = avCenter
          mmHeight = 3969
          mmLeft = 2647
          mmTop = 1325
          mmWidth = 9258
          BandType = 5
          GroupNo = 1
          LayerName = BandLayer6
        end
        object TXT_Producao: TppDBCalc
          UserName = 'TXT_Producao'
          HyperlinkEnabled = False
          DataField = 'Producao'
          DataPipeline = Pipeline
          DisplayFormat = '#,0;-#,0'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 6
          Font.Style = []
          ResetGroup = ppGroup2
          TextAlignment = taRightJustified
          Transparent = True
          VerticalAlignment = avCenter
          DataPipelineName = 'Pipeline'
          mmHeight = 3969
          mmLeft = 60325
          mmTop = 1323
          mmWidth = 10319
          BandType = 5
          GroupNo = 1
          LayerName = BandLayer6
        end
        object ppDBCalc4: TppDBCalc
          UserName = 'TXT_Quantidade2'
          HyperlinkEnabled = False
          DataField = 'PerdaUN'
          DataPipeline = Pipeline
          DisplayFormat = '#,0;-#,0'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 6
          Font.Style = []
          ResetGroup = ppGroup2
          TextAlignment = taRightJustified
          Transparent = True
          VerticalAlignment = avCenter
          DataPipelineName = 'Pipeline'
          mmHeight = 3969
          mmLeft = 74083
          mmTop = 1323
          mmWidth = 5292
          BandType = 5
          GroupNo = 1
          LayerName = BandLayer6
        end
        object TXT_Nominal: TppDBCalc
          UserName = 'TXT_Nominal'
          HyperlinkEnabled = False
          DataField = 'NominalTurno'
          DataPipeline = Pipeline
          DisplayFormat = '#,0;-#,0'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 6
          Font.Style = []
          ResetGroup = ppGroup2
          TextAlignment = taRightJustified
          Transparent = True
          VerticalAlignment = avCenter
          DataPipelineName = 'Pipeline'
          mmHeight = 3969
          mmLeft = 35190
          mmTop = 1320
          mmWidth = 10054
          BandType = 5
          GroupNo = 1
          LayerName = BandLayer6
        end
        object Var_CapacidadeDisponivel: TppDBCalc
          UserName = 'TXT_CapacidadeReal'
          HyperlinkEnabled = False
          DataField = 'CapacidadeDisponivel'
          DataPipeline = Pipeline
          DisplayFormat = '#,0;-#,0'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 6
          Font.Style = []
          ResetGroup = ppGroup2
          TextAlignment = taRightJustified
          Transparent = True
          VerticalAlignment = avCenter
          DataPipelineName = 'Pipeline'
          mmHeight = 3969
          mmLeft = 47361
          mmTop = 1323
          mmWidth = 10053
          BandType = 5
          GroupNo = 1
          LayerName = BandLayer6
        end
        object ppVariable1: TppVariable
          OnPrint = ppVariable1Print
          UserName = 'Variable1'
          BlankWhenZero = False
          CalcOrder = 0
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 6
          Font.Style = []
          TextAlignment = taRightJustified
          Transparent = True
          mmHeight = 3176
          mmLeft = 222515
          mmTop = 1852
          mmWidth = 8467
          BandType = 5
          GroupNo = 1
          LayerName = BandLayer6
        end
        object Var_PerdaKG: TppVariable
          OnPrint = Var_PerdaKGPrint
          UserName = 'Var_PerdaKG'
          BlankWhenZero = False
          CalcOrder = 1
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 6
          Font.Style = []
          TextAlignment = taRightJustified
          Transparent = True
          mmHeight = 3174
          mmLeft = 83346
          mmTop = 1853
          mmWidth = 5821
          BandType = 5
          GroupNo = 1
          LayerName = BandLayer6
        end
        object Var_ENominal: TppVariable
          OnPrint = Var_ENominalPrint
          UserName = 'Var_ENominal'
          BlankWhenZero = False
          CalcOrder = 2
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 6
          Font.Style = []
          TextAlignment = taRightJustified
          Transparent = True
          mmHeight = 3176
          mmLeft = 191030
          mmTop = 1853
          mmWidth = 11375
          BandType = 5
          GroupNo = 1
          LayerName = BandLayer6
        end
        object TXT_CapacidadeTeorica: TppDBCalc
          UserName = 'DBCalc3'
          HyperlinkEnabled = False
          DataField = 'TempoProducao'
          DataPipeline = Pipeline
          DisplayFormat = '#,0.0;(#,0.0)'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 6
          Font.Style = []
          ResetGroup = ppGroup2
          TextAlignment = taRightJustified
          Transparent = True
          Visible = False
          VerticalAlignment = avCenter
          DataPipelineName = 'Pipeline'
          mmHeight = 3969
          mmLeft = 180975
          mmTop = 1322
          mmWidth = 7937
          BandType = 5
          GroupNo = 1
          LayerName = BandLayer6
        end
        object Var_Produt: TppVariable
          OnPrint = Var_ProdutPrint
          UserName = 'Var_Produt'
          BlankWhenZero = False
          CalcOrder = 3
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 6
          Font.Style = []
          TextAlignment = taRightJustified
          Transparent = True
          mmHeight = 3176
          mmLeft = 206639
          mmTop = 1852
          mmWidth = 11114
          BandType = 5
          GroupNo = 1
          LayerName = BandLayer6
        end
        object ppDBCalc1: TppDBCalc
          UserName = 'DBCalc1'
          HyperlinkEnabled = False
          DataField = 'TempoParadaMin'
          DataPipeline = Pipeline
          DisplayFormat = '#,0.0;(#,0.0)'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 6
          Font.Style = []
          ResetGroup = ppGroup2
          TextAlignment = taRightJustified
          Transparent = True
          Visible = False
          VerticalAlignment = avCenter
          DataPipelineName = 'Pipeline'
          mmHeight = 3969
          mmLeft = 234157
          mmTop = 1323
          mmWidth = 8467
          BandType = 5
          GroupNo = 1
          LayerName = BandLayer6
        end
        object TXT_PesoPerda: TppDBCalc
          UserName = 'TXT_Producao1'
          HyperlinkEnabled = False
          DataField = 'PesoPerda'
          DataPipeline = Pipeline
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 6
          Font.Style = []
          ResetGroup = ppGroup2
          TextAlignment = taRightJustified
          Transparent = True
          Visible = False
          VerticalAlignment = avCenter
          DataPipelineName = 'Pipeline'
          mmHeight = 3969
          mmLeft = 105040
          mmTop = 1323
          mmWidth = 18785
          BandType = 5
          GroupNo = 1
          LayerName = BandLayer6
        end
        object TXT_ProducaoKG: TppDBCalc
          UserName = 'DBCalc2'
          HyperlinkEnabled = False
          DataField = 'ProducaoKG'
          DataPipeline = Pipeline
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 6
          Font.Style = []
          ResetGroup = ppGroup2
          TextAlignment = taRightJustified
          Transparent = True
          Visible = False
          VerticalAlignment = avCenter
          DataPipelineName = 'Pipeline'
          mmHeight = 3969
          mmLeft = 130175
          mmTop = 1323
          mmWidth = 17992
          BandType = 5
          GroupNo = 1
          LayerName = BandLayer6
        end
      end
    end
    object ppGroup3: TppGroup
      BreakName = 'Turno'
      DataPipeline = Pipeline
      GroupFileSettings.NewFile = False
      GroupFileSettings.EmailFile = False
      OutlineSettings.CreateNode = True
      ReprintOnSubsequentPage = False
      StartOnOddPage = False
      UserName = 'Group_Turno'
      mmNewColumnThreshold = 0
      mmNewPageThreshold = 0
      DataPipelineName = 'Pipeline'
      NewFile = False
      object ppGroupHeaderBand3: TppGroupHeaderBand
        Background.Brush.Style = bsClear
        PrintHeight = phDynamic
        mmBottomOffset = 0
        mmHeight = 794
        mmPrintPosition = 0
        object ppLine1: TppLine
          UserName = 'Line2'
          Weight = 0.750000000000000000
          mmHeight = 794
          mmLeft = 1588
          mmTop = 264
          mmWidth = 278078
          BandType = 3
          GroupNo = 2
          LayerName = BandLayer6
        end
      end
      object ppGroupFooterBand3: TppGroupFooterBand
        Background.Brush.Style = bsClear
        PrintHeight = phDynamic
        HideWhenOneDetail = False
        mmBottomOffset = 0
        mmHeight = 3969
        mmPrintPosition = 0
        object TXT_QtdVezesNoTurno: TppDBCalc
          UserName = 'TXT_QtdVezesNoTurno'
          HyperlinkEnabled = False
          DataField = 'Chave1'
          DataPipeline = Pipeline
          DisplayFormat = '#,0;-#,0'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 6
          Font.Style = []
          ResetGroup = ppGroup3
          TextAlignment = taRightJustified
          Transparent = True
          Visible = False
          VerticalAlignment = avCenter
          DBCalcType = dcCount
          DataPipelineName = 'Pipeline'
          mmHeight = 3969
          mmLeft = 137319
          mmTop = 0
          mmWidth = 20902
          BandType = 5
          GroupNo = 2
          LayerName = BandLayer6
        end
        object Var_TempoParada: TppVariable
          OnPrint = Var_TempoParadaPrint
          UserName = 'Var_TempoParada'
          BlankWhenZero = False
          CalcOrder = 0
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 6
          Font.Style = []
          TextAlignment = taRightJustified
          Transparent = True
          Visible = False
          mmHeight = 2646
          mmLeft = 172773
          mmTop = 530
          mmWidth = 16669
          BandType = 5
          GroupNo = 2
          LayerName = BandLayer6
        end
        object TXT_TempoParada: TppDBCalc
          UserName = 'TXT_TempoParada'
          HyperlinkEnabled = False
          DataField = 'TempoParadaMin'
          DataPipeline = Pipeline
          DisplayFormat = '#,0;-#,0'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 6
          Font.Style = []
          ResetGroup = ppGroup3
          TextAlignment = taRightJustified
          Transparent = True
          Visible = False
          VerticalAlignment = avCenter
          DataPipelineName = 'Pipeline'
          mmHeight = 3969
          mmLeft = 115623
          mmTop = 0
          mmWidth = 19579
          BandType = 5
          GroupNo = 2
          LayerName = BandLayer6
        end
      end
    end
    object ppDesignLayers3: TppDesignLayers
      object ppDesignLayer3: TppDesignLayer
        UserName = 'PageLayer3'
        LayerType = ltPage
        Index = 0
      end
      object ppDesignLayer4: TppDesignLayer
        UserName = 'BandLayer6'
        LayerType = ltBanded
        Index = 1
      end
    end
    object ppParameterList2: TppParameterList
    end
  end
  object Pipeline: TppDBPipeline
    DataSource = DS
    CloseDataSource = True
    UserName = 'Pipeline'
    Left = 171
    Top = 31
    object PipelineppField1: TppField
      FieldAlias = 'Chave0'
      FieldName = 'Chave0'
      FieldLength = 25
      DisplayWidth = 25
      Position = 0
    end
    object PipelineppField2: TppField
      FieldAlias = 'Chave1'
      FieldName = 'Chave1'
      FieldLength = 17
      DisplayWidth = 17
      Position = 1
    end
    object PipelineppField3: TppField
      FieldAlias = 'Chave2'
      FieldName = 'Chave2'
      FieldLength = 11
      DisplayWidth = 11
      Position = 2
    end
    object PipelineppField4: TppField
      FieldAlias = 'Area'
      FieldName = 'Area'
      FieldLength = 50
      DisplayWidth = 50
      Position = 3
    end
    object PipelineppField5: TppField
      FieldAlias = 'Setor'
      FieldName = 'Setor'
      FieldLength = 50
      DisplayWidth = 50
      Position = 4
    end
    object PipelineppField6: TppField
      FieldAlias = 'DataTurno'
      FieldName = 'DataTurno'
      FieldLength = 0
      DataType = dtDateTime
      DisplayWidth = 18
      Position = 5
    end
    object PipelineppField7: TppField
      FieldAlias = 'Mes'
      FieldName = 'Mes'
      FieldLength = 30
      DisplayWidth = 30
      Position = 6
    end
    object PipelineppField8: TppField
      Alignment = taRightJustify
      FieldAlias = 'Ano'
      FieldName = 'Ano'
      FieldLength = 0
      DataType = dtInteger
      DisplayWidth = 10
      Position = 7
    end
    object PipelineppField9: TppField
      FieldAlias = 'Recurso_ID'
      FieldName = 'Recurso_ID'
      FieldLength = 6
      DisplayWidth = 6
      Position = 8
    end
    object PipelineppField10: TppField
      FieldAlias = 'Recurso'
      FieldName = 'Recurso'
      FieldLength = 30
      DisplayWidth = 30
      Position = 9
    end
    object PipelineppField11: TppField
      Alignment = taRightJustify
      FieldAlias = 'Cavidade'
      FieldName = 'Cavidade'
      FieldLength = 0
      DataType = dtInteger
      DisplayWidth = 10
      Position = 10
    end
    object PipelineppField12: TppField
      FieldAlias = 'Grupo'
      FieldName = 'Grupo'
      FieldLength = 50
      DisplayWidth = 50
      Position = 11
    end
    object PipelineppField13: TppField
      FieldAlias = 'Tipo'
      FieldName = 'Tipo'
      FieldLength = 20
      DisplayWidth = 20
      Position = 12
    end
    object PipelineppField14: TppField
      FieldAlias = 'Item_FK'
      FieldName = 'Item_FK'
      FieldLength = 8
      DisplayWidth = 8
      Position = 13
    end
    object PipelineppField15: TppField
      FieldAlias = 'Item'
      FieldName = 'Item'
      FieldLength = 50
      DisplayWidth = 50
      Position = 14
    end
    object PipelineppField16: TppField
      FieldAlias = 'TurnoDescricao'
      FieldName = 'TurnoDescricao'
      FieldLength = 15
      DisplayWidth = 15
      Position = 15
    end
    object PipelineppField17: TppField
      FieldAlias = 'Turno'
      FieldName = 'Turno'
      FieldLength = 2
      DisplayWidth = 2
      Position = 16
    end
    object PipelineppField18: TppField
      Alignment = taRightJustify
      FieldAlias = 'Posicao'
      FieldName = 'Posicao'
      FieldLength = 0
      DataType = dtInteger
      DisplayWidth = 10
      Position = 17
    end
    object PipelineppField19: TppField
      Alignment = taRightJustify
      FieldAlias = 'NumItens'
      FieldName = 'NumItens'
      FieldLength = 0
      DataType = dtInteger
      DisplayWidth = 10
      Position = 18
    end
    object PipelineppField20: TppField
      Alignment = taRightJustify
      FieldAlias = 'Producao'
      FieldName = 'Producao'
      FieldLength = 0
      DataType = dtInteger
      DisplayWidth = 10
      Position = 19
    end
    object PipelineppField21: TppField
      Alignment = taRightJustify
      FieldAlias = 'PerdaUN'
      FieldName = 'PerdaUN'
      FieldLength = 0
      DataType = dtInteger
      DisplayWidth = 10
      Position = 20
    end
    object PipelineppField22: TppField
      Alignment = taRightJustify
      FieldAlias = 'PerdaKGPerc'
      FieldName = 'PerdaKGPerc'
      FieldLength = 0
      DataType = dtDouble
      DisplayWidth = 10
      Position = 21
    end
    object PipelineppField23: TppField
      Alignment = taRightJustify
      FieldAlias = 'PesoItem'
      FieldName = 'PesoItem'
      FieldLength = 0
      DataType = dtDouble
      DisplayWidth = 10
      Position = 22
    end
    object PipelineppField24: TppField
      Alignment = taRightJustify
      FieldAlias = 'ProducaoKG'
      FieldName = 'ProducaoKG'
      FieldLength = 0
      DataType = dtDouble
      DisplayWidth = 10
      Position = 23
    end
    object PipelineppField25: TppField
      Alignment = taRightJustify
      FieldAlias = 'PesoPerda'
      FieldName = 'PesoPerda'
      FieldLength = 0
      DataType = dtDouble
      DisplayWidth = 10
      Position = 24
    end
    object PipelineppField26: TppField
      Alignment = taRightJustify
      FieldAlias = 'PerdaUNPerc'
      FieldName = 'PerdaUNPerc'
      FieldLength = 0
      DataType = dtDouble
      DisplayWidth = 10
      Position = 25
    end
    object PipelineppField27: TppField
      FieldAlias = 'Semafaro_PerdaUNPerc'
      FieldName = 'Semafaro_PerdaUNPerc'
      FieldLength = 8
      DisplayWidth = 8
      Position = 26
    end
    object PipelineppField28: TppField
      FieldAlias = 'Causa_Perda'
      FieldName = 'Causa_Perda'
      FieldLength = 500
      DisplayWidth = 500
      Position = 27
    end
    object PipelineppField29: TppField
      Alignment = taRightJustify
      FieldAlias = 'CicloReal'
      FieldName = 'CicloReal'
      FieldLength = 0
      DataType = dtDouble
      DisplayWidth = 10
      Position = 28
    end
    object PipelineppField30: TppField
      Alignment = taRightJustify
      FieldAlias = 'CicloPadrao'
      FieldName = 'CicloPadrao'
      FieldLength = 0
      DataType = dtDouble
      DisplayWidth = 10
      Position = 29
    end
    object PipelineppField31: TppField
      Alignment = taRightJustify
      FieldAlias = 'Ciclo_Diferenca'
      FieldName = 'Ciclo_Diferenca'
      FieldLength = 0
      DataType = dtDouble
      DisplayWidth = 10
      Position = 30
    end
    object PipelineppField32: TppField
      FieldAlias = 'Semafaro_Ciclo'
      FieldName = 'Semafaro_Ciclo'
      FieldLength = 8
      DisplayWidth = 8
      Position = 31
    end
    object PipelineppField33: TppField
      FieldAlias = 'DataInicio'
      FieldName = 'DataInicio'
      FieldLength = 0
      DataType = dtDateTime
      DisplayWidth = 18
      Position = 32
    end
    object PipelineppField34: TppField
      FieldAlias = 'HoraInicio'
      FieldName = 'HoraInicio'
      FieldLength = 5
      DisplayWidth = 5
      Position = 33
    end
    object PipelineppField35: TppField
      FieldAlias = 'DataFim'
      FieldName = 'DataFim'
      FieldLength = 0
      DataType = dtDateTime
      DisplayWidth = 18
      Position = 34
    end
    object PipelineppField36: TppField
      FieldAlias = 'HoraFim'
      FieldName = 'HoraFim'
      FieldLength = 5
      DisplayWidth = 5
      Position = 35
    end
    object PipelineppField37: TppField
      Alignment = taRightJustify
      FieldAlias = 'NominalTurno'
      FieldName = 'NominalTurno'
      FieldLength = 0
      DataType = dtDouble
      DisplayWidth = 10
      Position = 36
    end
    object PipelineppField38: TppField
      Alignment = taRightJustify
      FieldAlias = 'EficienciaNominal'
      FieldName = 'EficienciaNominal'
      FieldLength = 0
      DataType = dtDouble
      DisplayWidth = 10
      Position = 37
    end
    object PipelineppField39: TppField
      FieldAlias = 'Semafaro_EficienciaNominal'
      FieldName = 'Semafaro_EficienciaNominal'
      FieldLength = 8
      DisplayWidth = 8
      Position = 38
    end
    object PipelineppField40: TppField
      Alignment = taRightJustify
      FieldAlias = 'TempoProducao'
      FieldName = 'TempoProducao'
      FieldLength = 0
      DataType = dtInteger
      DisplayWidth = 10
      Position = 39
    end
    object PipelineppField41: TppField
      Alignment = taRightJustify
      FieldAlias = 'TempoProducaoTotal'
      FieldName = 'TempoProducaoTotal'
      FieldLength = 0
      DataType = dtInteger
      DisplayWidth = 10
      Position = 40
    end
    object PipelineppField42: TppField
      Alignment = taRightJustify
      FieldAlias = 'TempoProducaoPer'
      FieldName = 'TempoProducaoPer'
      FieldLength = 0
      DataType = dtDouble
      DisplayWidth = 10
      Position = 41
    end
    object PipelineppField43: TppField
      Alignment = taRightJustify
      FieldAlias = 'TempoTotalDoTurno'
      FieldName = 'TempoTotalDoTurno'
      FieldLength = 0
      DataType = dtDouble
      DisplayWidth = 10
      Position = 42
    end
    object PipelineppField44: TppField
      Alignment = taRightJustify
      FieldAlias = 'TempoParadaMin'
      FieldName = 'TempoParadaMin'
      FieldLength = 0
      DataType = dtInteger
      DisplayWidth = 10
      Position = 43
    end
    object PipelineppField45: TppField
      FieldAlias = 'Causa_Improdutividade'
      FieldName = 'Causa_Improdutividade'
      FieldLength = 500
      DisplayWidth = 500
      Position = 44
    end
    object PipelineppField46: TppField
      Alignment = taRightJustify
      FieldAlias = 'NominalKG'
      FieldName = 'NominalKG'
      FieldLength = 0
      DataType = dtDouble
      DisplayWidth = 10
      Position = 45
    end
    object PipelineppField47: TppField
      Alignment = taRightJustify
      FieldAlias = 'CapacidadeTeorica'
      FieldName = 'CapacidadeTeorica'
      FieldLength = 0
      DataType = dtDouble
      DisplayWidth = 10
      Position = 46
    end
    object PipelineppField48: TppField
      Alignment = taRightJustify
      FieldAlias = 'ProducaoJustificada'
      FieldName = 'ProducaoJustificada'
      FieldLength = 0
      DataType = dtDouble
      DisplayWidth = 10
      Position = 47
    end
    object PipelineppField49: TppField
      Alignment = taRightJustify
      FieldAlias = 'CapacidadeDisponivel'
      FieldName = 'CapacidadeDisponivel'
      FieldLength = 0
      DataType = dtDouble
      DisplayWidth = 10
      Position = 48
    end
    object PipelineppField50: TppField
      FieldAlias = 'Semafaro'
      FieldName = 'Semafaro'
      FieldLength = 8
      DisplayWidth = 8
      Position = 49
    end
    object PipelineppField51: TppField
      Alignment = taRightJustify
      FieldAlias = 'ProducaoPer'
      FieldName = 'ProducaoPer'
      FieldLength = 0
      DataType = dtDouble
      DisplayWidth = 10
      Position = 50
    end
    object PipelineppField52: TppField
      Alignment = taRightJustify
      FieldAlias = 'Improdutividade'
      FieldName = 'Improdutividade'
      FieldLength = 0
      DataType = dtDouble
      DisplayWidth = 10
      Position = 51
    end
    object PipelineppField53: TppField
      Alignment = taRightJustify
      FieldAlias = 'TempoImprodutivo'
      FieldName = 'TempoImprodutivo'
      FieldLength = 0
      DataType = dtDouble
      DisplayWidth = 10
      Position = 52
    end
    object PipelineppField54: TppField
      Alignment = taRightJustify
      FieldAlias = 'CapacidadeKG'
      FieldName = 'CapacidadeKG'
      FieldLength = 0
      DataType = dtDouble
      DisplayWidth = 10
      Position = 53
    end
    object PipelineppField55: TppField
      Alignment = taRightJustify
      FieldAlias = 'Produtividade'
      FieldName = 'Produtividade'
      FieldLength = 0
      DataType = dtDouble
      DisplayWidth = 10
      Position = 54
    end
    object PipelineppField56: TppField
      FieldAlias = 'Semafaro_Produtividade'
      FieldName = 'Semafaro_Produtividade'
      FieldLength = 8
      DisplayWidth = 8
      Position = 55
    end
    object PipelineppField57: TppField
      Alignment = taRightJustify
      FieldAlias = 'CapacidadeTeoricaKG'
      FieldName = 'CapacidadeTeoricaKG'
      FieldLength = 0
      DataType = dtDouble
      DisplayWidth = 10
      Position = 56
    end
  end
  object DSP: TDataSetProvider
    DataSet = Query
    Left = 135
    Top = 30
  end
  object DS: TDataSource
    DataSet = CDS
    Left = 105
    Top = 30
  end
  object Query: TADOQuery
    Connection = Sys_ModuloConexaoDados.ADOConnection1
    CursorType = ctStatic
    DataSource = DS
    Parameters = <>
    SQL.Strings = (
      ''
      
        ' Select *  from [dbo].[Pcp_VW_OrdemProducaoApontamentoMIGUEL_Tot' +
        'vs]  '
      
        ' Where DataTurno =  CONVERT(Datetime,Convert(Varchar(10),'#39'20/02/' +
        '2017 10:10:49'#39',103),103)'
      ' AND rtrim(Area) = '#39'INJECAO'#39
      ' AND rtrim(Setor) in ('#39'INJECAO'#39','#39'INJECAO 04'#39')'
      
        ' Order by Setor, Recurso_ID desc, Turno, DataInicio, HoraInicio ' +
        ' '
      '')
    Left = 75
    Top = 30
    object QueryChave0: TStringField
      FieldName = 'Chave0'
      Size = 25
    end
    object QueryChave1: TStringField
      FieldName = 'Chave1'
      Size = 17
    end
    object QueryChave2: TStringField
      FieldName = 'Chave2'
      Size = 11
    end
    object QueryArea: TStringField
      FieldName = 'Area'
      Size = 50
    end
    object QuerySetor: TStringField
      FieldName = 'Setor'
      Size = 50
    end
    object QueryDataTurno: TDateTimeField
      FieldName = 'DataTurno'
    end
    object QueryMes: TStringField
      FieldName = 'Mes'
      ReadOnly = True
      Size = 30
    end
    object QueryAno: TIntegerField
      FieldName = 'Ano'
      ReadOnly = True
    end
    object QueryRecurso_ID: TStringField
      FieldName = 'Recurso_ID'
      Size = 6
    end
    object QueryRecurso: TStringField
      FieldName = 'Recurso'
      Size = 30
    end
    object QueryCavidade: TIntegerField
      FieldName = 'Cavidade'
    end
    object QueryGrupo: TStringField
      FieldName = 'Grupo'
      Size = 50
    end
    object QueryTipo: TStringField
      FieldName = 'Tipo'
    end
    object QueryItem_FK: TStringField
      FieldName = 'Item_FK'
      Size = 8
    end
    object QueryItem: TStringField
      FieldName = 'Item'
      Size = 50
    end
    object QueryTurnoDescricao: TStringField
      FieldName = 'TurnoDescricao'
      Size = 15
    end
    object QueryTurno: TStringField
      FieldName = 'Turno'
      Size = 2
    end
    object QueryPosicao: TIntegerField
      FieldName = 'Posicao'
    end
    object QueryNumItens: TIntegerField
      FieldName = 'NumItens'
    end
    object QueryProducao: TIntegerField
      FieldName = 'Producao'
    end
    object QueryPerdaUN: TIntegerField
      FieldName = 'PerdaUN'
    end
    object QueryPerdaKGPerc: TFloatField
      FieldName = 'PerdaKGPerc'
    end
    object QueryPesoItem: TFloatField
      FieldName = 'PesoItem'
    end
    object QueryProducaoKG: TFloatField
      FieldName = 'ProducaoKG'
    end
    object QueryPesoPerda: TFloatField
      FieldName = 'PesoPerda'
    end
    object QueryPerdaUNPerc: TFloatField
      FieldName = 'PerdaUNPerc'
    end
    object QuerySemafaro_PerdaUNPerc: TStringField
      FieldName = 'Semafaro_PerdaUNPerc'
      ReadOnly = True
      Size = 8
    end
    object QueryCausa_Perda: TStringField
      FieldName = 'Causa_Perda'
      Size = 500
    end
    object QueryCicloReal: TFloatField
      FieldName = 'CicloReal'
    end
    object QueryCicloPadrao: TFloatField
      FieldName = 'CicloPadrao'
      ReadOnly = True
    end
    object QueryCiclo_Diferenca: TFloatField
      FieldName = 'Ciclo_Diferenca'
      ReadOnly = True
    end
    object QuerySemafaro_Ciclo: TStringField
      FieldName = 'Semafaro_Ciclo'
      ReadOnly = True
      Size = 8
    end
    object QueryDataInicio: TDateTimeField
      FieldName = 'DataInicio'
    end
    object QueryHoraInicio: TStringField
      FieldName = 'HoraInicio'
      Size = 5
    end
    object QueryDataFim: TDateTimeField
      FieldName = 'DataFim'
    end
    object QueryHoraFim: TStringField
      FieldName = 'HoraFim'
      Size = 5
    end
    object QueryNominalTurno: TFloatField
      FieldName = 'NominalTurno'
    end
    object QueryEficienciaNominal: TFloatField
      FieldName = 'EficienciaNominal'
    end
    object QuerySemafaro_EficienciaNominal: TStringField
      FieldName = 'Semafaro_EficienciaNominal'
      ReadOnly = True
      Size = 8
    end
    object QueryTempoProducao: TIntegerField
      FieldName = 'TempoProducao'
    end
    object QueryTempoProducaoTotal: TIntegerField
      FieldName = 'TempoProducaoTotal'
    end
    object QueryTempoProducaoPer: TFloatField
      FieldName = 'TempoProducaoPer'
    end
    object QueryTempoTotalDoTurno: TFloatField
      FieldName = 'TempoTotalDoTurno'
    end
    object QueryTempoParadaMin: TIntegerField
      FieldName = 'TempoParadaMin'
    end
    object QueryCausa_Improdutividade: TStringField
      FieldName = 'Causa_Improdutividade'
      Size = 500
    end
    object QueryNominalKG: TFloatField
      FieldName = 'NominalKG'
    end
    object QueryCapacidadeTeorica: TFloatField
      FieldName = 'CapacidadeTeorica'
    end
    object QueryProducaoJustificada: TFloatField
      FieldName = 'ProducaoJustificada'
    end
    object QueryCapacidadeDisponivel: TFloatField
      FieldName = 'CapacidadeDisponivel'
    end
    object QuerySemafaro: TStringField
      FieldName = 'Semafaro'
      ReadOnly = True
      Size = 8
    end
    object QueryProducaoPer: TFloatField
      FieldName = 'ProducaoPer'
      ReadOnly = True
    end
    object QueryImprodutividade: TFloatField
      FieldName = 'Improdutividade'
      ReadOnly = True
    end
    object QueryTempoImprodutivo: TFloatField
      FieldName = 'TempoImprodutivo'
      ReadOnly = True
    end
    object QueryCapacidadeKG: TFloatField
      FieldName = 'CapacidadeKG'
      ReadOnly = True
    end
    object QueryProdutividade: TFloatField
      FieldName = 'Produtividade'
      ReadOnly = True
    end
    object QuerySemafaro_Produtividade: TStringField
      FieldName = 'Semafaro_Produtividade'
      ReadOnly = True
      Size = 8
    end
    object QueryCapacidadeTeoricaKG: TFloatField
      FieldName = 'CapacidadeTeoricaKG'
      ReadOnly = True
    end
  end
  object CDS: TClientDataSet
    Active = True
    Aggregates = <>
    Params = <>
    ProviderName = 'DSP'
    Left = 44
    Top = 31
    object CDSChave0: TStringField
      FieldName = 'Chave0'
      Size = 25
    end
    object CDSChave1: TStringField
      FieldName = 'Chave1'
      Size = 17
    end
    object CDSChave2: TStringField
      FieldName = 'Chave2'
      Size = 11
    end
    object CDSArea: TStringField
      FieldName = 'Area'
      Size = 50
    end
    object CDSSetor: TStringField
      FieldName = 'Setor'
      Size = 50
    end
    object CDSDataTurno: TDateTimeField
      FieldName = 'DataTurno'
    end
    object CDSMes: TStringField
      FieldName = 'Mes'
      ReadOnly = True
      Size = 30
    end
    object CDSAno: TIntegerField
      FieldName = 'Ano'
      ReadOnly = True
    end
    object CDSRecurso_ID: TStringField
      FieldName = 'Recurso_ID'
      Size = 6
    end
    object CDSRecurso: TStringField
      FieldName = 'Recurso'
      Size = 30
    end
    object CDSCavidade: TIntegerField
      FieldName = 'Cavidade'
    end
    object CDSGrupo: TStringField
      FieldName = 'Grupo'
      Size = 50
    end
    object CDSTipo: TStringField
      FieldName = 'Tipo'
    end
    object CDSItem_FK: TStringField
      FieldName = 'Item_FK'
      Size = 8
    end
    object CDSItem: TStringField
      FieldName = 'Item'
      Size = 50
    end
    object CDSTurnoDescricao: TStringField
      FieldName = 'TurnoDescricao'
      Size = 15
    end
    object CDSTurno: TStringField
      FieldName = 'Turno'
      Size = 2
    end
    object CDSPosicao: TIntegerField
      FieldName = 'Posicao'
    end
    object CDSNumItens: TIntegerField
      FieldName = 'NumItens'
    end
    object CDSProducao: TIntegerField
      FieldName = 'Producao'
    end
    object CDSPerdaUN: TIntegerField
      FieldName = 'PerdaUN'
    end
    object CDSPerdaKGPerc: TFloatField
      FieldName = 'PerdaKGPerc'
    end
    object CDSPesoItem: TFloatField
      FieldName = 'PesoItem'
    end
    object CDSProducaoKG: TFloatField
      FieldName = 'ProducaoKG'
    end
    object CDSPesoPerda: TFloatField
      FieldName = 'PesoPerda'
    end
    object CDSPerdaUNPerc: TFloatField
      FieldName = 'PerdaUNPerc'
    end
    object CDSSemafaro_PerdaUNPerc: TStringField
      FieldName = 'Semafaro_PerdaUNPerc'
      ReadOnly = True
      Size = 8
    end
    object CDSCausa_Perda: TStringField
      FieldName = 'Causa_Perda'
      Size = 500
    end
    object CDSCicloReal: TFloatField
      FieldName = 'CicloReal'
    end
    object CDSCicloPadrao: TFloatField
      FieldName = 'CicloPadrao'
      ReadOnly = True
    end
    object CDSCiclo_Diferenca: TFloatField
      FieldName = 'Ciclo_Diferenca'
      ReadOnly = True
    end
    object CDSSemafaro_Ciclo: TStringField
      FieldName = 'Semafaro_Ciclo'
      ReadOnly = True
      Size = 8
    end
    object CDSDataInicio: TDateTimeField
      FieldName = 'DataInicio'
    end
    object CDSHoraInicio: TStringField
      FieldName = 'HoraInicio'
      Size = 5
    end
    object CDSDataFim: TDateTimeField
      FieldName = 'DataFim'
    end
    object CDSHoraFim: TStringField
      FieldName = 'HoraFim'
      Size = 5
    end
    object CDSNominalTurno: TFloatField
      FieldName = 'NominalTurno'
    end
    object CDSEficienciaNominal: TFloatField
      FieldName = 'EficienciaNominal'
    end
    object CDSSemafaro_EficienciaNominal: TStringField
      FieldName = 'Semafaro_EficienciaNominal'
      ReadOnly = True
      Size = 8
    end
    object CDSTempoProducao: TIntegerField
      FieldName = 'TempoProducao'
    end
    object CDSTempoProducaoTotal: TIntegerField
      FieldName = 'TempoProducaoTotal'
    end
    object CDSTempoProducaoPer: TFloatField
      FieldName = 'TempoProducaoPer'
    end
    object CDSTempoTotalDoTurno: TFloatField
      FieldName = 'TempoTotalDoTurno'
    end
    object CDSTempoParadaMin: TIntegerField
      FieldName = 'TempoParadaMin'
    end
    object CDSCausa_Improdutividade: TStringField
      FieldName = 'Causa_Improdutividade'
      Size = 500
    end
    object CDSNominalKG: TFloatField
      FieldName = 'NominalKG'
    end
    object CDSCapacidadeTeorica: TFloatField
      FieldName = 'CapacidadeTeorica'
    end
    object CDSProducaoJustificada: TFloatField
      FieldName = 'ProducaoJustificada'
    end
    object CDSCapacidadeDisponivel: TFloatField
      FieldName = 'CapacidadeDisponivel'
    end
    object CDSSemafaro: TStringField
      FieldName = 'Semafaro'
      ReadOnly = True
      Size = 8
    end
    object CDSProducaoPer: TFloatField
      FieldName = 'ProducaoPer'
      ReadOnly = True
    end
    object CDSImprodutividade: TFloatField
      FieldName = 'Improdutividade'
      ReadOnly = True
    end
    object CDSTempoImprodutivo: TFloatField
      FieldName = 'TempoImprodutivo'
      ReadOnly = True
    end
    object CDSCapacidadeKG: TFloatField
      FieldName = 'CapacidadeKG'
      ReadOnly = True
    end
    object CDSProdutividade: TFloatField
      FieldName = 'Produtividade'
      ReadOnly = True
    end
    object CDSSemafaro_Produtividade: TStringField
      FieldName = 'Semafaro_Produtividade'
      ReadOnly = True
      Size = 8
    end
    object CDSCapacidadeTeoricaKG: TFloatField
      FieldName = 'CapacidadeTeoricaKG'
      ReadOnly = True
    end
  end
  object DSP_Total_Recurso: TDataSetProvider
    DataSet = Query_Total_Recurso
    Left = 135
    Top = 94
  end
  object DS_Total_Recurso: TDataSource
    DataSet = CDS_Total_Recurso
    Left = 105
    Top = 94
  end
  object Query_Total_Recurso: TADOQuery
    Connection = Sys_ModuloConexaoDados.ADOConnection1
    CursorType = ctStatic
    DataSource = DS_Total_Recurso
    Parameters = <>
    SQL.Strings = (
      'Select '
      '  '
      ' '#9'*   '
      '  '
      ' from [dbo].[Pcp_VW_OrdemProducaoApontamentoMIGUEL_Totvs]  '
      
        ' Where DataTurno =  CONVERT(Datetime,Convert(Varchar(10),'#39'13/10/' +
        '2016 10:10:49'#39',103),103)'
      
        '  AND rtrim(Area) in ('#39'GRAFICA'#39','#39'INJECAO'#39','#39'PALETIZACAO'#39','#39'PERSONA' +
        'LIZACAO'#39')'
      ' AND rtrim(Setor) = '#39'INJECAO'#39
      ' AND TurnoDescricao = '#39'TURNO 02'#39
      '  Order by Setor, Recurso_ID, Turno  ')
    Left = 75
    Top = 94
    object Query_Total_RecursoChave0: TStringField
      FieldName = 'Chave0'
      Size = 25
    end
    object Query_Total_RecursoChave1: TStringField
      FieldName = 'Chave1'
      Size = 17
    end
    object Query_Total_RecursoChave2: TStringField
      FieldName = 'Chave2'
      Size = 11
    end
    object Query_Total_RecursoArea: TStringField
      FieldName = 'Area'
      Size = 50
    end
    object Query_Total_RecursoSetor: TStringField
      FieldName = 'Setor'
      Size = 50
    end
    object Query_Total_RecursoDataTurno: TDateTimeField
      FieldName = 'DataTurno'
    end
    object Query_Total_RecursoMes: TStringField
      FieldName = 'Mes'
      ReadOnly = True
      Size = 30
    end
    object Query_Total_RecursoAno: TIntegerField
      FieldName = 'Ano'
      ReadOnly = True
    end
    object Query_Total_RecursoRecurso_ID: TStringField
      FieldName = 'Recurso_ID'
      Size = 6
    end
    object Query_Total_RecursoRecurso: TStringField
      FieldName = 'Recurso'
      Size = 30
    end
    object Query_Total_RecursoCavidade: TIntegerField
      FieldName = 'Cavidade'
    end
    object Query_Total_RecursoGrupo: TStringField
      FieldName = 'Grupo'
      Size = 50
    end
    object Query_Total_RecursoTipo: TStringField
      FieldName = 'Tipo'
    end
    object Query_Total_RecursoItem_FK: TStringField
      FieldName = 'Item_FK'
      Size = 8
    end
    object Query_Total_RecursoItem: TStringField
      FieldName = 'Item'
      Size = 50
    end
    object Query_Total_RecursoTurnoDescricao: TStringField
      FieldName = 'TurnoDescricao'
      Size = 15
    end
    object Query_Total_RecursoTurno: TStringField
      FieldName = 'Turno'
      Size = 2
    end
    object Query_Total_RecursoPosicao: TIntegerField
      FieldName = 'Posicao'
    end
    object Query_Total_RecursoNumItens: TIntegerField
      FieldName = 'NumItens'
    end
    object Query_Total_RecursoProducao: TIntegerField
      FieldName = 'Producao'
    end
    object Query_Total_RecursoPerdaUN: TIntegerField
      FieldName = 'PerdaUN'
    end
    object Query_Total_RecursoPerdaKGPerc: TFloatField
      FieldName = 'PerdaKGPerc'
    end
    object Query_Total_RecursoPesoItem: TFloatField
      FieldName = 'PesoItem'
    end
    object Query_Total_RecursoProducaoKG: TFloatField
      FieldName = 'ProducaoKG'
    end
    object Query_Total_RecursoPesoPerda: TFloatField
      FieldName = 'PesoPerda'
    end
    object Query_Total_RecursoPerdaUNPerc: TFloatField
      FieldName = 'PerdaUNPerc'
    end
    object Query_Total_RecursoSemafaro_PerdaUNPerc: TStringField
      FieldName = 'Semafaro_PerdaUNPerc'
      ReadOnly = True
      Size = 8
    end
    object Query_Total_RecursoCausa_Perda: TStringField
      FieldName = 'Causa_Perda'
      Size = 500
    end
    object Query_Total_RecursoCicloReal: TFloatField
      FieldName = 'CicloReal'
    end
    object Query_Total_RecursoCicloPadrao: TFloatField
      FieldName = 'CicloPadrao'
      ReadOnly = True
    end
    object Query_Total_RecursoCiclo_Diferenca: TFloatField
      FieldName = 'Ciclo_Diferenca'
      ReadOnly = True
    end
    object Query_Total_RecursoSemafaro_Ciclo: TStringField
      FieldName = 'Semafaro_Ciclo'
      ReadOnly = True
      Size = 8
    end
    object Query_Total_RecursoDataInicio: TDateTimeField
      FieldName = 'DataInicio'
    end
    object Query_Total_RecursoHoraInicio: TStringField
      FieldName = 'HoraInicio'
      Size = 5
    end
    object Query_Total_RecursoDataFim: TDateTimeField
      FieldName = 'DataFim'
    end
    object Query_Total_RecursoHoraFim: TStringField
      FieldName = 'HoraFim'
      Size = 5
    end
    object Query_Total_RecursoNominalTurno: TFloatField
      FieldName = 'NominalTurno'
    end
    object Query_Total_RecursoEficienciaNominal: TFloatField
      FieldName = 'EficienciaNominal'
    end
    object Query_Total_RecursoSemafaro_EficienciaNominal: TStringField
      FieldName = 'Semafaro_EficienciaNominal'
      ReadOnly = True
      Size = 8
    end
    object Query_Total_RecursoTempoProducao: TIntegerField
      FieldName = 'TempoProducao'
    end
    object Query_Total_RecursoTempoProducaoTotal: TIntegerField
      FieldName = 'TempoProducaoTotal'
    end
    object Query_Total_RecursoTempoProducaoPer: TFloatField
      FieldName = 'TempoProducaoPer'
    end
    object Query_Total_RecursoTempoTotalDoTurno: TFloatField
      FieldName = 'TempoTotalDoTurno'
    end
    object Query_Total_RecursoTempoParadaMin: TIntegerField
      FieldName = 'TempoParadaMin'
    end
    object Query_Total_RecursoCausa_Improdutividade: TStringField
      FieldName = 'Causa_Improdutividade'
      Size = 500
    end
    object Query_Total_RecursoNominalKG: TFloatField
      FieldName = 'NominalKG'
    end
    object Query_Total_RecursoCapacidadeTeorica: TFloatField
      FieldName = 'CapacidadeTeorica'
    end
    object Query_Total_RecursoProducaoJustificada: TFloatField
      FieldName = 'ProducaoJustificada'
    end
    object Query_Total_RecursoCapacidadeDisponivel: TFloatField
      FieldName = 'CapacidadeDisponivel'
    end
    object Query_Total_RecursoSemafaro: TStringField
      FieldName = 'Semafaro'
      ReadOnly = True
      Size = 8
    end
    object Query_Total_RecursoProducaoPer: TFloatField
      FieldName = 'ProducaoPer'
      ReadOnly = True
    end
    object Query_Total_RecursoImprodutividade: TFloatField
      FieldName = 'Improdutividade'
      ReadOnly = True
    end
    object Query_Total_RecursoTempoImprodutivo: TFloatField
      FieldName = 'TempoImprodutivo'
      ReadOnly = True
    end
    object Query_Total_RecursoCapacidadeKG: TFloatField
      FieldName = 'CapacidadeKG'
      ReadOnly = True
    end
    object Query_Total_RecursoProdutividade: TFloatField
      FieldName = 'Produtividade'
      ReadOnly = True
    end
    object Query_Total_RecursoSemafaro_Produtividade: TStringField
      FieldName = 'Semafaro_Produtividade'
      ReadOnly = True
      Size = 8
    end
    object Query_Total_RecursoCapacidadeTeoricaKG: TFloatField
      FieldName = 'CapacidadeTeoricaKG'
      ReadOnly = True
    end
  end
  object CDS_Total_Recurso: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DSP_Total_Recurso'
    Left = 44
    Top = 95
    object CDS_Total_RecursoChave0: TStringField
      FieldName = 'Chave0'
      Size = 25
    end
    object CDS_Total_RecursoChave1: TStringField
      FieldName = 'Chave1'
      Size = 17
    end
    object CDS_Total_RecursoChave2: TStringField
      FieldName = 'Chave2'
      Size = 11
    end
    object CDS_Total_RecursoArea: TStringField
      FieldName = 'Area'
      Size = 50
    end
    object CDS_Total_RecursoSetor: TStringField
      FieldName = 'Setor'
      Size = 50
    end
    object CDS_Total_RecursoDataTurno: TDateTimeField
      FieldName = 'DataTurno'
    end
    object CDS_Total_RecursoMes: TStringField
      FieldName = 'Mes'
      ReadOnly = True
      Size = 30
    end
    object CDS_Total_RecursoAno: TIntegerField
      FieldName = 'Ano'
      ReadOnly = True
    end
    object CDS_Total_RecursoRecurso_ID: TStringField
      FieldName = 'Recurso_ID'
      Size = 6
    end
    object CDS_Total_RecursoRecurso: TStringField
      FieldName = 'Recurso'
      Size = 30
    end
    object CDS_Total_RecursoCavidade: TIntegerField
      FieldName = 'Cavidade'
    end
    object CDS_Total_RecursoGrupo: TStringField
      FieldName = 'Grupo'
      Size = 50
    end
    object CDS_Total_RecursoTipo: TStringField
      FieldName = 'Tipo'
    end
    object CDS_Total_RecursoItem_FK: TStringField
      FieldName = 'Item_FK'
      Size = 8
    end
    object CDS_Total_RecursoItem: TStringField
      FieldName = 'Item'
      Size = 50
    end
    object CDS_Total_RecursoTurnoDescricao: TStringField
      FieldName = 'TurnoDescricao'
      Size = 15
    end
    object CDS_Total_RecursoTurno: TStringField
      FieldName = 'Turno'
      Size = 2
    end
    object CDS_Total_RecursoPosicao: TIntegerField
      FieldName = 'Posicao'
    end
    object CDS_Total_RecursoNumItens: TIntegerField
      FieldName = 'NumItens'
    end
    object CDS_Total_RecursoProducao: TIntegerField
      FieldName = 'Producao'
    end
    object CDS_Total_RecursoPerdaUN: TIntegerField
      FieldName = 'PerdaUN'
    end
    object CDS_Total_RecursoPerdaKGPerc: TFloatField
      FieldName = 'PerdaKGPerc'
    end
    object CDS_Total_RecursoPesoItem: TFloatField
      FieldName = 'PesoItem'
    end
    object CDS_Total_RecursoProducaoKG: TFloatField
      FieldName = 'ProducaoKG'
    end
    object CDS_Total_RecursoPesoPerda: TFloatField
      FieldName = 'PesoPerda'
    end
    object CDS_Total_RecursoPerdaUNPerc: TFloatField
      FieldName = 'PerdaUNPerc'
    end
    object CDS_Total_RecursoSemafaro_PerdaUNPerc: TStringField
      FieldName = 'Semafaro_PerdaUNPerc'
      ReadOnly = True
      Size = 8
    end
    object CDS_Total_RecursoCausa_Perda: TStringField
      FieldName = 'Causa_Perda'
      Size = 500
    end
    object CDS_Total_RecursoCicloReal: TFloatField
      FieldName = 'CicloReal'
    end
    object CDS_Total_RecursoCicloPadrao: TFloatField
      FieldName = 'CicloPadrao'
      ReadOnly = True
    end
    object CDS_Total_RecursoCiclo_Diferenca: TFloatField
      FieldName = 'Ciclo_Diferenca'
      ReadOnly = True
    end
    object CDS_Total_RecursoSemafaro_Ciclo: TStringField
      FieldName = 'Semafaro_Ciclo'
      ReadOnly = True
      Size = 8
    end
    object CDS_Total_RecursoDataInicio: TDateTimeField
      FieldName = 'DataInicio'
    end
    object CDS_Total_RecursoHoraInicio: TStringField
      FieldName = 'HoraInicio'
      Size = 5
    end
    object CDS_Total_RecursoDataFim: TDateTimeField
      FieldName = 'DataFim'
    end
    object CDS_Total_RecursoHoraFim: TStringField
      FieldName = 'HoraFim'
      Size = 5
    end
    object CDS_Total_RecursoNominalTurno: TFloatField
      FieldName = 'NominalTurno'
    end
    object CDS_Total_RecursoEficienciaNominal: TFloatField
      FieldName = 'EficienciaNominal'
    end
    object CDS_Total_RecursoSemafaro_EficienciaNominal: TStringField
      FieldName = 'Semafaro_EficienciaNominal'
      ReadOnly = True
      Size = 8
    end
    object CDS_Total_RecursoTempoProducao: TIntegerField
      FieldName = 'TempoProducao'
    end
    object CDS_Total_RecursoTempoProducaoTotal: TIntegerField
      FieldName = 'TempoProducaoTotal'
    end
    object CDS_Total_RecursoTempoProducaoPer: TFloatField
      FieldName = 'TempoProducaoPer'
    end
    object CDS_Total_RecursoTempoTotalDoTurno: TFloatField
      FieldName = 'TempoTotalDoTurno'
    end
    object CDS_Total_RecursoTempoParadaMin: TIntegerField
      FieldName = 'TempoParadaMin'
    end
    object CDS_Total_RecursoCausa_Improdutividade: TStringField
      FieldName = 'Causa_Improdutividade'
      Size = 500
    end
    object CDS_Total_RecursoNominalKG: TFloatField
      FieldName = 'NominalKG'
    end
    object CDS_Total_RecursoCapacidadeTeorica: TFloatField
      FieldName = 'CapacidadeTeorica'
    end
    object CDS_Total_RecursoProducaoJustificada: TFloatField
      FieldName = 'ProducaoJustificada'
    end
    object CDS_Total_RecursoCapacidadeDisponivel: TFloatField
      FieldName = 'CapacidadeDisponivel'
    end
    object CDS_Total_RecursoSemafaro: TStringField
      FieldName = 'Semafaro'
      ReadOnly = True
      Size = 8
    end
    object CDS_Total_RecursoProducaoPer: TFloatField
      FieldName = 'ProducaoPer'
      ReadOnly = True
    end
    object CDS_Total_RecursoImprodutividade: TFloatField
      FieldName = 'Improdutividade'
      ReadOnly = True
    end
    object CDS_Total_RecursoTempoImprodutivo: TFloatField
      FieldName = 'TempoImprodutivo'
      ReadOnly = True
    end
    object CDS_Total_RecursoCapacidadeKG: TFloatField
      FieldName = 'CapacidadeKG'
      ReadOnly = True
    end
    object CDS_Total_RecursoProdutividade: TFloatField
      FieldName = 'Produtividade'
      ReadOnly = True
    end
    object CDS_Total_RecursoSemafaro_Produtividade: TStringField
      FieldName = 'Semafaro_Produtividade'
      ReadOnly = True
      Size = 8
    end
    object CDS_Total_RecursoCapacidadeTeoricaKG: TFloatField
      FieldName = 'CapacidadeTeoricaKG'
      ReadOnly = True
    end
  end
  object DSP_Total_Turno: TDataSetProvider
    DataSet = Query_Total_Turno
    Left = 103
    Top = 157
  end
  object DS_Total_Turno: TDataSource
    DataSet = CDS_Total_Turno
    Left = 65
    Top = 157
  end
  object Query_Total_Turno: TADOQuery
    Connection = Sys_ModuloConexaoDados.ADOConnection1
    CursorType = ctStatic
    DataSource = DS_Total_Turno
    Parameters = <>
    SQL.Strings = (
      'Select '
      '  '
      ' '#9'*   '
      '  '
      ' from [dbo].[Pcp_VW_OrdemProducaoApontamentoMIGUEL_Totvs]  '
      
        ' Where DataTurno =  CONVERT(Datetime,Convert(Varchar(10),'#39'13/10/' +
        '2016 10:10:49'#39',103),103)'
      
        '  AND rtrim(Area) in ('#39'GRAFICA'#39','#39'INJECAO'#39','#39'PALETIZACAO'#39','#39'PERSONA' +
        'LIZACAO'#39')'
      ' AND rtrim(Setor) = '#39'INJECAO'#39
      ' AND TurnoDescricao = '#39'TURNO 02'#39
      '  Order by Setor, Recurso_ID, Turno  ')
    Left = 139
    Top = 157
    object Query_Total_TurnoChave0: TStringField
      FieldName = 'Chave0'
      Size = 25
    end
    object Query_Total_TurnoChave1: TStringField
      FieldName = 'Chave1'
      Size = 17
    end
    object Query_Total_TurnoChave2: TStringField
      FieldName = 'Chave2'
      Size = 11
    end
    object Query_Total_TurnoArea: TStringField
      FieldName = 'Area'
      Size = 50
    end
    object Query_Total_TurnoSetor: TStringField
      FieldName = 'Setor'
      Size = 50
    end
    object Query_Total_TurnoDataTurno: TDateTimeField
      FieldName = 'DataTurno'
    end
    object Query_Total_TurnoMes: TStringField
      FieldName = 'Mes'
      ReadOnly = True
      Size = 30
    end
    object Query_Total_TurnoAno: TIntegerField
      FieldName = 'Ano'
      ReadOnly = True
    end
    object Query_Total_TurnoRecurso_ID: TStringField
      FieldName = 'Recurso_ID'
      Size = 6
    end
    object Query_Total_TurnoRecurso: TStringField
      FieldName = 'Recurso'
      Size = 30
    end
    object Query_Total_TurnoCavidade: TIntegerField
      FieldName = 'Cavidade'
    end
    object Query_Total_TurnoGrupo: TStringField
      FieldName = 'Grupo'
      Size = 50
    end
    object Query_Total_TurnoTipo: TStringField
      FieldName = 'Tipo'
    end
    object Query_Total_TurnoItem_FK: TStringField
      FieldName = 'Item_FK'
      Size = 8
    end
    object Query_Total_TurnoItem: TStringField
      FieldName = 'Item'
      Size = 50
    end
    object Query_Total_TurnoTurnoDescricao: TStringField
      FieldName = 'TurnoDescricao'
      Size = 15
    end
    object Query_Total_TurnoTurno: TStringField
      FieldName = 'Turno'
      Size = 2
    end
    object Query_Total_TurnoPosicao: TIntegerField
      FieldName = 'Posicao'
    end
    object Query_Total_TurnoNumItens: TIntegerField
      FieldName = 'NumItens'
    end
    object Query_Total_TurnoProducao: TIntegerField
      FieldName = 'Producao'
    end
    object Query_Total_TurnoPerdaUN: TIntegerField
      FieldName = 'PerdaUN'
    end
    object Query_Total_TurnoPerdaKGPerc: TFloatField
      FieldName = 'PerdaKGPerc'
    end
    object Query_Total_TurnoPesoItem: TFloatField
      FieldName = 'PesoItem'
    end
    object Query_Total_TurnoProducaoKG: TFloatField
      FieldName = 'ProducaoKG'
    end
    object Query_Total_TurnoPesoPerda: TFloatField
      FieldName = 'PesoPerda'
    end
    object Query_Total_TurnoPerdaUNPerc: TFloatField
      FieldName = 'PerdaUNPerc'
    end
    object Query_Total_TurnoSemafaro_PerdaUNPerc: TStringField
      FieldName = 'Semafaro_PerdaUNPerc'
      ReadOnly = True
      Size = 8
    end
    object Query_Total_TurnoCausa_Perda: TStringField
      FieldName = 'Causa_Perda'
      Size = 500
    end
    object Query_Total_TurnoCicloReal: TFloatField
      FieldName = 'CicloReal'
    end
    object Query_Total_TurnoCicloPadrao: TFloatField
      FieldName = 'CicloPadrao'
      ReadOnly = True
    end
    object Query_Total_TurnoCiclo_Diferenca: TFloatField
      FieldName = 'Ciclo_Diferenca'
      ReadOnly = True
    end
    object Query_Total_TurnoSemafaro_Ciclo: TStringField
      FieldName = 'Semafaro_Ciclo'
      ReadOnly = True
      Size = 8
    end
    object Query_Total_TurnoDataInicio: TDateTimeField
      FieldName = 'DataInicio'
    end
    object Query_Total_TurnoHoraInicio: TStringField
      FieldName = 'HoraInicio'
      Size = 5
    end
    object Query_Total_TurnoDataFim: TDateTimeField
      FieldName = 'DataFim'
    end
    object Query_Total_TurnoHoraFim: TStringField
      FieldName = 'HoraFim'
      Size = 5
    end
    object Query_Total_TurnoNominalTurno: TFloatField
      FieldName = 'NominalTurno'
    end
    object Query_Total_TurnoEficienciaNominal: TFloatField
      FieldName = 'EficienciaNominal'
    end
    object Query_Total_TurnoSemafaro_EficienciaNominal: TStringField
      FieldName = 'Semafaro_EficienciaNominal'
      ReadOnly = True
      Size = 8
    end
    object Query_Total_TurnoTempoProducao: TIntegerField
      FieldName = 'TempoProducao'
    end
    object Query_Total_TurnoTempoProducaoTotal: TIntegerField
      FieldName = 'TempoProducaoTotal'
    end
    object Query_Total_TurnoTempoProducaoPer: TFloatField
      FieldName = 'TempoProducaoPer'
    end
    object Query_Total_TurnoTempoTotalDoTurno: TFloatField
      FieldName = 'TempoTotalDoTurno'
    end
    object Query_Total_TurnoTempoParadaMin: TIntegerField
      FieldName = 'TempoParadaMin'
    end
    object Query_Total_TurnoCausa_Improdutividade: TStringField
      FieldName = 'Causa_Improdutividade'
      Size = 500
    end
    object Query_Total_TurnoNominalKG: TFloatField
      FieldName = 'NominalKG'
    end
    object Query_Total_TurnoCapacidadeTeorica: TFloatField
      FieldName = 'CapacidadeTeorica'
    end
    object Query_Total_TurnoProducaoJustificada: TFloatField
      FieldName = 'ProducaoJustificada'
    end
    object Query_Total_TurnoCapacidadeDisponivel: TFloatField
      FieldName = 'CapacidadeDisponivel'
    end
    object Query_Total_TurnoSemafaro: TStringField
      FieldName = 'Semafaro'
      ReadOnly = True
      Size = 8
    end
    object Query_Total_TurnoProducaoPer: TFloatField
      FieldName = 'ProducaoPer'
      ReadOnly = True
    end
    object Query_Total_TurnoImprodutividade: TFloatField
      FieldName = 'Improdutividade'
      ReadOnly = True
    end
    object Query_Total_TurnoTempoImprodutivo: TFloatField
      FieldName = 'TempoImprodutivo'
      ReadOnly = True
    end
    object Query_Total_TurnoCapacidadeKG: TFloatField
      FieldName = 'CapacidadeKG'
      ReadOnly = True
    end
    object Query_Total_TurnoProdutividade: TFloatField
      FieldName = 'Produtividade'
      ReadOnly = True
    end
    object Query_Total_TurnoSemafaro_Produtividade: TStringField
      FieldName = 'Semafaro_Produtividade'
      ReadOnly = True
      Size = 8
    end
    object Query_Total_TurnoCapacidadeTeoricaKG: TFloatField
      FieldName = 'CapacidadeTeoricaKG'
      ReadOnly = True
    end
  end
  object CDS_Total_Turno: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DSP_Total_Turno'
    Left = 28
    Top = 157
    object CDS_Total_TurnoChave0: TStringField
      FieldName = 'Chave0'
      Size = 25
    end
    object CDS_Total_TurnoChave1: TStringField
      FieldName = 'Chave1'
      Size = 17
    end
    object CDS_Total_TurnoChave2: TStringField
      FieldName = 'Chave2'
      Size = 11
    end
    object CDS_Total_TurnoArea: TStringField
      FieldName = 'Area'
      Size = 50
    end
    object CDS_Total_TurnoSetor: TStringField
      FieldName = 'Setor'
      Size = 50
    end
    object CDS_Total_TurnoDataTurno: TDateTimeField
      FieldName = 'DataTurno'
    end
    object CDS_Total_TurnoMes: TStringField
      FieldName = 'Mes'
      ReadOnly = True
      Size = 30
    end
    object CDS_Total_TurnoAno: TIntegerField
      FieldName = 'Ano'
      ReadOnly = True
    end
    object CDS_Total_TurnoRecurso_ID: TStringField
      FieldName = 'Recurso_ID'
      Size = 6
    end
    object CDS_Total_TurnoRecurso: TStringField
      FieldName = 'Recurso'
      Size = 30
    end
    object CDS_Total_TurnoCavidade: TIntegerField
      FieldName = 'Cavidade'
    end
    object CDS_Total_TurnoGrupo: TStringField
      FieldName = 'Grupo'
      Size = 50
    end
    object CDS_Total_TurnoTipo: TStringField
      FieldName = 'Tipo'
    end
    object CDS_Total_TurnoItem_FK: TStringField
      FieldName = 'Item_FK'
      Size = 8
    end
    object CDS_Total_TurnoItem: TStringField
      FieldName = 'Item'
      Size = 50
    end
    object CDS_Total_TurnoTurnoDescricao: TStringField
      FieldName = 'TurnoDescricao'
      Size = 15
    end
    object CDS_Total_TurnoTurno: TStringField
      FieldName = 'Turno'
      Size = 2
    end
    object CDS_Total_TurnoPosicao: TIntegerField
      FieldName = 'Posicao'
    end
    object CDS_Total_TurnoNumItens: TIntegerField
      FieldName = 'NumItens'
    end
    object CDS_Total_TurnoProducao: TIntegerField
      FieldName = 'Producao'
    end
    object CDS_Total_TurnoPerdaUN: TIntegerField
      FieldName = 'PerdaUN'
    end
    object CDS_Total_TurnoPerdaKGPerc: TFloatField
      FieldName = 'PerdaKGPerc'
    end
    object CDS_Total_TurnoPesoItem: TFloatField
      FieldName = 'PesoItem'
    end
    object CDS_Total_TurnoProducaoKG: TFloatField
      FieldName = 'ProducaoKG'
    end
    object CDS_Total_TurnoPesoPerda: TFloatField
      FieldName = 'PesoPerda'
    end
    object CDS_Total_TurnoPerdaUNPerc: TFloatField
      FieldName = 'PerdaUNPerc'
    end
    object CDS_Total_TurnoSemafaro_PerdaUNPerc: TStringField
      FieldName = 'Semafaro_PerdaUNPerc'
      ReadOnly = True
      Size = 8
    end
    object CDS_Total_TurnoCausa_Perda: TStringField
      FieldName = 'Causa_Perda'
      Size = 500
    end
    object CDS_Total_TurnoCicloReal: TFloatField
      FieldName = 'CicloReal'
    end
    object CDS_Total_TurnoCicloPadrao: TFloatField
      FieldName = 'CicloPadrao'
      ReadOnly = True
    end
    object CDS_Total_TurnoCiclo_Diferenca: TFloatField
      FieldName = 'Ciclo_Diferenca'
      ReadOnly = True
    end
    object CDS_Total_TurnoSemafaro_Ciclo: TStringField
      FieldName = 'Semafaro_Ciclo'
      ReadOnly = True
      Size = 8
    end
    object CDS_Total_TurnoDataInicio: TDateTimeField
      FieldName = 'DataInicio'
    end
    object CDS_Total_TurnoHoraInicio: TStringField
      FieldName = 'HoraInicio'
      Size = 5
    end
    object CDS_Total_TurnoDataFim: TDateTimeField
      FieldName = 'DataFim'
    end
    object CDS_Total_TurnoHoraFim: TStringField
      FieldName = 'HoraFim'
      Size = 5
    end
    object CDS_Total_TurnoNominalTurno: TFloatField
      FieldName = 'NominalTurno'
    end
    object CDS_Total_TurnoEficienciaNominal: TFloatField
      FieldName = 'EficienciaNominal'
    end
    object CDS_Total_TurnoSemafaro_EficienciaNominal: TStringField
      FieldName = 'Semafaro_EficienciaNominal'
      ReadOnly = True
      Size = 8
    end
    object CDS_Total_TurnoTempoProducao: TIntegerField
      FieldName = 'TempoProducao'
    end
    object CDS_Total_TurnoTempoProducaoTotal: TIntegerField
      FieldName = 'TempoProducaoTotal'
    end
    object CDS_Total_TurnoTempoProducaoPer: TFloatField
      FieldName = 'TempoProducaoPer'
    end
    object CDS_Total_TurnoTempoTotalDoTurno: TFloatField
      FieldName = 'TempoTotalDoTurno'
    end
    object CDS_Total_TurnoTempoParadaMin: TIntegerField
      FieldName = 'TempoParadaMin'
    end
    object CDS_Total_TurnoCausa_Improdutividade: TStringField
      FieldName = 'Causa_Improdutividade'
      Size = 500
    end
    object CDS_Total_TurnoNominalKG: TFloatField
      FieldName = 'NominalKG'
    end
    object CDS_Total_TurnoCapacidadeTeorica: TFloatField
      FieldName = 'CapacidadeTeorica'
    end
    object CDS_Total_TurnoProducaoJustificada: TFloatField
      FieldName = 'ProducaoJustificada'
    end
    object CDS_Total_TurnoCapacidadeDisponivel: TFloatField
      FieldName = 'CapacidadeDisponivel'
    end
    object CDS_Total_TurnoSemafaro: TStringField
      FieldName = 'Semafaro'
      ReadOnly = True
      Size = 8
    end
    object CDS_Total_TurnoProducaoPer: TFloatField
      FieldName = 'ProducaoPer'
      ReadOnly = True
    end
    object CDS_Total_TurnoImprodutividade: TFloatField
      FieldName = 'Improdutividade'
      ReadOnly = True
    end
    object CDS_Total_TurnoTempoImprodutivo: TFloatField
      FieldName = 'TempoImprodutivo'
      ReadOnly = True
    end
    object CDS_Total_TurnoCapacidadeKG: TFloatField
      FieldName = 'CapacidadeKG'
      ReadOnly = True
    end
    object CDS_Total_TurnoProdutividade: TFloatField
      FieldName = 'Produtividade'
      ReadOnly = True
    end
    object CDS_Total_TurnoSemafaro_Produtividade: TStringField
      FieldName = 'Semafaro_Produtividade'
      ReadOnly = True
      Size = 8
    end
    object CDS_Total_TurnoCapacidadeTeoricaKG: TFloatField
      FieldName = 'CapacidadeTeoricaKG'
      ReadOnly = True
    end
  end
end
