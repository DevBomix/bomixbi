object Relatorio_ApontamentoProducaoPorTurno: TRelatorio_ApontamentoProducaoPorTurno
  Left = 0
  Top = 0
  Caption = 'Relatorio_ApontamentoProducaoPorTurno'
  ClientHeight = 189
  ClientWidth = 471
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  PixelsPerInch = 96
  TextHeight = 13
  object CDS_ApontamentoSinteticoPorTurno: TClientDataSet
    Active = True
    Aggregates = <>
    Params = <>
    ProviderName = 'DSP_ApontamentoSinteticoPorTurno'
    Left = 39
    Top = 23
    object CDS_ApontamentoSinteticoPorTurnoArea: TStringField
      FieldName = 'Area'
      Size = 50
    end
    object CDS_ApontamentoSinteticoPorTurnoDataTurno: TDateTimeField
      FieldName = 'DataTurno'
    end
    object CDS_ApontamentoSinteticoPorTurnoTurno: TStringField
      FieldName = 'Turno'
      Size = 2
    end
    object CDS_ApontamentoSinteticoPorTurnoTurnoDescricao: TStringField
      FieldName = 'TurnoDescricao'
      Size = 15
    end
    object CDS_ApontamentoSinteticoPorTurnoRecurso_ID: TStringField
      FieldName = 'Recurso_ID'
      Size = 6
    end
    object CDS_ApontamentoSinteticoPorTurnoRecurso: TStringField
      FieldName = 'Recurso'
      Size = 30
    end
    object CDS_ApontamentoSinteticoPorTurnoGrupo: TStringField
      FieldName = 'Grupo'
      Size = 10
    end
    object CDS_ApontamentoSinteticoPorTurnoProduto_ID: TStringField
      FieldName = 'Produto_ID'
      Size = 10
    end
    object CDS_ApontamentoSinteticoPorTurnoProduto: TStringField
      FieldName = 'Produto'
      Size = 100
    end
    object CDS_ApontamentoSinteticoPorTurnoQuantidade: TFloatField
      FieldName = 'Quantidade'
      ReadOnly = True
    end
    object CDS_ApontamentoSinteticoPorTurnoPeso: TFloatField
      FieldName = 'Peso'
      ReadOnly = True
    end
    object CDS_ApontamentoSinteticoPorTurnoQuantidadePerda: TFloatField
      FieldName = 'QuantidadePerda'
      ReadOnly = True
    end
    object CDS_ApontamentoSinteticoPorTurnoPesoPerda: TFloatField
      FieldName = 'PesoPerda'
      ReadOnly = True
    end
  end
  object Query_ApontamentoSinteticoPorTurno: TADOQuery
    Connection = Sys_ModuloConexaoDados.ADOConnection1
    CursorType = ctStatic
    DataSource = DS_ApontamentoSinteticoPorTurno
    Parameters = <>
    SQL.Strings = (
      ' Select '
      
        ' '#9' Area, DataTurno, Turno, TurnoDescricao, Recurso_ID, Recurso, ' +
        'Grupo, Produto_ID, Produto, Sum(ProduzidoA) as Quantidade, '
      
        #9' Sum(Peso) as Peso, Sum(Perda) as QuantidadePerda, Isnull(Sum(P' +
        'esoPerda),0) as PesoPerda  '
      ' from (  '
      ' Select  '
      '  '
      
        '   '#9'Apontamento.Setor, Apontamento.Area, Apontamento.Emissao, Ap' +
        'ontamento.DataTurno, DataIni, HoraIni, DataFin, HoraFin,  '
      
        '   '#9'Apontamento.TurnoDescricao, Recurso_ID, Apontamento.Recurso,' +
        ' Apontamento.CicloApontado, Apontamento.CicloDigitado, Apontamen' +
        'to.CicloPadrao,  '
      
        '   '#9'Apontamento.Dosagem, Apontamento.OrdemProducao, Grupo, Produ' +
        'to_ID, Apontamento.Produto, Cor, Apontamento.Item,   '
      
        '  '#9'Peso, Tipo, Lote, Armazem, Apontamento.Turno, QtdProduzida as' +
        ' ProduzidoA, Apontamento.Perda, Apontamento.PesoPerda AS PesoPer' +
        'da,  '
      
        '  '#9'Apontamento.Causa_Perda, Apontamento.Recno as Apontamento_Rec' +
        'no  '
      
        ' from BomixBI.dbo.Pcp_TB_OrdemProducaoApontamentoRelatorio Apont' +
        'amento (nolock) '
      ' Where Apontamento.Empresa = '#39'0101'#39
      
        ' AND Apontamento.DataTurno Between  CONVERT(Datetime,Convert(Var' +
        'char(10),'#39'18/10/2016 10:10:49'#39',103),103)'
      
        ' AND CONVERT(Datetime,Convert(Varchar(10),'#39'18/10/2016 10:10:49'#39',' +
        '103),103)'
      
        ' AND rtrim(Apontamento.Area) in ('#39'GRAFICA'#39','#39'INJECAO'#39','#39'PALETIZACA' +
        'O'#39','#39'PERSONALIZACAO'#39')'
      
        ' AND rtrim(Apontamento.Setor) in ('#39'INJECAO'#39','#39'PERSONALIZACAO'#39','#39'IN' +
        'JECAO 04'#39','#39'PALETIZACAO 04'#39','#39'IMPRESSAO DIGITAL'#39','#39'MONTAGEM DE ALCA' +
        #39','#39'PALETIZACAO PE'#39')'
      ') TB '
      
        'Group by Area, DataTurno, Turno, TurnoDescricao, Recurso_ID, Rec' +
        'urso, Grupo, Produto_ID, Produto  '
      
        '  Order by Area, DataTurno desc, TurnoDescricao, Recurso_ID, Rec' +
        'urso, Grupo, Produto_ID, Produto'
      '')
    Left = 70
    Top = 22
    object Query_ApontamentoSinteticoPorTurnoArea: TStringField
      FieldName = 'Area'
      Size = 50
    end
    object Query_ApontamentoSinteticoPorTurnoDataTurno: TDateTimeField
      FieldName = 'DataTurno'
    end
    object Query_ApontamentoSinteticoPorTurnoTurno: TStringField
      FieldName = 'Turno'
      Size = 2
    end
    object Query_ApontamentoSinteticoPorTurnoTurnoDescricao: TStringField
      FieldName = 'TurnoDescricao'
      Size = 15
    end
    object Query_ApontamentoSinteticoPorTurnoRecurso_ID: TStringField
      FieldName = 'Recurso_ID'
      Size = 6
    end
    object Query_ApontamentoSinteticoPorTurnoRecurso: TStringField
      FieldName = 'Recurso'
      Size = 30
    end
    object Query_ApontamentoSinteticoPorTurnoGrupo: TStringField
      FieldName = 'Grupo'
      Size = 10
    end
    object Query_ApontamentoSinteticoPorTurnoProduto_ID: TStringField
      FieldName = 'Produto_ID'
      Size = 10
    end
    object Query_ApontamentoSinteticoPorTurnoProduto: TStringField
      FieldName = 'Produto'
      Size = 100
    end
    object Query_ApontamentoSinteticoPorTurnoQuantidade: TFloatField
      FieldName = 'Quantidade'
      ReadOnly = True
    end
    object Query_ApontamentoSinteticoPorTurnoPeso: TFloatField
      FieldName = 'Peso'
      ReadOnly = True
    end
    object Query_ApontamentoSinteticoPorTurnoQuantidadePerda: TFloatField
      FieldName = 'QuantidadePerda'
      ReadOnly = True
    end
    object Query_ApontamentoSinteticoPorTurnoPesoPerda: TFloatField
      FieldName = 'PesoPerda'
      ReadOnly = True
    end
  end
  object DS_ApontamentoSinteticoPorTurno: TDataSource
    DataSet = CDS_ApontamentoSinteticoPorTurno
    Left = 100
    Top = 22
  end
  object DSP_ApontamentoSinteticoPorTurno: TDataSetProvider
    DataSet = Query_ApontamentoSinteticoPorTurno
    Left = 130
    Top = 22
  end
  object ppDBPipeline_ApontamentoSinteticoPorTurno: TppDBPipeline
    DataSource = DS_ApontamentoSinteticoPorTurno
    CloseDataSource = True
    UserName = 'DBPipeline_ApontamentoSinteticoPorTurno'
    Left = 166
    Top = 23
    object ppDBPipeline_ApontamentoSinteticoPorTurnoppField1: TppField
      FieldAlias = 'Area'
      FieldName = 'Area'
      FieldLength = 0
      DisplayWidth = 0
      Position = 0
    end
    object ppDBPipeline_ApontamentoSinteticoPorTurnoppField2: TppField
      FieldAlias = 'DataTurno'
      FieldName = 'DataTurno'
      FieldLength = 0
      DataType = dtDateTime
      DisplayWidth = 18
      Position = 1
    end
    object ppDBPipeline_ApontamentoSinteticoPorTurnoppField3: TppField
      FieldAlias = 'Turno'
      FieldName = 'Turno'
      FieldLength = 2
      DisplayWidth = 2
      Position = 2
    end
    object ppDBPipeline_ApontamentoSinteticoPorTurnoppField4: TppField
      FieldAlias = 'TurnoDescricao'
      FieldName = 'TurnoDescricao'
      FieldLength = 15
      DisplayWidth = 15
      Position = 3
    end
    object ppDBPipeline_ApontamentoSinteticoPorTurnoppField5: TppField
      FieldAlias = 'Recurso_ID'
      FieldName = 'Recurso_ID'
      FieldLength = 6
      DisplayWidth = 6
      Position = 4
    end
    object ppDBPipeline_ApontamentoSinteticoPorTurnoppField6: TppField
      FieldAlias = 'Recurso'
      FieldName = 'Recurso'
      FieldLength = 30
      DisplayWidth = 30
      Position = 5
    end
    object ppDBPipeline_ApontamentoSinteticoPorTurnoppField7: TppField
      FieldAlias = 'Grupo'
      FieldName = 'Grupo'
      FieldLength = 10
      DisplayWidth = 10
      Position = 6
    end
    object ppDBPipeline_ApontamentoSinteticoPorTurnoppField8: TppField
      FieldAlias = 'Produto_ID'
      FieldName = 'Produto_ID'
      FieldLength = 10
      DisplayWidth = 10
      Position = 7
    end
    object ppDBPipeline_ApontamentoSinteticoPorTurnoppField9: TppField
      FieldAlias = 'Produto'
      FieldName = 'Produto'
      FieldLength = 100
      DisplayWidth = 100
      Position = 8
    end
    object ppDBPipeline_ApontamentoSinteticoPorTurnoppField10: TppField
      Alignment = taRightJustify
      FieldAlias = 'Quantidade'
      FieldName = 'Quantidade'
      FieldLength = 0
      DataType = dtDouble
      DisplayWidth = 10
      Position = 9
    end
    object ppDBPipeline_ApontamentoSinteticoPorTurnoppField11: TppField
      Alignment = taRightJustify
      FieldAlias = 'Peso'
      FieldName = 'Peso'
      FieldLength = 0
      DataType = dtDouble
      DisplayWidth = 10
      Position = 10
    end
    object ppDBPipeline_ApontamentoSinteticoPorTurnoppField12: TppField
      Alignment = taRightJustify
      FieldAlias = 'QuantidadePerda'
      FieldName = 'QuantidadePerda'
      FieldLength = 0
      DataType = dtDouble
      DisplayWidth = 10
      Position = 11
    end
    object ppDBPipeline_ApontamentoSinteticoPorTurnoppField13: TppField
      Alignment = taRightJustify
      FieldAlias = 'PesoPerda'
      FieldName = 'PesoPerda'
      FieldLength = 0
      DataType = dtDouble
      DisplayWidth = 10
      Position = 12
    end
  end
  object ppApontamentoSinteticoPorTurno: TppReport
    AutoStop = False
    DataPipeline = ppDBPipeline_ApontamentoSinteticoPorTurno
    PrinterSetup.BinName = 'Default'
    PrinterSetup.DocumentName = 'Report'
    PrinterSetup.Orientation = poLandscape
    PrinterSetup.PaperName = 'A4'
    PrinterSetup.PrinterName = 'HP_TI (HP LaserJet P3010 Series)'
    PrinterSetup.SaveDeviceSettings = False
    PrinterSetup.mmMarginBottom = 6350
    PrinterSetup.mmMarginLeft = 6350
    PrinterSetup.mmMarginRight = 6350
    PrinterSetup.mmMarginTop = 6350
    PrinterSetup.mmPaperHeight = 210000
    PrinterSetup.mmPaperWidth = 297000
    PrinterSetup.PaperSize = 9
    AllowPrintToArchive = True
    AllowPrintToFile = True
    ArchiveFileName = '($MyDocuments)\ReportArchive.raf'
    DeviceType = 'Screen'
    DefaultFileDeviceType = 'XLSData'
    EmailSettings.ReportFormat = 'PDF'
    LanguageID = 'Default'
    OpenFile = False
    OutlineSettings.CreateNode = True
    OutlineSettings.CreatePageNodes = True
    OutlineSettings.Enabled = True
    OutlineSettings.Visible = True
    ThumbnailSettings.Enabled = True
    ThumbnailSettings.Visible = True
    ThumbnailSettings.DeadSpace = 30
    PDFSettings.EmbedFontOptions = [efUseSubset]
    PDFSettings.EncryptSettings.AllowCopy = True
    PDFSettings.EncryptSettings.AllowInteract = True
    PDFSettings.EncryptSettings.AllowModify = True
    PDFSettings.EncryptSettings.AllowPrint = True
    PDFSettings.EncryptSettings.Enabled = False
    PDFSettings.EncryptSettings.KeyLength = kl40Bit
    PDFSettings.FontEncoding = feAnsi
    PDFSettings.ImageCompressionLevel = 25
    PreviewFormSettings.WindowState = wsMaximized
    PreviewFormSettings.ZoomSetting = zs100Percent
    RTFSettings.DefaultFont.Charset = DEFAULT_CHARSET
    RTFSettings.DefaultFont.Color = clWindowText
    RTFSettings.DefaultFont.Height = -13
    RTFSettings.DefaultFont.Name = 'Arial'
    RTFSettings.DefaultFont.Style = []
    TextFileName = '($MyDocuments)\Report.xls'
    TextSearchSettings.DefaultString = '<Texto a localizar>'
    TextSearchSettings.Enabled = True
    XLSSettings.AppName = 'ReportBuilder'
    XLSSettings.Author = 'ReportBuilder'
    XLSSettings.Subject = 'Report'
    XLSSettings.Title = 'Report'
    Left = 194
    Top = 23
    Version = '15.04'
    mmColumnWidth = 197300
    DataPipelineName = 'ppDBPipeline_ApontamentoSinteticoPorTurno'
    object ppHeaderBand3: TppHeaderBand
      Background.Brush.Style = bsClear
      mmBottomOffset = 0
      mmHeight = 14817
      mmPrintPosition = 0
      object ppLabel51: TppLabel
        UserName = 'LBL_Periodo'
        HyperlinkEnabled = False
        Caption = 'LBL_Periodo'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = [fsBold]
        TextAlignment = taRightJustified
        Transparent = True
        mmHeight = 4191
        mmLeft = 260161
        mmTop = 1323
        mmWidth = 21886
        BandType = 0
        LayerName = BandLayer8
      end
      object ppLabel49: TppLabel
        UserName = 'ppLBL_TituloRelatorio'
        HyperlinkEnabled = False
        Caption = 'Apontamento de Produ'#231#227'o - Sint'#233'tico (Por Turno)'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 18
        Font.Style = [fsBold]
        Transparent = True
        mmHeight = 7673
        mmLeft = 32015
        mmTop = 3704
        mmWidth = 151077
        BandType = 0
        LayerName = BandLayer8
      end
      object ppImage3: TppImage
        UserName = 'ppIMG_Logo'
        AlignHorizontal = ahCenter
        AlignVertical = avCenter
        MaintainAspectRatio = False
        Stretch = True
        Picture.Data = {
          
          FFFF}
        mmHeight = 8731
        mmLeft = 3175
        mmTop = 2381
        mmWidth = 27252
        BandType = 0
        LayerName = BandLayer8
      end
      object ppLine10: TppLine
        UserName = 'Line1'
        Pen.Width = 3
        Weight = 2.250000000000000000
        mmHeight = 2646
        mmLeft = 1588
        mmTop = 11640
        mmWidth = 280723
        BandType = 0
        LayerName = BandLayer8
      end
    end
    object ppDetailBand6: TppDetailBand
      Background1.Brush.Style = bsClear
      Background2.Brush.Style = bsClear
      PrintHeight = phDynamic
      mmBottomOffset = 0
      mmHeight = 5821
      mmPrintPosition = 0
      object ppDBText52: TppDBText
        UserName = 'DBText30'
        HyperlinkEnabled = False
        DataField = 'PesoPerda'
        DataPipeline = ppDBPipeline_ApontamentoSinteticoPorTurno
        DisplayFormat = '#,0.00;(#,0.00)'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDBPipeline_ApontamentoSinteticoPorTurno'
        mmHeight = 3969
        mmLeft = 266434
        mmTop = 265
        mmWidth = 14552
        BandType = 4
        LayerName = BandLayer8
      end
      object ppDBText51: TppDBText
        UserName = 'DBText31'
        HyperlinkEnabled = False
        DataField = 'QuantidadePerda'
        DataPipeline = ppDBPipeline_ApontamentoSinteticoPorTurno
        DisplayFormat = '#,0;-#,0'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDBPipeline_ApontamentoSinteticoPorTurno'
        mmHeight = 3969
        mmLeft = 250295
        mmTop = 265
        mmWidth = 14288
        BandType = 4
        LayerName = BandLayer8
      end
      object ppDBText50: TppDBText
        UserName = 'DBText33'
        HyperlinkEnabled = False
        DataField = 'Peso'
        DataPipeline = ppDBPipeline_ApontamentoSinteticoPorTurno
        DisplayFormat = '#,0.00;(#,0.00)'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDBPipeline_ApontamentoSinteticoPorTurno'
        mmHeight = 3969
        mmLeft = 231245
        mmTop = 265
        mmWidth = 16669
        BandType = 4
        LayerName = BandLayer8
      end
      object ppDBText49: TppDBText
        UserName = 'DBText34'
        HyperlinkEnabled = False
        DataField = 'Quantidade'
        DataPipeline = ppDBPipeline_ApontamentoSinteticoPorTurno
        DisplayFormat = '#,0;-#,0'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDBPipeline_ApontamentoSinteticoPorTurno'
        mmHeight = 3969
        mmLeft = 211401
        mmTop = 265
        mmWidth = 17463
        BandType = 4
        LayerName = BandLayer8
      end
      object ppDBText48: TppDBText
        UserName = 'DBText201'
        HyperlinkEnabled = False
        DataField = 'Produto'
        DataPipeline = ppDBPipeline_ApontamentoSinteticoPorTurno
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        ParentDataPipeline = False
        Transparent = True
        DataPipelineName = 'ppDBPipeline_ApontamentoSinteticoPorTurno'
        mmHeight = 3969
        mmLeft = 91015
        mmTop = 265
        mmWidth = 119063
        BandType = 4
        LayerName = BandLayer8
      end
      object ppDBText47: TppDBText
        UserName = 'DBText36'
        HyperlinkEnabled = False
        DataField = 'Produto_ID'
        DataPipeline = ppDBPipeline_ApontamentoSinteticoPorTurno
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        Transparent = True
        DataPipelineName = 'ppDBPipeline_ApontamentoSinteticoPorTurno'
        mmHeight = 3969
        mmLeft = 73023
        mmTop = 265
        mmWidth = 16404
        BandType = 4
        LayerName = BandLayer8
      end
      object ppDBText45: TppDBText
        UserName = 'DBText38'
        HyperlinkEnabled = False
        DataField = 'Grupo'
        DataPipeline = ppDBPipeline_ApontamentoSinteticoPorTurno
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        Transparent = True
        DataPipelineName = 'ppDBPipeline_ApontamentoSinteticoPorTurno'
        mmHeight = 3969
        mmLeft = 53973
        mmTop = 265
        mmWidth = 16933
        BandType = 4
        LayerName = BandLayer8
      end
      object ppDBText53: TppDBText
        UserName = 'DBText53'
        HyperlinkEnabled = False
        DataField = 'Recurso'
        DataPipeline = ppDBPipeline_ApontamentoSinteticoPorTurno
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        Transparent = True
        DataPipelineName = 'ppDBPipeline_ApontamentoSinteticoPorTurno'
        mmHeight = 3969
        mmLeft = 3438
        mmTop = 265
        mmWidth = 48154
        BandType = 4
        LayerName = BandLayer8
      end
      object ppLine11: TppLine
        UserName = 'Line6'
        Border.Color = clSilver
        Border.Style = psDot
        Pen.Color = clSilver
        Pen.Style = psDot
        Weight = 0.750000000000000000
        mmHeight = 1058
        mmLeft = 1590
        mmTop = 4498
        mmWidth = 280723
        BandType = 4
        LayerName = BandLayer8
      end
    end
    object ppFooterBand3: TppFooterBand
      Background.Brush.Style = bsClear
      mmBottomOffset = 0
      mmHeight = 6879
      mmPrintPosition = 0
      object ppLine13: TppLine
        UserName = 'Line12'
        Pen.Width = 3
        Weight = 2.250000000000000000
        mmHeight = 2646
        mmLeft = 1590
        mmTop = 265
        mmWidth = 280721
        BandType = 8
        LayerName = BandLayer8
      end
      object ppLabel52: TppLabel
        UserName = 'ppLBL_UsuarioListaPorGerencia'
        HyperlinkEnabled = False
        Caption = 'ppLBL_Usuario'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = [fsBold]
        TextAlignment = taRightJustified
        Transparent = True
        mmHeight = 4233
        mmLeft = 165894
        mmTop = 1588
        mmWidth = 116417
        BandType = 8
        LayerName = BandLayer8
      end
    end
    object ppSummaryBand5: TppSummaryBand
      Background.Brush.Style = bsClear
      mmBottomOffset = 0
      mmHeight = 7673
      mmPrintPosition = 0
      object ppShape18: TppShape
        UserName = 'Shape18'
        Shape = stRoundRect
        mmHeight = 6085
        mmLeft = 184150
        mmTop = 529
        mmWidth = 98161
        BandType = 7
        LayerName = BandLayer8
      end
      object ppLabel58: TppLabel
        UserName = 'Label58'
        HyperlinkEnabled = False
        AutoSize = False
        Caption = 'Total Geral'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 9
        Font.Style = [fsBold]
        Transparent = True
        mmHeight = 3440
        mmLeft = 186002
        mmTop = 1852
        mmWidth = 21960
        BandType = 7
        LayerName = BandLayer8
      end
      object ppDBCalc21: TppDBCalc
        UserName = 'DBCalc21'
        HyperlinkEnabled = False
        DataField = 'QuantidadePerda'
        DataPipeline = ppDBPipeline_ApontamentoSinteticoPorTurno
        DisplayFormat = '#,0;-#,0'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDBPipeline_ApontamentoSinteticoPorTurno'
        mmHeight = 3440
        mmLeft = 249238
        mmTop = 1852
        mmWidth = 15081
        BandType = 7
        LayerName = BandLayer8
      end
      object ppDBCalc22: TppDBCalc
        UserName = 'DBCalc22'
        HyperlinkEnabled = False
        DataField = 'PesoPerda'
        DataPipeline = ppDBPipeline_ApontamentoSinteticoPorTurno
        DisplayFormat = '#,0.00;-#,0.00'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDBPipeline_ApontamentoSinteticoPorTurno'
        mmHeight = 3440
        mmLeft = 265642
        mmTop = 1852
        mmWidth = 15081
        BandType = 7
        LayerName = BandLayer8
      end
      object ppDBCalc23: TppDBCalc
        UserName = 'DBCalc23'
        HyperlinkEnabled = False
        DataField = 'Peso'
        DataPipeline = ppDBPipeline_ApontamentoSinteticoPorTurno
        DisplayFormat = '#,0.00;-#,0.00'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDBPipeline_ApontamentoSinteticoPorTurno'
        mmHeight = 3440
        mmLeft = 230982
        mmTop = 1852
        mmWidth = 16669
        BandType = 7
        LayerName = BandLayer8
      end
      object ppDBCalc24: TppDBCalc
        UserName = 'DBCalc24'
        HyperlinkEnabled = False
        DataField = 'Quantidade'
        DataPipeline = ppDBPipeline_ApontamentoSinteticoPorTurno
        DisplayFormat = '#,0;-#,0'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDBPipeline_ApontamentoSinteticoPorTurno'
        mmHeight = 3440
        mmLeft = 211138
        mmTop = 1852
        mmWidth = 17463
        BandType = 7
        LayerName = BandLayer8
      end
    end
    object ppPageStyle3: TppPageStyle
      Background.Brush.Style = bsClear
      EndPage = 0
      SinglePage = 0
      StartPage = 0
      mmBottomOffset = 0
      mmHeight = 0
      mmPrintPosition = 0
    end
    object ppGroup1: TppGroup
      BreakName = 'DataTurno'
      DataPipeline = ppDBPipeline_ApontamentoSinteticoPorTurno
      GroupFileSettings.NewFile = False
      GroupFileSettings.EmailFile = False
      OutlineSettings.CreateNode = True
      StartOnOddPage = False
      UserName = 'Group1'
      mmNewColumnThreshold = 0
      mmNewPageThreshold = 0
      DataPipelineName = 'ppDBPipeline_ApontamentoSinteticoPorTurno'
      NewFile = False
      object ppGroupHeaderBand14: TppGroupHeaderBand
        Background.Brush.Style = bsClear
        mmBottomOffset = 0
        mmHeight = 6350
        mmPrintPosition = 0
        object ppDBText55: TppDBText
          UserName = 'DBText55'
          HyperlinkEnabled = False
          DataField = 'DataTurno'
          DataPipeline = ppDBPipeline_ApontamentoSinteticoPorTurno
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 12
          Font.Style = [fsBold]
          Transparent = True
          DataPipelineName = 'ppDBPipeline_ApontamentoSinteticoPorTurno'
          mmHeight = 5027
          mmLeft = 1588
          mmTop = 530
          mmWidth = 22488
          BandType = 3
          GroupNo = 0
          LayerName = BandLayer8
        end
        object ppDBText56: TppDBText
          UserName = 'DBText56'
          HyperlinkEnabled = False
          DataField = 'Area'
          DataPipeline = ppDBPipeline_ApontamentoSinteticoPorTurno
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 12
          Font.Style = [fsBold]
          Transparent = True
          DataPipelineName = 'ppDBPipeline_ApontamentoSinteticoPorTurno'
          mmHeight = 5027
          mmLeft = 26723
          mmTop = 529
          mmWidth = 155311
          BandType = 3
          GroupNo = 0
          LayerName = BandLayer8
        end
      end
      object ppGroupFooterBand14: TppGroupFooterBand
        Background.Brush.Style = bsClear
        HideWhenOneDetail = False
        mmBottomOffset = 0
        mmHeight = 6615
        mmPrintPosition = 0
        object ppShape1: TppShape
          UserName = 'Shape1'
          Shape = stRoundRect
          mmHeight = 6085
          mmLeft = 183886
          mmTop = 265
          mmWidth = 98161
          BandType = 5
          GroupNo = 0
          LayerName = BandLayer8
        end
        object ppLabel1: TppLabel
          UserName = 'Label1'
          HyperlinkEnabled = False
          AutoSize = False
          Caption = 'Total do Dia'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          Transparent = True
          mmHeight = 3440
          mmLeft = 185738
          mmTop = 1588
          mmWidth = 21960
          BandType = 5
          GroupNo = 0
          LayerName = BandLayer8
        end
        object ppDBCalc1: TppDBCalc
          UserName = 'DBCalc1'
          HyperlinkEnabled = False
          DataField = 'QuantidadePerda'
          DataPipeline = ppDBPipeline_ApontamentoSinteticoPorTurno
          DisplayFormat = '#,0;-#,0'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = []
          ResetGroup = ppGroup1
          TextAlignment = taRightJustified
          Transparent = True
          DataPipelineName = 'ppDBPipeline_ApontamentoSinteticoPorTurno'
          mmHeight = 3440
          mmLeft = 249238
          mmTop = 1588
          mmWidth = 15081
          BandType = 5
          GroupNo = 0
          LayerName = BandLayer8
        end
        object ppDBCalc2: TppDBCalc
          UserName = 'DBCalc2'
          HyperlinkEnabled = False
          DataField = 'PesoPerda'
          DataPipeline = ppDBPipeline_ApontamentoSinteticoPorTurno
          DisplayFormat = '#,0.00;-#,0.00'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = []
          ResetGroup = ppGroup1
          TextAlignment = taRightJustified
          Transparent = True
          DataPipelineName = 'ppDBPipeline_ApontamentoSinteticoPorTurno'
          mmHeight = 3440
          mmLeft = 265642
          mmTop = 1588
          mmWidth = 15081
          BandType = 5
          GroupNo = 0
          LayerName = BandLayer8
        end
        object ppDBCalc3: TppDBCalc
          UserName = 'DBCalc3'
          HyperlinkEnabled = False
          DataField = 'Peso'
          DataPipeline = ppDBPipeline_ApontamentoSinteticoPorTurno
          DisplayFormat = '#,0.00;-#,0.00'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = []
          ResetGroup = ppGroup1
          TextAlignment = taRightJustified
          Transparent = True
          DataPipelineName = 'ppDBPipeline_ApontamentoSinteticoPorTurno'
          mmHeight = 3440
          mmLeft = 230982
          mmTop = 1588
          mmWidth = 16669
          BandType = 5
          GroupNo = 0
          LayerName = BandLayer8
        end
        object ppDBCalc4: TppDBCalc
          UserName = 'DBCalc4'
          HyperlinkEnabled = False
          DataField = 'Quantidade'
          DataPipeline = ppDBPipeline_ApontamentoSinteticoPorTurno
          DisplayFormat = '#,0;-#,0'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = []
          ResetGroup = ppGroup1
          TextAlignment = taRightJustified
          Transparent = True
          DataPipelineName = 'ppDBPipeline_ApontamentoSinteticoPorTurno'
          mmHeight = 3440
          mmLeft = 211138
          mmTop = 1588
          mmWidth = 17463
          BandType = 5
          GroupNo = 0
          LayerName = BandLayer8
        end
      end
    end
    object ppGroup2: TppGroup
      BreakName = 'TurnoDescricao'
      DataPipeline = ppDBPipeline_ApontamentoSinteticoPorTurno
      GroupFileSettings.NewFile = False
      GroupFileSettings.EmailFile = False
      PreventOrphans = True
      OutlineSettings.CreateNode = True
      StartOnOddPage = False
      UserName = 'Group2'
      mmNewColumnThreshold = 0
      mmNewPageThreshold = 0
      DataPipelineName = 'ppDBPipeline_ApontamentoSinteticoPorTurno'
      NewFile = False
      object ppGroupHeaderBand15: TppGroupHeaderBand
        Background.Brush.Style = bsClear
        mmBottomOffset = 0
        mmHeight = 12700
        mmPrintPosition = 0
        object ppShape6: TppShape
          UserName = 'Shape36'
          Shape = stRoundRect
          mmHeight = 6085
          mmLeft = 1585
          mmTop = 6615
          mmWidth = 280723
          BandType = 3
          GroupNo = 1
          LayerName = BandLayer8
        end
        object ppDBText54: TppDBText
          UserName = 'DBText54'
          HyperlinkEnabled = False
          DataField = 'TurnoDescricao'
          DataPipeline = ppDBPipeline_ApontamentoSinteticoPorTurno
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 12
          Font.Style = [fsBold]
          Transparent = True
          DataPipelineName = 'ppDBPipeline_ApontamentoSinteticoPorTurno'
          mmHeight = 5027
          mmLeft = 1588
          mmTop = 529
          mmWidth = 113506
          BandType = 3
          GroupNo = 1
          LayerName = BandLayer8
        end
        object ppLabel48: TppLabel
          UserName = 'Label5'
          HyperlinkEnabled = False
          AutoSize = False
          Caption = 'Produto'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          Transparent = True
          mmHeight = 4233
          mmLeft = 90750
          mmTop = 7408
          mmWidth = 30163
          BandType = 3
          GroupNo = 1
          LayerName = BandLayer8
        end
        object ppLabel47: TppLabel
          UserName = 'Label21'
          HyperlinkEnabled = False
          AutoSize = False
          Caption = 'Grupo'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          Transparent = True
          mmHeight = 4233
          mmLeft = 53708
          mmTop = 7408
          mmWidth = 16933
          BandType = 3
          GroupNo = 1
          LayerName = BandLayer8
        end
        object ppLabel46: TppLabel
          UserName = 'Label23'
          HyperlinkEnabled = False
          AutoSize = False
          Caption = 'ID'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          TextAlignment = taCentered
          Transparent = True
          mmHeight = 4233
          mmLeft = 72758
          mmTop = 7408
          mmWidth = 14817
          BandType = 3
          GroupNo = 1
          LayerName = BandLayer8
        end
        object ppLabel45: TppLabel
          UserName = 'Label24'
          HyperlinkEnabled = False
          AutoSize = False
          Caption = 'Produzida'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          TextAlignment = taRightJustified
          Transparent = True
          mmHeight = 4233
          mmLeft = 211135
          mmTop = 7408
          mmWidth = 17463
          BandType = 3
          GroupNo = 1
          LayerName = BandLayer8
        end
        object ppLabel44: TppLabel
          UserName = 'Label25'
          HyperlinkEnabled = False
          AutoSize = False
          Caption = 'Peso'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          TextAlignment = taRightJustified
          Transparent = True
          mmHeight = 4233
          mmLeft = 229921
          mmTop = 7408
          mmWidth = 17727
          BandType = 3
          GroupNo = 1
          LayerName = BandLayer8
        end
        object ppLabel43: TppLabel
          UserName = 'Label27'
          HyperlinkEnabled = False
          AutoSize = False
          Caption = 'Perda'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          TextAlignment = taRightJustified
          Transparent = True
          mmHeight = 4233
          mmLeft = 249235
          mmTop = 7408
          mmWidth = 15081
          BandType = 3
          GroupNo = 1
          LayerName = BandLayer8
        end
        object ppLabel42: TppLabel
          UserName = 'Label28'
          HyperlinkEnabled = False
          AutoSize = False
          Caption = 'Peso Pe'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          TextAlignment = taRightJustified
          Transparent = True
          mmHeight = 4233
          mmLeft = 266169
          mmTop = 7408
          mmWidth = 14817
          BandType = 3
          GroupNo = 1
          LayerName = BandLayer8
        end
        object ppLabel54: TppLabel
          UserName = 'Label54'
          HyperlinkEnabled = False
          AutoSize = False
          Caption = 'Recurso'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          Transparent = True
          mmHeight = 4233
          mmLeft = 3173
          mmTop = 7408
          mmWidth = 48154
          BandType = 3
          GroupNo = 1
          LayerName = BandLayer8
        end
      end
      object ppGroupFooterBand15: TppGroupFooterBand
        Background.Brush.Style = bsClear
        HideWhenOneDetail = False
        mmBottomOffset = 0
        mmHeight = 7938
        mmPrintPosition = 0
        object ppShape17: TppShape
          UserName = 'Shape17'
          Shape = stRoundRect
          mmHeight = 6085
          mmLeft = 183885
          mmTop = 265
          mmWidth = 98161
          BandType = 5
          GroupNo = 1
          LayerName = BandLayer8
        end
        object ppLabel57: TppLabel
          UserName = 'Label57'
          HyperlinkEnabled = False
          AutoSize = False
          Caption = 'Total do Turno'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          Transparent = True
          mmHeight = 3440
          mmLeft = 185737
          mmTop = 1588
          mmWidth = 21960
          BandType = 5
          GroupNo = 1
          LayerName = BandLayer8
        end
        object ppDBCalc17: TppDBCalc
          UserName = 'DBCalc17'
          HyperlinkEnabled = False
          DataField = 'QuantidadePerda'
          DataPipeline = ppDBPipeline_ApontamentoSinteticoPorTurno
          DisplayFormat = '#,0;-#,0'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = []
          ResetGroup = ppGroup2
          TextAlignment = taRightJustified
          Transparent = True
          DataPipelineName = 'ppDBPipeline_ApontamentoSinteticoPorTurno'
          mmHeight = 3440
          mmLeft = 249238
          mmTop = 1588
          mmWidth = 15081
          BandType = 5
          GroupNo = 1
          LayerName = BandLayer8
        end
        object ppDBCalc18: TppDBCalc
          UserName = 'DBCalc18'
          HyperlinkEnabled = False
          DataField = 'PesoPerda'
          DataPipeline = ppDBPipeline_ApontamentoSinteticoPorTurno
          DisplayFormat = '#,0.00;-#,0.00'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = []
          ResetGroup = ppGroup2
          TextAlignment = taRightJustified
          Transparent = True
          DataPipelineName = 'ppDBPipeline_ApontamentoSinteticoPorTurno'
          mmHeight = 3440
          mmLeft = 265642
          mmTop = 1588
          mmWidth = 15081
          BandType = 5
          GroupNo = 1
          LayerName = BandLayer8
        end
        object ppDBCalc19: TppDBCalc
          UserName = 'DBCalc19'
          HyperlinkEnabled = False
          DataField = 'Peso'
          DataPipeline = ppDBPipeline_ApontamentoSinteticoPorTurno
          DisplayFormat = '#,0.00;-#,0.00'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = []
          ResetGroup = ppGroup2
          TextAlignment = taRightJustified
          Transparent = True
          DataPipelineName = 'ppDBPipeline_ApontamentoSinteticoPorTurno'
          mmHeight = 3440
          mmLeft = 230982
          mmTop = 1588
          mmWidth = 16669
          BandType = 5
          GroupNo = 1
          LayerName = BandLayer8
        end
        object ppDBCalc20: TppDBCalc
          UserName = 'TXT_Quantidade4'
          HyperlinkEnabled = False
          DataField = 'Quantidade'
          DataPipeline = ppDBPipeline_ApontamentoSinteticoPorTurno
          DisplayFormat = '#,0;-#,0'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = []
          ResetGroup = ppGroup2
          TextAlignment = taRightJustified
          Transparent = True
          DataPipelineName = 'ppDBPipeline_ApontamentoSinteticoPorTurno'
          mmHeight = 3440
          mmLeft = 211138
          mmTop = 1588
          mmWidth = 17463
          BandType = 5
          GroupNo = 1
          LayerName = BandLayer8
        end
      end
    end
    object ppDesignLayers4: TppDesignLayers
      object ppDesignLayer7: TppDesignLayer
        UserName = 'PageLayer4'
        LayerType = ltPage
        Index = 0
      end
      object ppDesignLayer8: TppDesignLayer
        UserName = 'BandLayer8'
        LayerType = ltBanded
        Index = 1
      end
    end
    object ppParameterList3: TppParameterList
    end
  end
end
