...
begin
    ...
    if Relatorio_ApontamentoProducao.CBX_Tipo.Text = 'ANÁLISE DE OEE (OVERALL EQUIPMENT EFFECTIVENESS)' then
    begin
    ...
    if Relatorio_ApontamentoProducao.CBX_Tipo.Text = 'ANÁLISE DE PERDAS, EFICIÊNCIA E PRODUTIVIDADE' then
    begin
          If CDS.RecordCount > 0 Then
          begin
             ppLabel16.Caption :=  ' Data de Emissão Igual ' + ' ' + DateToStr(Relatorio_ApontamentoProducao.DT_DataEmissao1.Date);
             ppLabel17.Caption := 'Data Impressão: ' + DateTimeToStr(Date()) + ' - ' + Sys_FuncaoSistema.PNL_UsuarioAtivo.Caption;
...
begin
 ...
 if CDSTipo.AsString <> 'SEM PRODUÇÃO' then
  begin
      ...
      if CDSSemafaro_Disponibilidade.AsString = 'VERMELHO' then
            SHP_Disponibilidade.Brush.Color := clRed;

      if CDSSemafaro_Disponibilidade.AsString = 'AMARELO' then
            SHP_Disponibilidade.Brush.Color := clYellow;
...
begin
   ...
   if CDSDia_Tipo.AsString <> 'SEM PRODUÇÃO' then
    begin
        ...
        if CDSDia_Semafaro_Disponibilidade.AsString = 'VERMELHO' then
              SHP_Disponibilidade_Dia.Brush.Color := clRed;

        if CDSDia_Semafaro_Disponibilidade.AsString = 'AMARELO' then
              SHP_Disponibilidade_Dia.Brush.Color := clYellow;
