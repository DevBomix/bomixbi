...
begin
  ...
  Application.MessageBox('Não é permitida a edição de Solicitação de Matéria Prima' + #13 + '','Atenção',mb_iconerror + mb_ok);
  Exit;
  inherited;
...
// Validação **********************************************************************************************************
  ...
  begin
       Application.MessageBox('Não é permitido adicionar produto sem antes cadastrar uma solicitação!' + #13 + '','Atenção',mb_iconerror + mb_ok);
       Exit;
  end;
...
begin
  ...
  begin
      ...
      ppLBL_Data.Caption := 'Data Impressão: ' + DateToStr(Date()) + ' - ' + TimeToStr(Time());
...
begin
  ...
  begin
      ...
      if Sys_FuncaoSistema.PNL_Setor.Caption = 'PERSONALIZAÇÃO' then
      begin
      ...
      if Sys_FuncaoSistema.PNL_Setor.Caption = 'INJEÇÃO' then
      begin
      ...
      if (Sys_FuncaoSistema.PNL_Setor.Caption = 'TECNOLOGIA DA INFORMAÇÃO') then
      begin
          CBX_Origem.Items.Add('PE');
          CBX_Origem.Items.Add('PR');
          CBX_Origem.Text := 'PR';
