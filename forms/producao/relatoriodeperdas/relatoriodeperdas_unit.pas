...
begin
  ...
  try
      ...
      Relatorio_PosicaoEstoque.CBX_TipoRelatorio.Text := 'RELATÓRIO DE SALDO DE ESTOQUE DAS PERDAS';
...

{
     ...
     If PadraoBuscadorFiltro.RetornarWhereFiltro(CBX_Operador,PNL_Sinal,TXT_Valor1,TXT_Valor2,'Apont.H6_OP','Caracter') <> '' then
      ...
      begin
         ...
         LBL_Periodo.Caption :=  ' Data de Emissão Entre ' + ' ' + DateToStr(DT_DataEmissao1.Date) + ' a ' + DateToStr(DT_DataEmissao2.Date);
         ppLabel36.Caption := 'Data Impressão: ' + DateTimeToStr(Date()) + ' - ' + Sys_FuncaoSistema.PNL_UsuarioAtivo.Caption;
...
begin
      ...
      begin
         ...
         ppLabel21.Caption :=  ' Data de Emissão Entre ' + ' ' + DateToStr(DT_DataEmissao1.Date) + ' a ' + DateToStr(DT_DataEmissao2.Date);
         ppLabel28.Caption := 'Data Impressão: ' + DateTimeToStr(Date()) + ' - ' + Sys_FuncaoSistema.PNL_UsuarioAtivo.Caption;
...
begin
      ...
      begin
           ...
           ppLabel37.Caption :=  ' Data de Emissão Entre ' + ' ' + DateToStr(DT_DataEmissao1.Date) + ' a ' + DateToStr(DT_DataEmissao2.Date);
           ppLabel39.Caption := 'Data Impressão: ' + DateTimeToStr(Date()) + ' - ' + Sys_FuncaoSistema.PNL_UsuarioAtivo.Caption;
...
begin
    ...
    if CBX_Tipo.Text = 'RELATÓRIO DE PERDAS - ANALÍTICO' then
        RelatorioPerdasAnalitico;

    if CBX_Tipo.Text = 'RELATÓRIO DE ENTREGA AO ARMAZÉM' then
        RelatorioEntregaArmazem;

    if CBX_Tipo.Text = 'RELATÓRIO DE PERDAS SINTÉTICO - POR RECURSO' then
        RelatorioPerdasSinteticoPorRecurso;

    if CBX_Tipo.Text = 'RELATÓRIO DE PERDAS SINTÉTICO - POR MOTIVO' then
