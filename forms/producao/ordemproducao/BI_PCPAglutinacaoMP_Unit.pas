...
begin
  ...
  begin
      ppLabel20.Caption :=  'Data Impressão: ' + DateTimeToStr(Date()) + '  ' + Copy(DateTimeToStr(Time()),11,6)  + '         Usuário: ' + Sys_FuncaoSistema.PNL_UsuarioAtivo.Caption;
      ppImage1.Picture := Sys_FuncaoSistema.Img_LogoBomix.Picture;
      ppReport1.Print;
  end
  else
  begin
      ppLBL_Status.Caption :=  'Data Impressão: ' + DateTimeToStr(Date()) + '  ' + Copy(DateTimeToStr(Time()),11,6)  + '         Usuário: ' + Sys_FuncaoSistema.PNL_UsuarioAtivo.Caption;
...
begin
  inherited;
    ...
    begin
        Application.MessageBox('Defina o saldo','Atenção',mb_iconwarning + mb_ok);
