object OrdemProducao_ImprimirOP_TinturaBomix: TOrdemProducao_ImprimirOP_TinturaBomix
  Left = 0
  Top = 0
  Caption = 'OrdemProducao_ImprimirOP_TinturaBomix'
  ClientHeight = 459
  ClientWidth = 838
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  PixelsPerInch = 96
  TextHeight = 13
  object ppDBPipeline2: TppDBPipeline
    DataSource = DS_ImpressaoTinturariaOP
    CloseDataSource = True
    UserName = 'DBPipeline2'
    Left = 257
    Top = 11
  end
  object CDS_ImpressaoTinturariaOP: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DSP_ImpressaoTinturariaOP'
    Left = 304
    Top = 74
    object CDS_ImpressaoTinturariaOPOP: TStringField
      FieldName = 'OP'
      ReadOnly = True
      Size = 8
    end
    object CDS_ImpressaoTinturariaOPRecno: TIntegerField
      FieldName = 'Recno'
    end
    object CDS_ImpressaoTinturariaOPEmpresa: TStringField
      FieldName = 'Empresa'
      Size = 15
    end
    object CDS_ImpressaoTinturariaOPSetor: TStringField
      FieldName = 'Setor'
      Size = 50
    end
    object CDS_ImpressaoTinturariaOPOrdemProducao: TStringField
      FieldName = 'OrdemProducao'
      Size = 11
    end
    object CDS_ImpressaoTinturariaOPSaldoTinta: TFloatField
      FieldName = 'SaldoTinta'
    end
    object CDS_ImpressaoTinturariaOPCliente: TStringField
      FieldName = 'Cliente'
      Size = 100
    end
    object CDS_ImpressaoTinturariaOPProduto_ID: TStringField
      FieldName = 'Produto_ID'
      Size = 30
    end
    object CDS_ImpressaoTinturariaOPProduto: TStringField
      FieldName = 'Produto'
      Size = 150
    end
    object CDS_ImpressaoTinturariaOPArte_ID: TStringField
      FieldName = 'Arte_ID'
      Size = 30
    end
    object CDS_ImpressaoTinturariaOPArte: TStringField
      FieldName = 'Arte'
      Size = 100
    end
    object CDS_ImpressaoTinturariaOPQuantidade: TFloatField
      FieldName = 'Quantidade'
    end
    object CDS_ImpressaoTinturariaOPRecnoTinta: TIntegerField
      FieldName = 'RecnoTinta'
    end
    object CDS_ImpressaoTinturariaOPOPTinta: TStringField
      FieldName = 'OPTinta'
      Size = 11
    end
    object CDS_ImpressaoTinturariaOPTinta_ID: TStringField
      FieldName = 'Tinta_ID'
      Size = 30
    end
    object CDS_ImpressaoTinturariaOPTinta: TStringField
      FieldName = 'Tinta'
      Size = 150
    end
    object CDS_ImpressaoTinturariaOPQtdTinta: TFloatField
      FieldName = 'QtdTinta'
    end
    object CDS_ImpressaoTinturariaOPD4Recno: TIntegerField
      FieldName = 'D4Recno'
    end
    object CDS_ImpressaoTinturariaOPInsumo_ID: TStringField
      FieldName = 'Insumo_ID'
      ReadOnly = True
      Size = 30
    end
    object CDS_ImpressaoTinturariaOPInsumo: TStringField
      FieldName = 'Insumo'
      ReadOnly = True
      Size = 150
    end
    object CDS_ImpressaoTinturariaOPQtdEmpenhada: TFloatField
      FieldName = 'QtdEmpenhada'
    end
    object CDS_ImpressaoTinturariaOPUnidade: TStringField
      FieldName = 'Unidade'
      Size = 6
    end
    object CDS_ImpressaoTinturariaOPArmazem: TStringField
      FieldName = 'Armazem'
      Size = 2
    end
    object CDS_ImpressaoTinturariaOPPerc: TStringField
      FieldName = 'Perc'
      ReadOnly = True
      Size = 6
    end
    object CDS_ImpressaoTinturariaOPEstoque: TFloatField
      FieldName = 'Estoque'
      ReadOnly = True
    end
  end
  object Query_ImpressaoTinturariaOP: TADOQuery
    Connection = Sys_ModuloConexaoDados.ADOConnection1
    CursorType = ctStatic
    DataSource = DS_ImpressaoTinturariaOP
    Parameters = <>
    SQL.Strings = (
      'Select '
      
        #9'OP, Recno, Empresa, Setor, OrdemProducao, SaldoTinta, Cliente, ' +
        'Produto_ID, Produto, Arte_ID, Arte, Quantidade, RecnoTinta, OPTi' +
        'nta, Tinta_ID, Tinta, QtdTinta,'
      
        #9'D4.R_E_C_N_O_ as D4Recno, Rtrim(D4_COD) as Insumo_ID, Rtrim(B1_' +
        'DESC) as Insumo, D4_QUANT as QtdEmpenhada, B1_UM as Unidade, D4_' +
        'LOCAL as Armazem,'
      
        #9'Cast(Round((D4_QUANT * 100)/QtdTinta,2) as Varchar(5)) + '#39'%'#39' as' +
        ' Perc,'
      #9'Isnull(('
      #9#9#9
      #9#9#9'Select '
      #9#9#9#9'B2_QATU as Saldo '
      #9#9#9'from P12OFICIAL.dbo.SB2010 (nolock)'
      #9#9#9'Where B2_FILIAL = '#39'010101'#39' '
      #9#9#9'AND D_E_L_E_T_ <> '#39'*'#39' '
      #9#9#9'AND B2_LOCAL = ArmazemTinta'
      #9#9#9'AND B2_COD = Tinta_ID'
      ''
      #9'),0) as Estoque'
      ''
      'from ('
      ''
      #9#9'Select '
      
        #9#9#9'Substring(OrdemProducao,1,8) as OP, Recno, Empresa, Setor, Cl' +
        'iente, SaldoTinta,'
      
        #9#9#9'OrdemProducao, Produto_ID, Produto, Arte_ID, Arte, Quantidade' +
        ', '
      #9#9#9'RecnoTinta, OPTinta, Tinta_ID, Tinta, QtdTinta, ArmazemTinta'
      #9#9'from BOMIXBI.dbo.Pcp_TB_OrdemProducao OP (nolock)'
      #9#9'Inner Join ('
      ''
      #9#9#9#9#9'Select '
      
        #9#9#9#9#9#9'Substring(OrdemProducao,1,8) as OP, Recno as RecnoTinta, O' +
        'rdemProducao as OPTinta, Saldo as SaldoTinta, Produto_ID as Tint' +
        'a_ID, Produto as Tinta, Quantidade as QtdTinta, Armazem as Armaz' +
        'emTinta'
      #9#9#9#9#9'from BOMIXBI.dbo.Pcp_TB_OrdemProducao'
      #9#9#9#9#9'Where Substring(OrdemProducao,1,8) = '#39'P4782001'#39
      #9#9#9#9#9'AND Setor = '#39'TINTURARIA'#39
      ''
      #9#9') Tinta ON  Tinta.OP = Substring(OrdemProducao,1,8) '
      #9#9'Where OrdemProducao like '#39'P4782001%'#39
      #9#9'AND Setor = '#39'PERSONALIZACAO'#39
      '        AND OP.Status <> '#39'FINALIZADA'#39
      ''
      ') TB'
      
        'Inner Join P12OFICIAL.dbo.SD4010 D4 (nolock) ON D4_FILIAL = '#39'010' +
        '101'#39#9#9#9#9#9#9#9#9#9#9#9
      #9#9#9#9#9#9#9#9#9#9#9'AND D4.D_E_L_E_T_ <> '#39'*'#39
      #9#9#9#9#9#9#9#9#9#9#9'AND D4_OP = OPTinta'#9#9#9
      
        'Inner Join P12OFICIAL.dbo.SB1010 B1 (nolock) ON B1_FILIAL = '#39'010' +
        '1'#39#9#9#9#9#9#9#9#9#9#9#9
      #9#9#9#9#9#9#9#9#9#9#9'AND B1.D_E_L_E_T_ <> '#39'*'#39
      #9#9#9#9#9#9#9#9#9#9#9'AND B1_COD = D4_COD'
      ''
      'Order by Recno desc')
    Left = 335
    Top = 73
    object Query_ImpressaoTinturariaOPOP: TStringField
      FieldName = 'OP'
      ReadOnly = True
      Size = 8
    end
    object Query_ImpressaoTinturariaOPRecno: TIntegerField
      FieldName = 'Recno'
    end
    object Query_ImpressaoTinturariaOPEmpresa: TStringField
      FieldName = 'Empresa'
      Size = 15
    end
    object Query_ImpressaoTinturariaOPSetor: TStringField
      FieldName = 'Setor'
      Size = 50
    end
    object Query_ImpressaoTinturariaOPOrdemProducao: TStringField
      FieldName = 'OrdemProducao'
      Size = 11
    end
    object Query_ImpressaoTinturariaOPSaldoTinta: TFloatField
      FieldName = 'SaldoTinta'
    end
    object Query_ImpressaoTinturariaOPCliente: TStringField
      FieldName = 'Cliente'
      Size = 100
    end
    object Query_ImpressaoTinturariaOPProduto_ID: TStringField
      FieldName = 'Produto_ID'
      Size = 30
    end
    object Query_ImpressaoTinturariaOPProduto: TStringField
      FieldName = 'Produto'
      Size = 150
    end
    object Query_ImpressaoTinturariaOPArte_ID: TStringField
      FieldName = 'Arte_ID'
      Size = 30
    end
    object Query_ImpressaoTinturariaOPArte: TStringField
      FieldName = 'Arte'
      Size = 100
    end
    object Query_ImpressaoTinturariaOPQuantidade: TFloatField
      FieldName = 'Quantidade'
    end
    object Query_ImpressaoTinturariaOPRecnoTinta: TIntegerField
      FieldName = 'RecnoTinta'
    end
    object Query_ImpressaoTinturariaOPOPTinta: TStringField
      FieldName = 'OPTinta'
      Size = 11
    end
    object Query_ImpressaoTinturariaOPTinta_ID: TStringField
      FieldName = 'Tinta_ID'
      Size = 30
    end
    object Query_ImpressaoTinturariaOPTinta: TStringField
      FieldName = 'Tinta'
      Size = 150
    end
    object Query_ImpressaoTinturariaOPQtdTinta: TFloatField
      FieldName = 'QtdTinta'
    end
    object Query_ImpressaoTinturariaOPD4Recno: TIntegerField
      FieldName = 'D4Recno'
    end
    object Query_ImpressaoTinturariaOPInsumo_ID: TStringField
      FieldName = 'Insumo_ID'
      ReadOnly = True
      Size = 30
    end
    object Query_ImpressaoTinturariaOPInsumo: TStringField
      FieldName = 'Insumo'
      ReadOnly = True
      Size = 150
    end
    object Query_ImpressaoTinturariaOPQtdEmpenhada: TFloatField
      FieldName = 'QtdEmpenhada'
    end
    object Query_ImpressaoTinturariaOPUnidade: TStringField
      FieldName = 'Unidade'
      Size = 6
    end
    object Query_ImpressaoTinturariaOPArmazem: TStringField
      FieldName = 'Armazem'
      Size = 2
    end
    object Query_ImpressaoTinturariaOPPerc: TStringField
      FieldName = 'Perc'
      ReadOnly = True
      Size = 6
    end
    object Query_ImpressaoTinturariaOPEstoque: TFloatField
      FieldName = 'Estoque'
      ReadOnly = True
    end
  end
  object DS_ImpressaoTinturariaOP: TDataSource
    DataSet = CDS_ImpressaoTinturariaOP
    Left = 365
    Top = 73
  end
  object DSP_ImpressaoTinturariaOP: TDataSetProvider
    DataSet = Query_ImpressaoTinturariaOP
    Left = 395
    Top = 73
  end
  object ppImprimirTintura: TppReport
    AutoStop = False
    DataPipeline = ppDBPipeline2
    PassSetting = psTwoPass
    PrinterSetup.BinName = 'Default'
    PrinterSetup.DocumentName = 'Report'
    PrinterSetup.Duplex = dpNone
    PrinterSetup.PaperName = 'A4'
    PrinterSetup.PrinterName = 'pdfFactory Pro'
    PrinterSetup.SaveDeviceSettings = False
    PrinterSetup.mmMarginBottom = 5080
    PrinterSetup.mmMarginLeft = 5080
    PrinterSetup.mmMarginRight = 5080
    PrinterSetup.mmMarginTop = 5080
    PrinterSetup.mmPaperHeight = 297000
    PrinterSetup.mmPaperWidth = 210000
    PrinterSetup.PaperSize = 9
    AllowPrintToArchive = True
    AllowPrintToFile = True
    ArchiveFileName = '($MyDocuments)\ReportArchive.raf'
    DeviceType = 'PDF'
    DefaultFileDeviceType = 'PDF'
    EmailSettings.ReportFormat = 'PDF'
    LanguageID = 'Portuguese (Brazil)'
    OpenFile = True
    OutlineSettings.CreateNode = True
    OutlineSettings.CreatePageNodes = True
    OutlineSettings.Enabled = True
    OutlineSettings.Visible = True
    ThumbnailSettings.Enabled = True
    ThumbnailSettings.Visible = True
    ThumbnailSettings.DeadSpace = 30
    ThumbnailSettings.PageHighlight.Width = 3
    PDFSettings.EmbedFontOptions = [efUseSubset]
    PDFSettings.EncryptSettings.AllowCopy = True
    PDFSettings.EncryptSettings.AllowInteract = True
    PDFSettings.EncryptSettings.AllowModify = True
    PDFSettings.EncryptSettings.AllowPrint = True
    PDFSettings.EncryptSettings.AllowExtract = True
    PDFSettings.EncryptSettings.AllowAssemble = True
    PDFSettings.EncryptSettings.AllowQualityPrint = True
    PDFSettings.EncryptSettings.Enabled = False
    PDFSettings.EncryptSettings.KeyLength = kl40Bit
    PDFSettings.EncryptSettings.EncryptionType = etRC4
    PDFSettings.FontEncoding = feAnsi
    PDFSettings.ImageCompressionLevel = 25
    PDFSettings.PDFAFormat = pafNone
    PreviewFormSettings.PageBorder.mmPadding = 0
    PreviewFormSettings.WindowState = wsMaximized
    PreviewFormSettings.ZoomSetting = zs100Percent
    RTFSettings.DefaultFont.Charset = DEFAULT_CHARSET
    RTFSettings.DefaultFont.Color = clWindowText
    RTFSettings.DefaultFont.Height = -13
    RTFSettings.DefaultFont.Name = 'Arial'
    RTFSettings.DefaultFont.Style = []
    ShowPrintDialog = False
    TextFileName = '($MyDocuments)\Report.pdf'
    TextSearchSettings.DefaultString = '<Texto a localizar>'
    TextSearchSettings.Enabled = True
    XLSSettings.AppName = 'ReportBuilder'
    XLSSettings.Author = 'ReportBuilder'
    XLSSettings.Subject = 'Report'
    XLSSettings.Title = 'Report'
    XLSSettings.WorksheetName = 'Report'
    Left = 365
    Top = 11
    Version = '19.03'
    mmColumnWidth = 197300
    DataPipelineName = 'ppDBPipeline2'
    object ppHeaderBand1: TppHeaderBand
      Background.Brush.Style = bsClear
      Border.mmPadding = 0
      PrintHeight = phDynamic
      mmBottomOffset = 0
      mmHeight = 54769
      mmPrintPosition = 0
      object ppImage1: TppImage
        DesignLayer = ppDesignLayer4
        UserName = 'ppIMG_Logo'
        AlignHorizontal = ahCenter
        AlignVertical = avCenter
        MaintainAspectRatio = False
        Stretch = True
        Border.mmPadding = 0
        Picture.Data = {
          
          FFFF}
        mmHeight = 11906
        mmLeft = 3178
        mmTop = 3974
        mmWidth = 33338
        BandType = 0
        LayerName = BandLayer4
      end
      object ppLabel90: TppLabel
        DesignLayer = ppDesignLayer4
        UserName = 'ppLBL_TituloRelatorio'
        HyperlinkEnabled = False
        Border.mmPadding = 0
        Caption = 'Ordem de Produ'#231#227'o'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 18
        Font.Style = [fsBold]
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        Transparent = True
        mmHeight = 7408
        mmLeft = 3175
        mmTop = 16933
        mmWidth = 60590
        BandType = 0
        LayerName = BandLayer4
      end
      object ppLine9: TppLine
        DesignLayer = ppDesignLayer4
        UserName = 'Line1'
        Border.mmPadding = 0
        Pen.Width = 3
        Weight = 2.250000000000000000
        mmHeight = 1588
        mmLeft = 2381
        mmTop = 24884
        mmWidth = 195531
        BandType = 0
        LayerName = BandLayer4
      end
      object ppShape399: TppShape
        DesignLayer = ppDesignLayer4
        UserName = 'Shape1'
        mmHeight = 20636
        mmLeft = 2117
        mmTop = 27253
        mmWidth = 195795
        BandType = 0
        LayerName = BandLayer4
      end
      object ppLabel155: TppLabel
        DesignLayer = ppDesignLayer4
        UserName = 'Label2'
        HyperlinkEnabled = False
        AutoSize = False
        Border.mmPadding = 0
        Caption = 'Recurso:'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = [fsBold]
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        Transparent = True
        mmHeight = 4233
        mmLeft = 4501
        mmTop = 29383
        mmWidth = 20643
        BandType = 0
        LayerName = BandLayer4
      end
      object ppDBText25: TppDBText
        DesignLayer = ppDesignLayer4
        UserName = 'DBText4'
        Border.mmPadding = 0
        DataField = 'cliente'
        DataPipeline = ppDBPipeline2
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = []
        Transparent = True
        DataPipelineName = 'ppDBPipeline2'
        mmHeight = 4233
        mmLeft = 26731
        mmTop = 29383
        mmWidth = 119061
        BandType = 0
        LayerName = BandLayer4
      end
      object ppLabel1: TppLabel
        DesignLayer = ppDesignLayer4
        UserName = 'Label1'
        HyperlinkEnabled = False
        AutoSize = False
        Border.mmPadding = 0
        Caption = 'Arte'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = [fsBold]
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        Transparent = True
        mmHeight = 4233
        mmLeft = 4500
        mmTop = 35468
        mmWidth = 20643
        BandType = 0
        LayerName = BandLayer4
      end
      object ppDBText1: TppDBText
        DesignLayer = ppDesignLayer4
        UserName = 'DBText1'
        Border.mmPadding = 0
        DataField = 'Arte_ID'
        DataPipeline = ppDBPipeline2
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = []
        Transparent = True
        DataPipelineName = 'ppDBPipeline2'
        mmHeight = 4233
        mmLeft = 26731
        mmTop = 35468
        mmWidth = 12171
        BandType = 0
        LayerName = BandLayer4
      end
      object ppDBText2: TppDBText
        DesignLayer = ppDesignLayer4
        UserName = 'DBText2'
        Border.mmPadding = 0
        DataField = 'Arte'
        DataPipeline = ppDBPipeline2
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = []
        Transparent = True
        DataPipelineName = 'ppDBPipeline2'
        mmHeight = 4233
        mmLeft = 26723
        mmTop = 41010
        mmWidth = 104511
        BandType = 0
        LayerName = BandLayer4
      end
      object ppLabel2: TppLabel
        DesignLayer = ppDesignLayer4
        UserName = 'Label6'
        HyperlinkEnabled = False
        AutoSize = False
        Border.mmPadding = 0
        Caption = 'O.P. Balde:'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = [fsBold]
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        Transparent = True
        mmHeight = 4233
        mmLeft = 147372
        mmTop = 29373
        mmWidth = 21960
        BandType = 0
        LayerName = BandLayer4
      end
      object ppDBText3: TppDBText
        DesignLayer = ppDesignLayer4
        UserName = 'DBText7'
        Border.mmPadding = 0
        DataField = 'OrdemProducao'
        DataPipeline = ppDBPipeline2
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = []
        Transparent = True
        DataPipelineName = 'ppDBPipeline2'
        mmHeight = 4233
        mmLeft = 170391
        mmTop = 29373
        mmWidth = 24606
        BandType = 0
        LayerName = BandLayer4
      end
      object ppLabel3: TppLabel
        DesignLayer = ppDesignLayer4
        UserName = 'Label3'
        HyperlinkEnabled = False
        AutoSize = False
        Border.mmPadding = 0
        Caption = 'Produto'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = [fsBold]
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        Transparent = True
        mmHeight = 4233
        mmLeft = 147372
        mmTop = 35468
        mmWidth = 21960
        BandType = 0
        LayerName = BandLayer4
      end
      object ppDBText4: TppDBText
        DesignLayer = ppDesignLayer4
        UserName = 'DBText3'
        Border.mmPadding = 0
        DataField = 'Produto_ID'
        DataPipeline = ppDBPipeline2
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDBPipeline2'
        mmHeight = 4233
        mmLeft = 170391
        mmTop = 35468
        mmWidth = 24606
        BandType = 0
        LayerName = BandLayer4
      end
      object ppLabel4: TppLabel
        DesignLayer = ppDesignLayer4
        UserName = 'Label4'
        HyperlinkEnabled = False
        AutoSize = False
        Border.mmPadding = 0
        Caption = 'TINTAS PARA FABRICA'#199#195'O:'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = [fsBold]
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        Transparent = True
        mmHeight = 4763
        mmLeft = 2380
        mmTop = 48945
        mmWidth = 51594
        BandType = 0
        LayerName = BandLayer4
      end
      object ppLabel8: TppLabel
        DesignLayer = ppDesignLayer4
        UserName = 'Label8'
        HyperlinkEnabled = False
        AutoSize = False
        Border.mmPadding = 0
        Caption = 'Qtd. O.P'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = [fsBold]
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        Transparent = True
        mmHeight = 4233
        mmLeft = 147373
        mmTop = 41008
        mmWidth = 21960
        BandType = 0
        LayerName = BandLayer4
      end
      object ppDBText9: TppDBText
        DesignLayer = ppDesignLayer4
        UserName = 'DBText10'
        Border.mmPadding = 0
        DataField = 'Quantidade'
        DataPipeline = ppDBPipeline2
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDBPipeline2'
        mmHeight = 4233
        mmLeft = 170392
        mmTop = 41008
        mmWidth = 24606
        BandType = 0
        LayerName = BandLayer4
      end
    end
    object ppDetailBand2: TppDetailBand
      Background1.Brush.Style = bsClear
      Background2.Brush.Style = bsClear
      Border.mmPadding = 0
      PrintHeight = phDynamic
      mmBottomOffset = 0
      mmHeight = 5292
      mmPrintPosition = 0
      object ppLine10: TppLine
        DesignLayer = ppDesignLayer4
        UserName = 'Line10'
        Border.mmPadding = 0
        Pen.Style = psDot
        Weight = 0.750000000000000000
        mmHeight = 793
        mmLeft = 2383
        mmTop = 4499
        mmWidth = 194206
        BandType = 4
        LayerName = BandLayer4
      end
      object ppDBText42: TppDBText
        DesignLayer = ppDesignLayer4
        UserName = 'DBText42'
        Border.mmPadding = 0
        DataField = 'Insumo_ID'
        DataPipeline = ppDBPipeline2
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        Transparent = True
        DataPipelineName = 'ppDBPipeline2'
        mmHeight = 3333
        mmLeft = 3176
        mmTop = 795
        mmWidth = 15338
        BandType = 4
        LayerName = BandLayer4
      end
      object ppDBText43: TppDBText
        DesignLayer = ppDesignLayer4
        UserName = 'DBText43'
        Border.mmPadding = 0
        DataField = 'Insumo'
        DataPipeline = ppDBPipeline2
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        Transparent = True
        DataPipelineName = 'ppDBPipeline2'
        mmHeight = 3439
        mmLeft = 23813
        mmTop = 795
        mmWidth = 116417
        BandType = 4
        LayerName = BandLayer4
      end
      object ppDBText44: TppDBText
        DesignLayer = ppDesignLayer4
        UserName = 'DBText44'
        Border.mmPadding = 0
        DataField = 'Armazem'
        DataPipeline = ppDBPipeline2
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        TextAlignment = taCentered
        Transparent = True
        DataPipelineName = 'ppDBPipeline2'
        mmHeight = 3439
        mmLeft = 145248
        mmTop = 795
        mmWidth = 8724
        BandType = 4
        LayerName = BandLayer4
      end
      object ppDBText45: TppDBText
        DesignLayer = ppDesignLayer4
        UserName = 'DBText45'
        Border.mmPadding = 0
        DataField = 'QtdEmpenhada'
        DataPipeline = ppDBPipeline2
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDBPipeline2'
        mmHeight = 3439
        mmLeft = 155831
        mmTop = 795
        mmWidth = 15333
        BandType = 4
        LayerName = BandLayer4
      end
      object ppDBText40: TppDBText
        DesignLayer = ppDesignLayer4
        UserName = 'DBText40'
        Border.mmPadding = 0
        DataField = 'Perc'
        DataPipeline = ppDBPipeline2
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        TextAlignment = taCentered
        Transparent = True
        DataPipelineName = 'ppDBPipeline2'
        mmHeight = 3440
        mmLeft = 174886
        mmTop = 794
        mmWidth = 12178
        BandType = 4
        LayerName = BandLayer4
      end
      object ppDBText14: TppDBText
        DesignLayer = ppDesignLayer4
        UserName = 'DBText14'
        Border.mmPadding = 0
        DataField = 'Unidade'
        DataPipeline = ppDBPipeline2
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        TextAlignment = taCentered
        Transparent = True
        DataPipelineName = 'ppDBPipeline2'
        mmHeight = 3439
        mmLeft = 188649
        mmTop = 794
        mmWidth = 6876
        BandType = 4
        LayerName = BandLayer4
      end
    end
    object ppFooterBand2: TppFooterBand
      Background.Brush.Style = bsClear
      Border.mmPadding = 0
      mmBottomOffset = 0
      mmHeight = 18256
      mmPrintPosition = 0
      object ppLine16: TppLine
        DesignLayer = ppDesignLayer4
        UserName = 'Line12'
        Border.mmPadding = 0
        Pen.Width = 3
        Weight = 2.250000000000000000
        mmHeight = 2646
        mmLeft = 2381
        mmTop = 12973
        mmWidth = 195532
        BandType = 8
        LayerName = BandLayer4
      end
      object ppLine17: TppLine
        DesignLayer = ppDesignLayer4
        UserName = 'Line9'
        Border.mmPadding = 0
        Weight = 0.750000000000000000
        mmHeight = 2117
        mmLeft = 112713
        mmTop = 8211
        mmWidth = 82286
        BandType = 8
        LayerName = BandLayer4
      end
      object ppLBL_Data: TppLabel
        DesignLayer = ppDesignLayer4
        UserName = 'ppLBL_Data'
        HyperlinkEnabled = False
        Border.mmPadding = 0
        Caption = 'ppLBL_Data'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = [fsBold]
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        TextAlignment = taRightJustified
        Transparent = True
        mmHeight = 4233
        mmLeft = 75406
        mmTop = 13503
        mmWidth = 119592
        BandType = 8
        LayerName = BandLayer4
      end
      object ppLabel164: TppLabel
        DesignLayer = ppDesignLayer4
        UserName = 'Label164'
        HyperlinkEnabled = False
        AutoSize = False
        Border.mmPadding = 0
        Caption = 'RESPONS'#193'VEL DA CONFER'#202'NCIA  DA COR'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 9
        Font.Style = [fsBold]
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        Transparent = True
        mmHeight = 3704
        mmLeft = 112713
        mmTop = 9005
        mmWidth = 81756
        BandType = 8
        LayerName = BandLayer4
      end
      object ppLine1: TppLine
        DesignLayer = ppDesignLayer4
        UserName = 'Line2'
        Border.mmPadding = 0
        Weight = 0.750000000000000000
        mmHeight = 2117
        mmLeft = 2910
        mmTop = 8211
        mmWidth = 82286
        BandType = 8
        LayerName = BandLayer4
      end
      object ppLabel6: TppLabel
        DesignLayer = ppDesignLayer4
        UserName = 'Label5'
        HyperlinkEnabled = False
        AutoSize = False
        Border.mmPadding = 0
        Caption = 'RESPONS'#193'VEL PELA FABRICA'#199#195'O'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 9
        Font.Style = [fsBold]
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        Transparent = True
        mmHeight = 3704
        mmLeft = 2910
        mmTop = 9005
        mmWidth = 81756
        BandType = 8
        LayerName = BandLayer4
      end
    end
    object ppSummaryBand3: TppSummaryBand
      Background.Brush.Style = bsClear
      Border.mmPadding = 0
      PrintHeight = phDynamic
      mmBottomOffset = 0
      mmHeight = 0
      mmPrintPosition = 0
    end
    object ppPageStyle1: TppPageStyle
      Background.Brush.Style = bsClear
      Border.mmPadding = 0
      EndPage = 0
      SinglePage = 0
      StartPage = 0
      mmBottomOffset = 0
      mmHeight = 0
      mmPrintPosition = 0
    end
    object ppGroup1: TppGroup
      BreakName = 'RecnoTinta'
      DataPipeline = ppDBPipeline2
      GroupFileSettings.NewFile = False
      GroupFileSettings.EmailFile = False
      KeepTogether = True
      OutlineSettings.CreateNode = True
      StartOnOddPage = False
      mmNewColumnThreshold = 0
      mmNewPageThreshold = 0
      DataPipelineName = 'ppDBPipeline2'
      NewFile = False
      object ppGroupHeaderBand1: TppGroupHeaderBand
        Background.Brush.Style = bsClear
        Border.mmPadding = 0
        mmBottomOffset = 0
        mmHeight = 20902
        mmPrintPosition = 0
        object ppShape196: TppShape
          DesignLayer = ppDesignLayer4
          UserName = 'Shape196'
          mmHeight = 4498
          mmLeft = 2380
          mmTop = 16404
          mmWidth = 195527
          BandType = 3
          GroupNo = 0
          LayerName = BandLayer4
        end
        object ppLabel524: TppLabel
          DesignLayer = ppDesignLayer4
          UserName = 'Label524'
          HyperlinkEnabled = False
          AutoSize = False
          Border.mmPadding = 0
          Caption = 'Descri'#231#227'o'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
          FormFieldSettings.FormFieldType = fftNone
          Transparent = True
          VerticalAlignment = avCenter
          mmHeight = 3441
          mmLeft = 4498
          mmTop = 16933
          mmWidth = 16933
          BandType = 3
          GroupNo = 0
          LayerName = BandLayer4
        end
        object ppLabel526: TppLabel
          DesignLayer = ppDesignLayer4
          UserName = 'Label526'
          HyperlinkEnabled = False
          AutoSize = False
          Border.mmPadding = 0
          Caption = 'Local'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
          FormFieldSettings.FormFieldType = fftNone
          TextAlignment = taCentered
          Transparent = True
          VerticalAlignment = avCenter
          mmHeight = 3441
          mmLeft = 146041
          mmTop = 16933
          mmWidth = 8731
          BandType = 3
          GroupNo = 0
          LayerName = BandLayer4
        end
        object ppLabel527: TppLabel
          DesignLayer = ppDesignLayer4
          UserName = 'Label527'
          HyperlinkEnabled = False
          AutoSize = False
          Border.mmPadding = 0
          Caption = 'Empenho'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
          FormFieldSettings.FormFieldType = fftNone
          Transparent = True
          VerticalAlignment = avCenter
          mmHeight = 3441
          mmLeft = 156889
          mmTop = 16933
          mmWidth = 16669
          BandType = 3
          GroupNo = 0
          LayerName = BandLayer4
        end
        object ppLabel467: TppLabel
          DesignLayer = ppDesignLayer4
          UserName = 'Label467'
          HyperlinkEnabled = False
          AutoSize = False
          Border.mmPadding = 0
          Caption = 'UM'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
          FormFieldSettings.FormFieldType = fftNone
          Transparent = True
          mmHeight = 3705
          mmLeft = 189177
          mmTop = 16668
          mmWidth = 6879
          BandType = 3
          GroupNo = 0
          LayerName = BandLayer4
        end
        object ppLine45: TppLine
          DesignLayer = ppDesignLayer4
          UserName = 'Line45'
          Border.mmPadding = 0
          Position = lpRight
          Weight = 0.750000000000000000
          mmHeight = 4235
          mmLeft = 142337
          mmTop = 16403
          mmWidth = 3175
          BandType = 3
          GroupNo = 0
          LayerName = BandLayer4
        end
        object ppLine48: TppLine
          DesignLayer = ppDesignLayer4
          UserName = 'Line48'
          Border.mmPadding = 0
          Position = lpRight
          Weight = 0.750000000000000000
          mmHeight = 4234
          mmLeft = 152391
          mmTop = 16403
          mmWidth = 3175
          BandType = 3
          GroupNo = 0
          LayerName = BandLayer4
        end
        object ppLine50: TppLine
          DesignLayer = ppDesignLayer4
          UserName = 'Line50'
          Border.mmPadding = 0
          Position = lpRight
          Weight = 0.750000000000000000
          mmHeight = 4234
          mmLeft = 170117
          mmTop = 16403
          mmWidth = 3175
          BandType = 3
          GroupNo = 0
          LayerName = BandLayer4
        end
        object ppLabel353: TppLabel
          DesignLayer = ppDesignLayer4
          UserName = 'Label353'
          HyperlinkEnabled = False
          AutoSize = False
          Border.mmPadding = 0
          Caption = '%'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
          FormFieldSettings.FormFieldType = fftNone
          TextAlignment = taCentered
          Transparent = True
          VerticalAlignment = avCenter
          mmHeight = 3176
          mmLeft = 174886
          mmTop = 16668
          mmWidth = 12175
          BandType = 3
          GroupNo = 0
          LayerName = BandLayer4
        end
        object ppLine77: TppLine
          DesignLayer = ppDesignLayer4
          UserName = 'Line77'
          Border.mmPadding = 0
          Position = lpRight
          Weight = 0.750000000000000000
          mmHeight = 4235
          mmLeft = 185209
          mmTop = 16403
          mmWidth = 3175
          BandType = 3
          GroupNo = 0
          LayerName = BandLayer4
        end
        object ppDBBarCode10: TppDBBarCode
          DesignLayer = ppDesignLayer4
          UserName = 'DBBarCode3'
          AlignBarCode = ahLeft
          AutoSizeFont = False
          BarCodeType = bcCode39
          BarColor = clBlack
          Border.mmPadding = 0
          DataField = 'OPTinta'
          DataPipeline = ppDBPipeline2
          Alignment = taRightJustify
          AutoSize = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Name = 'Courier New'
          Font.Size = 8
          Font.Style = [fsBold]
          Transparent = True
          DataPipelineName = 'ppDBPipeline2'
          mmHeight = 13758
          mmLeft = 140229
          mmTop = 1588
          mmWidth = 59267
          BandType = 3
          GroupNo = 0
          LayerName = BandLayer4
          mmBarWidth = 254
          mmWideBarRatio = 76200
        end
        object ppDBText5: TppDBText
          DesignLayer = ppDesignLayer4
          UserName = 'DBText5'
          Border.mmPadding = 0
          DataField = 'Tinta'
          DataPipeline = ppDBPipeline2
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 10
          Font.Style = [fsBold]
          ResetGroup = ppGroup2
          Transparent = True
          DataPipelineName = 'ppDBPipeline2'
          mmHeight = 4233
          mmLeft = 2380
          mmTop = 8467
          mmWidth = 119063
          BandType = 3
          GroupNo = 0
          LayerName = BandLayer4
        end
        object ppDBText6: TppDBText
          DesignLayer = ppDesignLayer4
          UserName = 'DBText6'
          Border.mmPadding = 0
          DataField = 'OPTinta'
          DataPipeline = ppDBPipeline2
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 10
          Font.Style = [fsBold]
          ResetGroup = ppGroup2
          Transparent = True
          DataPipelineName = 'ppDBPipeline2'
          mmHeight = 4233
          mmLeft = 2378
          mmTop = 2910
          mmWidth = 119063
          BandType = 3
          GroupNo = 0
          LayerName = BandLayer4
        end
      end
      object ppGroupFooterBand1: TppGroupFooterBand
        Background.Brush.Style = bsClear
        Border.mmPadding = 0
        HideWhenOneDetail = False
        mmBottomOffset = 0
        mmHeight = 12700
        mmPrintPosition = 0
        object ppDBText7: TppDBText
          DesignLayer = ppDesignLayer4
          UserName = 'DBText8'
          Border.mmPadding = 0
          DataField = 'Estoque'
          DataPipeline = ppDBPipeline2
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          TextAlignment = taRightJustified
          Transparent = True
          DataPipelineName = 'ppDBPipeline2'
          mmHeight = 3969
          mmLeft = 150284
          mmTop = 6085
          mmWidth = 20902
          BandType = 5
          GroupNo = 0
          LayerName = BandLayer4
        end
        object ppLabel5: TppLabel
          DesignLayer = ppDesignLayer4
          UserName = 'ppLBL_TituloRelatorio1'
          HyperlinkEnabled = False
          Border.mmPadding = 0
          Caption = 'Estoque'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 10
          Font.Style = [fsBold]
          FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
          FormFieldSettings.FormFieldType = fftNone
          Transparent = True
          mmHeight = 3969
          mmLeft = 133086
          mmTop = 6085
          mmWidth = 14817
          BandType = 5
          GroupNo = 0
          LayerName = BandLayer4
        end
        object ppLabel7: TppLabel
          DesignLayer = ppDesignLayer4
          UserName = 'Label7'
          HyperlinkEnabled = False
          Border.mmPadding = 0
          Caption = 'Saldo a Produzir:'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 10
          Font.Style = [fsBold]
          FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
          FormFieldSettings.FormFieldType = fftNone
          Transparent = True
          mmHeight = 4233
          mmLeft = 118004
          mmTop = 794
          mmWidth = 29898
          BandType = 5
          GroupNo = 0
          LayerName = BandLayer4
        end
        object ppDBText8: TppDBText
          DesignLayer = ppDesignLayer4
          UserName = 'DBText9'
          Border.mmPadding = 0
          DataField = 'SaldoTinta'
          DataPipeline = ppDBPipeline2
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          TextAlignment = taRightJustified
          Transparent = True
          DataPipelineName = 'ppDBPipeline2'
          mmHeight = 3969
          mmLeft = 150284
          mmTop = 794
          mmWidth = 20902
          BandType = 5
          GroupNo = 0
          LayerName = BandLayer4
        end
      end
    end
    object ppGroup2: TppGroup
      BreakName = 'D4Recno'
      DataPipeline = ppDBPipeline2
      GroupFileSettings.NewFile = False
      GroupFileSettings.EmailFile = False
      KeepTogether = True
      OutlineSettings.CreateNode = True
      StartOnOddPage = False
      UserName = 'Group1'
      mmNewColumnThreshold = 0
      mmNewPageThreshold = 0
      DataPipelineName = 'ppDBPipeline2'
      NewFile = False
      object ppGroupHeaderBand2: TppGroupHeaderBand
        Background.Brush.Style = bsClear
        Border.mmPadding = 0
        mmBottomOffset = 0
        mmHeight = 0
        mmPrintPosition = 0
      end
      object ppGroupFooterBand2: TppGroupFooterBand
        Background.Brush.Style = bsClear
        Border.mmPadding = 0
        HideWhenOneDetail = False
        mmBottomOffset = 0
        mmHeight = 0
        mmPrintPosition = 0
      end
    end
    object ppDesignLayers3: TppDesignLayers
      object ppDesignLayer3: TppDesignLayer
        UserName = 'PageLayer2'
        LayerType = ltPage
        Index = 0
      end
      object ppDesignLayer4: TppDesignLayer
        UserName = 'BandLayer4'
        LayerType = ltBanded
        Index = 1
      end
    end
    object ppParameterList1: TppParameterList
    end
  end
  object ppBDEPipeline_Estrutura: TppBDEPipeline
    UserName = 'ppBDEPipeline_Estrutura'
    Left = 102
    Top = 165
  end
end
