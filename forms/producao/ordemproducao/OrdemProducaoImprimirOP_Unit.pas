...
begin
      ...
      begin
          Application.MessageBox('Acesso negado para Ordem que não esteja Pronta para Personalização','Atenção',mb_iconwarning + mb_ok);
          Exit;
      end;
      ...
      begin
          Application.MessageBox('Acesso negado para Imprimir Ordem com Classe C não definida.','Atenção',mb_iconwarning + mb_ok);
          Exit;
      end;

      if ((CDSTipoProduto.AsString = 'BALDE') OR (CDSTipoProduto.AsString = 'TAMPA') OR (CDSTipoProduto.AsString = 'ALÇA')) and (CDSClasseD.AsString = '') then
      begin
          Application.MessageBox('Acesso negado para Imprimir Ordem com Classe D não definida.','Atenção',mb_iconwarning + mb_ok);
          Exit;
      end;

      if CDSStatus.AsString = 'PREVISTA' then
      begin
          Application.MessageBox('Acesso negado para Imprimir Ordem com Status PREVISTA.','Atenção',mb_iconwarning + mb_ok);
          Exit;
      end;
...
begin
   ...
   If Application.MessageBox('Deseja retirar a marcação de insumo de todas as OP´s?','Atenção',MB_YESNO) = 6 then
   begin
...
begin
  ...
  if (CBX_Setor.Text <> 'PERSONALIZACAO') then
  begin
      Application.MessageBox('Esta ação é só aplicável para Ordens de Personalização','Atenção',mb_iconwarning + mb_ok);
      Exit;
  end;
