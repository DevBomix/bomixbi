object OrdemProducao_RelatoriosDaInjecaoADM: TOrdemProducao_RelatoriosDaInjecaoADM
  Left = 0
  Top = 0
  Caption = 'OrdemProducao_RelatoriosDaInjecaoADM'
  ClientHeight = 531
  ClientWidth = 880
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  PixelsPerInch = 96
  TextHeight = 13
  object ppDBPipeline5: TppDBPipeline
    DataSource = DS_InusmosAgrupados
    CloseDataSource = True
    UserName = 'DBPipeline5'
    Left = 313
    Top = 139
  end
  object CDS_InusmosAgrupados: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DSP_InusmosAgrupados'
    Left = 280
    Top = 186
    object CDS_InusmosAgrupadosEmpresa: TStringField
      FieldName = 'Empresa'
      Size = 15
    end
    object CDS_InusmosAgrupadosSetor: TStringField
      FieldName = 'Setor'
      Size = 50
    end
    object CDS_InusmosAgrupadosEmissao: TStringField
      FieldName = 'Emissao'
      ReadOnly = True
      Size = 10
    end
    object CDS_InusmosAgrupadosTipo: TStringField
      FieldName = 'Tipo'
      Size = 50
    end
    object CDS_InusmosAgrupadosProduto_ID: TStringField
      FieldName = 'Produto_ID'
      Size = 30
    end
    object CDS_InusmosAgrupadosProduto: TStringField
      FieldName = 'Produto'
      Size = 150
    end
    object CDS_InusmosAgrupadosSaldoProducao: TFloatField
      FieldName = 'SaldoProducao'
      ReadOnly = True
    end
    object CDS_InusmosAgrupadosEmpenho: TFloatField
      FieldName = 'Empenho'
      ReadOnly = True
    end
    object CDS_InusmosAgrupadosDestino_ID: TStringField
      FieldName = 'Destino_ID'
      Size = 2
    end
    object CDS_InusmosAgrupadosDestino: TStringField
      FieldName = 'Destino'
      Size = 30
    end
    object CDS_InusmosAgrupadosEstoqueDestino: TFloatField
      FieldName = 'EstoqueDestino'
    end
    object CDS_InusmosAgrupadosOrigem_ID: TStringField
      FieldName = 'Origem_ID'
      Size = 2
    end
    object CDS_InusmosAgrupadosOrigem: TStringField
      FieldName = 'Origem'
      Size = 30
    end
    object CDS_InusmosAgrupadosEstoqueOrigem: TFloatField
      FieldName = 'EstoqueOrigem'
    end
    object CDS_InusmosAgrupadosUnidade: TStringField
      FieldName = 'Unidade'
      Size = 2
    end
  end
  object Query_InusmosAgrupados: TADOQuery
    Connection = Sys_ModuloConexaoDados.ADOConnection1
    CursorType = ctStatic
    DataSource = DS_InusmosAgrupados
    Parameters = <>
    SQL.Strings = (
      ''
      ''
      'Select '
      
        #9'Empresa, Setor, Emissao, Tipo, Produto_ID, Produto, Sum(SaldoPr' +
        'oducao) as SaldoProducao, '
      
        #9'Sum(Empenho) as Empenho, Destino_ID, Destino, EstoqueDestino, O' +
        'rigem_ID, Origem, EstoqueOrigem, Unidade'
      ''
      'from ('
      ''
      #9' Select '
      '  '
      
        #9#9#9'Empresa, Setor, Convert(Varchar(10),GetDate(),103) as Emissao' +
        ', Tipo, Produto_ID, Produto, SaldoProducao, QtdPerda,'
      
        #9#9#9'Case When Unidade = '#39'UN'#39' then CEILING(Empenho) else Empenho e' +
        'nd as Empenho, '
      
        #9#9#9'Insumos.Destino_ID, Destino, EstoqueDestino.B2_QATU as Estoqu' +
        'eDestino, '
      
        #9#9#9'Insumos.Origem_ID, Origem, EstoqueOrigem.B2_QATU as EstoqueOr' +
        'igem, Unidade'
      ' '
      #9#9'  from ('#9
      #9'  '
      #9#9#9'Select '
      
        #9#9#9#9'  OP.Empresa, Empenho.Setor, Empenho.Tipo, Empenho.Produto_I' +
        'D, Empenho.Produto, '
      
        #9#9#9#9'  Empenho.Armazem as Origem_ID, Empenho.ArmazemDesc as Orige' +
        'm, '
      #9#9#9#9'  Sum(OP.Saldo) as Saldo,  '
      
        #9#9#9#9'  Sum(OP.QtdParcialSolicitada) as SaldoProducao, Sum(OP.Perd' +
        'aNoMomento) as PerdaNoMomento,  '
      #9#9#9#9'  Sum(FaltaUsar) as FaltaUsar, Empenho.QtdPerda,  '
      
        #9#9#9#9'  (Sum(OP.QtdParcialSolicitada) * 100)/Sum(OP.Saldo) as Perc' +
        'entual, '
      
        #9#9#9#9'  (Sum(FaltaUsar) * (Sum(OP.QtdParcialSolicitada) * 100)/Sum' +
        '(OP.Saldo))/100 + Empenho.QtdPerda as Empenho,   '
      
        #9#9#9#9'   Empenho.Unidade, Empenho.ArmazemConsumo as Destino_ID, Em' +
        'penho.ArmazemConsumoDesc as Destino '
      #9#9#9'from BomixBI.dbo.Pcp_TB_OrdemProducao OP (nolock) '
      
        #9#9#9'Inner Join BomixBI.dbo.Pcp_TB_ProdutoEmpenho Empenho (nolock)' +
        ' ON Empenho.C2_Recno = OP.Recno '
      #9#9#9'Where OP.Empresa = '#39'0101'#39
      
        #9#9#9'AND OP.Setor in ('#39'INJECAO'#39','#39'PERSONALIZACAO'#39','#39'INJECAO 04'#39','#39'PAL' +
        'ETIZACAO 04'#39','#39'IMPRESSAO DIGITAL'#39','#39'MONTAGEM DE ALCA'#39','#39'PALETIZACAO' +
        ' PE'#39','#39'TINTURARIA'#39')'
      #9#9#9'AND StatusInsumoOP <> '#39#39
      #9#9#9'AND OP.Status <> '#39'FINALIZADA'#39
      
        #9#9#9'Group By OP.Empresa, Empenho.Setor, Empenho.Tipo, Empenho.Pro' +
        'duto_ID, '
      
        #9#9#9#9' Empenho.Produto, Empenho.Armazem, Empenho.ArmazemDesc, Empe' +
        'nho.Unidade, '
      
        #9#9#9#9' Empenho.ArmazemConsumo, Empenho.ArmazemConsumoDesc, Empenho' +
        '.QtdPerda '
      ''
      #9#9'  ) Insumos '
      
        #9#9'  Inner Join P12OFICIAL.dbo.SB2010 EstoqueDestino (nolock) ON ' +
        'Substring(EstoqueDestino.B2_FILIAL,1,4) = Empresa '
      #9#9#9#9#9#9#9#9#9#9' AND EstoqueDestino.D_E_L_E_T_ <> '#39'*'#39
      #9#9#9#9#9#9#9#9#9#9' AND EstoqueDestino.B2_COD = Produto_ID   '
      #9#9#9#9#9#9#9#9#9#9' AND EstoqueDestino.B2_LOCAL = Destino_ID  '
      
        #9#9'  Inner Join P12OFICIAL.dbo.SB2010 EstoqueOrigem (nolock) ON S' +
        'ubstring(EstoqueOrigem.B2_FILIAL,1,4) = Empresa '
      #9#9#9#9#9#9#9#9#9#9' AND EstoqueOrigem.D_E_L_E_T_ <> '#39'*'#39
      #9#9#9#9#9#9#9#9#9#9' AND EstoqueOrigem.B2_COD = Produto_ID  '
      #9#9#9#9#9#9#9#9#9#9' AND EstoqueOrigem.B2_LOCAL = Origem_ID  '
      ''
      ') TB'
      
        'Group by Empresa, Setor, Emissao, Tipo, Produto_ID, Produto, Des' +
        'tino_ID, Destino, EstoqueDestino, Origem_ID, Origem, EstoqueOrig' +
        'em, Unidade'
      'Order by Setor, Tipo, Produto_ID '
      ''
      ''
      ''
      ''
      ''
      ''
      ''
      ''
      ''
      ''
      ''
      '')
    Left = 311
    Top = 185
    object Query_InusmosAgrupadosEmpresa: TStringField
      FieldName = 'Empresa'
      Size = 15
    end
    object Query_InusmosAgrupadosSetor: TStringField
      FieldName = 'Setor'
      Size = 50
    end
    object Query_InusmosAgrupadosEmissao: TStringField
      FieldName = 'Emissao'
      ReadOnly = True
      Size = 10
    end
    object Query_InusmosAgrupadosTipo: TStringField
      FieldName = 'Tipo'
      Size = 50
    end
    object Query_InusmosAgrupadosProduto_ID: TStringField
      FieldName = 'Produto_ID'
      Size = 30
    end
    object Query_InusmosAgrupadosProduto: TStringField
      FieldName = 'Produto'
      Size = 150
    end
    object Query_InusmosAgrupadosSaldoProducao: TFloatField
      FieldName = 'SaldoProducao'
      ReadOnly = True
    end
    object Query_InusmosAgrupadosEmpenho: TFloatField
      FieldName = 'Empenho'
      ReadOnly = True
    end
    object Query_InusmosAgrupadosDestino_ID: TStringField
      FieldName = 'Destino_ID'
      Size = 2
    end
    object Query_InusmosAgrupadosDestino: TStringField
      FieldName = 'Destino'
      Size = 30
    end
    object Query_InusmosAgrupadosEstoqueDestino: TFloatField
      FieldName = 'EstoqueDestino'
    end
    object Query_InusmosAgrupadosOrigem_ID: TStringField
      FieldName = 'Origem_ID'
      Size = 2
    end
    object Query_InusmosAgrupadosOrigem: TStringField
      FieldName = 'Origem'
      Size = 30
    end
    object Query_InusmosAgrupadosEstoqueOrigem: TFloatField
      FieldName = 'EstoqueOrigem'
    end
    object Query_InusmosAgrupadosUnidade: TStringField
      FieldName = 'Unidade'
      Size = 2
    end
  end
  object DS_InusmosAgrupados: TDataSource
    DataSet = CDS_InusmosAgrupados
    Left = 341
    Top = 185
  end
  object DSP_InusmosAgrupados: TDataSetProvider
    DataSet = Query_InusmosAgrupados
    Left = 371
    Top = 185
  end
  object ppImprimirInusmosAgrupados: TppReport
    AutoStop = False
    DataPipeline = ppDBPipeline5
    PassSetting = psTwoPass
    PrinterSetup.BinName = 'Default'
    PrinterSetup.DocumentName = 'Report'
    PrinterSetup.Duplex = dpVertical
    PrinterSetup.PaperName = 'A4'
    PrinterSetup.PrinterName = 'Default'
    PrinterSetup.SaveDeviceSettings = False
    PrinterSetup.mmMarginBottom = 6350
    PrinterSetup.mmMarginLeft = 6350
    PrinterSetup.mmMarginRight = 6350
    PrinterSetup.mmMarginTop = 6350
    PrinterSetup.mmPaperHeight = 297000
    PrinterSetup.mmPaperWidth = 210000
    PrinterSetup.PaperSize = 9
    AllowPrintToArchive = True
    AllowPrintToFile = True
    ArchiveFileName = '($MyDocuments)\ReportArchive.raf'
    DeviceType = 'Screen'
    DefaultFileDeviceType = 'XlsxData'
    EmailSettings.ReportFormat = 'PDF'
    LanguageID = 'Portuguese (Brazil)'
    OpenFile = False
    OutlineSettings.CreateNode = True
    OutlineSettings.CreatePageNodes = True
    OutlineSettings.Enabled = True
    OutlineSettings.Visible = True
    ThumbnailSettings.Enabled = True
    ThumbnailSettings.Visible = True
    ThumbnailSettings.DeadSpace = 30
    ThumbnailSettings.PageHighlight.Width = 3
    PDFSettings.EmbedFontOptions = [efUseSubset]
    PDFSettings.EncryptSettings.AllowCopy = True
    PDFSettings.EncryptSettings.AllowInteract = True
    PDFSettings.EncryptSettings.AllowModify = True
    PDFSettings.EncryptSettings.AllowPrint = True
    PDFSettings.EncryptSettings.AllowExtract = True
    PDFSettings.EncryptSettings.AllowAssemble = True
    PDFSettings.EncryptSettings.AllowQualityPrint = True
    PDFSettings.EncryptSettings.Enabled = False
    PDFSettings.EncryptSettings.KeyLength = kl40Bit
    PDFSettings.EncryptSettings.EncryptionType = etRC4
    PDFSettings.FontEncoding = feAnsi
    PDFSettings.ImageCompressionLevel = 25
    PDFSettings.PDFAFormat = pafNone
    PreviewFormSettings.PageBorder.mmPadding = 0
    PreviewFormSettings.PageDisplay = pdContinuous
    PreviewFormSettings.WindowState = wsMaximized
    PreviewFormSettings.ZoomSetting = zsPageWidth
    RTFSettings.DefaultFont.Charset = DEFAULT_CHARSET
    RTFSettings.DefaultFont.Color = clWindowText
    RTFSettings.DefaultFont.Height = -13
    RTFSettings.DefaultFont.Name = 'Arial'
    RTFSettings.DefaultFont.Style = []
    TextFileName = '($MyDocuments)\Report.xlsx'
    TextSearchSettings.DefaultString = '<Texto a localizar>'
    TextSearchSettings.Enabled = True
    XLSSettings.AppName = 'ReportBuilder'
    XLSSettings.Author = 'ReportBuilder'
    XLSSettings.Subject = 'Report'
    XLSSettings.Title = 'Report'
    XLSSettings.WorksheetName = 'Report'
    Left = 351
    Top = 139
    Version = '19.03'
    mmColumnWidth = 197300
    DataPipelineName = 'ppDBPipeline5'
    object ppHeaderBand5: TppHeaderBand
      Background.Brush.Style = bsClear
      Border.mmPadding = 0
      PrintHeight = phDynamic
      mmBottomOffset = 0
      mmHeight = 15346
      mmPrintPosition = 0
      object ppImage4: TppImage
        DesignLayer = ppDesignLayer10
        UserName = 'ppIMG_Logo'
        AlignHorizontal = ahCenter
        AlignVertical = avCenter
        MaintainAspectRatio = False
        Stretch = True
        Border.mmPadding = 0
        Picture.Data = {
          
          FFFF}
        mmHeight = 11906
        mmLeft = 1323
        mmTop = 794
        mmWidth = 33338
        BandType = 0
        LayerName = BandLayer5
      end
      object ppLabel33: TppLabel
        DesignLayer = ppDesignLayer10
        UserName = 'ppLBL_TituloRelatorio'
        HyperlinkEnabled = False
        Border.mmPadding = 0
        Caption = 'Empenhos Agrupados no Dia '
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 18
        Font.Style = [fsBold]
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        Transparent = True
        mmHeight = 7673
        mmLeft = 37571
        mmTop = 2646
        mmWidth = 91281
        BandType = 0
        LayerName = BandLayer5
      end
      object ppLine29: TppLine
        DesignLayer = ppDesignLayer10
        UserName = 'Line1'
        Border.mmPadding = 0
        Pen.Width = 3
        Weight = 2.250000000000000000
        mmHeight = 1588
        mmLeft = 529
        mmTop = 13758
        mmWidth = 195792
        BandType = 0
        LayerName = BandLayer5
      end
      object ppDBText7: TppDBText
        DesignLayer = ppDesignLayer10
        UserName = 'DBText7'
        Border.mmPadding = 0
        DataField = 'Emissao'
        DataPipeline = ppDBPipeline5
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 18
        Font.Style = [fsBold]
        Transparent = True
        DataPipelineName = 'ppDBPipeline5'
        mmHeight = 7678
        mmLeft = 130175
        mmTop = 2646
        mmWidth = 35719
        BandType = 0
        LayerName = BandLayer5
      end
    end
    object ppDetailBand5: TppDetailBand
      Background1.Brush.Style = bsClear
      Background2.Brush.Style = bsClear
      Border.mmPadding = 0
      PrintHeight = phDynamic
      mmBottomOffset = 0
      mmHeight = 5821
      mmPrintPosition = 0
      object ppDBText1: TppDBText
        DesignLayer = ppDesignLayer10
        UserName = 'DBText1'
        Border.mmPadding = 0
        DataField = 'Tipo'
        DataPipeline = ppDBPipeline5
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 6
        Font.Style = []
        Transparent = True
        DataPipelineName = 'ppDBPipeline5'
        mmHeight = 3704
        mmLeft = 2646
        mmTop = 1059
        mmWidth = 18781
        BandType = 4
        LayerName = BandLayer5
      end
      object ppLine5: TppLine
        DesignLayer = ppDesignLayer10
        UserName = 'Line6'
        Border.mmPadding = 0
        Pen.Color = clSilver
        Pen.Style = psDot
        Weight = 0.750000000000000000
        mmHeight = 1058
        mmLeft = 2649
        mmTop = 4761
        mmWidth = 191817
        BandType = 4
        LayerName = BandLayer5
      end
      object ppDBText2: TppDBText
        DesignLayer = ppDesignLayer10
        UserName = 'DBText2'
        Border.mmPadding = 0
        DataField = 'Produto_ID'
        DataPipeline = ppDBPipeline5
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 6
        Font.Style = []
        Transparent = True
        DataPipelineName = 'ppDBPipeline5'
        mmHeight = 3704
        mmLeft = 22750
        mmTop = 1058
        mmWidth = 13225
        BandType = 4
        LayerName = BandLayer5
      end
      object ppDBText4: TppDBText
        DesignLayer = ppDesignLayer10
        UserName = 'DBText4'
        Border.mmPadding = 0
        DataField = 'Empenho'
        DataPipeline = ppDBPipeline5
        DisplayFormat = '#,0.00;-#,0.00'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 6
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDBPipeline5'
        mmHeight = 3704
        mmLeft = 132821
        mmTop = 1058
        mmWidth = 17455
        BandType = 4
        LayerName = BandLayer5
      end
      object ppDBText5: TppDBText
        DesignLayer = ppDesignLayer10
        UserName = 'DBText5'
        Border.mmPadding = 0
        DataField = 'EstoqueOrigem'
        DataPipeline = ppDBPipeline5
        DisplayFormat = '#,0.00;-#,0.00'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 6
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDBPipeline5'
        mmHeight = 3704
        mmLeft = 151607
        mmTop = 1058
        mmWidth = 16938
        BandType = 4
        LayerName = BandLayer5
      end
      object ppDBText13: TppDBText
        DesignLayer = ppDesignLayer10
        UserName = 'DBText13'
        Border.mmPadding = 0
        DataField = 'EstoqueDestino'
        DataPipeline = ppDBPipeline5
        DisplayFormat = '#,0.00;-#,0.00'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 6
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDBPipeline5'
        mmHeight = 3704
        mmLeft = 169858
        mmTop = 1058
        mmWidth = 17726
        BandType = 4
        LayerName = BandLayer5
      end
      object ppDBText14: TppDBText
        DesignLayer = ppDesignLayer10
        UserName = 'DBText14'
        Border.mmPadding = 0
        DataField = 'Unidade'
        DataPipeline = ppDBPipeline5
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 6
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDBPipeline5'
        mmHeight = 3704
        mmLeft = 188929
        mmTop = 1058
        mmWidth = 5274
        BandType = 4
        LayerName = BandLayer5
      end
      object ppDBText12: TppDBText
        DesignLayer = ppDesignLayer10
        UserName = 'DBText12'
        Border.mmPadding = 0
        DataField = 'SaldoProducao'
        DataPipeline = ppDBPipeline5
        DisplayFormat = '#,0;-#,0'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 6
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDBPipeline5'
        mmHeight = 3704
        mmLeft = 114013
        mmTop = 1058
        mmWidth = 17997
        BandType = 4
        LayerName = BandLayer5
      end
      object ppDBMemo1: TppDBMemo
        DesignLayer = ppDesignLayer10
        UserName = 'DBMemo1'
        Border.mmPadding = 0
        CharWrap = True
        DataField = 'Produto'
        DataPipeline = ppDBPipeline5
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 6
        Font.Style = []
        RemoveEmptyLines = False
        Stretch = True
        Transparent = True
        DataPipelineName = 'ppDBPipeline5'
        mmHeight = 3175
        mmLeft = 37571
        mmTop = 1058
        mmWidth = 74877
        BandType = 4
        LayerName = BandLayer5
        mmBottomOffset = 0
        mmOverFlowOffset = 0
        mmStopPosition = 0
        mmMinHeight = 0
        mmLeading = 0
      end
    end
    object ppFooterBand5: TppFooterBand
      Background.Brush.Style = bsClear
      Border.mmPadding = 0
      mmBottomOffset = 0
      mmHeight = 5821
      mmPrintPosition = 0
      object ppLine12: TppLine
        DesignLayer = ppDesignLayer10
        UserName = 'Line12'
        Border.mmPadding = 0
        Pen.Width = 3
        Weight = 2.250000000000000000
        mmHeight = 2646
        mmLeft = 529
        mmTop = 0
        mmWidth = 195792
        BandType = 8
        LayerName = BandLayer5
      end
      object ppLBL_Data: TppLabel
        DesignLayer = ppDesignLayer10
        UserName = 'ppLBL_DataListaPorGerencia'
        HyperlinkEnabled = False
        Border.mmPadding = 0
        Caption = 'ppLBL_Data'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = [fsBold]
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        TextAlignment = taRightJustified
        Transparent = True
        mmHeight = 4233
        mmLeft = 59796
        mmTop = 1058
        mmWidth = 135996
        BandType = 8
        LayerName = BandLayer5
      end
    end
    object ppSummaryBand1: TppSummaryBand
      Background.Brush.Style = bsClear
      Border.mmPadding = 0
      PrintHeight = phDynamic
      mmBottomOffset = 0
      mmHeight = 0
      mmPrintPosition = 0
    end
    object ppPageStyle5: TppPageStyle
      Background.Brush.Style = bsClear
      Border.mmPadding = 0
      EndPage = 0
      SinglePage = 0
      StartPage = 0
      mmBottomOffset = 0
      mmHeight = 0
      mmPrintPosition = 0
    end
    object ppGroup1: TppGroup
      BreakName = 'Setor'
      DataPipeline = ppDBPipeline5
      GroupFileSettings.NewFile = False
      GroupFileSettings.EmailFile = False
      OutlineSettings.CreateNode = True
      NewPage = True
      StartOnOddPage = False
      UserName = 'Group1'
      mmNewColumnThreshold = 0
      mmNewPageThreshold = 0
      DataPipelineName = 'ppDBPipeline5'
      NewFile = False
      object ppGroupHeaderBand1: TppGroupHeaderBand
        Background.Brush.Style = bsClear
        Border.mmPadding = 0
        mmBottomOffset = 0
        mmHeight = 23548
        mmPrintPosition = 0
        object ppShape1: TppShape
          DesignLayer = ppDesignLayer10
          UserName = 'Shape1'
          mmHeight = 5821
          mmLeft = 1321
          mmTop = 17720
          mmWidth = 194467
          BandType = 3
          GroupNo = 0
          LayerName = BandLayer5
        end
        object ppDBText6: TppDBText
          DesignLayer = ppDesignLayer10
          UserName = 'DBText6'
          Border.mmPadding = 0
          DataField = 'Setor'
          DataPipeline = ppDBPipeline5
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 11
          Font.Style = [fsBold]
          Transparent = True
          DataPipelineName = 'ppDBPipeline5'
          mmHeight = 5556
          mmLeft = 2646
          mmTop = 1059
          mmWidth = 77258
          BandType = 3
          GroupNo = 0
          LayerName = BandLayer5
        end
        object ppLabel1: TppLabel
          DesignLayer = ppDesignLayer10
          UserName = 'Label1'
          HyperlinkEnabled = False
          AutoSize = False
          Border.mmPadding = 0
          Caption = 'Tipo'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
          FormFieldSettings.FormFieldType = fftNone
          Transparent = True
          VerticalAlignment = avCenter
          mmHeight = 3969
          mmLeft = 2646
          mmTop = 18511
          mmWidth = 7938
          BandType = 3
          GroupNo = 0
          LayerName = BandLayer5
        end
        object ppLabel8: TppLabel
          DesignLayer = ppDesignLayer10
          UserName = 'Label8'
          HyperlinkEnabled = False
          AutoSize = False
          Border.mmPadding = 0
          Caption = 'Origem'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = [fsBold]
          FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
          FormFieldSettings.FormFieldType = fftNone
          Transparent = True
          mmHeight = 4763
          mmLeft = 2646
          mmTop = 7143
          mmWidth = 16139
          BandType = 3
          GroupNo = 0
          LayerName = BandLayer5
        end
        object ppDBText8: TppDBText
          DesignLayer = ppDesignLayer10
          UserName = 'DBText8'
          Border.mmPadding = 0
          DataField = 'Origem_ID'
          DataPipeline = ppDBPipeline5
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = []
          Transparent = True
          DataPipelineName = 'ppDBPipeline5'
          mmHeight = 4498
          mmLeft = 20636
          mmTop = 7143
          mmWidth = 7144
          BandType = 3
          GroupNo = 0
          LayerName = BandLayer5
        end
        object ppDBText9: TppDBText
          DesignLayer = ppDesignLayer10
          UserName = 'DBText9'
          Border.mmPadding = 0
          DataField = 'Origem'
          DataPipeline = ppDBPipeline5
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = []
          Transparent = True
          DataPipelineName = 'ppDBPipeline5'
          mmHeight = 4498
          mmLeft = 29627
          mmTop = 7143
          mmWidth = 94192
          BandType = 3
          GroupNo = 0
          LayerName = BandLayer5
        end
        object ppDBText11: TppDBText
          DesignLayer = ppDesignLayer10
          UserName = 'DBText11'
          Border.mmPadding = 0
          DataField = 'Destino'
          DataPipeline = ppDBPipeline5
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = []
          Transparent = True
          DataPipelineName = 'ppDBPipeline5'
          mmHeight = 4498
          mmLeft = 29627
          mmTop = 11638
          mmWidth = 94192
          BandType = 3
          GroupNo = 0
          LayerName = BandLayer5
        end
        object ppDBText10: TppDBText
          DesignLayer = ppDesignLayer10
          UserName = 'DBText10'
          Border.mmPadding = 0
          DataField = 'Destino_ID'
          DataPipeline = ppDBPipeline5
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = []
          Transparent = True
          DataPipelineName = 'ppDBPipeline5'
          mmHeight = 4498
          mmLeft = 20636
          mmTop = 11638
          mmWidth = 7144
          BandType = 3
          GroupNo = 0
          LayerName = BandLayer5
        end
        object ppLabel9: TppLabel
          DesignLayer = ppDesignLayer10
          UserName = 'Label9'
          HyperlinkEnabled = False
          AutoSize = False
          Border.mmPadding = 0
          Caption = 'Destino'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = [fsBold]
          FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
          FormFieldSettings.FormFieldType = fftNone
          Transparent = True
          mmHeight = 4763
          mmLeft = 2646
          mmTop = 11903
          mmWidth = 16139
          BandType = 3
          GroupNo = 0
          LayerName = BandLayer5
        end
        object ppLabel3: TppLabel
          DesignLayer = ppDesignLayer10
          UserName = 'Label6'
          HyperlinkEnabled = False
          AutoSize = False
          Border.mmPadding = 0
          Caption = 'ID'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
          FormFieldSettings.FormFieldType = fftNone
          Transparent = True
          VerticalAlignment = avCenter
          mmHeight = 3969
          mmLeft = 22750
          mmTop = 18511
          mmWidth = 7938
          BandType = 3
          GroupNo = 0
          LayerName = BandLayer5
        end
        object ppLabel10: TppLabel
          DesignLayer = ppDesignLayer10
          UserName = 'Label10'
          HyperlinkEnabled = False
          AutoSize = False
          Border.mmPadding = 0
          Caption = 'Produto'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
          FormFieldSettings.FormFieldType = fftNone
          Transparent = True
          VerticalAlignment = avCenter
          mmHeight = 3969
          mmLeft = 37564
          mmTop = 18511
          mmWidth = 17992
          BandType = 3
          GroupNo = 0
          LayerName = BandLayer5
        end
        object ppLabel11: TppLabel
          DesignLayer = ppDesignLayer10
          UserName = 'Label101'
          HyperlinkEnabled = False
          AutoSize = False
          Border.mmPadding = 0
          Caption = 'Empenho'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
          FormFieldSettings.FormFieldType = fftNone
          TextAlignment = taRightJustified
          Transparent = True
          VerticalAlignment = avCenter
          mmHeight = 3969
          mmLeft = 132821
          mmTop = 18511
          mmWidth = 17455
          BandType = 3
          GroupNo = 0
          LayerName = BandLayer5
        end
        object ppLabel13: TppLabel
          DesignLayer = ppDesignLayer10
          UserName = 'Label13'
          HyperlinkEnabled = False
          AutoSize = False
          Border.mmPadding = 0
          Caption = 'Est.'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
          FormFieldSettings.FormFieldType = fftNone
          Transparent = True
          VerticalAlignment = avCenter
          mmHeight = 3969
          mmLeft = 157430
          mmTop = 18511
          mmWidth = 6590
          BandType = 3
          GroupNo = 0
          LayerName = BandLayer5
        end
        object ppLabel15: TppLabel
          DesignLayer = ppDesignLayer10
          UserName = 'Label15'
          HyperlinkEnabled = False
          AutoSize = False
          Border.mmPadding = 0
          Caption = 'UN'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
          FormFieldSettings.FormFieldType = fftNone
          TextAlignment = taRightJustified
          Transparent = True
          VerticalAlignment = avCenter
          mmHeight = 3969
          mmLeft = 189710
          mmTop = 18511
          mmWidth = 4496
          BandType = 3
          GroupNo = 0
          LayerName = BandLayer5
        end
        object ppLabel2: TppLabel
          DesignLayer = ppDesignLayer10
          UserName = 'Label2'
          HyperlinkEnabled = False
          AutoSize = False
          Border.mmPadding = 0
          Caption = 'Produ'#231#227'o'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
          FormFieldSettings.FormFieldType = fftNone
          TextAlignment = taRightJustified
          Transparent = True
          VerticalAlignment = avCenter
          mmHeight = 3969
          mmLeft = 113218
          mmTop = 18513
          mmWidth = 18792
          BandType = 3
          GroupNo = 0
          LayerName = BandLayer5
        end
        object ppDBText17: TppDBText
          DesignLayer = ppDesignLayer10
          UserName = 'DBText17'
          Border.mmPadding = 0
          DataField = 'Origem_ID'
          DataPipeline = ppDBPipeline5
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = [fsBold]
          TextAlignment = taCentered
          Transparent = True
          VerticalAlignment = avCenter
          DataPipelineName = 'ppDBPipeline5'
          mmHeight = 3968
          mmLeft = 164305
          mmTop = 18778
          mmWidth = 4494
          BandType = 3
          GroupNo = 0
          LayerName = BandLayer5
        end
        object ppDBText18: TppDBText
          DesignLayer = ppDesignLayer10
          UserName = 'DBText18'
          Border.mmPadding = 0
          DataField = 'Destino_ID'
          DataPipeline = ppDBPipeline5
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = [fsBold]
          TextAlignment = taCentered
          Transparent = True
          VerticalAlignment = avCenter
          DataPipelineName = 'ppDBPipeline5'
          mmHeight = 3968
          mmLeft = 183090
          mmTop = 18777
          mmWidth = 4499
          BandType = 3
          GroupNo = 0
          LayerName = BandLayer5
        end
        object ppLabel12: TppLabel
          DesignLayer = ppDesignLayer10
          UserName = 'Label12'
          HyperlinkEnabled = False
          AutoSize = False
          Border.mmPadding = 0
          Caption = 'Est.'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
          FormFieldSettings.FormFieldType = fftNone
          Transparent = True
          VerticalAlignment = avCenter
          mmHeight = 3969
          mmLeft = 175957
          mmTop = 18513
          mmWidth = 6068
          BandType = 3
          GroupNo = 0
          LayerName = BandLayer5
        end
      end
      object ppGroupFooterBand1: TppGroupFooterBand
        Background.Brush.Style = bsClear
        Border.mmPadding = 0
        HideWhenOneDetail = False
        mmBottomOffset = 0
        mmHeight = 7144
        mmPrintPosition = 0
      end
    end
    object ppDesignLayers5: TppDesignLayers
      object ppDesignLayer9: TppDesignLayer
        UserName = 'PageLayer5'
        LayerType = ltPage
        Index = 0
      end
      object ppDesignLayer10: TppDesignLayer
        UserName = 'BandLayer5'
        LayerType = ltBanded
        Index = 1
      end
    end
    object ppParameterList5: TppParameterList
    end
  end
  object CDS_InsumosPorRecursos: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DSP_InsumosPorRecursos'
    Left = 168
    Top = 24
    object CDS_InsumosPorRecursosEmpresa: TStringField
      FieldName = 'Empresa'
      Size = 15
    end
    object CDS_InsumosPorRecursosSetor: TStringField
      FieldName = 'Setor'
      Size = 50
    end
    object CDS_InsumosPorRecursosInjetora_FK: TStringField
      FieldName = 'Injetora_FK'
      Size = 6
    end
    object CDS_InsumosPorRecursosInjetora: TStringField
      FieldName = 'Injetora'
      Size = 30
    end
    object CDS_InsumosPorRecursosEmissao: TStringField
      FieldName = 'Emissao'
      ReadOnly = True
      Size = 10
    end
    object CDS_InsumosPorRecursosTipo: TStringField
      FieldName = 'Tipo'
      Size = 50
    end
    object CDS_InsumosPorRecursosProduto_ID: TStringField
      FieldName = 'Produto_ID'
      Size = 30
    end
    object CDS_InsumosPorRecursosProduto: TStringField
      FieldName = 'Produto'
      Size = 150
    end
    object CDS_InsumosPorRecursosSaldoProducao: TFloatField
      FieldName = 'SaldoProducao'
      ReadOnly = True
    end
    object CDS_InsumosPorRecursosEmpenho: TFloatField
      FieldName = 'Empenho'
      ReadOnly = True
    end
    object CDS_InsumosPorRecursosEstoque: TFloatField
      FieldName = 'Estoque'
    end
    object CDS_InsumosPorRecursosSaldoEstoque: TFloatField
      FieldName = 'SaldoEstoque'
      ReadOnly = True
    end
    object CDS_InsumosPorRecursosUnidade: TStringField
      FieldName = 'Unidade'
      Size = 2
    end
    object CDS_InsumosPorRecursosDestino_ID: TStringField
      FieldName = 'Destino_ID'
      Size = 2
    end
    object CDS_InsumosPorRecursosDestino: TStringField
      FieldName = 'Destino'
      Size = 30
    end
    object CDS_InsumosPorRecursosOrigem_ID: TStringField
      FieldName = 'Origem_ID'
      Size = 2
    end
    object CDS_InsumosPorRecursosOrigem: TStringField
      FieldName = 'Origem'
      Size = 30
    end
  end
  object Query_InsumosPorRecursos: TADOQuery
    Connection = Sys_ModuloConexaoDados.ADOConnection1
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      '     Select '
      '  '
      
        '        Empresa, Setor, Injetora_FK, Injetora, Convert(Varchar(1' +
        '0),GetDate(),103) as Emissao, Tipo, Produto_ID, Produto,  '
      
        '        SaldoProducao, Empenho, Estoque.B2_QATU as Estoque, Esto' +
        'que.B2_QATU - Empenho as SaldoEstoque,  '
      
        '        Unidade, Insumos.Destino_ID, Destino, Insumos.Origem_ID,' +
        ' Origem '
      ' '
      '      from ('#9'Select '
      
        '          OP.Empresa, Empenho.Setor, OP.Injetora_FK, OP.Injetora' +
        ', Empenho.Tipo, Empenho.Produto_ID, Empenho.Produto, '
      
        '          Empenho.Armazem as Origem_ID, Empenho.ArmazemDesc as O' +
        'rigem, '
      '          Sum(OP.QtdParcialSolicitada) as SaldoProducao, '
      
        '          (Sum(OP.QtdParcialSolicitada) * 100)/Sum(OP.Saldo) as ' +
        'Percentual, '
      
        '          (Sum(FaltaUsar) * (Sum(OP.QtdParcialSolicitada) * 100)' +
        '/Sum(OP.Saldo))/100 as Empenho, '
      
        '           Empenho.Unidade, Empenho.ArmazemConsumo as Destino_ID' +
        ', Empenho.ArmazemConsumoDesc as Destino '
      '        from BomixBI.dbo.Pcp_TB_OrdemProducao OP (nolock) '
      
        '        Inner Join BomixBI.dbo.Pcp_TB_ProdutoEmpenho Empenho (no' +
        'lock) ON Empenho.C2_Recno = OP.Recno '
      '        Where OP.Empresa = '#39'0101'#39
      
        '        AND OP.Setor in ('#39'INJECAO'#39','#39'PERSONALIZACAO'#39','#39'INJECAO 04'#39 +
        ','#39'PALETIZACAO 04'#39','#39'IMPRESSAO DIGITAL'#39','#39'MONTAGEM DE ALCA'#39','#39'PALETI' +
        'ZACAO PE'#39')'
      '        AND StatusInsumoOP <> '#39#39
      '        AND OP.Status <> '#39'FINALIZADA'#39
      
        '        Group By OP.Empresa, Empenho.Setor, OP.Injetora_FK, OP.I' +
        'njetora, Empenho.Tipo, Empenho.Produto_ID, '
      
        '             Empenho.Produto, Empenho.Armazem, Empenho.ArmazemDe' +
        'sc, Empenho.Unidade, '
      '             Empenho.ArmazemConsumo, Empenho.ArmazemConsumoDesc '
      '      ) Insumos '
      
        '      Inner Join P12OFICIAL.dbo.SB2010 Estoque (nolock) ON Subst' +
        'ring(B2_FILIAL,1,4) = Empresa '
      '                               AND Estoque.D_E_L_E_T_ <> '#39'*'#39
      '                               AND B2_COD = Produto_ID  '
      '                               AND B2_LOCAL = Destino_ID '
      '      Order by Setor, Injetora_FK, Injetora'
      '')
    Left = 204
    Top = 24
    object Query_InsumosPorRecursosEmpresa: TStringField
      FieldName = 'Empresa'
      Size = 15
    end
    object Query_InsumosPorRecursosSetor: TStringField
      FieldName = 'Setor'
      Size = 50
    end
    object Query_InsumosPorRecursosInjetora_FK: TStringField
      FieldName = 'Injetora_FK'
      Size = 6
    end
    object Query_InsumosPorRecursosInjetora: TStringField
      FieldName = 'Injetora'
      Size = 30
    end
    object Query_InsumosPorRecursosEmissao: TStringField
      FieldName = 'Emissao'
      ReadOnly = True
      Size = 10
    end
    object Query_InsumosPorRecursosTipo: TStringField
      FieldName = 'Tipo'
      Size = 50
    end
    object Query_InsumosPorRecursosProduto_ID: TStringField
      FieldName = 'Produto_ID'
      Size = 30
    end
    object Query_InsumosPorRecursosProduto: TStringField
      FieldName = 'Produto'
      Size = 150
    end
    object Query_InsumosPorRecursosSaldoProducao: TFloatField
      FieldName = 'SaldoProducao'
      ReadOnly = True
    end
    object Query_InsumosPorRecursosEmpenho: TFloatField
      FieldName = 'Empenho'
      ReadOnly = True
    end
    object Query_InsumosPorRecursosEstoque: TFloatField
      FieldName = 'Estoque'
    end
    object Query_InsumosPorRecursosSaldoEstoque: TFloatField
      FieldName = 'SaldoEstoque'
      ReadOnly = True
    end
    object Query_InsumosPorRecursosUnidade: TStringField
      FieldName = 'Unidade'
      Size = 2
    end
    object Query_InsumosPorRecursosDestino_ID: TStringField
      FieldName = 'Destino_ID'
      Size = 2
    end
    object Query_InsumosPorRecursosDestino: TStringField
      FieldName = 'Destino'
      Size = 30
    end
    object Query_InsumosPorRecursosOrigem_ID: TStringField
      FieldName = 'Origem_ID'
      Size = 2
    end
    object Query_InsumosPorRecursosOrigem: TStringField
      FieldName = 'Origem'
      Size = 30
    end
  end
  object DS_InsumosPorRecursos: TDataSource
    DataSet = CDS_InsumosPorRecursos
    Left = 240
    Top = 24
  end
  object DSP_InsumosPorRecursos: TDataSetProvider
    DataSet = Query_InsumosPorRecursos
    Constraints = False
    Left = 274
    Top = 24
  end
  object ppDBPipeline1: TppDBPipeline
    DataSource = DS_InsumosPorRecursos
    CloseDataSource = True
    UserName = 'DBPipeline1'
    Left = 169
    Top = 91
  end
  object ppImprimirInusmosPorRecurso: TppReport
    AutoStop = False
    DataPipeline = ppDBPipeline1
    PassSetting = psTwoPass
    PrinterSetup.BinName = 'Default'
    PrinterSetup.DocumentName = 'Report'
    PrinterSetup.Duplex = dpVertical
    PrinterSetup.PaperName = 'A4'
    PrinterSetup.PrinterName = 'Default'
    PrinterSetup.SaveDeviceSettings = False
    PrinterSetup.mmMarginBottom = 6350
    PrinterSetup.mmMarginLeft = 6350
    PrinterSetup.mmMarginRight = 6350
    PrinterSetup.mmMarginTop = 6350
    PrinterSetup.mmPaperHeight = 297000
    PrinterSetup.mmPaperWidth = 210000
    PrinterSetup.PaperSize = 9
    AllowPrintToArchive = True
    AllowPrintToFile = True
    ArchiveFileName = '($MyDocuments)\ReportArchive.raf'
    DeviceType = 'Screen'
    DefaultFileDeviceType = 'XLSReport'
    EmailSettings.ReportFormat = 'PDF'
    LanguageID = 'Portuguese (Brazil)'
    OpenFile = False
    OutlineSettings.CreateNode = True
    OutlineSettings.CreatePageNodes = True
    OutlineSettings.Enabled = True
    OutlineSettings.Visible = True
    ThumbnailSettings.Enabled = True
    ThumbnailSettings.Visible = True
    ThumbnailSettings.DeadSpace = 30
    ThumbnailSettings.PageHighlight.Width = 3
    PDFSettings.EmbedFontOptions = [efUseSubset]
    PDFSettings.EncryptSettings.AllowCopy = True
    PDFSettings.EncryptSettings.AllowInteract = True
    PDFSettings.EncryptSettings.AllowModify = True
    PDFSettings.EncryptSettings.AllowPrint = True
    PDFSettings.EncryptSettings.AllowExtract = True
    PDFSettings.EncryptSettings.AllowAssemble = True
    PDFSettings.EncryptSettings.AllowQualityPrint = True
    PDFSettings.EncryptSettings.Enabled = False
    PDFSettings.EncryptSettings.KeyLength = kl40Bit
    PDFSettings.EncryptSettings.EncryptionType = etRC4
    PDFSettings.FontEncoding = feAnsi
    PDFSettings.ImageCompressionLevel = 25
    PDFSettings.PDFAFormat = pafNone
    PreviewFormSettings.PageBorder.mmPadding = 0
    PreviewFormSettings.WindowState = wsMaximized
    PreviewFormSettings.ZoomSetting = zsPageWidth
    RTFSettings.DefaultFont.Charset = DEFAULT_CHARSET
    RTFSettings.DefaultFont.Color = clWindowText
    RTFSettings.DefaultFont.Height = -13
    RTFSettings.DefaultFont.Name = 'Arial'
    RTFSettings.DefaultFont.Style = []
    TextFileName = '($MyDocuments)\Report.xls'
    TextSearchSettings.DefaultString = '<Texto a localizar>'
    TextSearchSettings.Enabled = True
    XLSSettings.AppName = 'ReportBuilder'
    XLSSettings.Author = 'ReportBuilder'
    XLSSettings.Subject = 'Report'
    XLSSettings.Title = 'Report'
    XLSSettings.WorksheetName = 'Report'
    Left = 207
    Top = 91
    Version = '19.03'
    mmColumnWidth = 197300
    DataPipelineName = 'ppDBPipeline1'
    object ppHeaderBand1: TppHeaderBand
      Background.Brush.Style = bsClear
      Border.mmPadding = 0
      PrintHeight = phDynamic
      mmBottomOffset = 0
      mmHeight = 15346
      mmPrintPosition = 0
      object ppLine1: TppLine
        DesignLayer = ppDesignLayer2
        UserName = 'Line1'
        Border.mmPadding = 0
        Pen.Width = 3
        Weight = 2.250000000000000000
        mmHeight = 1588
        mmLeft = 529
        mmTop = 13758
        mmWidth = 195792
        BandType = 0
        LayerName = BandLayer2
      end
      object ppLabel4: TppLabel
        DesignLayer = ppDesignLayer2
        UserName = 'ppLBL_TituloRelatorio'
        HyperlinkEnabled = False
        Border.mmPadding = 0
        Caption = 'Empenhos Agrupados Por Recursos'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 18
        Font.Style = [fsBold]
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        Transparent = True
        mmHeight = 7673
        mmLeft = 37571
        mmTop = 2646
        mmWidth = 110861
        BandType = 0
        LayerName = BandLayer2
      end
      object ppImage1: TppImage
        DesignLayer = ppDesignLayer2
        UserName = 'ppIMG_Logo'
        AlignHorizontal = ahCenter
        AlignVertical = avCenter
        MaintainAspectRatio = False
        Stretch = True
        Border.mmPadding = 0
        Picture.Data = {
          
          FFFF}
        mmHeight = 11906
        mmLeft = 1323
        mmTop = 794
        mmWidth = 33338
        BandType = 0
        LayerName = BandLayer2
      end
    end
    object ppDetailBand1: TppDetailBand
      Background1.Brush.Style = bsClear
      Background2.Brush.Style = bsClear
      Border.mmPadding = 0
      PrintHeight = phDynamic
      mmBottomOffset = 0
      mmHeight = 5821
      mmPrintPosition = 0
      object ppDBMemo2: TppDBMemo
        DesignLayer = ppDesignLayer2
        UserName = 'DBMemo1'
        Border.mmPadding = 0
        CharWrap = True
        DataField = 'Produto'
        DataPipeline = ppDBPipeline1
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 6
        Font.Style = []
        RemoveEmptyLines = False
        Stretch = True
        Transparent = True
        DataPipelineName = 'ppDBPipeline1'
        mmHeight = 3175
        mmLeft = 37571
        mmTop = 1058
        mmWidth = 107950
        BandType = 4
        LayerName = BandLayer2
        mmBottomOffset = 0
        mmOverFlowOffset = 0
        mmStopPosition = 0
        mmMinHeight = 0
        mmLeading = 0
      end
      object ppDBText20: TppDBText
        DesignLayer = ppDesignLayer2
        UserName = 'DBText12'
        Border.mmPadding = 0
        DataField = 'SaldoProducao'
        DataPipeline = ppDBPipeline1
        DisplayFormat = '#,0.00;-#,0.00'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 6
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDBPipeline1'
        mmHeight = 3704
        mmLeft = 149523
        mmTop = 1058
        mmWidth = 17997
        BandType = 4
        LayerName = BandLayer2
      end
      object ppDBText19: TppDBText
        DesignLayer = ppDesignLayer2
        UserName = 'DBText14'
        Border.mmPadding = 0
        DataField = 'Unidade'
        DataPipeline = ppDBPipeline1
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 6
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDBPipeline1'
        mmHeight = 3704
        mmLeft = 188929
        mmTop = 1058
        mmWidth = 5274
        BandType = 4
        LayerName = BandLayer2
      end
      object ppDBText16: TppDBText
        DesignLayer = ppDesignLayer2
        UserName = 'DBText4'
        Border.mmPadding = 0
        DataField = 'Empenho'
        DataPipeline = ppDBPipeline1
        DisplayFormat = '#,0.00;-#,0.00'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 6
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDBPipeline1'
        mmHeight = 3704
        mmLeft = 168331
        mmTop = 1058
        mmWidth = 18780
        BandType = 4
        LayerName = BandLayer2
      end
      object ppDBText15: TppDBText
        DesignLayer = ppDesignLayer2
        UserName = 'DBText2'
        Border.mmPadding = 0
        DataField = 'Produto_ID'
        DataPipeline = ppDBPipeline1
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 6
        Font.Style = []
        Transparent = True
        DataPipelineName = 'ppDBPipeline1'
        mmHeight = 3704
        mmLeft = 22750
        mmTop = 1058
        mmWidth = 13225
        BandType = 4
        LayerName = BandLayer2
      end
      object ppLine2: TppLine
        DesignLayer = ppDesignLayer2
        UserName = 'Line6'
        Border.mmPadding = 0
        Pen.Color = clSilver
        Pen.Style = psDot
        Weight = 0.750000000000000000
        mmHeight = 1058
        mmLeft = 2649
        mmTop = 4761
        mmWidth = 191817
        BandType = 4
        LayerName = BandLayer2
      end
      object ppDBText3: TppDBText
        DesignLayer = ppDesignLayer2
        UserName = 'DBText1'
        Border.mmPadding = 0
        DataField = 'Tipo'
        DataPipeline = ppDBPipeline1
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 6
        Font.Style = []
        Transparent = True
        DataPipelineName = 'ppDBPipeline1'
        mmHeight = 3704
        mmLeft = 2646
        mmTop = 1059
        mmWidth = 18781
        BandType = 4
        LayerName = BandLayer2
      end
    end
    object ppFooterBand1: TppFooterBand
      Background.Brush.Style = bsClear
      Border.mmPadding = 0
      mmBottomOffset = 0
      mmHeight = 5821
      mmPrintPosition = 0
      object ppLabel5: TppLabel
        DesignLayer = ppDesignLayer2
        UserName = 'ppLBL_DataListaPorGerencia'
        HyperlinkEnabled = False
        Border.mmPadding = 0
        Caption = 'ppLBL_Data'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = [fsBold]
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        TextAlignment = taRightJustified
        Transparent = True
        mmHeight = 4233
        mmLeft = 59796
        mmTop = 1058
        mmWidth = 135996
        BandType = 8
        LayerName = BandLayer2
      end
      object ppLine3: TppLine
        DesignLayer = ppDesignLayer2
        UserName = 'Line12'
        Border.mmPadding = 0
        Pen.Width = 3
        Weight = 2.250000000000000000
        mmHeight = 2646
        mmLeft = 529
        mmTop = 0
        mmWidth = 195792
        BandType = 8
        LayerName = BandLayer2
      end
    end
    object ppSummaryBand2: TppSummaryBand
      Background.Brush.Style = bsClear
      Border.mmPadding = 0
      PrintHeight = phDynamic
      mmBottomOffset = 0
      mmHeight = 0
      mmPrintPosition = 0
    end
    object ppPageStyle1: TppPageStyle
      Background.Brush.Style = bsClear
      Border.mmPadding = 0
      EndPage = 0
      SinglePage = 0
      StartPage = 0
      mmBottomOffset = 0
      mmHeight = 0
      mmPrintPosition = 0
    end
    object ppGroup2: TppGroup
      BreakName = 'Setor'
      DataPipeline = ppDBPipeline1
      GroupFileSettings.NewFile = False
      GroupFileSettings.EmailFile = False
      OutlineSettings.CreateNode = True
      NewPage = True
      StartOnOddPage = False
      UserName = 'Group1'
      mmNewColumnThreshold = 0
      mmNewPageThreshold = 0
      DataPipelineName = 'ppDBPipeline1'
      NewFile = False
      object ppGroupHeaderBand2: TppGroupHeaderBand
        Background.Brush.Style = bsClear
        Border.mmPadding = 0
        mmBottomOffset = 0
        mmHeight = 7408
        mmPrintPosition = 0
        object ppDBText21: TppDBText
          DesignLayer = ppDesignLayer2
          UserName = 'DBText6'
          Border.mmPadding = 0
          DataField = 'Setor'
          DataPipeline = ppDBPipeline1
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 14
          Font.Style = [fsBold]
          Transparent = True
          DataPipelineName = 'ppDBPipeline1'
          mmHeight = 5556
          mmLeft = 1321
          mmTop = 529
          mmWidth = 71173
          BandType = 3
          GroupNo = 0
          LayerName = BandLayer2
        end
      end
      object ppGroupFooterBand2: TppGroupFooterBand
        Background.Brush.Style = bsClear
        Border.mmPadding = 0
        HideWhenOneDetail = False
        mmBottomOffset = 0
        mmHeight = 7144
        mmPrintPosition = 0
      end
    end
    object ppGroup3: TppGroup
      BreakName = 'Injetora_FK'
      DataPipeline = ppDBPipeline1
      GroupFileSettings.NewFile = False
      GroupFileSettings.EmailFile = False
      KeepTogether = True
      OutlineSettings.CreateNode = True
      StartOnOddPage = False
      UserName = 'Group3'
      mmNewColumnThreshold = 0
      mmNewPageThreshold = 0
      DataPipelineName = 'ppDBPipeline1'
      NewFile = False
      object ppGroupHeaderBand3: TppGroupHeaderBand
        Background.Brush.Style = bsClear
        Border.mmPadding = 0
        mmBottomOffset = 0
        mmHeight = 13494
        mmPrintPosition = 0
        object ppShape2: TppShape
          DesignLayer = ppDesignLayer2
          UserName = 'Shape1'
          mmHeight = 5821
          mmLeft = 1323
          mmTop = 7673
          mmWidth = 194469
          BandType = 3
          GroupNo = 1
          LayerName = BandLayer2
        end
        object ppLabel24: TppLabel
          DesignLayer = ppDesignLayer2
          UserName = 'Label2'
          HyperlinkEnabled = False
          AutoSize = False
          Border.mmPadding = 0
          Caption = 'Produ'#231#227'o'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
          FormFieldSettings.FormFieldType = fftNone
          TextAlignment = taRightJustified
          Transparent = True
          mmHeight = 3969
          mmLeft = 148752
          mmTop = 8469
          mmWidth = 18785
          BandType = 3
          GroupNo = 1
          LayerName = BandLayer2
        end
        object ppLabel23: TppLabel
          DesignLayer = ppDesignLayer2
          UserName = 'Label15'
          HyperlinkEnabled = False
          AutoSize = False
          Border.mmPadding = 0
          Caption = 'UN'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
          FormFieldSettings.FormFieldType = fftNone
          TextAlignment = taRightJustified
          Transparent = True
          mmHeight = 3969
          mmLeft = 189707
          mmTop = 8469
          mmWidth = 4498
          BandType = 3
          GroupNo = 1
          LayerName = BandLayer2
        end
        object ppLabel20: TppLabel
          DesignLayer = ppDesignLayer2
          UserName = 'Label101'
          HyperlinkEnabled = False
          AutoSize = False
          Border.mmPadding = 0
          Caption = 'Empenho'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
          FormFieldSettings.FormFieldType = fftNone
          TextAlignment = taRightJustified
          Transparent = True
          mmHeight = 3969
          mmLeft = 168331
          mmTop = 8469
          mmWidth = 18785
          BandType = 3
          GroupNo = 1
          LayerName = BandLayer2
        end
        object ppLabel19: TppLabel
          DesignLayer = ppDesignLayer2
          UserName = 'Label10'
          HyperlinkEnabled = False
          AutoSize = False
          Border.mmPadding = 0
          Caption = 'Produto'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
          FormFieldSettings.FormFieldType = fftNone
          Transparent = True
          mmHeight = 3969
          mmLeft = 37571
          mmTop = 8469
          mmWidth = 17992
          BandType = 3
          GroupNo = 1
          LayerName = BandLayer2
        end
        object ppLabel18: TppLabel
          DesignLayer = ppDesignLayer2
          UserName = 'Label6'
          HyperlinkEnabled = False
          AutoSize = False
          Border.mmPadding = 0
          Caption = 'ID'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
          FormFieldSettings.FormFieldType = fftNone
          Transparent = True
          mmHeight = 3969
          mmLeft = 22754
          mmTop = 8469
          mmWidth = 7938
          BandType = 3
          GroupNo = 1
          LayerName = BandLayer2
        end
        object ppLabel6: TppLabel
          DesignLayer = ppDesignLayer2
          UserName = 'Label1'
          HyperlinkEnabled = False
          AutoSize = False
          Border.mmPadding = 0
          Caption = 'Tipo'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
          FormFieldSettings.FormFieldType = fftNone
          Transparent = True
          mmHeight = 3969
          mmLeft = 2646
          mmTop = 8469
          mmWidth = 7938
          BandType = 3
          GroupNo = 1
          LayerName = BandLayer2
        end
        object ppDBText27: TppDBText
          DesignLayer = ppDesignLayer2
          UserName = 'DBText27'
          Border.mmPadding = 0
          DataField = 'Injetora'
          DataPipeline = ppDBPipeline1
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 9
          Font.Style = [fsBold]
          Transparent = True
          DataPipelineName = 'ppDBPipeline1'
          mmHeight = 4761
          mmLeft = 1323
          mmTop = 2648
          mmWidth = 105040
          BandType = 3
          GroupNo = 1
          LayerName = BandLayer2
        end
      end
      object ppGroupFooterBand3: TppGroupFooterBand
        Background.Brush.Style = bsClear
        Border.mmPadding = 0
        HideWhenOneDetail = False
        mmBottomOffset = 0
        mmHeight = 0
        mmPrintPosition = 0
      end
    end
    object ppDesignLayers1: TppDesignLayers
      object ppDesignLayer1: TppDesignLayer
        UserName = 'PageLayer2'
        LayerType = ltPage
        Index = 0
      end
      object ppDesignLayer2: TppDesignLayer
        UserName = 'BandLayer2'
        LayerType = ltBanded
        Index = 1
      end
    end
    object ppParameterList1: TppParameterList
    end
  end
end
