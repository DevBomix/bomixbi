...
type
  TSys_FuncaoRestricoesInformacoes = class(TForm)
    ...
    function GetAcessoGerenciaDeFormaRestrita: String;
...
begin
  ...
  VLC_Select := VLC_Select +  '			AND ' + PLC_CampoVendedor +  ' in ' + #13;
  VLC_Select := VLC_Select +  '		 (  ' + #13;
  VLC_Select := VLC_Select +  '       Select Distinct Z6_CODVEND   ' + #13;
  VLC_Select := VLC_Select +  '       from P12Oficial.dbo.SZ6010 (nolock) ' + #13;
  VLC_Select := VLC_Select +  '       Where D_E_L_E_T_ <> ' + '''' + '*' + '''' + #13;
  VLC_Select := VLC_Select +  '       AND Z6_FILIAL = ' + '''' + Copy(Sys_FuncaoSistema.PNL_EmpresaTotvs.Caption,1,4) + '''' + #13;
...
//  VLC_Select := VLC_Select + ' (Select Totvs from BomixBI.dbo.Sys_TB_Usuario (nolock) where Usuario_ID = ' + Sys_FuncaoSistema.PNL_UsuarioAtivoID.Caption + '))' + #13;
  ...
  VLC_Select :=  '            AND ' + PLC_CampoVendedor +  ' in ' + Sys_FuncaoSistema.TXT_PermissaoVendedor.Caption + #13;
  Result := VLC_Select;

end;


function TSys_FuncaoRestricoesInformacoes.GetAcessoGerenciaDeFormaRestrita: String;
var
  VLC_Select : String;
  Query : TADOQuery;
  VLC_Filtro : String;
begin
   ...
   VLC_Select := ' Select Distinct Z6_CODVEND from P12Oficial.dbo.SZ6010 (nolock) Where D_E_L_E_T_ <> ' + '''' + '*' + '''' + #13;
   ...
   begin
    ...
    repeat
        VLC_Filtro := VLC_Filtro + '''' + Query.FieldByName('Z6_CODVEND').AsString + '''' + ',';
        Query.Next;
    until (Query.Eof);

    VLC_Filtro := VLC_Filtro + ')';

   if Query.RecordCount = 0 then  Result := ''  else Result := (StringReplace(VLC_Filtro,',)',')', [rfReplaceAll]));;
...
begin
  ...
  begin
        VLC_Select := VLC_Select +  '		  AND Vendedor_ID in ' + #13;
        VLC_Select := VLC_Select +  '		 (  ' + #13;
        VLC_Select := VLC_Select +  '       Select Distinct Z6_CODVEND   ' + #13;
        VLC_Select := VLC_Select +  '       from P12Oficial.dbo.SZ6010 (nolock) ' + #13;
        VLC_Select := VLC_Select +  '       Where D_E_L_E_T_ <> ' + '''' + '*' + '''' + #13;
