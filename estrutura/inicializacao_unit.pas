...
begin
  ...
  // Comercial - Diário de Nota Fiscal por Transportadora
  ...
  // Comercial - Relatório  CIT - Conhecimento Int. de Transporte
  ...
  //Comercial Relatório Faturamento Periódico
  ...
  //Comercial Relatório - Etiqueta\Envelope para Feiras
  ...
  // Comercial Relatório - Pedidos com Data PCP em Atraso
  ...
  // Comercial Relatório - Periódico de Faturamento x Produção
  ...
  // Comercial Relatório - Último Faturamento (Cliente)
  ...
  // Comercial Relatório - Último Faturamento (Produto)
  ...
  // Comercial Relatório - Ranking de Clientes
  ...
  // Comercial - Relatório - Controle de Rótulos
  ...
  // Comercial Relatório - Relação de Notas Fiscais
  ...
  // Comercial Relatório - Vendas Por Mix de Produtos
  ...
  begin
    ...
    // Comercial Relatório - Parametrizado de Vendas (NOVO)
  ...
  // Comercial Relatório - Parametrizado de Vendas (Antigo)
  If Sys_FuncaoSistema.GetVerificaSeExistePermissao(20,Sys_FuncaoSistema.PNL_LoginAtivo.Caption) = False then
  begin
  ...
  // Comercial Relatório - Mapas Anual de Vendas
  If Sys_FuncaoSistema.GetVerificaSeExistePermissao(20,Sys_FuncaoSistema.PNL_LoginAtivo.Caption) = False then
  begin
  ...
  // Comercial Relatório - Média de Prazos de Pagamento
  ...
  // Comercial Relatório - Tonelagem Diária
  ...
  // Comercial Relatório - Posição de Estoque x Faturamento
  ...
  // Comercial - Relatório Comparativo Vendas Anuais
  If Sys_FuncaoSistema.GetVerificaSeExistePermissao(164,Sys_FuncaoSistema.PNL_LoginAtivo.Caption) = False then
  begin
      Sys_Principal.BTN_Comercial_CompracaoVendas.Visible := False;
      Sys_Principal.Comercial_ComparativoVendasAnuais.Visible := False;
  end
  else
  begin
      Sys_Principal.BTN_Comercial_CompracaoVendas.Visible := True;
      Sys_Principal.Comercial_ComparativoVendasAnuais.Visible := True;
  end;
...
begin
  ...
  // Painel de Controle - Acesso do Usuário ao Sistema
...
begin
    ...
    // Produção - Administração de Ciclo
    ...
    // Produção - Administração de Resina e Master
    ...
    // Produção - Etiqueta de Apontamento
    ...
    // Produção - Ordem de Produção PCP
    ...
    // Produção - Transferência Produção (P1 ou P4) -> Transbordo (TE)
    ...
    // Produção - Ordem de Solicitação de Matéria Prima
    ...
    // Produção - Relatório de Apontamento de Produção
   ...
   // Produção - Relatório de Apontamento Personalização
   ...
   // Produção - Relatório de Apontamento Paletização
    ...
    // Produção - Relatório Controle de Estoque
    ...
    // Produção - Relatório Horas Improdutivas
    ...
    // Produção - Relatório Controle de Entrada e Saida (Loop)
    ...
    Sys_Principal.Producao_EntradaSaidaLoop.Visible := False else Sys_Principal.Producao_EntradaSaidaLoop.Visible := True;
     ...
     // Produção Relatório - Posição de Estoque
    ...
    // Produção Relatório - Perdas de Produção
...
begin
  ...
  //Qualidade - Especificação Técnica **************************************************************************************************************
...
begin
  ...
  // Gestão de Pessoas - Cadastro de Acessorios
  ...
  Sys_Principal.GestaoPessoal_CadastroAcessorios.Visible := False else Sys_Principal.GestaoPessoal_CadastroAcessorios.Visible := True;
   ...
   // Gestão de Pessoas - Impressão de Crachá
  ...
  begin
   ...
   // Gestão de Pessoas - Entrega Material
...

{
  ...
  // 04 - Menu Gestão Pessoal ********************************************************************************************************
  if Sys_FuncaoSistema.GetVerificaSeExistePermissaoModulo('GESTÂO PESSOAL',Sys_FuncaoSistema.PNL_LoginAtivo.Caption) = True then
  begin
       Sys_Principal.ActionMainMenuBar1.ActionControls[2].Visible := True;
       Sys_Principal.PNL_MenuGestaoPessoal.Visible := True;
       Sys_Principal.LBL_Modulo.Caption := 'Gestão Pessoal';
  end
  else
  begin
  ...
  // 03 - Menu Produção ******************************************************************************************************************
  if Sys_FuncaoSistema.GetVerificaSeExistePermissaoModulo('PCP / PRODUÇÃO',Sys_FuncaoSistema.PNL_LoginAtivo.Caption) = True then
  begin
       Sys_Principal.ActionMainMenuBar1.ActionControls[1].Visible := True;
       Sys_Principal.PNL_MenuProducaoPCP.Visible := True;
       Sys_Principal.LBL_Modulo.Caption := 'PCP / Produção';
  end
  else
  begin
  ...
  begin
   ...
   if Sys_Principal.LBL_Modulo.Caption = 'Comercial' then
    ...
    if Sys_Principal.LBL_Modulo.Caption = 'PCP / Produção' then
    begin
      Sys_Principal.PNL_MenuComercial.Visible := False;
      Sys_Principal.PNL_MenuProducaoPCP.Visible := True;
      Sys_Principal.PNL_MenuQualidade.Visible := False;
    ...
    if Sys_Principal.LBL_Modulo.Caption = 'Gestão Pessoal' then
    begin
      Sys_Principal.PNL_MenuComercial.Visible := False;
      Sys_Principal.PNL_MenuProducaoPCP.Visible := False;
      Sys_Principal.PNL_MenuQualidade.Visible := False;
...
begin
   ...
   // Verifica se o Usuário tem Restrição de Informação para apenas alguma(s) Gerência(s)
   ...
   begin
       Sys_FuncaoSistema.TXT_RestricaoComercial.Caption := 'Não';
   end;
...
begin
  ...
  begin
      Application.MessageBox('Não foi encontrado na pasta do sistema o arquivo BomixUpdate.exe.' + #13 + 'Favor solicitar ao suporte TI a sua inclusão','Atenção',mb_iconwarning + mb_ok);
      Exit;
  end;
  ...
  begin
       Application.MessageBox('Login ou Senha não conferem.' + #13 + 'Tente outra vez!','Atenção',mb_iconerror + mb_ok);
       TXT_Senha.Text := '';
       TXT_Login.Text := '';
       TXT_Login.SetFocus;
       Exit;
