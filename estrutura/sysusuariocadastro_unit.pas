...
begin
  ...
  begin
       Application.MessageBox('Acesso negado para Controle de Armazem.' + #13 + 'Consulte o Administrador do sistema','Atenção',mb_iconwarning + mb_ok);
   end
   else
   begin
        inherited;
...
begin
  ...
  begin
   ...
   begin
        ...
        // Atualizar os Dados de relação entre Armazens e Usuários
...
begin
  ...
  If Sys_FuncaoSistema.GetVerificaSeExistePermissao(27,Sys_FuncaoSistema.PNL_LoginAtivo.Caption) = False then
  begin
       Application.MessageBox('Acesso negado para Retirar Restrinção.' + #13 + 'Consulte o Administrador do sistema','Atenção',mb_iconwarning + mb_ok);
   end
   else
   begin
...
begin
  ...
  If Sys_FuncaoSistema.GetVerificaSeExistePermissao(28,Sys_FuncaoSistema.PNL_LoginAtivo.Caption) = False then
  begin
       Application.MessageBox('Acesso negado para Definir Restrinção.' + #13 + 'Consulte o Administrador do sistema','Atenção',mb_iconwarning + mb_ok);
   end
   else
   begin
        inherited;
        ...
        try
            ...
            begin
              Application.MessageBox('Todas as restrições já estão vinculadas a este usuário!' + #13 + '','Atenção',mb_iconerror + mb_ok);
              Exit;
            end;
...
begin
  ...
  if (LBL_Modo.Caption = 'Modo de Visualização') OR (LBL_Modo.Caption = 'Modo de Pesquisa') then
  begin
      inherited;
...
begin
   ...
   if CHK_MobileEntregaMaterias_Producao.Checked = True then VLB_MobileEntregaMaterias_Producao := 'SIM' else  VLB_MobileEntregaMaterias_Producao := 'NÃO';
   if CHK_MobileEntregaMaterias_Personalizacao.Checked = True then VLB_MobileEntregaMaterias_Personalizacao := 'SIM' else  VLB_MobileEntregaMaterias_Personalizacao := 'NÃO';
   if CHK_MobileEntregaMaterias_Logistica.Checked = True then VLB_MobileEntregaMaterias_Logistica := 'SIM' else  VLB_MobileEntregaMaterias_Logistica := 'NÃO';
...
// Validação **********************************************************************************************************
  ...
  begin
       Application.MessageBox('Campo Obrigatório: Defina o Login!' + #13 + '','Atenção',mb_iconerror + mb_ok);
       TXT_Login.SetFocus;
       Exit;
  end;
  ...
  begin
       Application.MessageBox('Campo Obrigatório: Defina a identificação deste usuário no sistema da Totvs!' + #13 + '','Atenção',mb_iconerror + mb_ok);
  ...
  begin
       Application.MessageBox('Campo Obrigatório: Defina o Grupo de Usuário!' + #13 + '','Atenção',mb_iconerror + mb_ok);
       Exit;
  end;
...
// ******************************************************************************************************************
  ...
  begin
      ...
      begin
         Application.MessageBox('Campo em Duplicidade. Defina outro Login para o Usuário!' + #13 + '','Atenção',mb_iconerror + mb_ok);
         TXT_Login.SetFocus;
         Exit;
      end;
